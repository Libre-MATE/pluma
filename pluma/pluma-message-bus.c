 ...
 * PlumaMessageType *message_type = pluma_message_bus_register
 * ("/plugins/example", "method", 0, "arg1", G_TYPE_STRING, NULL);
 * </programlisting>
 * </example>
 * <example>
...
static void listener_free(Listener *listener) {
  if (listener->destroy_data) listener->destroy_data(listener->userdata);
...
static void pluma_message_bus_finalize(GObject *object) {
  PlumaMessageBus *bus = PLUMA_MESSAGE_BUS(object);

  if (bus->priv->idle_id != 0) g_source_remove(bus->priv->idle_id);
...
static void pluma_message_bus_class_init(PlumaMessageBusClass *klass) {
  ...
  message_bus_signals[DISPATCH] = g_signal_new(
      ...
      G_STRUCT_OFFSET(PlumaMessageBusClass, dispatch), NULL, NULL, NULL,
      G_TYPE_NONE, 1, PLUMA_TYPE_MESSAGE);
  ...
  message_bus_signals[REGISTERED] = g_signal_new(
      ...
      G_STRUCT_OFFSET(PlumaMessageBusClass, registered), NULL, NULL, NULL,
      G_TYPE_NONE, 1, PLUMA_TYPE_MESSAGE_TYPE);
  ...
  message_bus_signals[UNREGISTERED] = g_signal_new(
      ...
      G_STRUCT_OFFSET(PlumaMessageBusClass, unregistered), NULL, NULL, NULL,
      G_TYPE_NONE, 1, PLUMA_TYPE_MESSAGE_TYPE);
}

static Message *message_new(PlumaMessageBus *bus, const gchar *object_path,
                            const gchar *method) {
...
static Message *lookup_message(PlumaMessageBus *bus, const gchar *object_path,
                               const gchar *method, gboolean create) {
  gchar *identifier;
  Message *message;
  ...
  if (!message && !create) return NULL;

  if (!message) message = message_new(bus, object_path, method);

  return message;
}

static guint add_listener(PlumaMessageBus *bus, Message *message,
                          PlumaMessageCallback callback, gpointer userdata,
                          GDestroyNotify destroy_data) {
  Listener *listener;
  IdMap *idmap;
...
static void remove_listener(PlumaMessageBus *bus, Message *message,
                            GList *listener) {
  Listener *lst;
...
static void block_listener(PlumaMessageBus *bus, Message *message,
                           GList *listener) {
  Listener *lst;
...
static void unblock_listener(PlumaMessageBus *bus, Message *message,
                             GList *listener) {
  Listener *lst;
...
static void dispatch_message_real(PlumaMessageBus *bus, Message *msg,
                                  PlumaMessage *message) {
  GList *item;
...
static void pluma_message_bus_dispatch_real(PlumaMessageBus *bus,
  ...
  if (msg) dispatch_message_real(bus, msg, message);
}

static void dispatch_message(PlumaMessageBus *bus, PlumaMessage *message) {
  g_signal_emit(bus, message_bus_signals[DISPATCH], 0, message);
}

static gboolean idle_dispatch(PlumaMessageBus *bus) {
  GList *list;
  GList *item;
...
static void process_by_id(PlumaMessageBus *bus, guint id,
...
static void process_by_match(PlumaMessageBus *bus, const gchar *object_path,
                             const gchar *method, PlumaMessageCallback callback,
                             gpointer userdata, MatchCallback processor) {
  Message *message;
  GList *item;
...
static void pluma_message_bus_init(PlumaMessageBus *self) {
  ...
  self->priv->idmap = g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL,
...
PlumaMessageType *pluma_message_bus_lookup(PlumaMessageBus *bus,
  ...
  identifier = pluma_message_type_identifier(object_path, method);
  message_type =
      PLUMA_MESSAGE_TYPE(g_hash_table_lookup(bus->priv->types, identifier));
...
PlumaMessageType *pluma_message_bus_register(PlumaMessageBus *bus,
  ...
  if (pluma_message_bus_is_registered(bus, object_path, method)) {
    g_warning("Message type for '%s.%s' is already registered", object_path,
              method);
    return NULL;
  }
...
static void pluma_message_bus_unregister_real(PlumaMessageBus *bus,
                                              PlumaMessageType *message_type,
...
static gboolean unregister_each(const gchar *identifier,
                                PlumaMessageType *message_type,
                                UnregisterInfo *info) {
...
void pluma_message_bus_unregister_all(PlumaMessageBus *bus,
  ...
  g_hash_table_foreach_remove(bus->priv->types, (GHRFunc)unregister_each,
                              &info);
}
...
static void foreach_type(const gchar *key, PlumaMessageType *message_type,
                         ForeachInfo *info) {
  pluma_message_type_ref(message_type);
...
void pluma_message_bus_foreach(PlumaMessageBus *bus,
                               PlumaMessageBusForeach func, gpointer userdata) {
 ...
 *
 */
guint pluma_message_bus_connect(PlumaMessageBus *bus, const gchar *object_path,
                                const gchar *method,
                                PlumaMessageCallback callback,
                                gpointer userdata,
                                GDestroyNotify destroy_data) {
 ...
 *
 */
void pluma_message_bus_disconnect(PlumaMessageBus *bus, guint id) {
  g_return_if_fail(PLUMA_IS_MESSAGE_BUS(bus));
...
void pluma_message_bus_disconnect_by_func(PlumaMessageBus *bus,
  ...
  process_by_match(bus, object_path, method, callback, userdata,
                   remove_listener);
}
 ...
 *
 */
void pluma_message_bus_block(PlumaMessageBus *bus, guint id) {
  g_return_if_fail(PLUMA_IS_MESSAGE_BUS(bus));
 ...
 *
 */
void pluma_message_bus_unblock(PlumaMessageBus *bus, guint id) {
  g_return_if_fail(PLUMA_IS_MESSAGE_BUS(bus));
...
static gboolean validate_message(PlumaMessage *message) {
  if (!pluma_message_validate(message)) {
    g_warning("Message '%s.%s' is invalid",
              pluma_message_get_object_path(message),
...
static void send_message_real(PlumaMessageBus *bus, PlumaMessage *message) {
  ...
  if (bus->priv->idle_id == 0)
    bus->priv->idle_id =
        g_idle_add_full(G_PRIORITY_HIGH, (GSourceFunc)idle_dispatch, bus, NULL);
}
...
static void send_message_sync_real(PlumaMessageBus *bus,
                                   PlumaMessage *message) {
  if (!validate_message(message)) {
    return;
  }
...
static PlumaMessage *create_message(PlumaMessageBus *bus,
                                    const gchar *object_path,
                                    const gchar *method, va_list var_args) {
  PlumaMessageType *message_type;
  ...
  return pluma_message_type_instantiate_valist(message_type, var_args);
}
 ...
 *
 */
void pluma_message_bus_send(PlumaMessageBus *bus, const gchar *object_path,
                            const gchar *method, ...) {
  va_list var_args;
  PlumaMessage *message;
 ...
 *
 * Return value: (transfer full): the constructed #PlumaMessage. The caller owns
 * a reference to the #PlumaMessage and should call g_object_unref() when it is
 * no longer needed
 */
PlumaMessage *pluma_message_bus_send_sync(PlumaMessageBus *bus,
  ...
  if (message) send_message_sync_real(bus, message);
