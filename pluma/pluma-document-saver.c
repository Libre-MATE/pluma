...
#define REMOTE_QUERY_ATTRIBUTES          \
...
G_DEFINE_TYPE_WITH_PRIVATE(PlumaDocumentSaver, pluma_document_saver,
                           G_TYPE_OBJECT)

static void pluma_document_saver_set_property(GObject *object, guint prop_id,
                                              const GValue *value,
                                              GParamSpec *pspec) {
...
static void pluma_document_saver_get_property(GObject *object, guint prop_id,
                                              GValue *value,
                                              GParamSpec *pspec) {
  PlumaDocumentSaver *saver = PLUMA_DOCUMENT_SAVER(object);
...
static void pluma_document_saver_finalize(GObject *object) {
  PlumaDocumentSaverPrivate *priv =
      pluma_document_saver_get_instance_private(PLUMA_DOCUMENT_SAVER(object));
...
static void pluma_document_saver_dispose(GObject *object) {
  PlumaDocumentSaverPrivate *priv =
      pluma_document_saver_get_instance_private(PLUMA_DOCUMENT_SAVER(object));
...
static AsyncData *async_data_new(PlumaDocumentSaver *gvsaver) {
  AsyncData *async;
...
static void async_data_free(AsyncData *async) {
...
static void pluma_document_saver_class_init(PlumaDocumentSaverClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string(
          "uri", "URI", "The URI this PlumaDocumentSaver saves the document to",
          "",
          G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS));
  ...
  g_object_class_install_property(
      ...
      g_param_spec_boxed(
          "encoding", "URI", "The encoding of the saved file",
          PLUMA_TYPE_ENCODING,
          G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS));
  ...
  g_object_class_install_property(
      ...
      g_param_spec_flags("flags", "Flags", "The flags for the saving operation",
                         PLUMA_TYPE_DOCUMENT_SAVE_FLAGS, 0,
                         G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));

  signals[SAVING] = g_signal_new(
      "saving", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PlumaDocumentSaverClass, saving), NULL, NULL, NULL,
      G_TYPE_NONE, 2, G_TYPE_BOOLEAN, G_TYPE_POINTER);
}

static void pluma_document_saver_init(PlumaDocumentSaver *saver) {
...
PlumaDocumentSaver *pluma_document_saver_new(
    PlumaDocument *doc, const gchar *uri, const PlumaEncoding *encoding,
    PlumaDocumentNewlineType newline_type, PlumaDocumentSaveFlags flags) {
  return PLUMA_DOCUMENT_SAVER(g_object_new(
      PLUMA_TYPE_DOCUMENT_SAVER, "document", doc, "uri", uri, "encoding",
      encoding, "newline_type", newline_type, "flags", flags, NULL));
}

static void remote_save_completed_or_failed(PlumaDocumentSaver *gvsaver,
                                            AsyncData *async) {
  pluma_document_saver_saving(PLUMA_DOCUMENT_SAVER(gvsaver), TRUE,
                              gvsaver->priv->error);

  if (async) async_data_free(async);
}

static void async_failed(AsyncData *async, GError *error) {
 ...
 */
static void cancel_output_stream_ready_cb(GOutputStream *stream,
                                          GAsyncResult *result,
                                          AsyncData *async) {
  GError *error;
...
static void cancel_output_stream(AsyncData *async) {
  GCancellable *cancellable;
  ...
  g_output_stream_close_async(
      ...
      (GAsyncReadyCallback)cancel_output_stream_ready_cb, async);

  g_object_unref(cancellable);
}

static void cancel_output_stream_and_fail(AsyncData *async, GError *error) {
...
static void remote_get_info_cb(GFile *source, GAsyncResult *res,
                               AsyncData *async) {
  PlumaDocumentSaver *saver;
  GFileInfo *info;
  GError *error = NULL;
  ...
  if (info != NULL) {
    if (saver->priv->info != NULL) g_object_unref(saver->priv->info);
...
static void close_async_ready_get_info_cb(GOutputStream *stream,
                                          GAsyncResult *res, AsyncData *async) {
  GError *error = NULL;
  ...
  if (!g_output_stream_close_finish(stream, res, &error)) {
    pluma_debug_message(DEBUG_SAVER, "Closing stream error: %s",
                        error->message);
  ...
  g_file_query_info_async(async->saver->priv->gfile, REMOTE_QUERY_ATTRIBUTES,
                          G_FILE_QUERY_INFO_NONE, G_PRIORITY_HIGH,
                          async->cancellable,
                          (GAsyncReadyCallback)remote_get_info_cb, async);
}

static void write_complete(AsyncData *async) {
  GError *error = NULL;

  /* first we close the input stream */
  pluma_debug_message(DEBUG_SAVER, "Close input stream");
  if (!g_input_stream_close(async->saver->priv->input, async->cancellable,
                            &error)) {
    pluma_debug_message(DEBUG_SAVER, "Closing input stream error: %s",
                        error->message);
  ...
  g_output_stream_close_async(
      async->saver->priv->stream, G_PRIORITY_HIGH, async->cancellable,
      (GAsyncReadyCallback)close_async_ready_get_info_cb, async);
}
...
static void async_write_cb(GOutputStream *stream, GAsyncResult *res,
                           AsyncData *async) {
  PlumaDocumentSaver *saver;
...
static void write_file_chunk(AsyncData *async) {
  ...
  g_output_stream_write_async(
      G_OUTPUT_STREAM(saver->priv->stream), async->buffer + async->written,
      async->read - async->written, G_PRIORITY_HIGH, async->cancellable,
      (GAsyncReadyCallback)async_write_cb, async);
}
...
static void read_file_chunk(AsyncData *async) {
  ...
  async->read =
      g_input_stream_read(saver->priv->input, async->buffer, WRITE_CHUNK_SIZE,
...
static void async_replace_ready_callback(GFile *source, GAsyncResult *res,
                                         AsyncData *async) {
  PlumaDocumentSaver *saver;
  ...
  if (saver->priv->encoding != pluma_encoding_get_utf8()) {
    converter = g_charset_converter_new(
        pluma_encoding_get_charset(saver->priv->encoding), "UTF-8", NULL);
    saver->priv->stream = g_converter_output_stream_new(
        G_OUTPUT_STREAM(file_stream), G_CONVERTER(converter));
  ...
  saver->priv->input = pluma_document_input_stream_new(
      GTK_TEXT_BUFFER(saver->priv->document), saver->priv->newline_type);

  saver->priv->size = pluma_document_input_stream_get_total_size(
      PLUMA_DOCUMENT_INPUT_STREAM(saver->priv->input));

  read_file_chunk(async);
}

static void begin_write(AsyncData *async) {
  PlumaDocumentSaver *saver;
  gboolean backup;
   ...
   */
  backup = (saver->priv->keep_backup &&
            pluma_document_is_local(saver->priv->document));

  pluma_debug_message(DEBUG_SAVER, "File contents size: %" G_GINT64_FORMAT,
                      saver->priv->size);
  pluma_debug_message(DEBUG_SAVER, "Calling replace_async");
  pluma_debug_message(DEBUG_SAVER, backup ? "Keep backup" : "Discard backup");

  g_file_replace_async(saver->priv->gfile, NULL, backup, G_FILE_CREATE_NONE,
                       G_PRIORITY_HIGH, async->cancellable,
                       (GAsyncReadyCallback)async_replace_ready_callback,
                       async);
}

static void mount_ready_callback(GFile *file, GAsyncResult *res,
                                 AsyncData *async) {
  GError *error = NULL;
  gboolean mounted;
...
static void recover_not_mounted(AsyncData *async) {
  PlumaDocument *doc;
  ...
  g_file_mount_enclosing_volume(
      async->saver->priv->gfile, G_MOUNT_MOUNT_NONE, mount_operation,
      async->cancellable, (GAsyncReadyCallback)mount_ready_callback, async);

  g_object_unref(mount_operation);
}

static void check_modification_callback(GFile *source, GAsyncResult *res,
                                        AsyncData *async) {
  PlumaDocumentSaver *saver;
  GError *error = NULL;
  GFileInfo *info;
  ...
  if (info == NULL) {
    ...
    if (error->code != G_IO_ERROR_NOT_FOUND) {
      pluma_debug_message(DEBUG_SAVER, "Error getting modification: %s",
                          error->message);
  ...
  if (info != NULL &&
    ...
    if (g_file_info_has_attribute(info, G_FILE_ATTRIBUTE_TIME_MODIFIED_USEC)) {
      guint32 usec = g_file_info_get_attribute_uint32(
...
static void check_modified_async(AsyncData *async) {
  ...
  g_file_query_info_async(
      async->saver->priv->gfile,
      G_FILE_ATTRIBUTE_TIME_MODIFIED "," G_FILE_ATTRIBUTE_TIME_MODIFIED_USEC,
      G_FILE_QUERY_INFO_NONE, G_PRIORITY_HIGH, async->cancellable,
      (GAsyncReadyCallback)check_modification_callback, async);
}

static gboolean save_remote_file_real(PlumaDocumentSaver *saver) {
  AsyncData *async;
...
void pluma_document_saver_save(PlumaDocumentSaver *saver, gint64 *old_mtime) {
  pluma_debug(DEBUG_SAVER);
  ...
  if ((saver->priv->flags & PLUMA_DOCUMENT_SAVE_PRESERVE_BACKUP) != 0)
    ...
    saver->priv->keep_backup = g_settings_get_boolean(
        saver->priv->editor_settings, PLUMA_SETTINGS_CREATE_BACKUP_COPY);
  ...
  g_timeout_add_full(G_PRIORITY_HIGH, 0, (GSourceFunc)save_remote_file_real,
                     saver, NULL);
}

void pluma_document_saver_saving(PlumaDocumentSaver *saver, gboolean completed,
                                 GError *error) {
