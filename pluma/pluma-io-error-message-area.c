...
static void info_bar_add_icon_button_with_text(GtkInfoBar *infobar,
                                               const gchar *text,
                                               const gchar *icon_id,
                                               gint response_id) {
  GtkWidget *button;
...
static void set_message_area_text_and_icon(GtkWidget *message_area,
...
static GtkWidget *create_io_loading_error_message_area(
  ...
  gtk_button_set_image(
      GTK_BUTTON(gtk_info_bar_add_button(GTK_INFO_BAR(message_area),
  ...
  set_message_area_text_and_icon(message_area, "dialog-error", primary_text,
                                 secondary_text);

  if (recoverable_error) {
    info_bar_add_icon_button_with_text(GTK_INFO_BAR(message_area), _("_Retry"),
                                       "view-refresh", GTK_RESPONSE_OK);
  }

  return message_area;
}

static gboolean parse_gio_error(gint code, gchar **error_message,
                                gchar **message_details, const gchar *uri,
  ...
  switch (code) {
    ...
    case G_IO_ERROR_NOT_DIRECTORY:
      *error_message =
          g_strdup_printf(_("Could not find the file %s."), uri_for_display);
      *message_details =
    ...
    case G_IO_ERROR_NOT_SUPPORTED: {
      ...
      if ((scheme_string != NULL) && g_utf8_validate(scheme_string, -1, NULL)) {
        ...
        *message_details = g_strdup_printf(
            _("pluma cannot handle %s locations."), scheme_markup);
    ...
    case G_IO_ERROR_NOT_MOUNTABLE_FILE:
      *message_details =
          g_strdup(_("The location of the file cannot be mounted."));
      break;

    case G_IO_ERROR_NOT_MOUNTED:
      *message_details =
          g_strdup(_("The location of the file cannot be accessed because it "
                     "is not mounted."));
    ...
    case G_IO_ERROR_INVALID_FILENAME:
      *error_message =
          g_strdup_printf(_("%s is not a valid location."), uri_for_display);
      *message_details =
          g_strdup(_("Please check that you typed the "
                     "location correctly and try again."));
      break;
       ...
       */
      {
        ...
        if (pluma_utils_decode_uri(uri, NULL, NULL, &hn, NULL, NULL)) {
          if (hn != NULL) {
            ...
            *message_details =
                g_strdup_printf(_("Host %s could not be found. "
        ...
        if (!*message_details) {
          ...
          *message_details =
              g_strdup_printf(_("Hostname was invalid. "
...
static gboolean parse_pluma_error(gint code, gchar **error_message,
                                  gchar **message_details, const gchar *uri,
                                  const gchar *uri_for_display) {
  gboolean ret = TRUE;
...
static void parse_error(const GError *error, gchar **error_message,
                        gchar **message_details, const gchar *uri,
                        const gchar *uri_for_display) {
  gboolean ret = FALSE;

  if (error->domain == G_IO_ERROR) {
    ret = parse_gio_error(error->code, error_message, message_details, uri,
                          uri_for_display);
  } else if (error->domain == PLUMA_DOCUMENT_ERROR) {
    ret = parse_pluma_error(error->code, error_message, message_details, uri,
                            uri_for_display);
  }

  if (!ret) {
    g_warning("Hit unhandled case %d (%s) in %s.", error->code, error->message,
              G_STRFUNC);
    *message_details =
...
GtkWidget *pluma_unrecoverable_reverting_error_message_area_new(
  ...
  g_return_val_if_fail(
      (error->domain == PLUMA_DOCUMENT_ERROR) || (error->domain == G_IO_ERROR),
      NULL);
   ...
   */
  temp_uri_for_display = pluma_utils_str_middle_truncate(
  ...
  if (is_gio_error(error, G_IO_ERROR_NOT_FOUND)) {
    message_details =
        g_strdup(_("pluma cannot find the file. "
  ...
  if (error_message == NULL) {
    error_message =
        g_strdup_printf(_("Could not revert the file %s."), uri_for_display);
  }
...
static void create_combo_box(GtkWidget *message_area, GtkWidget *vbox) {
  GtkWidget *hbox;
  GtkWidget *label;
  GtkWidget *menu;
  gchar *label_markup;
  ...
  g_object_set_data(G_OBJECT(message_area), "pluma-message-area-encoding-menu",
                    menu);
...
static GtkWidget *create_conversion_error_message_area(
    const gchar *primary_text, const gchar *secondary_text,
...
GtkWidget *pluma_io_loading_error_message_area_new(
  ...
  if (is_gio_error(error, G_IO_ERROR_TOO_MANY_LINKS)) {
    ...
    message_details = g_strconcat(
        _("pluma has not been able to detect "
  ...
  if (error_message == NULL) {
    error_message =
        g_strdup_printf(_("Could not open the file %s."), uri_for_display);
  }
  ...
  if (convert_error) {
    ...
    message_area = create_io_loading_error_message_area(
...
GtkWidget *pluma_file_already_open_warning_message_area_new(const gchar *uri) {
  ...
  primary_text = g_strdup_printf(
      _("This file (%s) is already open in another pluma window."),
      uri_for_display);
  g_free(uri_for_display);
  ...
  secondary_text =
      _("pluma opened this instance of the file in a non-editable way. "
...
GtkWidget *pluma_externally_modified_saving_error_message_area_new(
  ...
  g_return_val_if_fail(error->code == PLUMA_DOCUMENT_ERROR_EXTERNALLY_MODIFIED,
                       NULL);
  ...
  info_bar_add_icon_button_with_text(GTK_INFO_BAR(message_area),
                                     _("S_ave Anyway"), "document-save",
                                     GTK_RESPONSE_YES);
  gtk_info_bar_add_button(GTK_INFO_BAR(message_area), _("D_on't Save"),
                          GTK_RESPONSE_CANCEL);
  gtk_info_bar_set_message_type(GTK_INFO_BAR(message_area),
                                GTK_MESSAGE_WARNING);
  ...
  // "modification" could be interpreted as the changes he made in the document.
  // beside "reading" is not accurate (since last load/save)
  primary_text = g_strdup_printf(
      _("The file %s has been modified since reading it."), uri_for_display);
  g_free(uri_for_display);
...
GtkWidget *pluma_no_backup_saving_error_message_area_new(const gchar *uri,
  ...
  create_backup_copy = g_settings_get_boolean(
      editor_settings, PLUMA_SETTINGS_CREATE_BACKUP_COPY);
  g_object_unref(editor_settings);

  // FIXME: review this messages
  if (create_backup_copy)
    primary_text = g_strdup_printf(
        _("Could not create a backup file while saving %s"), uri_for_display);
  else
    primary_text = g_strdup_printf(
  ...
  secondary_text =
      _("pluma could not back up the old copy of the file before saving the "
        "new one. "
...
GtkWidget *pluma_unrecoverable_saving_error_message_area_new(
  ...
  if (is_gio_error(error, G_IO_ERROR_NOT_SUPPORTED)) {
    ...
    if ((scheme_string != NULL) && g_utf8_validate(scheme_string, -1, NULL)) {
      ...
      /* Translators: %s is a URI scheme (like for example http:, ftp:, etc.) */
      message_details =
          g_strdup_printf(_("pluma cannot handle %s locations in write mode. "
      ...
      message_details =
          g_strdup(_("pluma cannot handle this location in write mode. "
                     "Please check that you typed the "
                     "location correctly and try again."));
    }
    ...
    message_details =
        g_strdup(_("%s is not a valid location. "
                   "Please check that you typed the "
                   "location correctly and try again."));
  } else if (is_gio_error(error, G_IO_ERROR_PERMISSION_DENIED)) {
    message_details = g_strdup(
  ...
  if (error_message == NULL) {
    error_message =
        g_strdup_printf(_("Could not save the file %s."), uri_for_display);
  }
...
GtkWidget *pluma_externally_modified_message_area_new(
  ...
  info_bar_add_icon_button_with_text(GTK_INFO_BAR(message_area), _("_Reload"),
                                     "view-refresh", GTK_RESPONSE_OK);
  ...
  set_message_area_text_and_icon(message_area, "gtk-dialog-warning",
