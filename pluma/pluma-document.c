...
#define GTK_SOURCE_CHECK_VERSION(major, minor, micro) \
  (GTK_SOURCE_MAJOR_VERSION > (major) ||              \
   (GTK_SOURCE_MAJOR_VERSION == (major) &&            \
    GTK_SOURCE_MINOR_VERSION > (minor)) ||            \
   (GTK_SOURCE_MAJOR_VERSION == (major) &&            \
    GTK_SOURCE_MINOR_VERSION == (minor) &&            \
    GTK_SOURCE_MICRO_VERSION >= (micro)))

static void pluma_document_load_real(PlumaDocument *doc, const gchar *uri,
                                     const PlumaEncoding *encoding,
                                     gint line_pos, gboolean create);
static void pluma_document_save_real(PlumaDocument *doc, const gchar *uri,
                                     const PlumaEncoding *encoding,
                                     PlumaDocumentSaveFlags flags);
static void to_search_region_range(PlumaDocument *doc, GtkTextIter *start,
                                   GtkTextIter *end);
static void insert_text_cb(PlumaDocument *doc, GtkTextIter *pos,
                           const gchar *text, gint length);

static void delete_range_cb(PlumaDocument *doc, GtkTextIter *start,
                            GtkTextIter *end);
...
G_DEFINE_TYPE_WITH_PRIVATE(PlumaDocument, pluma_document,
...
static gint get_untitled_number(void) {
  ...
  while (TRUE) {
    if (g_hash_table_lookup(allocated_untitled_numbers, GINT_TO_POINTER(i)) ==
        NULL) {
      g_hash_table_insert(allocated_untitled_numbers, GINT_TO_POINTER(i),
                          GINT_TO_POINTER(i));
...
static void pluma_document_dispose(GObject *object) {
...
static void pluma_document_finalize(GObject *object) {
  PlumaDocument *doc = PLUMA_DOCUMENT(object);
...
static void pluma_document_get_property(GObject *object, guint prop_id,
  ...
  switch (prop_id) {
    ...
    case PROP_SHORTNAME:
      g_value_take_string(value,
                          pluma_document_get_short_name_for_display(doc));
      break;
...
static void pluma_document_set_property(GObject *object, guint prop_id,
                                        const GValue *value,
                                        GParamSpec *pspec) {
  PlumaDocument *doc = PLUMA_DOCUMENT(object);
  ...
  switch (prop_id) {
    case PROP_ENABLE_SEARCH_HIGHLIGHTING:
      ...
      break;
    case PROP_NEWLINE_TYPE:
      pluma_document_set_newline_type(doc, g_value_get_enum(value));
      break;
    case PROP_SHORTNAME:
      pluma_document_set_short_name_for_display(doc, g_value_get_string(value));
      break;
    case PROP_CONTENT_TYPE:
      pluma_document_set_content_type(doc, g_value_get_string(value));
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID(object, prop_id, pspec);
      break;
...
static void pluma_document_changed(GtkTextBuffer *buffer) {
  emit_cursor_moved(PLUMA_DOCUMENT(buffer));

  GTK_TEXT_BUFFER_CLASS(pluma_document_parent_class)->changed(buffer);
}

static void pluma_document_class_init(PlumaDocumentClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string("uri", "URI", "The document's URI", NULL,
                          G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));

  g_object_class_install_property(
      object_class, PROP_SHORTNAME,
      g_param_spec_string("shortname", "Short Name",
                          "The document's short name", NULL,
                          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  g_object_class_install_property(
      object_class, PROP_CONTENT_TYPE,
      g_param_spec_string("content-type", "Content Type",
                          "The document's Content Type", NULL,
                          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  g_object_class_install_property(
      object_class, PROP_MIME_TYPE,
      g_param_spec_string("mime-type", "MIME Type", "The document's MIME Type",
                          "text/plain",
                          G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));
  ...
  g_object_class_install_property(
      ...
      g_param_spec_boolean("read-only", "Read Only",
                           "Whether the document is read only or not", FALSE,
                           G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));

  g_object_class_install_property(
      object_class, PROP_ENCODING,
      g_param_spec_boxed(
          "encoding", "Encoding", "The PlumaEncoding used for the document",
          PLUMA_TYPE_ENCODING, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));

  g_object_class_install_property(
      object_class, PROP_CAN_SEARCH_AGAIN,
      g_param_spec_boolean(
          "can-search-again", "Can search again",
          "Whether it's possible to search again in the document", FALSE,
          G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));

  g_object_class_install_property(
      object_class, PROP_ENABLE_SEARCH_HIGHLIGHTING,
      g_param_spec_boolean("enable-search-highlighting",
                           "Enable Search Highlighting",
                           "Whether all the occurrences of the searched string "
                           "must be highlighted",
  ...
  g_object_class_install_property(
      ...
      g_param_spec_enum(
          "newline-type", "Newline type", "The accepted types of line ending",
          PLUMA_TYPE_DOCUMENT_NEWLINE_TYPE, PLUMA_DOCUMENT_NEWLINE_TYPE_LF,
  ...
  document_signals[CURSOR_MOVED] = g_signal_new(
      ...
      G_STRUCT_OFFSET(PlumaDocumentClass, cursor_moved), NULL, NULL, NULL,
      G_TYPE_NONE, 0);
  ...
  document_signals[LOAD] =
      g_signal_new("load", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(PlumaDocumentClass, load), NULL, NULL, NULL,
                   G_TYPE_NONE, 4, G_TYPE_STRING,
                   /* we rely on the fact that the PlumaEncoding pointer stays
                    * the same forever */
                   PLUMA_TYPE_ENCODING | G_SIGNAL_TYPE_STATIC_SCOPE, G_TYPE_INT,
                   G_TYPE_BOOLEAN);

  document_signals[LOADING] = g_signal_new(
      "loading", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PlumaDocumentClass, loading), NULL, NULL, NULL,
      G_TYPE_NONE, 2, G_TYPE_UINT64, G_TYPE_UINT64);

  document_signals[LOADED] = g_signal_new(
      "loaded", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PlumaDocumentClass, loaded), NULL, NULL, NULL,
  ...
  document_signals[SAVE] =
      g_signal_new("save", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(PlumaDocumentClass, save), NULL, NULL, NULL,
  ...
  document_signals[SAVING] = g_signal_new(
      "saving", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PlumaDocumentClass, saving), NULL, NULL, NULL,
      G_TYPE_NONE, 2, G_TYPE_UINT64, G_TYPE_UINT64);

  document_signals[SAVED] = g_signal_new(
      "saved", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PlumaDocumentClass, saved), NULL, NULL, NULL, G_TYPE_NONE,
      1, G_TYPE_POINTER);

  document_signals[SEARCH_HIGHLIGHT_UPDATED] = g_signal_new(
      "search_highlight_updated", G_OBJECT_CLASS_TYPE(object_class),
      G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PlumaDocumentClass, search_highlight_updated), NULL, NULL,
      NULL, G_TYPE_NONE, 2, GTK_TYPE_TEXT_ITER | G_SIGNAL_TYPE_STATIC_SCOPE,
      GTK_TYPE_TEXT_ITER | G_SIGNAL_TYPE_STATIC_SCOPE);
}

#if !GTK_SOURCE_CHECK_VERSION(4, 3, 1)
static gboolean file_with_bom(GFile *file) {
  FILE *testfile;
  gchar c;
  int i;
  gchar bom[3];
  ...
  if ((bom[0] == '\357') && (bom[1] == '\273') && (bom[2] == '\277'))
    return TRUE;
  else
    return FALSE;
}
...
static void set_language(PlumaDocument *doc, GtkSourceLanguage *lang,
  ...
  if (new_lang_id) is_bom_lang = g_strv_contains(bom_langs, new_lang_id);
  ...
  if (set_by_user && (doc->priv->uri != NULL)) {
    pluma_document_set_metadata(
        doc, PLUMA_METADATA_ATTRIBUTE_LANGUAGE,
        (lang == NULL) ? "_NORMAL_" : gtk_source_language_get_id(lang), NULL);
  }

  doc->priv->language_set_by_user = set_by_user;
}

static void set_encoding(PlumaDocument *doc, const PlumaEncoding *encoding,
                         gboolean set_by_user) {
  g_return_if_fail(encoding != NULL);

  pluma_debug(DEBUG_DOCUMENT);

  if (doc->priv->encoding == encoding) return;
  ...
  if (set_by_user) {
    ...
    pluma_document_set_metadata(doc, PLUMA_METADATA_ATTRIBUTE_ENCODING, charset,
                                NULL);
  }
...
static GtkSourceStyleScheme *get_default_style_scheme(
  ...
  scheme_id =
      g_settings_get_string(editor_settings, PLUMA_SETTINGS_COLOR_SCHEME);
  def_style = gtk_source_style_scheme_manager_get_scheme(manager, scheme_id);

  if (def_style == NULL) {
    g_warning(
        "Default style scheme '%s' cannot be found, falling back to 'classic' "
        "style scheme ",
        scheme_id);
    ...
    if (def_style == NULL) {
      g_warning(
          "Style scheme 'classic' cannot be found, check your GtkSourceView "
          "installation.");
    }
  }
...
static void on_uri_changed(PlumaDocument *doc, GParamSpec *pspec,
                           gpointer useless) {
  ...
  if (location != NULL) {
    ...
    doc->priv->metadata_info = g_file_query_info(
        location, METADATA_QUERY, G_FILE_QUERY_INFO_NONE, NULL, &error);
...
static GtkSourceLanguage *guess_language(PlumaDocument *doc,
  ...
  if (data != NULL) {
    ...
    if (strcmp(data, "_NORMAL_") != 0) {
      language = gtk_source_language_manager_get_language(
          pluma_get_language_manager(), data);
    }
    ...
    language = gtk_source_language_manager_guess_language(
        pluma_get_language_manager(), basename, content_type);
...
static void on_content_type_changed(PlumaDocument *doc, GParamSpec *pspec,
                                    gpointer useless) {
  if (!doc->priv->language_set_by_user) {
...
static void pluma_document_init(PlumaDocument *doc) {
  ...
  undo_actions = g_settings_get_uint(doc->priv->editor_settings,
                                     PLUMA_SETTINGS_MAX_UNDO_ACTIONS);
  ...
  g_signal_connect_after(doc, "delete-range", G_CALLBACK(delete_range_cb),
                         NULL);

  g_signal_connect(doc, "notify::content-type",
...
static void set_content_type_no_guess(PlumaDocument *doc,
                                      const gchar *content_type) {
  pluma_debug(DEBUG_DOCUMENT);
...
static void set_content_type(PlumaDocument *doc, const gchar *content_type) {
  pluma_debug(DEBUG_DOCUMENT);
 ...
 */
void pluma_document_set_content_type(PlumaDocument *doc,
                                     const gchar *content_type) {
...
static void set_uri(PlumaDocument *doc, const gchar *uri) {
  pluma_debug(DEBUG_DOCUMENT);

  g_return_if_fail((uri == NULL) || pluma_utils_is_valid_uri(uri));

  if (uri != NULL) {
    if (doc->priv->uri == uri) return;
...
void pluma_document_set_uri(PlumaDocument *doc, const gchar *uri) {
  g_return_if_fail(PLUMA_IS_DOCUMENT(doc));
 ...
 * @name: (allow-none):
 */
void pluma_document_set_short_name_for_display(PlumaDocument *doc,
...
gchar *pluma_document_get_content_type(PlumaDocument *doc) {
  g_return_val_if_fail(PLUMA_IS_DOCUMENT(doc), NULL);
...
static gboolean set_readonly(PlumaDocument *doc, gboolean readonly) {
  pluma_debug(DEBUG_DOCUMENT);

  readonly = (readonly != FALSE);

  if (doc->priv->readonly == readonly) return FALSE;
 ...
 */
void _pluma_document_set_readonly(PlumaDocument *doc, gboolean readonly) {
  pluma_debug(DEBUG_DOCUMENT);
...
gboolean _pluma_document_check_externally_modified(PlumaDocument *doc) {
  GFile *gfile;
  GFileInfo *info;
  ...
  if (info != NULL) {
    ...
    if (g_file_info_has_attribute(info, G_FILE_ATTRIBUTE_TIME_MODIFIED)) {
      ...
      timeval = g_file_info_get_attribute_uint64(
      ...
      if (g_file_info_has_attribute(info,
        ...
        usec = g_file_info_get_attribute_uint32(
...
static void reset_temp_loading_data(PlumaDocument *doc) {
...
static void document_loader_loaded(PlumaDocumentLoader *loader,
                                   const GError *error, PlumaDocument *doc) {
  /* load was successful */
  if (error == NULL ||
    ...
    set_encoding(doc, pluma_document_loader_get_encoding(loader),
                 (doc->priv->requested_encoding != NULL));

    set_content_type(doc, content_type);

    pluma_document_set_newline_type(
        doc, pluma_document_loader_get_newline_type(loader));

    restore_cursor = g_settings_get_boolean(
        doc->priv->editor_settings, PLUMA_SETTINGS_RESTORE_CURSOR_POSITION);
    ...
    /* else, if enabled, to the position stored in the metadata */
      ...
      gtk_text_buffer_get_iter_at_offset(GTK_TEXT_BUFFER(doc), &iter,
                                         MAX(offset, 0));
...
static void document_loader_loading(PlumaDocumentLoader *loader,
                                    gboolean completed, const GError *error,
                                    PlumaDocument *doc) {
  if (completed) {
...
static void pluma_document_load_real(PlumaDocument *doc, const gchar *uri,
  ...
  g_signal_connect(doc->priv->loader, "loading",
                   G_CALLBACK(document_loader_loading), doc);
 ...
 */
void pluma_document_load(PlumaDocument *doc, const gchar *uri,
...
gboolean pluma_document_load_cancel(PlumaDocument *doc) {
  ...
  if (doc->priv->loader == NULL) return FALSE;

  return pluma_document_loader_cancel(doc->priv->loader);
}

static void document_saver_saving(PlumaDocumentSaver *saver, gboolean completed,
                                  const GError *error, PlumaDocument *doc) {
  pluma_debug(DEBUG_DOCUMENT);
...
static void pluma_document_save_real(PlumaDocument *doc, const gchar *uri,
  ...
  doc->priv->saver = pluma_document_saver_new(doc, uri, encoding,
                                              doc->priv->newline_type, flags);

  g_signal_connect(doc->priv->saver, "saving",
                   G_CALLBACK(document_saver_saving), doc);

  doc->priv->requested_encoding = encoding;

  pluma_document_saver_save(doc->priv->saver, &doc->priv->mtime);
}
 ...
 */
void pluma_document_save(PlumaDocument *doc, PlumaDocumentSaveFlags flags) {
  g_return_if_fail(PLUMA_IS_DOCUMENT(doc));
  g_return_if_fail(doc->priv->uri != NULL);

  g_signal_emit(doc, document_signals[SAVE], 0, doc->priv->uri,
                doc->priv->encoding, flags);
}
 ...
 * to be emitted.
 */
void pluma_document_save_as(PlumaDocument *doc, const gchar *uri,
                            const PlumaEncoding *encoding,
                            PlumaDocumentSaveFlags flags) {
  g_return_if_fail(PLUMA_IS_DOCUMENT(doc));
  g_return_if_fail(uri != NULL);
  ...
  g_signal_emit(doc, document_signals[SAVE], 0, uri, encoding,
                flags | PLUMA_DOCUMENT_SAVE_IGNORE_MTIME);
}

gboolean pluma_document_insert_file(PlumaDocument *doc, GtkTextIter *iter,
                                    const gchar *uri,
                                    const PlumaEncoding *encoding) {
  g_return_val_if_fail(PLUMA_IS_DOCUMENT(doc), FALSE);
  g_return_val_if_fail(iter != NULL, FALSE);
  g_return_val_if_fail(gtk_text_iter_get_buffer(iter) == GTK_TEXT_BUFFER(doc),
                       FALSE);
 ...
 */
gboolean pluma_document_goto_line(PlumaDocument *doc, gint line) {
  gboolean ret = TRUE;
  guint line_count;
  GtkTextIter iter;
...
gboolean pluma_document_goto_line_offset(PlumaDocument *doc, gint line,
                                         gint line_offset) {
  gboolean ret = TRUE;
  guint offset_count;
  GtkTextIter iter;
...
static gint compute_num_of_lines(const gchar *text) {
  const gchar *p;
  gint len;
  gint n = 1;
 ...
 * @flags:
 **/
void pluma_document_set_search_text(PlumaDocument *doc, const gchar *text,
                                    guint flags) {
  ...
  if (text != NULL) {
    ...
    doc->priv->num_of_lines_search_text =
        compute_num_of_lines(doc->priv->search_text);
    update_to_search_region = TRUE;
  }

  if (!PLUMA_SEARCH_IS_DONT_SET_FLAGS(flags)) {
    if (doc->priv->search_flags != flags) update_to_search_region = TRUE;
  ...
  if (notify) g_object_notify(G_OBJECT(doc), "can-search-again");
}
 ...
 * @flags: (allow-none):
 */
gchar *pluma_document_get_search_text(PlumaDocument *doc, guint *flags) {
  g_return_val_if_fail(PLUMA_IS_DOCUMENT(doc), NULL);
 ...
 * @text: (allow-none):
 **/
void pluma_document_set_last_replace_text(PlumaDocument *doc,
                                          const gchar *text) {
  g_return_if_fail(PLUMA_IS_DOCUMENT(doc));

  g_free(doc->priv->last_replace_text);

  pluma_debug_message(DEBUG_SEARCH, "last_replace_text = %s",
                      text == NULL ? "NULL" : text);
 ...
 * @doc:
 */
gchar *pluma_document_get_last_replace_text(PlumaDocument *doc) {
  g_return_val_if_fail(PLUMA_IS_DOCUMENT(doc), NULL);
...
gboolean pluma_document_search_forward(PlumaDocument *doc,
  ...
  g_return_val_if_fail(PLUMA_IS_DOCUMENT(doc), FALSE);
  g_return_val_if_fail((start == NULL) || (gtk_text_iter_get_buffer(start) ==
                                           GTK_TEXT_BUFFER(doc)),
                       FALSE);
  g_return_val_if_fail(
      (end == NULL) || (gtk_text_iter_get_buffer(end) == GTK_TEXT_BUFFER(doc)),
      FALSE);
  ...
  while (!found) {
    if (!PLUMA_SEARCH_IS_MATCH_REGEX(doc->priv->search_flags)) {
      found = gtk_text_iter_forward_search(&iter, doc->priv->search_text,
                                           search_flags, &m_start, &m_end, end);
    } else {
      found = pluma_gtk_text_iter_regex_search(
          &iter, doc->priv->search_text, search_flags, &m_start, &m_end, end,
          TRUE, &doc->priv->last_replace_text);
    }
  ...
  if (found && (match_start != NULL)) *match_start = m_start;

  if (found && (match_end != NULL)) *match_end = m_end;
  ...
  while (!found) {
    if (!PLUMA_SEARCH_IS_MATCH_REGEX(doc->priv->search_flags)) {
      found = gtk_text_iter_backward_search(
          &iter, doc->priv->search_text, search_flags, &m_start, &m_end, start);
    } else {
      found = pluma_gtk_text_iter_regex_search(
          &iter, doc->priv->search_text, search_flags, &m_start, &m_end, start,
          FALSE, &doc->priv->last_replace_text);
    }
...
gint pluma_document_replace_all(PlumaDocument *doc, const gchar *find,
  ...
  brackets_highlighting = gtk_source_buffer_get_highlight_matching_brackets(
  ...
  do {
    if (!PLUMA_SEARCH_IS_MATCH_REGEX(flags)) {
      found = gtk_text_iter_forward_search(&iter, search_text, search_flags,
                                           &m_start, &m_end, NULL);
    } else {
      g_free(replace_text);
      replace_text = g_strdup(replace);
      found = pluma_gtk_text_iter_regex_search(&iter, search_text, search_flags,
                                               &m_start, &m_end, NULL, TRUE,
                                               &replace_text);
      replace_text_len = strlen(replace_text);
    }
 ...
 * @lang: (allow-none):
 **/
void pluma_document_set_language(PlumaDocument *doc, GtkSourceLanguage *lang) {
  g_return_if_fail(PLUMA_IS_DOCUMENT(doc));
 ...
 */
GtkSourceLanguage *pluma_document_get_language(PlumaDocument *doc) {
  g_return_val_if_fail(PLUMA_IS_DOCUMENT(doc), NULL);

  return gtk_source_buffer_get_language(GTK_SOURCE_BUFFER(doc));
}

const PlumaEncoding *pluma_document_get_encoding(PlumaDocument *doc) {
  g_return_val_if_fail(PLUMA_IS_DOCUMENT(doc), NULL);

  return doc->priv->encoding;
}

glong _pluma_document_get_seconds_since_last_save_or_load(PlumaDocument *doc) {
  pluma_debug(DEBUG_DOCUMENT);

  g_return_val_if_fail(PLUMA_IS_DOCUMENT(doc), -1);

  return ((g_get_real_time() - doc->priv->time_of_last_save_or_load) /
          G_USEC_PER_SEC);
}

static void get_search_match_colors(PlumaDocument *doc,
  ...
  if (style_scheme == NULL) goto fallback;

  style = gtk_source_style_scheme_get_style(style_scheme, "search-match");
  if (style == NULL) goto fallback;

  g_object_get(style, "foreground-set", foreground_set, "foreground", &fg,
               "background-set", background_set, "background", &bg, NULL);
...
static void sync_found_tag(PlumaDocument *doc, GParamSpec *pspec,
                           gpointer data) {
  GdkRGBA fg;
  GdkRGBA bg;
  gboolean fg_set;
  ...
  g_object_set(doc->priv->found_tag, "foreground-rgba", fg_set ? &fg : NULL,
               NULL);
  g_object_set(doc->priv->found_tag, "background-rgba", bg_set ? &bg : NULL,
               NULL);
}

static void text_tag_set_highest_priority(GtkTextTag *tag,
                                          GtkTextBuffer *buffer) {
...
static void search_region(PlumaDocument *doc, GtkTextIter *start,
                          GtkTextIter *end) {
  GtkTextIter iter;
  GtkTextIter m_start;
  GtkTextIter m_end;
  ...
  if (doc->priv->found_tag == NULL) {
    doc->priv->found_tag =
        gtk_text_buffer_create_tag(GTK_TEXT_BUFFER(doc), "found", NULL);

    sync_found_tag(doc, NULL, NULL);

    g_signal_connect(doc, "notify::style-scheme", G_CALLBACK(sync_found_tag),
                     NULL);
  }
  ...
  if (doc->priv->search_text == NULL) return;
  ...
  gtk_text_iter_get_offset (start), gtk_text_iter_get_line (end),
  gtk_text_iter_get_offset (end));
  */

  gtk_text_buffer_remove_tag(buffer, doc->priv->found_tag, start, end);

  if (*doc->priv->search_text == '\0') return;
  ...
  do {
    if ((end != NULL) && gtk_text_iter_is_end(end)) end = NULL;
...
static void to_search_region_range(PlumaDocument *doc, GtkTextIter *start,
  ...
  if (doc->priv->to_search_region == NULL) return;
...
void _pluma_document_search_region(PlumaDocument *doc, const GtkTextIter *start,
                                   const GtkTextIter *end) {
  ...
  */
  ...
  region = pluma_text_region_intersect(doc->priv->to_search_region, start, end);
  if (region) {
    gint i;
    ...
    pluma_text_region_nth_subregion(region, 0, &start_search, NULL);

    pluma_text_region_nth_subregion(region, i - 1, NULL, &end_search);
    ...
    pluma_text_region_subtract(doc->priv->to_search_region, start, end);
  }
}
...
void pluma_document_set_enable_search_highlighting(PlumaDocument *doc,
  ...
  if ((doc->priv->to_search_region != NULL) == enable) return;
  ...
  if (doc->priv->to_search_region != NULL) {
    ...
    if (doc->priv->found_tag != NULL) {
      ...
      gtk_text_buffer_remove_tag(GTK_TEXT_BUFFER(doc), doc->priv->found_tag,
                                 &begin, &end);
    }

    pluma_text_region_destroy(doc->priv->to_search_region, TRUE);
...
PlumaDocumentNewlineType pluma_document_get_newline_type(PlumaDocument *doc) {
  g_return_val_if_fail(PLUMA_IS_DOCUMENT(doc), 0);
...
void _pluma_document_set_mount_operation_factory(
    PlumaDocument *doc, PlumaMountOperationFactory callback,
    gpointer userdata) {
  g_return_if_fail(PLUMA_IS_DOCUMENT(doc));
...
GMountOperation *_pluma_document_create_mount_operation(PlumaDocument *doc) {
  g_return_val_if_fail(PLUMA_IS_DOCUMENT(doc), NULL);

  if (doc->priv->mount_operation_factory == NULL)
    return g_mount_operation_new();
  else
    return doc->priv->mount_operation_factory(
        doc, doc->priv->mount_operation_userdata);
}

#ifndef ENABLE_GVFS_METADATA
gchar *pluma_document_get_metadata(PlumaDocument *doc, const gchar *key) {
  gchar *value = NULL;
...
void pluma_document_set_metadata(PlumaDocument *doc, const gchar *first_key,
                                 ...) {
  const gchar *key;
  const gchar *value;
  va_list var_args;
  ...
  for (key = first_key; key; key = va_arg(var_args, const gchar *)) {
    ...
    pluma_metadata_manager_set(doc->priv->uri, key, value);
  }
...
gchar *pluma_document_get_metadata(PlumaDocument *doc, const gchar *key) {
  ...
  if (doc->priv->metadata_info &&
    ...
    value = g_strdup(
        g_file_info_get_attribute_string(doc->priv->metadata_info, key));
  }

  return value;
}

static void set_attributes_cb(GObject *source, GAsyncResult *res,
                              gpointer useless) {
...
void pluma_document_set_metadata(PlumaDocument *doc, const gchar *first_key,
  ...
  if (location != NULL) {
    g_file_set_attributes_async(location, info, G_FILE_QUERY_INFO_NONE,
                                G_PRIORITY_DEFAULT, NULL, set_attributes_cb,
                                NULL);
