...
static void last_search_data_restore_position(PlumaSearchDialog *dlg) {
  LastSearchData *data;
...
static void last_search_data_store_position(PlumaSearchDialog *dlg) {
  LastSearchData *data;
  ...
  if (data == NULL) {
    ...
    g_object_set_data_full(G_OBJECT(dlg), PLUMA_LAST_SEARCH_DATA_KEY, data,
...
static void text_found(PlumaWindow *window, gint occurrences) {
  if (occurrences > 1) {
...
static void text_not_found(PlumaWindow *window, const gchar *text) {
  gchar *searched;
...
static gboolean run_search(PlumaView *view, gboolean wrap_around,
  ...
  if (!search_backwards) {
    gtk_text_buffer_get_selection_bounds(GTK_TEXT_BUFFER(doc), NULL,
                                         &start_iter);

    found = pluma_document_search_forward(doc, &start_iter, NULL, &match_start,
                                          &match_end);
  } else {
    gtk_text_buffer_get_selection_bounds(GTK_TEXT_BUFFER(doc), &start_iter,
                                         NULL);

    found = pluma_document_search_backward(doc, NULL, &start_iter, &match_start,
                                           &match_end);
  }

  if (!found && wrap_around) {
    if (!search_backwards)
      found = pluma_document_search_forward(doc, NULL,
...
static void do_find(PlumaSearchDialog *dialog, PlumaWindow *window) {
  ...
  if (!parse_escapes) {
    entry_text = pluma_utils_escape_search_text(
        pluma_search_dialog_get_search_text(dialog));
  } else {
  ...
  if (match_regex) {
    pluma_document_set_last_replace_text(
        doc, pluma_search_dialog_get_replace_text(dialog));
  }
  ...
  gtk_dialog_set_response_sensitive(
      GTK_DIALOG(dialog), PLUMA_SEARCH_DIALOG_REPLACE_RESPONSE, found);
}
...
static void replace_selected_text(GtkTextBuffer *buffer, const gchar *replace) {
...
static void do_replace(PlumaSearchDialog *dialog, PlumaWindow *window) {
  PlumaDocument *doc;
  ...
  parse_escapes = pluma_search_dialog_get_parse_escapes(dialog);
  if (!parse_escapes) {
    search_entry_text = pluma_utils_escape_search_text(
        pluma_search_dialog_get_search_text(dialog));
  } else {
  ...
  if (!parse_escapes) {
    replace_entry_text = pluma_utils_escape_search_text(
        pluma_search_dialog_get_replace_text(dialog));
  } else {
  ...
  if (selected_text != NULL) {
    if (!match_regex) {
      need_refind =
          ...
          (!match_case &&
           !g_utf8_caselessnmatch(selected_text, unescaped_search_text,
                                  strlen(selected_text),
                                  strlen(unescaped_search_text)) != 0);
    } else {
      need_refind = !g_regex_match_simple(unescaped_search_text, selected_text,
  ...
  if (!match_regex)
    unescaped_replace_text =
        pluma_utils_unescape_search_text(replace_entry_text);
  else
    unescaped_replace_text =
        g_strdup(pluma_document_get_last_replace_text(doc));
...
static void do_replace_all(PlumaSearchDialog *dialog, PlumaWindow *window) {
  PlumaView *active_view;
  PlumaDocument *doc;
  const gchar *search_entry_text;
  const gchar *replace_entry_text;
  ...
  count = pluma_document_replace_all(doc, search_entry_text, replace_entry_text,
                                     flags);
  ...
  if (count > 0) {
    ...
    if (!parse_escapes) {
      text_not_found(window,
                     pluma_utils_unescape_search_text(search_entry_text));
    } else {
...
static void search_dialog_response_cb(PlumaSearchDialog *dialog,
...
static void search_dialog_destroyed(PlumaWindow *window,
                                    PlumaSearchDialog *dialog) {
  pluma_debug(DEBUG_COMMANDS);

  g_object_set_data(G_OBJECT(window), PLUMA_SEARCH_DIALOG_KEY, NULL);
  g_object_set_data(G_OBJECT(dialog), PLUMA_LAST_SEARCH_DATA_KEY, NULL);
}

static GtkWidget *create_dialog(PlumaWindow *window, gboolean show_replace) {
  GtkWidget *dialog;

  dialog = pluma_search_dialog_new(GTK_WINDOW(window), show_replace);

  g_signal_connect(dialog, "response", G_CALLBACK(search_dialog_response_cb),
                   window);
  g_signal_connect(dialog, "delete-event",
                   G_CALLBACK(search_dialog_delete_event_cb), NULL);

  g_object_set_data(G_OBJECT(window), PLUMA_SEARCH_DIALOG_KEY, dialog);
...
void _pluma_cmd_search_find(GtkAction *action, PlumaWindow *window) {
  ...
  if (data == NULL) {
    ...
    if (pluma_search_dialog_get_show_replace(
            PLUMA_SEARCH_DIALOG(search_dialog)))
  ...
  selection_exists =
      get_selected_text(GTK_TEXT_BUFFER(doc), &find_text, &sel_len);
  ...
  if (selection_exists && find_text != NULL && sel_len < 80) {
     ...
     * box).
     */
    parse_escapes = pluma_search_dialog_get_parse_escapes(
        PLUMA_SEARCH_DIALOG(search_dialog));
    search_text =
        pluma_search_dialog_get_search_text(PLUMA_SEARCH_DIALOG(search_dialog));
    if (!(search_text != NULL &&
...
void _pluma_cmd_search_replace(GtkAction *action, PlumaWindow *window) {
  ...
  if (data == NULL) {
    ...
    /* turn the dialog into a find dialog if needed */
    if (!pluma_search_dialog_get_show_replace(
            PLUMA_SEARCH_DIALOG(replace_dialog)))
  ...
  if (selection_exists && find_text != NULL && sel_len < 80) {
    ...
    parse_escapes = pluma_search_dialog_get_parse_escapes(
        PLUMA_SEARCH_DIALOG(replace_dialog));
    search_text = pluma_search_dialog_get_search_text(
        PLUMA_SEARCH_DIALOG(replace_dialog));
    if (!(search_text != NULL &&
          !strcmp(pluma_utils_unescape_search_text(search_text), find_text) &&
          parse_escapes)) {
...
static void do_find_again(PlumaWindow *window, gboolean backward) {
  PlumaView *active_view;
  ...
  if (data != NULL)
    wrap_around =
        pluma_search_dialog_get_wrap_around(PLUMA_SEARCH_DIALOG(data));
...
void _pluma_cmd_search_incremental_search(GtkAction *action,
   ...
   * the corresponding binding.
   */
  gtk_bindings_activate(G_OBJECT(active_view), GDK_KEY_k, GDK_CONTROL_MASK);
}
