...
static void pluma_statusbar_dispose(GObject *object) {
...
static void pluma_statusbar_class_init(PlumaStatusbarClass *klass) {
...
static void pluma_statusbar_init(PlumaStatusbar *statusbar) {
  GtkWidget *hbox;
  ...
  gtk_box_pack_end(GTK_BOX(statusbar), statusbar->priv->overwrite_mode_label,
                   FALSE, TRUE, 0);
  ...
  gtk_box_pack_end(GTK_BOX(statusbar), statusbar->priv->cursor_position_label,
                   FALSE, TRUE, 0);
  ...
  statusbar->priv->print_image =
      gtk_image_new_from_icon_name("document-print", GTK_ICON_SIZE_MENU);

  gtk_widget_show(hbox);

  gtk_box_pack_start(GTK_BOX(hbox), statusbar->priv->load_image, FALSE, TRUE,
                     4);
  gtk_box_pack_start(GTK_BOX(hbox), statusbar->priv->save_image, FALSE, TRUE,
                     4);
  gtk_box_pack_start(GTK_BOX(hbox), statusbar->priv->print_image, FALSE, TRUE,
                     4);

  gtk_box_pack_start(GTK_BOX(statusbar), statusbar->priv->state_frame, FALSE,
                     TRUE, 0);
  ...
  error_image =
      gtk_image_new_from_icon_name("dialog-error", GTK_ICON_SIZE_MENU);
  ...
  gtk_event_box_set_visible_window(
      GTK_EVENT_BOX(statusbar->priv->error_event_box), FALSE);
  ...
  gtk_box_pack_start(GTK_BOX(statusbar), statusbar->priv->error_frame, FALSE,
                     TRUE, 0);

  gtk_box_reorder_child(GTK_BOX(statusbar), statusbar->priv->error_frame, 0);
}
...
void pluma_statusbar_set_cursor_position(PlumaStatusbar *statusbar, gint line,
  ...
  if ((line >= 0) || (col >= 0)) {
    /* Translators: "Ln" is an abbreviation for "Line", Col is an abbreviation
    for "Column". Please, use abbreviations if possible to avoid space problems.
  */
...
static gboolean remove_message_timeout(PlumaStatusbar *statusbar) {
...
void pluma_statusbar_flash_message(PlumaStatusbar *statusbar, guint context_id,
  ...
  statusbar->priv->flash_timeout = g_timeout_add(
...
void pluma_statusbar_set_window_state(PlumaStatusbar *statusbar,
  ...
  if (state & PLUMA_WINDOW_STATE_ERROR) {
    ...
    tip = g_strdup_printf(
        ngettext("There is a tab with errors", "There are %d tabs with errors",
                 num_of_errors),
        num_of_errors);
