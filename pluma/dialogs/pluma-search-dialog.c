...
G_DEFINE_TYPE_WITH_PRIVATE(PlumaSearchDialog, pluma_search_dialog,
                           GTK_TYPE_DIALOG)

enum { PROP_0, PROP_SHOW_REPLACE };

static void pluma_search_dialog_set_property(GObject *object, guint prop_id,
                                             const GValue *value,
                                             GParamSpec *pspec) {
  ...
  switch (prop_id) {
    case PROP_SHOW_REPLACE:
      pluma_search_dialog_set_show_replace(dlg, g_value_get_boolean(value));
      break;
    default:
...
static void pluma_search_dialog_get_property(GObject *object, guint prop_id,
...
static gboolean show_replace(PlumaSearchDialog *dlg) {
  pluma_search_dialog_set_show_replace(dlg, TRUE);

  return TRUE;
}

static void pluma_search_dialog_class_init(PlumaSearchDialogClass *klass) {
  ...
  dialog_signals[SHOW_REPLACE] =
      g_signal_new("show_replace", G_TYPE_FROM_CLASS(object_class),
                   G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                   G_STRUCT_OFFSET(PlumaSearchDialogClass, show_replace), NULL,
                   NULL, NULL, G_TYPE_BOOLEAN, 0);

  g_object_class_install_property(
      object_class, PROP_SHOW_REPLACE,
      g_param_spec_boolean("show-replace", "Show Replace",
...
static void search_text_entry_changed(GtkEditable *editable,
  ...
  if (*search_string != '\0') {
    ...
    gtk_dialog_set_response_sensitive(
        GTK_DIALOG(dialog), PLUMA_SEARCH_DIALOG_REPLACE_ALL_RESPONSE, TRUE);
  } else {
    gtk_dialog_set_response_sensitive(GTK_DIALOG(dialog),
                                      PLUMA_SEARCH_DIALOG_FIND_RESPONSE, FALSE);
    gtk_dialog_set_response_sensitive(
        GTK_DIALOG(dialog), PLUMA_SEARCH_DIALOG_REPLACE_RESPONSE, FALSE);
    gtk_dialog_set_response_sensitive(
        GTK_DIALOG(dialog), PLUMA_SEARCH_DIALOG_REPLACE_ALL_RESPONSE, FALSE);
  }
}

static void response_handler(PlumaSearchDialog *dialog, gint response_id,
                             gpointer data) {
  const gchar *str;
  ...
  switch (response_id) {
    ...
    case PLUMA_SEARCH_DIALOG_REPLACE_ALL_RESPONSE:
      ...
      if (*str != '\0') {
        ...
        pluma_history_entry_prepend_text(
            PLUMA_HISTORY_ENTRY(dialog->priv->replace_entry), text);
    ...
    case PLUMA_SEARCH_DIALOG_FIND_RESPONSE:
      ...
      if (*str != '\0') {
        ...
        text = pluma_utils_unescape_search_text(str);
        pluma_history_entry_prepend_text(
            PLUMA_HISTORY_ENTRY(dialog->priv->search_entry), text);
...
static void show_replace_widgets(PlumaSearchDialog *dlg,
                                 gboolean show_replace) {
  if (show_replace) {
...
static void pluma_search_dialog_init(PlumaSearchDialog *dlg) {
  GtkWidget *content;
  GtkWidget *error_widget;
  gboolean ret;
  gchar *root_objects[] = {"search_dialog_content", NULL};
  ...
  ret = pluma_utils_get_ui_objects(
      PLUMA_DATADIR "/ui/pluma-search-dialog.ui", root_objects, &error_widget,
      "search_dialog_content", &content, "grid", &dlg->priv->grid,
      "search_label", &dlg->priv->search_label, "replace_with_label",
      &dlg->priv->replace_label, "match_case_checkbutton",
      &dlg->priv->match_case_checkbutton, "match_regex_checkbutton",
      &dlg->priv->match_regex_checkbutton, "entire_word_checkbutton",
      &dlg->priv->entire_word_checkbutton, "search_backwards_checkbutton",
      &dlg->priv->backwards_checkbutton, "wrap_around_checkbutton",
      &dlg->priv->wrap_around_checkbutton, "parse_escapes_checkbutton",
      &dlg->priv->parse_escapes_checkbutton, NULL);
  ...
  dlg->priv->search_entry = pluma_history_entry_new("history-search-for", TRUE);
  ...
  gtk_grid_attach_next_to(GTK_GRID(dlg->priv->grid), dlg->priv->search_entry,
                          dlg->priv->search_label, GTK_POS_RIGHT, 1, 1);

  dlg->priv->replace_entry =
      pluma_history_entry_new("history-replace-with", TRUE);
  pluma_history_entry_set_escape_func(
  ...
  gtk_entry_set_activates_default(GTK_ENTRY(dlg->priv->replace_text_entry),
                                  TRUE);
  gtk_widget_show(dlg->priv->replace_entry);
  gtk_grid_attach_next_to(GTK_GRID(dlg->priv->grid), dlg->priv->replace_entry,
                          dlg->priv->replace_label, GTK_POS_RIGHT, 1, 1);
  ...
  gtk_button_set_image(
      GTK_BUTTON(dlg->priv->find_button),
  ...
  dlg->priv->replace_button =
      pluma_gtk_button_new_with_icon(_("_Replace"), "edit-find-replace");

  gtk_dialog_add_action_widget(GTK_DIALOG(dlg), dlg->priv->replace_all_button,
                               PLUMA_SEARCH_DIALOG_REPLACE_ALL_RESPONSE);
  gtk_dialog_add_action_widget(GTK_DIALOG(dlg), dlg->priv->replace_button,
                               PLUMA_SEARCH_DIALOG_REPLACE_RESPONSE);
  gtk_dialog_add_action_widget(GTK_DIALOG(dlg), dlg->priv->find_button,
  ...
  gtk_dialog_set_response_sensitive(GTK_DIALOG(dlg),
                                    PLUMA_SEARCH_DIALOG_FIND_RESPONSE, FALSE);
  gtk_dialog_set_response_sensitive(
      GTK_DIALOG(dlg), PLUMA_SEARCH_DIALOG_REPLACE_RESPONSE, FALSE);
  gtk_dialog_set_response_sensitive(
      GTK_DIALOG(dlg), PLUMA_SEARCH_DIALOG_REPLACE_ALL_RESPONSE, FALSE);
  ...
  g_signal_connect(dlg->priv->search_text_entry, "insert_text",
                   G_CALLBACK(insert_text_handler), NULL);
  g_signal_connect(dlg->priv->replace_text_entry, "insert_text",
                   G_CALLBACK(insert_text_handler), NULL);
  g_signal_connect(dlg->priv->search_text_entry, "changed",
...
GtkWidget *pluma_search_dialog_new(GtkWindow *parent, gboolean show_replace) {
  ...
  dlg = g_object_new(PLUMA_TYPE_SEARCH_DIALOG, "show-replace", show_replace,
                     NULL);
...
void pluma_search_dialog_set_show_replace(PlumaSearchDialog *dialog,
  ...
  if (dialog->priv->ui_error) return;
...
void pluma_search_dialog_set_search_text(PlumaSearchDialog *dialog,
  ...
  gtk_dialog_set_response_sensitive(
      GTK_DIALOG(dialog), PLUMA_SEARCH_DIALOG_FIND_RESPONSE, (*text != '\0'));
...
void pluma_search_dialog_set_match_case(PlumaSearchDialog *dialog,
  ...
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(dialog->priv->match_case_checkbutton), match_case);
}
...
gboolean pluma_search_dialog_get_match_case(PlumaSearchDialog *dialog) {
  ...
  return gtk_toggle_button_get_active(
      GTK_TOGGLE_BUTTON(dialog->priv->match_case_checkbutton));
}
...
void pluma_search_dialog_set_match_regex(PlumaSearchDialog *dialog,
  ...
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(dialog->priv->match_regex_checkbutton), match_case);
}
...
gboolean pluma_search_dialog_get_match_regex(PlumaSearchDialog *dialog) {
  ...
  return gtk_toggle_button_get_active(
      GTK_TOGGLE_BUTTON(dialog->priv->match_regex_checkbutton));
}
...
void pluma_search_dialog_set_entire_word(PlumaSearchDialog *dialog,
  ...
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(dialog->priv->entire_word_checkbutton), entire_word);
}
...
gboolean pluma_search_dialog_get_entire_word(PlumaSearchDialog *dialog) {
  ...
  return gtk_toggle_button_get_active(
      GTK_TOGGLE_BUTTON(dialog->priv->entire_word_checkbutton));
}
...
void pluma_search_dialog_set_backwards(PlumaSearchDialog *dialog,
  ...
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(dialog->priv->backwards_checkbutton), backwards);
}
...
gboolean pluma_search_dialog_get_backwards(PlumaSearchDialog *dialog) {
  ...
  return gtk_toggle_button_get_active(
      GTK_TOGGLE_BUTTON(dialog->priv->backwards_checkbutton));
}
...
void pluma_search_dialog_set_wrap_around(PlumaSearchDialog *dialog,
  ...
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(dialog->priv->wrap_around_checkbutton), wrap_around);
}
...
gboolean pluma_search_dialog_get_wrap_around(PlumaSearchDialog *dialog) {
  ...
  return gtk_toggle_button_get_active(
      GTK_TOGGLE_BUTTON(dialog->priv->wrap_around_checkbutton));
}
...
void pluma_search_dialog_set_parse_escapes(PlumaSearchDialog *dialog,
  ...
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(dialog->priv->parse_escapes_checkbutton),
      parse_escapes);
}
