...
G_DEFINE_TYPE_WITH_PRIVATE(PlumaPreferencesDialog, pluma_preferences_dialog,
                           GTK_TYPE_DIALOG)

static void pluma_preferences_dialog_dispose(GObject *object) {
...
static void pluma_preferences_dialog_class_init(
    PlumaPreferencesDialogClass *klass) {
...
static void dialog_response_handler(GtkDialog *dlg, gint res_id) {
  ...
  switch (res_id) {
    case GTK_RESPONSE_HELP:
      pluma_help_display(GTK_WINDOW(dlg), NULL, "pluma-prefs");
...
static void on_auto_save_changed(GSettings *settings, const gchar *key,
...
static void draw_spaces_checkbutton_toggled(GtkToggleButton *button,
                                            PlumaPreferencesDialog *dlg) {
  ...
  g_return_if_fail(button ==
                   GTK_TOGGLE_BUTTON(dlg->priv->draw_spaces_checkbutton));
  ...
  g_settings_set_enum(dlg->priv->editor_settings, PLUMA_SETTINGS_DRAWER_SPACE,
                      setting);

#ifdef GTK_SOURCE_VERSION_3_24
  if (setting == DRAW_NONE)
    gtk_toggle_button_set_active(
        GTK_TOGGLE_BUTTON(dlg->priv->draw_trailing_spaces_checkbutton), FALSE);

  gtk_widget_set_sensitive(
      GTK_WIDGET(dlg->priv->draw_trailing_spaces_checkbutton),
      setting > DRAW_NONE);
  gtk_toggle_button_set_inconsistent(
      GTK_TOGGLE_BUTTON(dlg->priv->draw_trailing_spaces_checkbutton),
      setting == DRAW_NONE);
#endif
}

static void draw_trailing_spaces_checkbutton_toggled(
    GtkToggleButton *button, PlumaPreferencesDialog *dlg) {
  pluma_debug(DEBUG_PREFS);

  g_return_if_fail(
      button == GTK_TOGGLE_BUTTON(dlg->priv->draw_trailing_spaces_checkbutton));
  ...
  if (gtk_toggle_button_get_active(button))
    ...
    if (draw_spaces)
      g_settings_set_enum(dlg->priv->editor_settings,
                          PLUMA_SETTINGS_DRAWER_SPACE, DRAW_ALL);
    else
      g_settings_set_enum(dlg->priv->editor_settings,
                          PLUMA_SETTINGS_DRAWER_SPACE, DRAW_NONE);
  }
}

static void draw_tabs_checkbutton_toggled(GtkToggleButton *button,
                                          PlumaPreferencesDialog *dlg) {
  DrawSpacesSettings setting;
  pluma_debug(DEBUG_PREFS);

  g_return_if_fail(button ==
                   GTK_TOGGLE_BUTTON(dlg->priv->draw_tabs_checkbutton));
  ...
  g_settings_set_enum(dlg->priv->editor_settings, PLUMA_SETTINGS_DRAWER_TAB,
                      setting);
#ifdef GTK_SOURCE_VERSION_3_24
  if (setting == DRAW_NONE)
    gtk_toggle_button_set_active(
        GTK_TOGGLE_BUTTON(dlg->priv->draw_trailing_tabs_checkbutton), FALSE);

  gtk_widget_set_sensitive(
      GTK_WIDGET(dlg->priv->draw_trailing_tabs_checkbutton),
      setting > DRAW_NONE);
  gtk_toggle_button_set_inconsistent(
      GTK_TOGGLE_BUTTON(dlg->priv->draw_trailing_tabs_checkbutton),
      setting == DRAW_NONE);
#endif
}

static void draw_trailing_tabs_checkbutton_toggled(
    GtkToggleButton *button, PlumaPreferencesDialog *dlg) {
  pluma_debug(DEBUG_PREFS);

  g_return_if_fail(
      button == GTK_TOGGLE_BUTTON(dlg->priv->draw_trailing_tabs_checkbutton));
...
static void setup_editor_page(PlumaPreferencesDialog *dlg) {
  gboolean auto_save;
  ...
  g_settings_bind(dlg->priv->editor_settings, PLUMA_SETTINGS_TABS_SIZE,
                  dlg->priv->tabs_width_spinbutton, "value",
                  G_SETTINGS_BIND_GET | G_SETTINGS_BIND_SET);
  g_settings_bind(dlg->priv->editor_settings, PLUMA_SETTINGS_INSERT_SPACES,
                  dlg->priv->insert_spaces_checkbutton, "active",
                  G_SETTINGS_BIND_GET | G_SETTINGS_BIND_SET);
  g_settings_bind(dlg->priv->editor_settings, PLUMA_SETTINGS_AUTO_INDENT,
                  dlg->priv->auto_indent_checkbutton, "active",
                  G_SETTINGS_BIND_GET | G_SETTINGS_BIND_SET);
  g_settings_bind(dlg->priv->editor_settings, PLUMA_SETTINGS_CREATE_BACKUP_COPY,
                  dlg->priv->backup_copy_checkbutton, "active",
                  G_SETTINGS_BIND_GET | G_SETTINGS_BIND_SET);
  g_settings_bind(dlg->priv->editor_settings, PLUMA_SETTINGS_BRACKET_MATCHING,
                  dlg->priv->bracket_matching_checkbutton, "active",
                  G_SETTINGS_BIND_GET | G_SETTINGS_BIND_SET);
  g_settings_bind(dlg->priv->editor_settings, PLUMA_SETTINGS_AUTO_SAVE,
                  dlg->priv->auto_save_checkbutton, "active",
                  G_SETTINGS_BIND_GET | G_SETTINGS_BIND_SET);
  g_settings_bind(dlg->priv->editor_settings, PLUMA_SETTINGS_AUTO_SAVE_INTERVAL,
                  dlg->priv->auto_save_spinbutton, "value",
                  G_SETTINGS_BIND_GET | G_SETTINGS_BIND_SET |
                      G_SETTINGS_BIND_NO_SENSITIVITY);
  g_settings_bind(dlg->priv->editor_settings, PLUMA_SETTINGS_DRAWER_NEWLINE,
                  dlg->priv->draw_newlines_checkbutton, "active",
                  G_SETTINGS_BIND_GET | G_SETTINGS_BIND_SET);

  g_signal_connect(dlg->priv->editor_settings, "changed::auto-save",
  ...
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(dlg->priv->draw_trailing_spaces_checkbutton),
      draw_spaces == DRAW_TRAILING);
#else
  gtk_widget_set_sensitive(
      GTK_WIDGET(dlg->priv->draw_trailing_spaces_checkbutton), FALSE);
  gtk_toggle_button_set_inconsistent(
      GTK_TOGGLE_BUTTON(dlg->priv->draw_trailing_spaces_checkbutton), TRUE);
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(dlg->priv->draw_trailing_spaces_checkbutton), FALSE);
#endif
  gtk_toggle_button_set_active(
  ...
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(dlg->priv->draw_trailing_tabs_checkbutton),
      draw_tabs == DRAW_TRAILING);
#else
  gtk_widget_set_sensitive(
      GTK_WIDGET(dlg->priv->draw_trailing_tabs_checkbutton), FALSE);
  gtk_toggle_button_set_inconsistent(
      GTK_TOGGLE_BUTTON(dlg->priv->draw_trailing_tabs_checkbutton), FALSE);
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(dlg->priv->draw_trailing_tabs_checkbutton), FALSE);
  ...
  g_signal_connect(dlg->priv->draw_spaces_checkbutton, "toggled",
                   G_CALLBACK(draw_spaces_checkbutton_toggled), dlg);
  g_signal_connect(dlg->priv->draw_trailing_spaces_checkbutton, "toggled",
                   G_CALLBACK(draw_trailing_spaces_checkbutton_toggled), dlg);
  g_signal_connect(dlg->priv->draw_tabs_checkbutton, "toggled",
                   G_CALLBACK(draw_tabs_checkbutton_toggled), dlg);
  g_signal_connect(dlg->priv->draw_trailing_tabs_checkbutton, "toggled",
                   G_CALLBACK(draw_trailing_tabs_checkbutton_toggled), dlg);
}

static gboolean split_button_state = TRUE;

static void wrap_mode_checkbutton_toggled(GtkToggleButton *button,
                                          PlumaPreferencesDialog *dlg) {
  GtkWrapMode mode;

  if (!gtk_toggle_button_get_active(
          GTK_TOGGLE_BUTTON(dlg->priv->wrap_text_checkbutton))) {
    mode = GTK_WRAP_NONE;
  ...
  pluma_settings_set_wrap_mode(dlg->priv->editor_settings,
                               PLUMA_SETTINGS_WRAP_MODE, mode);
}

static void right_margin_checkbutton_toggled(GtkToggleButton *button,
                                             PlumaPreferencesDialog *dlg) {
  gboolean active;
  ...
  g_settings_set_boolean(dlg->priv->editor_settings,
                         PLUMA_SETTINGS_DISPLAY_RIGHT_MARGIN, active);

  gtk_widget_set_sensitive(dlg->priv->right_margin_position_spinbutton, active);
}

static void grid_checkbutton_toggled(GtkToggleButton *button,
                                     PlumaPreferencesDialog *dlg) {
  ...
  g_settings_set_enum(dlg->priv->editor_settings,
                      PLUMA_SETTINGS_BACKGROUND_PATTERN, background_type);
}

static void setup_view_page(PlumaPreferencesDialog *dlg) {
  ...
  display_right_margin = g_settings_get_boolean(
      dlg->priv->editor_settings, PLUMA_SETTINGS_DISPLAY_RIGHT_MARGIN);
  ...
  g_settings_bind(dlg->priv->editor_settings,
                  ...
                  dlg->priv->display_line_numbers_checkbutton, "active",
                  G_SETTINGS_BIND_GET | G_SETTINGS_BIND_SET);
  g_settings_bind(dlg->priv->editor_settings,
                  PLUMA_SETTINGS_HIGHLIGHT_CURRENT_LINE,
                  dlg->priv->highlight_current_line_checkbutton, "active",
                  G_SETTINGS_BIND_GET | G_SETTINGS_BIND_SET);
  g_settings_bind(dlg->priv->editor_settings,
                  PLUMA_SETTINGS_RIGHT_MARGIN_POSITION,
                  dlg->priv->right_margin_position_spinbutton, "value",
                  G_SETTINGS_BIND_GET | G_SETTINGS_BIND_SET |
                      G_SETTINGS_BIND_NO_SENSITIVITY);
  g_settings_bind(dlg->priv->editor_settings,
                  PLUMA_SETTINGS_DISPLAY_OVERVIEW_MAP,
                  dlg->priv->display_overview_map_checkbutton, "active",
                  G_SETTINGS_BIND_GET | G_SETTINGS_BIND_SET);
  ...
  g_signal_connect(dlg->priv->display_grid_checkbutton, "toggled",
                   G_CALLBACK(grid_checkbutton_toggled), dlg);
}

static void on_use_default_font_changed(GSettings *settings, const gchar *key,
                                        PlumaPreferencesDialog *dlg) {
  gboolean value;
...
static void setup_font_colors_page_font_section(PlumaPreferencesDialog *dlg) {
  ...
  label =
      g_strdup_printf(_("_Use the system fixed width font (%s)"), system_font);
  ...
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(dlg->priv->default_font_checkbutton), use_default_font);

  /* Connect signals */
  g_signal_connect(dlg->priv->editor_settings, "changed::use-default-font",
                   G_CALLBACK(on_use_default_font_changed), dlg);
  g_settings_bind(dlg->priv->editor_settings, PLUMA_SETTINGS_USE_DEFAULT_FONT,
                  dlg->priv->default_font_checkbutton, "active",
                  G_SETTINGS_BIND_GET | G_SETTINGS_BIND_SET);
  g_settings_bind(dlg->priv->editor_settings, PLUMA_SETTINGS_EDITOR_FONT,
                  dlg->priv->font_button, "font-name",
                  G_SETTINGS_BIND_GET | G_SETTINGS_BIND_SET);
...
static void set_buttons_sensisitivity_according_to_scheme(
    PlumaPreferencesDialog *dlg, const gchar *scheme_id) {
  gboolean editable;

  editable = (scheme_id != NULL) &&
             _pluma_style_scheme_manager_scheme_is_pluma_user_scheme(
                 pluma_get_style_scheme_manager(), scheme_id);

  gtk_widget_set_sensitive(dlg->priv->uninstall_scheme_button, editable);
}

static void style_scheme_changed(GtkWidget *treeview,
                                 PlumaPreferencesDialog *dlg) {
  GtkTreePath *path;
  GtkTreeIter iter;
  gchar *id;

  gtk_tree_view_get_cursor(GTK_TREE_VIEW(dlg->priv->schemes_treeview), &path,
                           NULL);
  gtk_tree_model_get_iter(GTK_TREE_MODEL(dlg->priv->schemes_treeview_model),
                          &iter, path);
  gtk_tree_path_free(path);
  gtk_tree_model_get(GTK_TREE_MODEL(dlg->priv->schemes_treeview_model), &iter,
                     ID_COLUMN, &id, -1);
...
static const gchar *ensure_color_scheme_id(PlumaPreferencesDialog *dlg,
                                           const gchar *id) {
  ...
  if (scheme == NULL) {
    /* Cannot determine default style scheme -> broken GtkSourceView
     * installation */
    return NULL;
  }
...
static const gchar *populate_color_scheme_list(PlumaPreferencesDialog *dlg,
                                               const gchar *def_id) {
  GSList *schemes;
  GSList *l;
  ...
  if (def_id == NULL) {
    g_warning(
        "Cannot build the list of available color schemes.\n"
        "Please check your GtkSourceView installation.");
    return NULL;
  }

  schemes = pluma_style_scheme_manager_list_schemes_sorted(
  ...
  while (l != NULL) {
    ...
    gtk_list_store_set(dlg->priv->schemes_treeview_model, &iter, ID_COLUMN, id,
                       NAME_COLUMN, name, DESC_COLUMN, description, -1);
    ...
    if (strcmp(id, def_id) == 0) {
      ...
      selection = gtk_tree_view_get_selection(
          GTK_TREE_VIEW(dlg->priv->schemes_treeview));
...
static void add_scheme_chooser_response_cb(GtkDialog *chooser, gint res_id,
                                           PlumaPreferencesDialog *dlg) {
  gchar *filename;
  ...
  scheme_id = _pluma_style_scheme_manager_install_scheme(
      pluma_get_style_scheme_manager(), filename);
  g_free(filename);
...
static GtkWidget *scheme_file_chooser_dialog_new_valist(
  ...
  while (button_text) {
    ...
    if (g_strcmp0(button_text, "process-stop") == 0)
      pluma_dialog_add_button(GTK_DIALOG(result), _("_Cancel"), button_text,
                              response_id);
    else
...
static GtkWidget *scheme_file_chooser_dialog_new(const gchar *title,
                                                 GtkWindow *parent,
...
static void install_scheme_clicked(GtkButton *button,
                                   PlumaPreferencesDialog *dlg) {
  GtkWidget *chooser;
  ...
  chooser = scheme_file_chooser_dialog_new(
      _("Add Scheme"), GTK_WINDOW(dlg), GTK_FILE_CHOOSER_ACTION_OPEN,
      "process-stop", GTK_RESPONSE_CANCEL, NULL);

  pluma_dialog_add_button(GTK_DIALOG(chooser), _("A_dd Scheme"), "list-add",
                          GTK_RESPONSE_ACCEPT);
...
static void uninstall_scheme_clicked(GtkButton *button,
                                     PlumaPreferencesDialog *dlg) {
  GtkTreeSelection *selection;
  GtkTreeModel *model;
  GtkTreeIter iter;
  ...
  selection =
      gtk_tree_view_get_selection(GTK_TREE_VIEW(dlg->priv->schemes_treeview));
  ...
  if (gtk_tree_selection_get_selected(selection, &model, &iter)) {
    ...
    if (!_pluma_style_scheme_manager_uninstall_scheme(
            pluma_get_style_scheme_manager(), id)) {
      pluma_warning(GTK_WINDOW(dlg), _("Could not remove color scheme \"%s\"."),
                    name);
    } else {
      ...
      if (real_new_id != NULL) {
        g_settings_set_string(dlg->priv->editor_settings,
                              PLUMA_SETTINGS_COLOR_SCHEME, real_new_id);
      }
    }
...
static void scheme_description_cell_data_func(GtkTreeViewColumn *column,
...
static void setup_font_colors_page_style_scheme_section(
  ...
  dlg->priv->schemes_treeview_model = gtk_list_store_new(
      NUM_COLUMNS, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING);

  gtk_tree_sortable_set_sort_column_id(
      GTK_TREE_SORTABLE(dlg->priv->schemes_treeview_model), 0,
      GTK_SORT_ASCENDING);
  ...
  gtk_tree_view_column_set_cell_data_func(
      column, renderer, scheme_description_cell_data_func, dlg, NULL);
  ...
  gtk_button_set_image(
      GTK_BUTTON(dlg->priv->uninstall_scheme_button),
...
static void setup_font_colors_page(PlumaPreferencesDialog *dlg) {
  setup_font_colors_page_font_section(dlg);
  setup_font_colors_page_style_scheme_section(dlg);
}

static void setup_plugins_page(PlumaPreferencesDialog *dlg) {
...
static void pluma_preferences_dialog_init(PlumaPreferencesDialog *dlg) {
  GtkWidget *error_widget;
  gboolean ret;
  gchar *root_objects[] = {"notebook",    "adjustment1",          "adjustment2",
                           "adjustment3", "install_scheme_image", NULL};
  ...
  pluma_dialog_add_button(GTK_DIALOG(dlg), _("_Close"), "window-close",
                          GTK_RESPONSE_CLOSE);
  ...
  g_signal_connect(dlg, "response", G_CALLBACK(dialog_response_handler), NULL);

  ret = pluma_utils_get_ui_objects(
      PLUMA_DATADIR "/ui/pluma-preferences-dialog.ui", root_objects,
      &error_widget,
      ...
      &dlg->priv->display_overview_map_checkbutton, "display_grid_checkbutton",
      &dlg->priv->display_grid_checkbutton, "wrap_text_checkbutton",
      &dlg->priv->wrap_text_checkbutton, "split_checkbutton",
      ...
      &dlg->priv->draw_trailing_spaces_checkbutton, "draw_tabs_checkbutton",
      &dlg->priv->draw_tabs_checkbutton, "draw_trailing_tabs_checkbutton",
      ...
      "font_button", &dlg->priv->font_button, "font_hbox",
      &dlg->priv->font_hbox,

      "schemes_treeview", &dlg->priv->schemes_treeview, "install_scheme_button",
      &dlg->priv->install_scheme_button, "uninstall_scheme_button",
      &dlg->priv->uninstall_scheme_button,
  ...
  g_signal_connect(dlg->priv->notebook, "scroll-event",
...
void pluma_show_preferences_dialog(PlumaWindow *parent) {
  ...
  if (preferences_dialog == NULL) {
    preferences_dialog =
        GTK_WIDGET(g_object_new(PLUMA_TYPE_PREFERENCES_DIALOG, NULL));
