...
static void pluma_message_finalize(GObject *object) {
...
static void pluma_message_get_property(GObject *object, guint prop_id,
  ...
  switch (prop_id) {
    case PROP_OBJECT_PATH:
      g_value_set_string(value,
                         pluma_message_type_get_object_path(msg->priv->type));
      break;
    case PROP_METHOD:
...
static void pluma_message_set_property(GObject *object, guint prop_id,
...
static GValue *add_value(PlumaMessage *message, const gchar *key) {
  GValue *value;
...
static void pluma_message_class_init(PlumaMessageClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string("object-path", "OBJECT_PATH",
                          "The message object path", NULL,
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string("method", "METHOD", "The message method", NULL,
                          G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));
  ...
  g_object_class_install_property(
      ...
      g_param_spec_boxed(
          "type", "TYPE", "The message type", PLUMA_TYPE_MESSAGE_TYPE,
          G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS));
}

static void destroy_value(GValue *value) {
  g_value_unset(value);
  g_free(value);
}

static void pluma_message_init(PlumaMessage *self) {
...
static gboolean set_value_real(GValue *to, const GValue *from) {
  GType from_type;
  GType to_type;
  ...
  if (!g_type_is_a(from_type, to_type)) {
    if (!g_value_transform(from, to)) {
      g_warning("%s: Unable to make conversion from %s to %s", G_STRLOC,
                g_type_name(from_type), g_type_name(to_type));
      return FALSE;
    }
...
static inline GValue *value_lookup(PlumaMessage *message, const gchar *key,
                                   gboolean create) {
  GValue *ret = (GValue *)g_hash_table_lookup(message->priv->values, key);

  if (!ret && create) ret = add_value(message, key);
...
void pluma_message_set_valist(PlumaMessage *message, va_list var_args) {
  ...
  while ((key = va_arg(var_args, const gchar *)) != NULL) {
    ...
    if (!container) {
      g_warning("%s: Cannot set value for %s, does not exist", G_STRLOC, key);
 ...
 *
 */
void pluma_message_set_value(PlumaMessage *message, const gchar *key,
                             GValue *value) {
  GValue *container;
  g_return_if_fail(PLUMA_IS_MESSAGE(message));
 ...
 *
 */
void pluma_message_set_valuesv(PlumaMessage *message, const gchar **keys,
 ...
 *
 */
void pluma_message_get_value(PlumaMessage *message, const gchar *key,
                             GValue *value) {
  GValue *container;
  ...
  if (!container) {
    g_warning("%s: Invalid key `%s'", G_STRLOC, key);
    return;
  }
...
static void validate_key(const gchar *key, GType type, gboolean required,
                         ValidateInfo *info) {
  GValue *value;

  if (!info->valid || !required) return;

  value = value_lookup(info->message, key, FALSE);

  if (!value) info->valid = FALSE;
}
...
gboolean pluma_message_validate(PlumaMessage *message) {
  ...
  if (!message->priv->valid) {
    pluma_message_type_foreach(message->priv->type,
                               (PlumaMessageTypeForeach)validate_key, &info);
