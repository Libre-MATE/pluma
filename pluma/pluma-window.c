...
static void pluma_window_get_property(GObject *object, guint prop_id,
                                      GValue *value, GParamSpec *pspec) {
  PlumaWindow *window = PLUMA_WINDOW(object);

  switch (prop_id) {
    case PROP_STATE:
      g_value_set_enum(value, pluma_window_get_state(window));
      break;
    default:
...
static void save_panes_state(PlumaWindow *window) {
  gint pane_page;
  ...
  pane_page =
      _pluma_panel_get_active_item_id(PLUMA_PANEL(window->priv->side_panel));
  if (pane_page != 0)
    g_settings_set_int(window->priv->editor_settings,
                       PLUMA_SETTINGS_SIDE_PANEL_ACTIVE_PAGE, pane_page);
  ...
  pane_page =
      _pluma_panel_get_active_item_id(PLUMA_PANEL(window->priv->bottom_panel));
  if (pane_page != 0)
    g_settings_set_int(window->priv->editor_settings,
                       PLUMA_SETTINGS_BOTTOM_PANEL_ACTIVE_PAGE, pane_page);
}

static void pluma_window_dispose(GObject *object) {
  PlumaWindow *window;
  ...
  if (!window->priv->dispose_has_run) {
    ...
    peas_engine_garbage_collect(
        PEAS_ENGINE(pluma_plugins_engine_get_default()));
...
#if GLIB_CHECK_VERSION(2, 62, 0)
    g_clear_signal_handler(&window->priv->recents_handler_id, recent_manager);
#else
...
static void pluma_window_finalize(GObject *object) {
  PlumaWindow *window;
...
static gboolean pluma_window_window_state_event(GtkWidget *widget,
                                                GdkEventWindowState *event) {
  PlumaWindow *window = PLUMA_WINDOW(widget);

  window->priv->window_state = event->new_window_state;

  return GTK_WIDGET_CLASS(pluma_window_parent_class)
      ->window_state_event(widget, event);
}

static gboolean pluma_window_configure_event(GtkWidget *widget,
 ...
 */
static gboolean pluma_window_key_press_event(GtkWidget *widget,
                                             GdkEventKey *event) {
  ...
  if (event->state & GDK_CONTROL_MASK) {
    ...
    if ((event->keyval == GDK_KEY_plus) || (event->keyval == GDK_KEY_KP_Add)) {
      ...
      if (!g_settings_get_boolean(settings, PLUMA_SETTINGS_USE_DEFAULT_FONT) &&
          (nsize < 73)) {
      ...
      g_settings_set_boolean(
          settings, PLUMA_SETTINGS_DISPLAY_LINE_NUMBERS,
          !g_settings_get_boolean(settings,
                                  PLUMA_SETTINGS_DISPLAY_LINE_NUMBERS));
    }

    if (g_settings_get_boolean(settings,
                               PLUMA_SETTINGS_CTRL_TABS_SWITCH_TABS)) {
      GtkNotebook *notebook =
          GTK_NOTEBOOK(_pluma_window_get_notebook(PLUMA_WINDOW(window)));
...
static void pluma_window_tab_removed(PlumaWindow *window, PlumaTab *tab) {
  peas_engine_garbage_collect(PEAS_ENGINE(pluma_plugins_engine_get_default()));
}

static void pluma_window_class_init(PlumaWindowClass *klass) {
  ...
  signals[TAB_ADDED] = g_signal_new(
      ...
      G_STRUCT_OFFSET(PlumaWindowClass, tab_added), NULL, NULL, NULL,
  ...
  signals[TAB_REMOVED] = g_signal_new(
      ...
      G_STRUCT_OFFSET(PlumaWindowClass, tab_removed), NULL, NULL, NULL,
      G_TYPE_NONE, 1, PLUMA_TYPE_TAB);
  signals[TABS_REORDERED] = g_signal_new(
      "tabs_reordered", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(PlumaWindowClass, tabs_reordered), NULL, NULL, NULL,
      G_TYPE_NONE, 0);
  signals[ACTIVE_TAB_CHANGED] = g_signal_new(
      "active_tab_changed", G_OBJECT_CLASS_TYPE(object_class),
      G_SIGNAL_RUN_FIRST, G_STRUCT_OFFSET(PlumaWindowClass, active_tab_changed),
      NULL, NULL, NULL, G_TYPE_NONE, 1, PLUMA_TYPE_TAB);
  ...
  g_object_class_install_property(
      ...
      g_param_spec_flags("state", "State", "The window's state",
                         PLUMA_TYPE_WINDOW_STATE, PLUMA_WINDOW_STATE_NORMAL,
                         G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));
}

static void menu_item_select_cb(GtkMenuItem *proxy, PlumaWindow *window) {
  GtkAction *action;
  char *message;
...
static void menu_item_deselect_cb(GtkMenuItem *proxy, PlumaWindow *window) {
  gtk_statusbar_pop(GTK_STATUSBAR(window->priv->statusbar),
                    window->priv->tip_message_cid);
}

static void connect_proxy_cb(GtkUIManager *manager, GtkAction *action,
                             GtkWidget *proxy, PlumaWindow *window) {
...
static void apply_toolbar_style(PlumaWindow *window, GtkWidget *toolbar) {
...
static gboolean set_toolbar_style(PlumaWindow *window, PlumaWindow *origin) {
  gboolean visible;
  ...
  action = gtk_action_group_get_action(
      window->priv->always_sensitive_action_group, "ViewToolbar");
...
static void update_next_prev_doc_sensitivity(PlumaWindow *window,
                                             PlumaTab *tab) {
  gint tab_number;
  GtkNotebook *notebook;
  GtkAction *action;
...
static void update_next_prev_doc_sensitivity_per_window(PlumaWindow *window) {
  PlumaTab *tab;
  GtkAction *action;
...
static void received_clipboard_contents(GtkClipboard *clipboard,
  ...
  if (window->priv->active_tab != NULL) {
    ...
    sens =
        state_normal && gtk_selection_data_targets_include_text(selection_data);
  } else {
    sens = FALSE;
  }
...
static void set_paste_sensitivity_according_to_clipboard(
    PlumaWindow *window, GtkClipboard *clipboard) {
  GdkDisplay *display;
  ...
  if (gdk_display_supports_selection_notification(display)) {
    ...
    action =
        gtk_action_group_get_action(window->priv->action_group, "EditPaste");
...
static void set_sensitivity_according_to_tab(PlumaWindow *window,
                                             PlumaTab *tab) {
  PlumaDocument *doc;
  PlumaView *view;
  GtkAction *action;
  ...
  enable_syntax_highlighting = g_settings_get_boolean(
      window->priv->editor_settings, PLUMA_SETTINGS_SYNTAX_HIGHLIGHTING);
  ...
  gtk_action_set_sensitive(
      action, (state_normal ||
               ...
               (state == PLUMA_TAB_STATE_SHOWING_PRINT_PREVIEW)) &&
                  ...
                  !(lockdown & PLUMA_LOCKDOWN_SAVE_TO_DISK) && (cansave) &&
                  (editable));

  action =
      gtk_action_group_get_action(window->priv->action_group, "FileSaveAs");
  gtk_action_set_sensitive(
      action, (state_normal || (state == PLUMA_TAB_STATE_SAVING_ERROR) ||
               (state == PLUMA_TAB_STATE_EXTERNALLY_MODIFIED_NOTIFICATION) ||
               (state == PLUMA_TAB_STATE_SHOWING_PRINT_PREVIEW)) &&
                  !(lockdown & PLUMA_LOCKDOWN_SAVE_TO_DISK));

  action =
      gtk_action_group_get_action(window->priv->action_group, "FileRevert");
  gtk_action_set_sensitive(
      action, (state_normal ||
               (state == PLUMA_TAB_STATE_EXTERNALLY_MODIFIED_NOTIFICATION)) &&
  ...
  action = gtk_action_group_get_action(window->priv->action_group,
                                       "FilePrintPreview");
  gtk_action_set_sensitive(
      action, state_normal && !(lockdown & PLUMA_LOCKDOWN_PRINTING));

  action = gtk_action_group_get_action(window->priv->action_group, "FilePrint");
  gtk_action_set_sensitive(
      action,
      (state_normal || (state == PLUMA_TAB_STATE_SHOWING_PRINT_PREVIEW)) &&
  ...
  gtk_action_set_sensitive(
      action, (state != PLUMA_TAB_STATE_CLOSING) &&
  ...
  gtk_action_set_sensitive(action, state_normal && gtk_source_buffer_can_undo(
                                                       GTK_SOURCE_BUFFER(doc)));

  action = gtk_action_group_get_action(window->priv->action_group, "EditRedo");
  gtk_action_set_sensitive(action, state_normal && gtk_source_buffer_can_redo(
                                                       GTK_SOURCE_BUFFER(doc)));

  action = gtk_action_group_get_action(window->priv->action_group, "EditCut");
  gtk_action_set_sensitive(
      action, state_normal && editable &&
  ...
  action =
      gtk_action_group_get_action(window->priv->action_group, "EditDelete");
  gtk_action_set_sensitive(
      action, state_normal && editable &&
                  gtk_text_buffer_get_has_selection(GTK_TEXT_BUFFER(doc)));

  action =
      gtk_action_group_get_action(window->priv->action_group, "SearchFind");
  gtk_action_set_sensitive(
      action, (state_normal ||
               state == PLUMA_TAB_STATE_EXTERNALLY_MODIFIED_NOTIFICATION));
  ...
  action =
      gtk_action_group_get_action(window->priv->action_group, "SearchReplace");
  gtk_action_set_sensitive(action, state_normal && editable);

  b = pluma_document_get_can_search_again(doc);
  action =
      gtk_action_group_get_action(window->priv->action_group, "SearchFindNext");
  gtk_action_set_sensitive(
      action, (state_normal ||
               state == PLUMA_TAB_STATE_EXTERNALLY_MODIFIED_NOTIFICATION) &&
                  b);
  ...
  action =
      gtk_action_group_get_action(window->priv->action_group, "SearchGoToLine");
  gtk_action_set_sensitive(
      action, (state_normal ||
               state == PLUMA_TAB_STATE_EXTERNALLY_MODIFIED_NOTIFICATION));
  ...
  action = gtk_action_group_get_action(window->priv->action_group,
                                       "ViewHighlightMode");
  gtk_action_set_sensitive(
      action, (state != PLUMA_TAB_STATE_CLOSING) && enable_syntax_highlighting);
...
static void language_toggled(GtkToggleAction *action, PlumaWindow *window) {
  PlumaDocument *doc;
  GtkSourceLanguage *lang;
  const gchar *lang_id;

  if (gtk_toggle_action_get_active(action) == FALSE) return;
  ...
  if (strcmp(lang_id, LANGUAGE_NONE) == 0) {
    ...
    lang = gtk_source_language_manager_get_language(
        pluma_get_language_manager(), lang_id);
    if (lang == NULL) {
...
static gchar *escape_section_name(const gchar *name) {
  gchar *ret;
...
static void create_language_menu_item(GtkSourceLanguage *lang, gint index,
                                      guint ui_id, PlumaWindow *window) {
  ...
  section_action = gtk_action_group_get_action(
      window->priv->languages_action_group, escaped_section);
  ...
  path = g_strdup_printf(
      "/MenuBar/ViewMenu/ViewHighlightModeMenu/LanguagesMenuPlaceholder/%s",
      escaped_section);
  ...
  normal_action = gtk_action_group_get_action(
      window->priv->languages_action_group, LANGUAGE_NONE);
  ...
  gtk_ui_manager_add_ui(window->priv->manager, ui_id, path, lang_id, lang_id,
...
static void create_languages_menu(PlumaWindow *window) {
  ...
  action_none =
      gtk_radio_action_new(LANGUAGE_NONE, _("Plain Text"),
                           _("Disable syntax highlighting"), NULL, -1);

  gtk_action_group_add_action(window->priv->languages_action_group,
                              GTK_ACTION(action_none));
  g_object_unref(action_none);

  g_signal_connect(action_none, "activate", G_CALLBACK(language_toggled),
                   window);

  id = gtk_ui_manager_new_merge_id(window->priv->manager);

  gtk_ui_manager_add_ui(
      window->priv->manager, id,
      "/MenuBar/ViewMenu/ViewHighlightModeMenu/LanguagesMenuPlaceholder",
  ...
  languages = pluma_language_manager_list_languages_sorted(
      pluma_get_language_manager(), FALSE);
...
static void update_languages_menu(PlumaWindow *window) {
  PlumaDocument *doc;
  GList *actions;
  GList *l;
  GtkAction *action;
  ...
  for (l = actions; l != NULL; l = l->next) {
    g_signal_handlers_block_by_func(GTK_ACTION(l->data),
                                    G_CALLBACK(language_toggled), window);
  }
...
void _pluma_recent_add(PlumaWindow *window, const gchar *uri,
                       const gchar *mime) {
  GtkRecentManager *recent_manager;
  GtkRecentData recent_data;

  static gchar *groups[2] = {"pluma", NULL};
...
void _pluma_recent_remove(PlumaWindow *window, const gchar *uri) {
  GtkRecentManager *recent_manager;
...
static void open_recent_file(const gchar *uri, PlumaWindow *window) {
  GSList *uris = NULL;
...
static void recents_menu_activate(GtkAction *action, PlumaWindow *window) {
  GtkRecentInfo *info;
  const gchar *uri;
...
static void update_recent_files_menu(PlumaWindow *window) {
  ...
  max_recents = g_settings_get_uint(window->priv->editor_settings,
                                    PLUMA_SETTINGS_MAX_RECENTS);
  ...
  for (l = actions; l != NULL; l = l->next) {
    g_signal_handlers_disconnect_by_func(
        GTK_ACTION(l->data), G_CALLBACK(recents_menu_activate), window);
  ...
  for (l = items; l != NULL; l = l->next) {
    ...
    if (!gtk_recent_info_has_group(info, "pluma")) continue;
  ...
  for (l = filtered_items; l != NULL; l = l->next) {
    ...
    g_signal_connect(action, "activate", G_CALLBACK(recents_menu_activate),
                     window);
    ...
    gtk_ui_manager_add_ui(p->manager, p->recents_menu_ui_id,
                          ...
                          action_name, action_name, GTK_UI_MANAGER_MENUITEM,
                          FALSE);
...
static void set_non_homogeneus(GtkWidget *widget, gpointer data) {
  gtk_tool_item_set_homogeneous(GTK_TOOL_ITEM(widget), FALSE);
}

static void toolbar_visibility_changed(GtkWidget *toolbar,
                                       PlumaWindow *window) {
  gboolean visible;
  GtkAction *action;
...
static GtkWidget *setup_toolbar_open_button(PlumaWindow *window,
                                            GtkWidget *toolbar) {
  GtkRecentManager *recent_manager;
  ...
  g_signal_connect(toolbar_recent_menu, "item_activated",
                   G_CALLBACK(recent_chooser_item_activated), window);

  /* add the custom Open button to the toolbar */
  open_button = gtk_menu_tool_button_new(
  ...
  action = gtk_action_group_get_action(
      window->priv->always_sensitive_action_group, "FileOpen");
...
static void create_menu_bar_and_toolbar(PlumaWindow *window,
                                        GtkWidget *main_box) {
  ...
  gtk_action_group_add_actions(
      action_group, pluma_always_sensitive_menu_entries,
      G_N_ELEMENTS(pluma_always_sensitive_menu_entries), window);
  gtk_action_group_add_toggle_actions(
      action_group, pluma_always_sensitive_toggle_menu_entries,
      G_N_ELEMENTS(pluma_always_sensitive_toggle_menu_entries), window);
  ...
  gtk_action_group_set_translation_domain(action_group, NULL);
  gtk_action_group_add_actions(action_group, pluma_menu_entries,
                               G_N_ELEMENTS(pluma_menu_entries), window);
  gtk_ui_manager_insert_action_group(manager, action_group, 0);
  g_object_unref(action_group);
  ...
  gtk_action_group_set_translation_domain(action_group, NULL);
  gtk_action_group_add_actions(action_group, pluma_quit_menu_entries,
                               G_N_ELEMENTS(pluma_quit_menu_entries), window);
  ...
  gtk_action_group_set_translation_domain(action_group, NULL);
  gtk_action_group_add_actions(action_group, pluma_close_menu_entries,
                               G_N_ELEMENTS(pluma_close_menu_entries), window);
  ...
  gtk_action_group_set_translation_domain(action_group, NULL);
  gtk_action_group_add_toggle_actions(
      action_group, pluma_panes_toggle_menu_entries,
      G_N_ELEMENTS(pluma_panes_toggle_menu_entries), window);
  ...
  gtk_ui_manager_add_ui_from_file(manager, PLUMA_DATADIR "/ui/pluma-ui.xml",
                                  &error);
  if (error != NULL) {
    g_warning("Could not merge %s: %s", PLUMA_DATADIR "/ui/pluma-ui.xml",
              error->message);
    g_error_free(error);
  }

  /* show tooltips in the statusbar */
  g_signal_connect(manager, "connect_proxy", G_CALLBACK(connect_proxy_cb),
                   window);
  g_signal_connect(manager, "disconnect_proxy", G_CALLBACK(disconnect_proxy_cb),
                   window);
  ...
  recent_manager = gtk_recent_manager_get_default();
  window->priv->recents_handler_id = g_signal_connect(
      recent_manager, "changed", G_CALLBACK(recent_manager_changed), window);
  update_recent_files_menu(window);
  ...
  gtk_style_context_add_class(
      gtk_widget_get_style_context(window->priv->toolbar),
  ...
  g_signal_connect_after(window->priv->toolbar, "show",
                         G_CALLBACK(toolbar_visibility_changed), window);
  g_signal_connect_after(window->priv->toolbar, "hide",
                         G_CALLBACK(toolbar_visibility_changed), window);
}

static void documents_list_menu_activate(GtkToggleAction *action,
                                         PlumaWindow *window) {
  gint n;
...
static gchar *get_menu_tip_for_tab(PlumaTab *tab) {
  PlumaDocument *doc;
  gchar *uri;
  gchar *ruri;
  gchar *tip;
...
static void update_documents_list_menu(PlumaWindow *window) {
  PlumaWindowPrivate *p = window->priv;
  GList *actions, *l;
  gint n, i;
  guint id;
  ...
  for (l = actions; l != NULL; l = l->next) {
    g_signal_handlers_disconnect_by_func(
        GTK_ACTION(l->data), G_CALLBACK(documents_list_menu_activate), window);
  ...
  for (i = 0; i < n; i++) {
    ...
    if (group != NULL) gtk_radio_action_set_group(action, group);
    ...
    gtk_ui_manager_add_ui(
        p->manager, id, "/MenuBar/DocumentsMenu/DocumentsListPlaceholder",
...
static gboolean set_statusbar_style(PlumaWindow *window, PlumaWindow *origin) {
  GtkAction *action;
  gboolean visible;
  ...
  action = gtk_action_group_get_action(
      window->priv->always_sensitive_action_group, "ViewStatusbar");
...
static void statusbar_visibility_changed(GtkWidget *statusbar,
                                         PlumaWindow *window) {
  gboolean visible;
  GtkAction *action;
...
static void tab_width_combo_changed(PlumaStatusComboBox *combo,
                                    GtkMenuItem *item, PlumaWindow *window) {
  PlumaView *view;
  guint width_data = 0;
  ...
  width_data =
      GPOINTER_TO_INT(g_object_get_data(G_OBJECT(item), TAB_WIDTH_DATA));

  if (width_data == 0) return;
...
static void use_spaces_toggled(GtkCheckMenuItem *item, PlumaWindow *window) {
  PlumaView *view;
  ...
  gtk_source_view_set_insert_spaces_instead_of_tabs(
      GTK_SOURCE_VIEW(view), gtk_check_menu_item_get_active(item));
  g_signal_handler_unblock(view, window->priv->spaces_instead_of_tabs_id);
}

static void language_combo_changed(PlumaStatusComboBox *combo,
                                   GtkMenuItem *item, PlumaWindow *window) {
  PlumaDocument *doc;
  ...
  language =
      GTK_SOURCE_LANGUAGE(g_object_get_data(G_OBJECT(item), LANGUAGE_DATA));
...
static void fill_tab_width_combo(PlumaWindow *window) {
  static TabWidthDefinition defs[] = {{"2", 2},
                                      {"4", 4},
                                      {"8", 8},
                                      {"", 0}, /* custom size */
                                      {NULL, 0}};
  ...
  while (defs[i].label != NULL) {
    ...
    g_object_set_data(G_OBJECT(item), TAB_WIDTH_DATA,
                      GINT_TO_POINTER(defs[i].width));

    pluma_status_combo_box_add_item(combo, GTK_MENU_ITEM(item), defs[i].label);

    if (defs[i].width != 0) gtk_widget_show(item);
...
static void fill_language_combo(PlumaWindow *window) {
  ...
  pluma_status_combo_box_add_item(
      PLUMA_STATUS_COMBO_BOX(window->priv->language_combo),
  ...
  for (item = languages; item; item = item->next) {
    ...
    g_object_set_data_full(G_OBJECT(menu_item), LANGUAGE_DATA,
                           g_object_ref(lang), (GDestroyNotify)g_object_unref);
...
static void create_statusbar(PlumaWindow *window, GtkWidget *main_box) {
  pluma_debug(DEBUG_WINDOW);
  ...
  gtk_box_pack_end(GTK_BOX(window->priv->statusbar),
                   window->priv->tab_width_combo, FALSE, TRUE, 0);
  ...
  gtk_box_pack_end(GTK_BOX(window->priv->statusbar),
                   window->priv->language_combo, FALSE, TRUE, 0);
...
static PlumaWindow *clone_window(PlumaWindow *origin) {
  PlumaWindow *window;
  GdkScreen *screen;
  PlumaApp *app;
  gint panel_page;
  ...
  if ((origin->priv->window_state & GDK_WINDOW_STATE_MAXIMIZED) != 0) {
    ...
    gtk_window_set_default_size(GTK_WINDOW(window), origin->priv->width,
                                origin->priv->height);
  ...
  panel_page =
      _pluma_panel_get_active_item_id(PLUMA_PANEL(origin->priv->side_panel));
  _pluma_panel_set_active_item_by_id(PLUMA_PANEL(window->priv->side_panel),
                                     panel_page);

  panel_page =
      _pluma_panel_get_active_item_id(PLUMA_PANEL(origin->priv->bottom_panel));
...
static void update_cursor_position_statusbar(GtkTextBuffer *buffer,
                                             PlumaWindow *window) {
  gint row, col;
  GtkTextIter iter;
  PlumaView *view;
...
static void update_overwrite_mode_statusbar(GtkTextView *view,
                                            PlumaWindow *window) {
  if (view != GTK_TEXT_VIEW(pluma_window_get_active_view(window))) return;
...
static void set_title(PlumaWindow *window) {
  ...
  if (len > MAX_TITLE_LENGTH) {
    ...
    if (file != NULL) {
       ...
       */
      dirname =
          pluma_utils_str_middle_truncate(str, MAX(20, MAX_TITLE_LENGTH - len));
      g_free(str);
    }
  }
  ...
  if (pluma_document_get_readonly(doc)) {
    if (dirname != NULL)
      title = g_strdup_printf("%s [%s] (%s) - Pluma", name, _("Read-Only"),
                              dirname);
    else
      title = g_strdup_printf("%s [%s] - Pluma", name, _("Read-Only"));
  } else {
    if (dirname != NULL)
      title = g_strdup_printf("%s (%s) - Pluma", name, dirname);
    else
      title = g_strdup_printf("%s - Pluma", name);
  }
...
static void set_tab_width_item_blocked(PlumaWindow *window, GtkMenuItem *item) {
  g_signal_handlers_block_by_func(window->priv->tab_width_combo,
                                  tab_width_combo_changed, window);

  pluma_status_combo_box_set_item(
      PLUMA_STATUS_COMBO_BOX(window->priv->tab_width_combo), item);

  g_signal_handlers_unblock_by_func(window->priv->tab_width_combo,
                                    tab_width_combo_changed, window);
}

static void spaces_instead_of_tabs_changed(GObject *object, GParamSpec *pspec,
                                           PlumaWindow *window) {
...
static void tab_width_changed(GObject *object, GParamSpec *pspec,
                              PlumaWindow *window) {
  GList *items;
  GList *item;
  PlumaStatusComboBox *combo =
  ...
  for (item = items; item; item = item->next) {
    guint tab_width = GPOINTER_TO_INT(
        g_object_get_data(G_OBJECT(item->data), TAB_WIDTH_DATA));
    ...
    if (GTK_IS_SEPARATOR_MENU_ITEM(item->next->data)) {
      if (!found) {
        ...
        pluma_status_combo_box_set_item_text(combo, GTK_MENU_ITEM(item->data),
                                             text);
...
static void language_changed(GObject *object, GParamSpec *pspec,
                             PlumaWindow *window) {
  GList *items;
  GList *item;
  PlumaStatusComboBox *combo =
  ...
  for (item = items; item; item = item->next) {
    GtkSourceLanguage *lang =
        g_object_get_data(G_OBJECT(item->data), LANGUAGE_DATA);

    if ((new_id == NULL && lang == NULL) ||
        (new_id != NULL && lang != NULL &&
         strcmp(gtk_source_language_get_id(lang), new_id) == 0)) {
      g_signal_handlers_block_by_func(window->priv->language_combo,
                                      language_combo_changed, window);
...
static void notebook_switch_page(GtkNotebook *book, GtkWidget *pg,
                                 gint page_num, PlumaWindow *window) {
  PlumaView *view;
  PlumaTab *tab;
  GtkAction *action;
  gchar *action_name;
  ...
  if (tab == window->priv->active_tab) return;
  ...
  if (window->priv->active_tab) {
    ...
    g_clear_signal_handler(&window->priv->tab_width_id, active_tab_view);
  ...
  action = gtk_action_group_get_action(
      window->priv->documents_list_action_group, action_name);

  /* sometimes the action doesn't exist yet, and the proper action
   * is set active during the documents list menu creation
   * CHECK: would it be nicer if active_tab was a property and we monitored the
   * notify signal?
   */
  if (action != NULL)
    gtk_toggle_action_set_active(GTK_TOGGLE_ACTION(action), TRUE);
  ...
  window->priv->tab_width_id = g_signal_connect(
      view, "notify::tab-width", G_CALLBACK(tab_width_changed), window);
  window->priv->spaces_instead_of_tabs_id =
      g_signal_connect(view, "notify::insert-spaces-instead-of-tabs",
                       G_CALLBACK(spaces_instead_of_tabs_changed), window);

  window->priv->language_changed_id =
      g_signal_connect(pluma_tab_get_document(tab), "notify::language",
  ...
  g_signal_emit(G_OBJECT(window), signals[ACTIVE_TAB_CHANGED], 0,
                window->priv->active_tab);
}

static void set_sensitivity_according_to_window_state(PlumaWindow *window) {
  GtkAction *action;
  PlumaLockdownMask lockdown;
  ...
  action =
      gtk_action_group_get_action(window->priv->action_group, "FileCloseAll");
  gtk_action_set_sensitive(
      action, !(window->priv->state & PLUMA_WINDOW_STATE_SAVING) &&
                  !(window->priv->state & PLUMA_WINDOW_STATE_PRINTING));

  action =
      gtk_action_group_get_action(window->priv->action_group, "FileSaveAll");
  gtk_action_set_sensitive(
      action, !(window->priv->state & PLUMA_WINDOW_STATE_PRINTING) &&
                  !(lockdown & PLUMA_LOCKDOWN_SAVE_TO_DISK));

  action = gtk_action_group_get_action(
      window->priv->always_sensitive_action_group, "FileNew");
  gtk_action_set_sensitive(
      action, !(window->priv->state & PLUMA_WINDOW_STATE_SAVING_SESSION));
...
static void update_tab_autosave(GtkWidget *widget, gpointer data) {
...
static void analyze_tab_state(PlumaTab *tab, PlumaWindow *window) {
  PlumaTabState ts;
...
static void update_window_state(PlumaWindow *window) {
  ...
  gtk_container_foreach(GTK_CONTAINER(window->priv->notebook),
                        (GtkCallback)analyze_tab_state, window);
...
static void sync_state(PlumaTab *tab, GParamSpec *pspec, PlumaWindow *window) {
  pluma_debug(DEBUG_WINDOW);
...
static void sync_name(PlumaTab *tab, GParamSpec *pspec, PlumaWindow *window) {
  GtkAction *action;
  gchar *action_name;
  gchar *tab_name;
  gchar *escaped_name;
...
static PlumaWindow *get_drop_window(GtkWidget *widget) {
  GtkWidget *target_window;

  target_window = gtk_widget_get_toplevel(widget);
  g_return_val_if_fail(PLUMA_IS_WINDOW(target_window), NULL);

  if ((PLUMA_WINDOW(target_window)->priv->state &
       PLUMA_WINDOW_STATE_SAVING_SESSION) != 0)
    return NULL;

  return PLUMA_WINDOW(target_window);
}

static void load_uris_from_drop(PlumaWindow *window, gchar **uri_list) {
  GSList *uris = NULL;
  gint i;
...
static void drop_uris_cb(GtkWidget *widget, gchar **uri_list) {
  PlumaWindow *window;
...
static void fullscreen_controls_show(PlumaWindow *window) {
  GdkScreen *screen;
  GdkDisplay *display;
  ...
  gtk_window_move(GTK_WINDOW(window->priv->fullscreen_controls), fs_rect.x,
                  fs_rect.y - h + 1);

  gtk_widget_show_all(window->priv->fullscreen_controls);
}

static gboolean run_fullscreen_animation(gpointer data) {
  ...
  gtk_window_get_position(GTK_WINDOW(window->priv->fullscreen_controls), &x,
                          &y);
...
static void show_hide_fullscreen_toolbar(PlumaWindow *window, gboolean show,
                                         gint height) {
  GtkSettings *settings;
  gboolean enable_animations;

  settings = gtk_widget_get_settings(GTK_WIDGET(window));
  g_object_get(G_OBJECT(settings), "gtk-enable-animations", &enable_animations,
               NULL);
...
static gboolean on_fullscreen_controls_enter_notify_event(
    GtkWidget *widget, GdkEventCrossing *event, PlumaWindow *window) {
  show_hide_fullscreen_toolbar(window, TRUE, 0);

  return FALSE;
}

static gboolean on_fullscreen_controls_leave_notify_event(
    GtkWidget *widget, GdkEventCrossing *event, PlumaWindow *window) {
  GdkDevice *device;
  gint w, h;
  gint x, y;
...
static void fullscreen_controls_build(PlumaWindow *window) {
  PlumaWindowPrivate *priv = window->priv;
  GtkWidget *toolbar;
  GtkAction *action;

  if (priv->fullscreen_controls != NULL) return;
  ...
  gtk_container_foreach(GTK_CONTAINER(toolbar), (GtkCallback)set_non_homogeneus,
                        NULL);
...
static void can_search_again(PlumaDocument *doc, GParamSpec *pspec,
                             PlumaWindow *window) {
  gboolean sensitive;
  GtkAction *action;

  if (doc != pluma_window_get_active_document(window)) return;
...
static void can_undo(PlumaDocument *doc, GParamSpec *pspec,
                     PlumaWindow *window) {
  GtkAction *action;
  gboolean sensitive;
...
static void can_redo(PlumaDocument *doc, GParamSpec *pspec,
                     PlumaWindow *window) {
  GtkAction *action;
  gboolean sensitive;
...
static void selection_changed(PlumaDocument *doc, GParamSpec *pspec,
                              PlumaWindow *window) {
  PlumaTab *tab;
  PlumaView *view;
  GtkAction *action;
...
static void sync_languages_menu(PlumaDocument *doc, GParamSpec *pspec,
                                PlumaWindow *window) {
  update_languages_menu(window);
  peas_extension_set_call(window->priv->extensions, "update_state");
}

static void readonly_changed(PlumaDocument *doc, GParamSpec *pspec,
                             PlumaWindow *window) {
...
static void editable_changed(PlumaView *view, GParamSpec *arg1,
                             PlumaWindow *window) {
  peas_extension_set_call(window->priv->extensions, "update_state");
}

static void update_sensitivity_according_to_open_tabs(PlumaWindow *window) {
  GtkAction *action;
...
static void notebook_tab_added(PlumaNotebook *notebook, PlumaTab *tab,
                               PlumaWindow *window) {
  PlumaView *view;
  PlumaDocument *doc;
  ...
  g_return_if_fail((window->priv->state & PLUMA_WINDOW_STATE_SAVING_SESSION) ==
                   0);
  ...
  g_signal_connect(doc, "cursor-moved",
                   G_CALLBACK(update_cursor_position_statusbar), window);
  g_signal_connect(doc, "notify::can-search-again",
                   G_CALLBACK(can_search_again), window);
  ...
  g_signal_connect(doc, "notify::has-selection", G_CALLBACK(selection_changed),
                   window);
  g_signal_connect(doc, "notify::language", G_CALLBACK(sync_languages_menu),
                   window);
  g_signal_connect(doc, "notify::read-only", G_CALLBACK(readonly_changed),
                   window);
  g_signal_connect(view, "toggle_overwrite",
                   G_CALLBACK(update_overwrite_mode_statusbar), window);
  g_signal_connect(view, "notify::editable", G_CALLBACK(editable_changed),
                   window);
...
static void notebook_tab_removed(PlumaNotebook *notebook, PlumaTab *tab,
                                 PlumaWindow *window) {
  PlumaView *view;
  PlumaDocument *doc;
  ...
  g_signal_handlers_disconnect_by_func(doc, G_CALLBACK(can_search_again),
                                       window);
  ...
  g_signal_handlers_disconnect_by_func(doc, G_CALLBACK(selection_changed),
                                       window);
  g_signal_handlers_disconnect_by_func(doc, G_CALLBACK(sync_languages_menu),
                                       window);
  g_signal_handlers_disconnect_by_func(doc, G_CALLBACK(readonly_changed),
                                       window);
  g_signal_handlers_disconnect_by_func(
      view, G_CALLBACK(update_overwrite_mode_statusbar), window);
  g_signal_handlers_disconnect_by_func(view, G_CALLBACK(editable_changed),
                                       window);
  ...
  if (window->priv->num_tabs == 0) {
    ...
    pluma_statusbar_set_cursor_position(
        PLUMA_STATUSBAR(window->priv->statusbar), -1, -1);
...
static void notebook_tabs_reordered(PlumaNotebook *notebook,
                                    PlumaWindow *window) {
  update_documents_list_menu(window);
  update_next_prev_doc_sensitivity_per_window(window);
...
static void notebook_tab_detached(PlumaNotebook *notebook, PlumaTab *tab,
                                  PlumaWindow *window) {
  ...
  pluma_notebook_move_tab(
      notebook, PLUMA_NOTEBOOK(_pluma_window_get_notebook(new_window)), tab, 0);
...
static void notebook_tab_close_request(PlumaNotebook *notebook, PlumaTab *tab,
                                       GtkWindow *window) {
...
static gboolean show_notebook_popup_menu(GtkNotebook *notebook,
                                         PlumaWindow *window,
...
static gboolean notebook_button_press_event(GtkNotebook *notebook,
                                            GdkEventButton *event,
                                            PlumaWindow *window) {
  if (event->type == GDK_BUTTON_PRESS) {
    if (event->button == 3)
      ...
      PlumaTab *tab;
      tab = pluma_window_get_active_tab(window);
      notebook_tab_close_request(PLUMA_NOTEBOOK(notebook), tab,
                                 GTK_WINDOW(window));
    }
...
static gboolean notebook_scroll_event(GtkNotebook *notebook,
...
static gboolean notebook_popup_menu(GtkNotebook *notebook,
                                    PlumaWindow *window) {
...
static void side_panel_size_allocate(GtkWidget *widget,
                                     GtkAllocation *allocation,
                                     PlumaWindow *window) {
  window->priv->side_panel_size = allocation->width;
}

static void bottom_panel_size_allocate(GtkWidget *widget,
                                       GtkAllocation *allocation,
                                       PlumaWindow *window) {
  window->priv->bottom_panel_size = allocation->height;
}

static void hpaned_restore_position(GtkWidget *widget, PlumaWindow *window) {
  gint pos;
  ...
  g_signal_connect(window->priv->side_panel, "size-allocate",
...
static void vpaned_restore_position(GtkWidget *widget, PlumaWindow *window) {
  GtkAllocation allocation;
  gint pos;
  ...
  /* start monitoring the size */
  g_signal_connect(window->priv->bottom_panel, "size-allocate",
...
static void side_panel_visibility_changed(GtkWidget *side_panel,
                                          PlumaWindow *window) {
  gboolean visible;
  GtkAction *action;
  ...
  if (!g_settings_get_boolean(window->priv->editor_settings,
                              "show-tabs-with-side-pane")) {
    if (visible)
      gtk_notebook_set_show_tabs(GTK_NOTEBOOK(window->priv->notebook), FALSE);
    else
      gtk_notebook_set_show_tabs(
          GTK_NOTEBOOK(window->priv->notebook),
          g_settings_get_boolean(window->priv->editor_settings,
                                 "show-single-tab") ||
  ...
  g_settings_set_boolean(window->priv->editor_settings,
                         PLUMA_SETTINGS_SIDE_PANE_VISIBLE, visible);
  ...
  if (!visible && window->priv->active_tab != NULL)
    gtk_widget_grab_focus(
        GTK_WIDGET(pluma_tab_get_view(PLUMA_TAB(window->priv->active_tab))));
}

static void create_side_panel(PlumaWindow *window) {
  ...
  g_signal_connect_after(window->priv->side_panel, "show",
                         G_CALLBACK(side_panel_visibility_changed), window);
  g_signal_connect_after(window->priv->side_panel, "hide",
                         G_CALLBACK(side_panel_visibility_changed), window);

  documents_panel = pluma_documents_panel_new(window);
  pluma_panel_add_item_with_icon(PLUMA_PANEL(window->priv->side_panel),
                                 documents_panel, _("Documents"),
                                 "text-x-generic");
}

static void bottom_panel_visibility_changed(PlumaPanel *bottom_panel,
                                            PlumaWindow *window) {
  gboolean visible;
  GtkAction *action;
  ...
  g_settings_set_boolean(window->priv->editor_settings,
                         PLUMA_SETTINGS_BOTTOM_PANE_VISIBLE, visible);
...
static void bottom_panel_item_removed(PlumaPanel *panel, GtkWidget *item,
                                      PlumaWindow *window) {
...
static void bottom_panel_item_added(PlumaPanel *panel, GtkWidget *item,
                                    PlumaWindow *window) {
  ...
  if (pluma_panel_get_n_items(panel) == 1) {
    ...
    if (show) gtk_widget_show(GTK_WIDGET(panel));
  }
}

static void create_bottom_panel(PlumaWindow *window) {
  pluma_debug(DEBUG_WINDOW);
  ...
  g_signal_connect_after(window->priv->bottom_panel, "show",
                         G_CALLBACK(bottom_panel_visibility_changed), window);
  g_signal_connect_after(window->priv->bottom_panel, "hide",
                         G_CALLBACK(bottom_panel_visibility_changed), window);
}

static void init_panels_visibility(PlumaWindow *window) {
  gint active_page;
  ...
  bottom_pane_visible = g_settings_get_boolean(
      window->priv->editor_settings, PLUMA_SETTINGS_BOTTOM_PANE_VISIBLE);
...
static void window_realized(GtkWidget *window, gpointer *data) {
  ...
  g_signal_connect(clipboard, "owner_change",
                   G_CALLBACK(clipboard_owner_change), window);
}
...
static void check_window_is_active(PlumaWindow *window, GParamSpec *property,
                                   gpointer useless) {
...
static void connect_notebook_signals(PlumaWindow *window, GtkWidget *notebook) {
  g_signal_connect(notebook, "switch-page", G_CALLBACK(notebook_switch_page),
                   window);
  g_signal_connect(notebook, "tab-added", G_CALLBACK(notebook_tab_added),
                   window);
  g_signal_connect(notebook, "tab-removed", G_CALLBACK(notebook_tab_removed),
                   window);
  g_signal_connect(notebook, "tabs-reordered",
                   G_CALLBACK(notebook_tabs_reordered), window);
  g_signal_connect(notebook, "tab-detached", G_CALLBACK(notebook_tab_detached),
                   window);
  g_signal_connect(notebook, "tab-close-request",
                   G_CALLBACK(notebook_tab_close_request), window);
  g_signal_connect(notebook, "button-press-event",
                   G_CALLBACK(notebook_button_press_event), window);
  g_signal_connect(notebook, "popup-menu", G_CALLBACK(notebook_popup_menu),
                   window);
  g_signal_connect(notebook, "scroll-event", G_CALLBACK(notebook_scroll_event),
                   window);
}

static void add_notebook(PlumaWindow *window, GtkWidget *notebook) {
...
static void on_extension_added(PeasExtensionSet *extensions,
                               PeasPluginInfo *info, PeasExtension *exten,
                               PlumaWindow *window) {
  peas_extension_call(exten, "activate", window);
}

static void on_extension_removed(PeasExtensionSet *extensions,
                                 PeasPluginInfo *info, PeasExtension *exten,
                                 PlumaWindow *window) {
...
static void pluma_window_init(PlumaWindow *window) {
  GtkWidget *main_box;
  GtkTargetList *tl;
  ...
  window->priv->side_panel_size = g_settings_get_int(
      window->priv->editor_settings, PLUMA_SETTINGS_SIDE_PANEL_SIZE);
  window->priv->bottom_panel_size = g_settings_get_int(
      window->priv->editor_settings, PLUMA_SETTINGS_BOTTOM_PANEL_SIZE);

  g_signal_connect_after(window->priv->hpaned, "map",
                         G_CALLBACK(hpaned_restore_position), window);
  g_signal_connect_after(window->priv->vpaned, "map",
                         G_CALLBACK(vpaned_restore_position), window);
  ...
  g_signal_connect(window, "notify::is-active",
                   G_CALLBACK(check_window_is_active), NULL);
...
PlumaView *pluma_window_get_active_view(PlumaWindow *window) {
  ...
  if (window->priv->active_tab == NULL) return NULL;
 ...
 */
PlumaTab *pluma_window_create_tab(PlumaWindow *window, gboolean jump_to) {
  PlumaTab *tab;
  ...
  pluma_notebook_add_tab(PLUMA_NOTEBOOK(window->priv->notebook), tab, -1,
                         jump_to);
...
PlumaTab *pluma_window_create_tab_from_uri(PlumaWindow *window,
  ...
  tab = _pluma_tab_new_from_uri(uri, encoding, line_pos, create);
  if (tab == NULL) return NULL;

  gtk_widget_show(tab);

  pluma_notebook_add_tab(PLUMA_NOTEBOOK(window->priv->notebook), PLUMA_TAB(tab),
                         -1, jump_to);
...
PlumaTab *pluma_window_get_active_tab(PlumaWindow *window) {
  ...
  return (window->priv->active_tab == NULL)
             ? NULL
             : PLUMA_TAB(window->priv->active_tab);
}

static void add_document(PlumaTab *tab, GList **res) {
  PlumaDocument *doc;
...
static void add_view(PlumaTab *tab, GList **res) {
  PlumaView *view;
 ...
 * Closes the @tab.
 */
void pluma_window_close_tab(PlumaWindow *window, PlumaTab *tab) {
  g_return_if_fail(PLUMA_IS_WINDOW(window));
  g_return_if_fail(PLUMA_IS_TAB(tab));
  g_return_if_fail(
      (pluma_tab_get_state(tab) != PLUMA_TAB_STATE_SAVING) &&
 ...
 */
void pluma_window_close_tabs(PlumaWindow *window, const GList *tabs) {
  g_return_if_fail(PLUMA_IS_WINDOW(window));
  g_return_if_fail(!(window->priv->state & PLUMA_WINDOW_STATE_SAVING) &&
                   !(window->priv->state & PLUMA_WINDOW_STATE_SAVING_SESSION));
  ...
  while (tabs != NULL) {
    if (tabs->next == NULL) window->priv->removing_tabs = FALSE;
...
PlumaWindow *_pluma_window_move_tab_to_new_window(PlumaWindow *window,
  ...
  g_return_val_if_fail(
      gtk_notebook_get_n_pages(GTK_NOTEBOOK(window->priv->notebook)) > 1, NULL);

  new_window = clone_window(window);

  pluma_notebook_move_tab(PLUMA_NOTEBOOK(window->priv->notebook),
                          PLUMA_NOTEBOOK(new_window->priv->notebook), tab, -1);
 ...
 */
void pluma_window_set_active_tab(PlumaWindow *window, PlumaTab *tab) {
  gint page_num;
...
void _pluma_window_set_default_location(PlumaWindow *window, GFile *location) {
  GFile *dir;
...
static void hide_notebook_tabs_on_fullscreen(GtkNotebook *notebook,
                                             GParamSpec *pspec,
                                             PlumaWindow *window) {
...
void _pluma_window_fullscreen(PlumaWindow *window) {
  ...
  if (_pluma_window_is_fullscreen(window)) return;
  ...
  g_signal_handlers_block_by_func(window->priv->toolbar,
                                  toolbar_visibility_changed, window);
  gtk_widget_hide(window->priv->toolbar);

  g_signal_handlers_block_by_func(window->priv->statusbar,
                                  statusbar_visibility_changed, window);
  gtk_widget_hide(window->priv->statusbar);
...
void _pluma_window_unfullscreen(PlumaWindow *window) {
  ...
  g_signal_handlers_disconnect_by_func(
      window->priv->notebook, hide_notebook_tabs_on_fullscreen, window);
  ...
  if (visible) gtk_widget_show(window->priv->toolbar);
 ...
 *
 * Returns: (transfer none): the #PlumaTab that matches with the given
 * @location.
 */
...
void _pluma_window_get_default_size(gint *width, gint *height) {
