...
static void set_view(PlumaPrintJob *job, PlumaView *view) {
  job->priv->view = view;
  job->priv->doc =
      PLUMA_DOCUMENT(gtk_text_view_get_buffer(GTK_TEXT_VIEW(view)));
}

static void pluma_print_job_get_property(GObject *object, guint prop_id,
...
static void pluma_print_job_set_property(GObject *object, guint prop_id,
                                         const GValue *value,
                                         GParamSpec *pspec) {
  PlumaPrintJob *job = PLUMA_PRINT_JOB(object);
...
static void pluma_print_job_finalize(GObject *object) {
  PlumaPrintJob *job = PLUMA_PRINT_JOB(object);

  g_free(job->priv->status_string);

  if (job->priv->compositor != NULL) g_object_unref(job->priv->compositor);

  if (job->priv->operation != NULL) g_object_unref(job->priv->operation);

  G_OBJECT_CLASS(pluma_print_job_parent_class)->finalize(object);
}

static void pluma_print_job_dispose(GObject *object) {
  PlumaPrintJob *job = PLUMA_PRINT_JOB(object);
...
static void pluma_print_job_class_init(PlumaPrintJobClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object(
          "view", "Pluma View", "Pluma View to print", PLUMA_TYPE_VIEW,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS | G_PARAM_CONSTRUCT_ONLY));

  print_job_signals[PRINTING] = g_signal_new(
      "printing", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PlumaPrintJobClass, printing), NULL, NULL, NULL,
      G_TYPE_NONE, 1, G_TYPE_UINT);

  print_job_signals[SHOW_PREVIEW] = g_signal_new(
      "show-preview", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PlumaPrintJobClass, show_preview), NULL, NULL, NULL,
      G_TYPE_NONE, 1, GTK_TYPE_WIDGET);

  print_job_signals[DONE] =
      g_signal_new("done", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(PlumaPrintJobClass, done), NULL, NULL, NULL,
                   G_TYPE_NONE, 2, G_TYPE_UINT, G_TYPE_POINTER);
}

static void line_numbers_checkbutton_toggled(GtkToggleButton *button,
                                             PlumaPrintJob *job) {
...
static void wrap_mode_checkbutton_toggled(GtkToggleButton *button,
  ...
  if (!gtk_toggle_button_get_active(
          GTK_TOGGLE_BUTTON(job->priv->text_wrapping_checkbutton))) {
...
static void restore_button_clicked(GtkButton *button, PlumaPrintJob *job)
...
static GObject *create_custom_widget_cb(GtkPrintOperation *operation,
  ...
  ret = pluma_utils_get_ui_objects(
      PLUMA_DATADIR "/ui/pluma-print-preferences.ui", root_objects,
      &error_widget, "contents", &widget, "syntax_checkbutton",
      &job->priv->syntax_checkbutton, "line_numbers_checkbutton",
      &job->priv->line_numbers_checkbutton, "line_numbers_hbox",
      &job->priv->line_numbers_hbox, "line_numbers_spinbutton",
      &job->priv->line_numbers_spinbutton, "page_header_checkbutton",
      &job->priv->page_header_checkbutton, "text_wrapping_checkbutton",
      &job->priv->text_wrapping_checkbutton, "do_not_split_checkbutton",
      &job->priv->do_not_split_checkbutton, "fonts_table",
      &job->priv->fonts_table, "body_font_label", &job->priv->body_font_label,
      "body_fontbutton", &job->priv->body_fontbutton, "headers_font_label",
      &job->priv->headers_font_label, "headers_fontbutton",
      &job->priv->headers_fontbutton, "numbers_font_label",
      &job->priv->numbers_font_label, "numbers_fontbutton",
      &job->priv->numbers_fontbutton, "restore_button",
      &job->priv->restore_button, NULL);
  ...
  line_numbers = g_settings_get_uint(job->priv->print_settings,
  ...
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(job->priv->page_header_checkbutton), print_header);

  /* Line numbers */
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(job->priv->line_numbers_checkbutton), line_numbers > 0);
  ...
  if (line_numbers > 0) {
    ...
    gtk_spin_button_set_value(
        GTK_SPIN_BUTTON(job->priv->line_numbers_spinbutton), 1);
...
static void custom_widget_apply_cb(GtkPrintOperation *operation,
                                   GtkWidget *widget, PlumaPrintJob *job) {
  gboolean syntax, page_header;
  const gchar *body, *header, *numbers;

  syntax = gtk_toggle_button_get_active(
      GTK_TOGGLE_BUTTON(job->priv->syntax_checkbutton));
  page_header = gtk_toggle_button_get_active(
      GTK_TOGGLE_BUTTON(job->priv->page_header_checkbutton));
  body =
      gtk_font_chooser_get_font(GTK_FONT_CHOOSER(job->priv->body_fontbutton));
  header = gtk_font_chooser_get_font(
      GTK_FONT_CHOOSER(job->priv->headers_fontbutton));
  numbers = gtk_font_chooser_get_font(
      GTK_FONT_CHOOSER(job->priv->numbers_fontbutton));
  ...
  g_settings_set_string(job->priv->print_settings,
                        PLUMA_SETTINGS_PRINT_FONT_BODY_PANGO, body);
  g_settings_set_string(job->priv->print_settings,
                        PLUMA_SETTINGS_PRINT_FONT_HEADER_PANGO, header);
  g_settings_set_string(job->priv->print_settings,
                        PLUMA_SETTINGS_PRINT_FONT_NUMBERS_PANGO, numbers);

  if (gtk_toggle_button_get_active(
          GTK_TOGGLE_BUTTON(job->priv->line_numbers_checkbutton))) {
    g_settings_set_uint(
        job->priv->print_settings, PLUMA_SETTINGS_PRINT_LINE_NUMBERS,
  ...
  if (!gtk_toggle_button_get_active(
    ...
    pluma_settings_set_wrap_mode(job->priv->print_settings,
                                 PLUMA_SETTINGS_PRINT_WRAP_MODE, GTK_WRAP_NONE);
  } else {
    if (gtk_toggle_button_get_active(
...
static void create_compositor(PlumaPrintJob *job) {
  ...
  print_font_header = g_settings_get_string(
      job->priv->print_settings, PLUMA_SETTINGS_PRINT_FONT_HEADER_PANGO);
  print_font_numbers = g_settings_get_string(
      job->priv->print_settings, PLUMA_SETTINGS_PRINT_FONT_NUMBERS_PANGO);
  syntax_hl = g_settings_get_boolean(job->priv->print_settings,
                                     PLUMA_SETTINGS_PRINT_SYNTAX_HIGHLIGHTING);
  print_line_numbers = g_settings_get_uint(job->priv->print_settings,
                                           PLUMA_SETTINGS_PRINT_LINE_NUMBERS);
  print_header = g_settings_get_boolean(job->priv->print_settings,
                                        PLUMA_SETTINGS_PRINT_HEADER);
  wrap_mode = pluma_settings_get_wrap_mode(job->priv->print_settings,
  ...
  job->priv->compositor = GTK_SOURCE_PRINT_COMPOSITOR(g_object_new(
      GTK_SOURCE_TYPE_PRINT_COMPOSITOR, "buffer",
      GTK_SOURCE_BUFFER(job->priv->doc), "tab-width",
      gtk_source_view_get_tab_width(GTK_SOURCE_VIEW(job->priv->view)),
      "highlight-syntax",
      gtk_source_buffer_get_highlight_syntax(
          GTK_SOURCE_BUFFER(job->priv->doc)) &&
          syntax_hl,
      "wrap-mode", wrap_mode, "print-line-numbers", print_line_numbers,
      "print-header", print_header, "print-footer", FALSE, "body-font-name",
      print_font_body, "line-numbers-font-name", print_font_numbers,
  ...
  if (print_header) {
    ...
    gtk_source_print_compositor_set_header_format(
        job->priv->compositor, TRUE, left, NULL, _("Page %N of %Q"));
...
static void begin_print_cb(GtkPrintOperation *operation,
                           GtkPrintContext *context, PlumaPrintJob *job) {
...
static void preview_ready(GtkPrintOperationPreview *gtk_preview,
                          GtkPrintContext *context, PlumaPrintJob *job) {
...
static void preview_destroyed(GtkWidget *preview,
                              GtkPrintOperationPreview *gtk_preview) {
  gtk_print_operation_preview_end_preview(gtk_preview);
}

static gboolean preview_cb(GtkPrintOperation *op,
                           GtkPrintOperationPreview *gtk_preview,
                           GtkPrintContext *context, GtkWindow *parent,
                           PlumaPrintJob *job) {
  job->priv->preview = pluma_print_preview_new(op, gtk_preview, context);

  g_signal_connect_after(gtk_preview, "ready", G_CALLBACK(preview_ready), job);

  /* FIXME: should this go in the preview widget itself? */
  g_signal_connect(job->priv->preview, "destroy", G_CALLBACK(preview_destroyed),
                   gtk_preview);
...
static gboolean paginate_cb(GtkPrintOperation *operation,
  ...
  job->priv->progress = gtk_source_print_compositor_get_pagination_progress(
      job->priv->compositor);

  /* When previewing, the progress is just for pagination, when printing
   * it's split between pagination and rendering */
  if (!job->priv->is_preview) job->priv->progress /= 2.0;
...
static void draw_page_cb(GtkPrintOperation *operation, GtkPrintContext *context,
                         gint page_nr, PlumaPrintJob *job) {
  gint n_pages;
  ...
  if (!job->priv->is_preview) {
    ...
    job->priv->status_string =
        g_strdup_printf("Rendering page %d of %d...", page_nr + 1, n_pages);
  ...
  gtk_source_print_compositor_draw_page(job->priv->compositor, context,
                                        page_nr);
}

static void end_print_cb(GtkPrintOperation *operation, GtkPrintContext *context,
                         PlumaPrintJob *job) {
...
static void done_cb(GtkPrintOperation *operation,
                    GtkPrintOperationResult result, PlumaPrintJob *job) {
  GError *error = NULL;
...
/* Note that pluma_print_job_print can can only be called once on a given
 * PlumaPrintJob */
GtkPrintOperationResult pluma_print_job_print(PlumaPrintJob *job,
  ...
  g_return_val_if_fail(job->priv->compositor == NULL,
  ...
  g_signal_connect(priv->operation, "begin-print", G_CALLBACK(begin_print_cb),
                   job);
  g_signal_connect(priv->operation, "preview", G_CALLBACK(preview_cb), job);
  g_signal_connect(priv->operation, "paginate", G_CALLBACK(paginate_cb), job);
  g_signal_connect(priv->operation, "draw-page", G_CALLBACK(draw_page_cb), job);
  g_signal_connect(priv->operation, "end-print", G_CALLBACK(end_print_cb), job);
  g_signal_connect(priv->operation, "done", G_CALLBACK(done_cb), job);

  return gtk_print_operation_run(priv->operation, action, parent, error);
}

static void pluma_print_job_init(PlumaPrintJob *job) {
...
PlumaPrintJob *pluma_print_job_new(PlumaView *view) {
  ...
  job = PLUMA_PRINT_JOB(g_object_new(PLUMA_TYPE_PRINT_JOB, "view", view, NULL));
