...
#define PLUMA_SESSION_LIST_OF_DOCS_TO_SAVE \
  "pluma-session-list-of-docs-to-save-key"

static void save_window_session(GKeyFile *state_file, const gchar *group_name,
                                PlumaWindow *window) {
  const gchar *role;
  int width, height;
  PlumaPanel *panel;
  ...
  for (l = docs; l != NULL; l = g_list_next(l)) {
    ...
    if (uri != NULL) g_ptr_array_add(doc_array, uri);
  }
  g_list_free(docs);
  ...
  if (doc_array->len) {
    ...
    g_key_file_set_string_list(state_file, group_name, "documents",
                               (const char **)doc_array->pdata, doc_array->len);
    for (i = 0; i < doc_array->len; i++) g_free(doc_array->pdata[i]);
  }
  g_ptr_array_free(doc_array, TRUE);
}

static void client_save_state_cb(EggSMClient *client, GKeyFile *state_file,
                                 gpointer user_data) {
  const GList *windows;
  gchar *group_name;
  int n;
  ...
  while (windows != NULL) {
    ...
    save_window_session(state_file, group_name, PLUMA_WINDOW(windows->data));
    g_free(group_name);
...
static void window_handled(PlumaWindow *window) {
...
static void window_state_change(PlumaWindow *window, GParamSpec *pspec,
                                gpointer data) {
  ...
  /* we are still saving */
  if (state & PLUMA_WINDOW_STATE_SAVING) return;

  unsaved_docs = pluma_window_get_unsaved_documents(window);

  docs_to_save =
      g_object_get_data(G_OBJECT(window), PLUMA_SESSION_LIST_OF_DOCS_TO_SAVE);
  ...
  if (done) {
    ...
    g_object_set_data(G_OBJECT(window), PLUMA_SESSION_LIST_OF_DOCS_TO_SAVE,
                      NULL);
...
static void close_confirmation_dialog_response_handler(
  ...
  switch (response_id) {
    case GTK_RESPONSE_YES:
      ...
      g_signal_connect(window, "notify::state", G_CALLBACK(window_state_change),
                       NULL);
    ...
    default:
      ...
      for (l = window_dirty_list; l != NULL; l = l->next)
        g_signal_handlers_disconnect_by_func(window, window_state_change, NULL);
...
static void show_confirmation_dialog(PlumaWindow *window) {
  GList *unsaved_docs;
  GtkWidget *dlg;
...
static void client_quit_requested_cb(EggSMClient *client, gpointer data) {
  PlumaApp *app;
  const GList *l;
...
void pluma_session_init(void) {
  ...
  if (master_client) return;

  master_client = egg_sm_client_get();
  g_signal_connect(master_client, "save_state",
                   G_CALLBACK(client_save_state_cb), NULL);
  g_signal_connect(master_client, "quit_requested",
                   G_CALLBACK(client_quit_requested_cb), NULL);
  g_signal_connect(master_client, "quit", G_CALLBACK(client_quit_cb), NULL);
}
...
static void parse_window(GKeyFile *state_file, const char *group_name) {
  PlumaWindow *window;
  ...
  width = g_key_file_get_integer(state_file, group_name, "width", &error);
  if (error) {
    g_clear_error(&error);
    width = -1;
  }
  height = g_key_file_get_integer(state_file, group_name, "height", &error);
  if (error) {
    g_clear_error(&error);
    height = -1;
  }
  ...
  visible = g_key_file_get_boolean(state_file, group_name, "side-panel-visible",
                                   &error);
  if (error) {
    g_clear_error(&error);
    visible = FALSE;
  ...
  active_document =
      g_key_file_get_string(state_file, group_name, "active-document", NULL);
  documents = g_key_file_get_string_list(state_file, group_name, "documents",
                                         NULL, NULL);
  if (documents) {
    int i;
    ...
    for (i = 0; documents[i]; i++) {
      ...
      pluma_debug_message(DEBUG_SESSION, "URI: %s (%s)", documents[i],
                          jump_to ? "active" : "not active");
      pluma_window_create_tab_from_uri(window, documents[i], NULL, 0, FALSE,
                                       jump_to);
    }
...
gboolean pluma_session_load(void) {
  ...
  if (state_file == NULL) return FALSE;
