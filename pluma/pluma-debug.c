...
void pluma_debug_init(void) {
  ...
  if (g_getenv("PLUMA_DEBUG_VIEW") != NULL) debug = debug | PLUMA_DEBUG_VIEW;
  if (g_getenv("PLUMA_DEBUG_SEARCH") != NULL)
    debug = debug | PLUMA_DEBUG_SEARCH;
  if (g_getenv("PLUMA_DEBUG_PREFS") != NULL) debug = debug | PLUMA_DEBUG_PREFS;
  if (g_getenv("PLUMA_DEBUG_PRINT") != NULL) debug = debug | PLUMA_DEBUG_PRINT;
  if (g_getenv("PLUMA_DEBUG_PLUGINS") != NULL)
    debug = debug | PLUMA_DEBUG_PLUGINS;
  if (g_getenv("PLUMA_DEBUG_TAB") != NULL) debug = debug | PLUMA_DEBUG_TAB;
  ...
  if (g_getenv("PLUMA_DEBUG_APP") != NULL) debug = debug | PLUMA_DEBUG_APP;
  if (g_getenv("PLUMA_DEBUG_SESSION") != NULL)
    debug = debug | PLUMA_DEBUG_SESSION;
  if (g_getenv("PLUMA_DEBUG_UTILS") != NULL) debug = debug | PLUMA_DEBUG_UTILS;
  ...
  if (g_getenv("PLUMA_DEBUG_SAVER") != NULL) debug = debug | PLUMA_DEBUG_SAVER;

out:

#ifdef ENABLE_PROFILING
  if (debug != PLUMA_NO_DEBUG) timer = g_timer_new();
#endif
  return;
}

void pluma_debug_message(PlumaDebugSection section, const gchar *file,
...
void pluma_debug(PlumaDebugSection section, const gchar *file, gint line,
