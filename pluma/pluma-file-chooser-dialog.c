...
G_DEFINE_TYPE_WITH_PRIVATE(PlumaFileChooserDialog, pluma_file_chooser_dialog,
                           GTK_TYPE_FILE_CHOOSER_DIALOG)

static void pluma_file_chooser_dialog_dispose(GObject *object) {
...
static void pluma_file_chooser_dialog_class_init(
    PlumaFileChooserDialogClass *klass) {
  GObjectClass *object_class = G_OBJECT_CLASS(klass);

  object_class->dispose = pluma_file_chooser_dialog_dispose;
}

static void create_option_menu(PlumaFileChooserDialog *dialog) {
  GtkWidget *label;
  GtkWidget *menu;
  ...
  gtk_box_pack_start(GTK_BOX(dialog->priv->extra_widget), label, FALSE, TRUE,
                     0);
...
static void update_newline_visibility(PlumaFileChooserDialog *dialog) {
...
static void newline_combo_append(GtkComboBox *combo, GtkListStore *store,
                                 GtkTreeIter *iter, const gchar *label,
...
static void create_newline_combo(PlumaFileChooserDialog *dialog) {
  ...
  store =
      gtk_list_store_new(2, G_TYPE_STRING, PLUMA_TYPE_DOCUMENT_NEWLINE_TYPE);
  ...
  newline_combo_append(GTK_COMBO_BOX(combo), store, &iter, _("Unix/Linux"),
                       PLUMA_DOCUMENT_NEWLINE_TYPE_LF);

  newline_combo_append(GTK_COMBO_BOX(combo), store, &iter, _("Mac OS Classic"),
                       PLUMA_DOCUMENT_NEWLINE_TYPE_CR);

  newline_combo_append(GTK_COMBO_BOX(combo), store, &iter, _("Windows"),
...
static void create_extra_widget(PlumaFileChooserDialog *dialog) {
...
static void action_changed(PlumaFileChooserDialog *dialog, GParamSpec *pspec,
                           gpointer data) {
...
static void filter_changed(PlumaFileChooserDialog *dialog, GParamSpec *pspec,
                           gpointer data) {
  ...
  if (filter != NULL) {
    ...
    if (strcmp(name, ALL_TEXT_FILES) == 0) id = 1;
...
static gboolean all_text_files_filter(const GtkFileFilterInfo *filter_info,
                                      gpointer data) {
  ...
  if (known_mime_types == NULL) {
    ...
    while ((languages != NULL) && (*languages != NULL)) {
      ...
      if (mime_types_srtv == NULL) continue;
      ...
      for (i = 0; mime_types_srtv[i] != NULL; i++) {
        if (!g_content_type_is_a(mime_types_srtv[i], "text/plain")) {
          ...
          known_mime_types =
              g_slist_prepend(known_mime_types, g_strdup(mime_types_srtv[i]));
        }
      }
   ...
   * unrelated to "text/plain" that pluma recognizes */

  if (filter_info->mime_type == NULL) return FALSE;
  ...
  if (strncmp(filter_info->mime_type, "text/", 5) == 0) return TRUE;

  mime_types = known_mime_types;
  while (mime_types != NULL) {
    if (g_content_type_is_a(filter_info->mime_type,
...
static void pluma_file_chooser_dialog_init(PlumaFileChooserDialog *dialog) {
...
static GtkWidget *pluma_file_chooser_dialog_new_valist(
    const gchar *title, GtkWindow *parent, GtkFileChooserAction action,
    const PlumaEncoding *encoding, const gchar *first_button_text,
    va_list varargs) {
  GtkWidget *result;
  ...
  result =
      g_object_new(PLUMA_TYPE_FILE_CHOOSER_DIALOG, "title", title, "local-only",
  ...
  g_signal_connect(result, "notify::action", G_CALLBACK(action_changed), NULL);

  if (encoding != NULL)
    pluma_encodings_combo_box_set_selected_encoding(
        PLUMA_ENCODINGS_COMBO_BOX(
            PLUMA_FILE_CHOOSER_DIALOG(result)->priv->option_menu),
        encoding);

  active_filter = g_settings_get_int(
      PLUMA_FILE_CHOOSER_DIALOG(result)->priv->filter_settings,
  ...
  gtk_file_filter_add_custom(filter, GTK_FILE_FILTER_MIME_TYPE,
                             all_text_files_filter, NULL, NULL);
  gtk_file_chooser_add_filter(GTK_FILE_CHOOSER(result), filter);
  ...
  g_signal_connect(result, "notify::filter", G_CALLBACK(filter_changed), NULL);
  ...
  while (button_text) {
    ...
    if (g_strcmp0(button_text, "process-stop") == 0)
      ...
      pluma_dialog_add_button(GTK_DIALOG(result), _("_Open"), button_text,
                              response_id);
    else if (g_strcmp0(button_text, "document-save") == 0)
      pluma_dialog_add_button(GTK_DIALOG(result), _("_Save"), button_text,
                              response_id);
    else
...
GtkWidget *pluma_file_chooser_dialog_new(const gchar *title, GtkWindow *parent,
  ...
  result = pluma_file_chooser_dialog_new_valist(title, parent, action, encoding,
...
void pluma_file_chooser_dialog_set_encoding(PlumaFileChooserDialog *dialog,
  ...
  pluma_encodings_combo_box_set_selected_encoding(
      PLUMA_ENCODINGS_COMBO_BOX(dialog->priv->option_menu), encoding);
}
...
const PlumaEncoding *pluma_file_chooser_dialog_get_encoding(
  ...
  g_return_val_if_fail(PLUMA_IS_ENCODINGS_COMBO_BOX(dialog->priv->option_menu),
                       NULL);
...
PlumaDocumentNewlineType pluma_file_chooser_dialog_get_newline_type(
  ...
  g_return_val_if_fail(PLUMA_IS_FILE_CHOOSER_DIALOG(dialog),
  ...
  gtk_tree_model_get(GTK_TREE_MODEL(dialog->priv->newline_store), &iter, 1,
                     &newline_type, -1);
