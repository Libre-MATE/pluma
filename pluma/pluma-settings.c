...
static void pluma_settings_dispose(GObject *object) {
  PlumaSettingsPrivate *priv =
      pluma_settings_get_instance_private(PLUMA_SETTINGS(object));
...
static void pluma_settings_finalize(GObject *object) {
...
static void pluma_settings_space_drawer_generic(GSettings *settings, gint level,
  ...
  while (l != NULL) {
    pluma_set_source_space_drawer_by_level(GTK_SOURCE_VIEW(l->data), level,
                                           type);
    l = l->next;
  }
...
#else
static void pluma_settings_draw_generic(GSettings *settings, gint level,
                                        GtkSourceSpaceTypeFlags type) {
  GList *views;
  GList *l;
  ...
  while (l != NULL) {
    ...
    if (level > 0)
      ...
      value &= ~type;
    gtk_source_view_set_draw_spaces(GTK_SOURCE_VIEW(l->data), value);
    l = l->next;
  }
...
static void on_lockdown_changed(GSettings *settings, const gchar *key,
                                gpointer useless) {
  gboolean locked;
  PlumaApp *app;
  ...
  if (strcmp(key, PLUMA_SETTINGS_LOCKDOWN_COMMAND_LINE) == 0)
    _pluma_app_set_lockdown_bit(app, PLUMA_LOCKDOWN_COMMAND_LINE, locked);
  else if (strcmp(key, PLUMA_SETTINGS_LOCKDOWN_PRINTING) == 0)
    _pluma_app_set_lockdown_bit(app, PLUMA_LOCKDOWN_PRINTING, locked);
  else if (strcmp(key, PLUMA_SETTINGS_LOCKDOWN_PRINT_SETUP) == 0)
    _pluma_app_set_lockdown_bit(app, PLUMA_LOCKDOWN_PRINT_SETUP, locked);
  else if (strcmp(key, PLUMA_SETTINGS_LOCKDOWN_SAVE_TO_DISK) == 0)
    _pluma_app_set_lockdown_bit(app, PLUMA_LOCKDOWN_SAVE_TO_DISK, locked);
}

static void set_font(PlumaSettings *self, const gchar *font) {
  GList *views, *l;
  guint ts;

  ts = g_settings_get_uint(self->priv->editor_settings,
...
static void on_system_font_changed(GSettings *settings, const gchar *key,
                                   PlumaSettings *self) {
...
static void on_editor_font_changed(GSettings *settings, const gchar *key,
                                   PlumaSettings *self) {
  gboolean use_default_font;
  gchar *font;
...
static void on_scheme_changed(GSettings *settings, const gchar *key,
                              PlumaSettings *self) {
  ...
  if (self->priv->old_scheme != NULL &&
      (strcmp(scheme, self->priv->old_scheme) == 0)) {
    g_free(scheme);
    return;
  }
  ...
  style = gtk_source_style_scheme_manager_get_scheme(
      pluma_get_style_scheme_manager(), scheme);

  if (style == NULL) {
    g_warning("Default style scheme '%s' not found, falling back to 'classic'",
              scheme);

    style = gtk_source_style_scheme_manager_get_scheme(
        pluma_get_style_scheme_manager(), "classic");
...
static void on_auto_save_interval_changed(GSettings *settings, const gchar *key,
                                          PlumaSettings *self) {
  GList *docs, *l;
...
static void on_undo_actions_limit_changed(GSettings *settings, const gchar *key,
                                          PlumaSettings *self) {
  GList *docs, *l;
  gint ul;
...
static void on_wrap_mode_changed(GSettings *settings, const gchar *key,
                                 PlumaSettings *self) {
...
static void on_tabs_size_changed(GSettings *settings, const gchar *key,
                                 PlumaSettings *self) {
  GList *views, *l;
  guint ts;
...
static void on_insert_spaces_changed(GSettings *settings, const gchar *key,
                                     PlumaSettings *self) {
  GList *views, *l;
  gboolean spaces;
...
static void on_auto_indent_changed(GSettings *settings, const gchar *key,
                                   PlumaSettings *self) {
  GList *views, *l;
  gboolean enable;
...
static void on_display_line_numbers_changed(GSettings *settings,
                                            const gchar *key,
                                            PlumaSettings *self) {
  GList *views, *l;
...
static void on_hl_current_line_changed(GSettings *settings, const gchar *key,
                                       PlumaSettings *self) {
  GList *views, *l;
  gboolean hl;
...
static void on_bracket_matching_changed(GSettings *settings, const gchar *key,
                                        PlumaSettings *self) {
  GList *docs, *l;
  gboolean enable;
...
static void on_display_right_margin_changed(GSettings *settings,
                                            const gchar *key,
                                            PlumaSettings *self) {
  GList *views, *l;
  gboolean display;
...
static void on_right_margin_position_changed(GSettings *settings,
                                             const gchar *key,
                                             PlumaSettings *self) {
  GList *views, *l;
  guint pos;
...
static void on_smart_home_end_changed(GSettings *settings, const gchar *key,
                                      PlumaSettings *self) {
...
static void on_syntax_highlighting_changed(GSettings *settings,
                                           const gchar *key,
                                           PlumaSettings *self) {
  const GList *windows;
  GList *docs, *l;
...
static void on_search_highlighting_changed(GSettings *settings,
                                           const gchar *key,
                                           PlumaSettings *self) {
  GList *docs, *l;
  gboolean enable;
...
static void on_max_recents_changed(GSettings *settings, const gchar *key,
                                   PlumaSettings *self) {
  const GList *windows;
  guint max;
  ...
  while (windows != NULL) {
    ...
    gtk_recent_chooser_set_limit(
        GTK_RECENT_CHOOSER(w->priv->toolbar_recent_menu), max);
...
static void on_draw_spaces_changed(GSettings *settings, gchar *key,
                                   gpointer user_data) {
  pluma_debug(DEBUG_PREFS);

  if (strcmp(key, PLUMA_SETTINGS_DRAWER_SPACE)) return;
...
#else
  pluma_settings_draw_generic(settings, g_settings_get_enum(settings, key),
                              GTK_SOURCE_DRAW_SPACES_SPACE);
#endif
}

static void on_draw_tabs_changed(GSettings *settings, gchar *key,
                                 gpointer user_data) {
  pluma_debug(DEBUG_PREFS);

  if (strcmp(key, PLUMA_SETTINGS_DRAWER_TAB)) return;

#ifdef GTK_SOURCE_VERSION_3_24
  pluma_settings_space_drawer_generic(
      settings, g_settings_get_enum(settings, key), GTK_SOURCE_SPACE_TYPE_TAB);
#else
  pluma_settings_draw_generic(settings, g_settings_get_enum(settings, key),
...
static void on_draw_newlines_changed(GSettings *settings, gchar *key,
                                     gpointer user_data) {
  pluma_debug(DEBUG_PREFS);

  if (strcmp(key, PLUMA_SETTINGS_DRAWER_NEWLINE)) return;

#ifdef GTK_SOURCE_VERSION_3_24
  pluma_settings_space_drawer_generic(
      settings, g_settings_get_boolean(settings, key) ? 1 : 0,
...
static void on_draw_nbsp_changed(GSettings *settings, gchar *key,
                                 gpointer user_data) {
  pluma_debug(DEBUG_PREFS);

  if (strcmp(key, PLUMA_SETTINGS_DRAWER_NBSP)) return;

#ifdef GTK_SOURCE_VERSION_3_24
  pluma_settings_space_drawer_generic(
      settings, g_settings_get_enum(settings, key), GTK_SOURCE_SPACE_TYPE_NBSP);
#else
  pluma_settings_draw_generic(settings, g_settings_get_enum(settings, key),
...
static void pluma_settings_class_init(PlumaSettingsClass *klass) {
...
static void pluma_settings_init(PlumaSettings *self) {
  ...
  g_signal_connect(self->priv->lockdown_settings, "changed",
                   G_CALLBACK(on_lockdown_changed), NULL);
  ...
  /* editor changes */
  g_signal_connect(self->priv->editor_settings, "changed::use-default-font",
                   G_CALLBACK(on_use_default_font_changed), self);
  g_signal_connect(self->priv->editor_settings, "changed::editor-font",
                   G_CALLBACK(on_editor_font_changed), self);
  g_signal_connect(self->priv->editor_settings, "changed::color-scheme",
                   G_CALLBACK(on_scheme_changed), self);
  g_signal_connect(self->priv->editor_settings, "changed::auto-save",
                   G_CALLBACK(on_auto_save_changed), self);
  g_signal_connect(self->priv->editor_settings, "changed::auto-save-interval",
                   G_CALLBACK(on_auto_save_interval_changed), self);
  g_signal_connect(self->priv->editor_settings, "changed::undo-actions-limit",
                   G_CALLBACK(on_undo_actions_limit_changed), self);
  g_signal_connect(self->priv->editor_settings, "changed::wrap-mode",
                   G_CALLBACK(on_wrap_mode_changed), self);
  g_signal_connect(self->priv->editor_settings, "changed::tabs-size",
                   G_CALLBACK(on_tabs_size_changed), self);
  g_signal_connect(self->priv->editor_settings, "changed::insert-spaces",
                   G_CALLBACK(on_insert_spaces_changed), self);
  g_signal_connect(self->priv->editor_settings, "changed::auto-indent",
                   G_CALLBACK(on_auto_indent_changed), self);
  g_signal_connect(self->priv->editor_settings, "changed::display-line-numbers",
                   G_CALLBACK(on_display_line_numbers_changed), self);
  g_signal_connect(self->priv->editor_settings,
                   "changed::highlight-current-line",
                   G_CALLBACK(on_hl_current_line_changed), self);
  g_signal_connect(self->priv->editor_settings, "changed::bracket-matching",
                   G_CALLBACK(on_bracket_matching_changed), self);
  g_signal_connect(self->priv->editor_settings, "changed::display-right-margin",
                   G_CALLBACK(on_display_right_margin_changed), self);
  g_signal_connect(self->priv->editor_settings,
                   "changed::right-margin-position",
                   G_CALLBACK(on_right_margin_position_changed), self);
  g_signal_connect(self->priv->editor_settings, "changed::smart-home-end",
                   G_CALLBACK(on_smart_home_end_changed), self);
  g_signal_connect(self->priv->editor_settings, "changed::syntax-highlighting",
                   G_CALLBACK(on_syntax_highlighting_changed), self);
  g_signal_connect(self->priv->editor_settings, "changed::search-highlighting",
                   G_CALLBACK(on_search_highlighting_changed), self);
  g_signal_connect(self->priv->editor_settings, "changed::max-recents",
                   G_CALLBACK(on_max_recents_changed), self);
  g_signal_connect(self->priv->editor_settings,
                   "changed::enable-space-drawer-space",
                   G_CALLBACK(on_draw_spaces_changed), NULL);
  g_signal_connect(self->priv->editor_settings,
                   "changed::enable-space-drawer-tab",
                   G_CALLBACK(on_draw_tabs_changed), NULL);
  g_signal_connect(self->priv->editor_settings,
                   "changed::enable-space-drawer-newline",
                   G_CALLBACK(on_draw_newlines_changed), NULL);
  g_signal_connect(self->priv->editor_settings,
                   "changed::enable-space-drawer-nbsp",
                   G_CALLBACK(on_draw_nbsp_changed), NULL);
}
...
PlumaLockdownMask pluma_settings_get_lockdown(PlumaSettings *self) {
  ...
  if (command_line) lockdown |= PLUMA_LOCKDOWN_COMMAND_LINE;

  if (printing) lockdown |= PLUMA_LOCKDOWN_PRINTING;

  if (print_setup) lockdown |= PLUMA_LOCKDOWN_PRINT_SETUP;

  if (save_to_disk) lockdown |= PLUMA_LOCKDOWN_SAVE_TO_DISK;
...
GtkSourceSmartHomeEndType pluma_settings_get_smart_home_end(
    PlumaSettings *self) {
  GtkSourceSmartHomeEndType res;
  gchar *str;

  g_return_val_if_fail(PLUMA_IS_SETTINGS(self),
...
static gchar *get_wrap_str(guint mode) {
  gchar *str;
...
void pluma_settings_set_wrap_mode(GSettings *settings, const gchar *key,
                                  GtkWrapMode mode) {
  gchar *str;
...
GSList *pluma_settings_get_list(GSettings *settings, const gchar *key) {
  gchar **array;
  GSList *list = NULL;
  gint i;
...
GSList *pluma_settings_get_writable_vfs_schemes(GSettings *settings) {
  ...
  strings =
      pluma_settings_get_list(settings, PLUMA_SETTINGS_WRITABLE_VFS_SCHEMES);
