...
G_DEFINE_TYPE_WITH_PRIVATE(PlumaPluginsEngine, pluma_plugins_engine,
                           PEAS_TYPE_ENGINE)

PlumaPluginsEngine *default_engine = NULL;

static void pluma_plugins_engine_init(PlumaPluginsEngine *engine) {
  GError *error = NULL;
  ...
  if (!g_irepository_require(g_irepository_get_default(), "Peas", "1.0", 0,
                             &error)) {
    g_warning("Could not load Peas repository: %s", error->message);
    g_clear_error(&error);
  }

  if (!g_irepository_require(g_irepository_get_default(), "PeasGtk", "1.0", 0,
                             &error)) {
  ...
  if (!g_irepository_require_private(g_irepository_get_default(),
                                     LIBDIR "/girepository-1.0", "Pluma", "1.0",
                                     0, &error)) {
  ...
  peas_engine_add_search_path(PEAS_ENGINE(engine), user_plugins_dir,
                              user_plugins_dir);
  g_free(user_plugins_dir);

  peas_engine_add_search_path(PEAS_ENGINE(engine), PLUMA_LIBDIR "/plugins",
                              PLUMA_DATADIR "/plugins");

  g_settings_bind(engine->priv->plugin_settings, PLUMA_SETTINGS_ACTIVE_PLUGINS,
                  engine, "loaded-plugins", G_SETTINGS_BIND_DEFAULT);
}

static void pluma_plugins_engine_dispose(GObject *object) {
...
static void pluma_plugins_engine_class_init(PlumaPluginsEngineClass *klass) {
...
PlumaPluginsEngine *pluma_plugins_engine_get_default(void) {
  if (default_engine != NULL) return default_engine;

  default_engine =
      PLUMA_PLUGINS_ENGINE(g_object_new(PLUMA_TYPE_PLUGINS_ENGINE, NULL));
