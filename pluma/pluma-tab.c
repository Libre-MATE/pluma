...
enum { PROP_0, PROP_NAME, PROP_STATE, PROP_AUTO_SAVE, PROP_AUTO_SAVE_INTERVAL };

static gboolean pluma_tab_auto_save(PlumaTab *tab);

static void install_auto_save_timeout(PlumaTab *tab) {
  gint timeout;
  ...
  timeout = g_timeout_add_seconds(tab->priv->auto_save_interval * 60,
                                  (GSourceFunc)pluma_tab_auto_save, tab);

  tab->priv->auto_save_timeout = timeout;
}

static gboolean install_auto_save_timeout_if_needed(PlumaTab *tab) {
  PlumaDocument *doc;
  ...
  g_return_val_if_fail(
      (tab->priv->state == PLUMA_TAB_STATE_NORMAL) ||
          ...
          (tab->priv->state == PLUMA_TAB_STATE_CLOSING),
      FALSE);

  if (tab->priv->state == PLUMA_TAB_STATE_CLOSING) return FALSE;

  doc = pluma_tab_get_document(tab);

  if (tab->priv->auto_save && !pluma_document_is_untitled(doc) &&
...
static void remove_auto_save_timeout(PlumaTab *tab) {
  pluma_debug(DEBUG_TAB);
...
static void pluma_tab_get_property(GObject *object, guint prop_id,
                                   GValue *value, GParamSpec *pspec) {
  PlumaTab *tab = PLUMA_TAB(object);

  switch (prop_id) {
    case PROP_NAME:
      g_value_take_string(value, _pluma_tab_get_name(tab));
      break;
    case PROP_STATE:
      g_value_set_enum(value, pluma_tab_get_state(tab));
      break;
    case PROP_AUTO_SAVE:
      g_value_set_boolean(value, pluma_tab_get_auto_save_enabled(tab));
      break;
    case PROP_AUTO_SAVE_INTERVAL:
      g_value_set_int(value, pluma_tab_get_auto_save_interval(tab));
      break;
    default:
...
static void pluma_tab_set_property(GObject *object, guint prop_id,
                                   const GValue *value, GParamSpec *pspec) {
  PlumaTab *tab = PLUMA_TAB(object);

  switch (prop_id) {
    case PROP_AUTO_SAVE:
      pluma_tab_set_auto_save_enabled(tab, g_value_get_boolean(value));
      break;
    case PROP_AUTO_SAVE_INTERVAL:
      pluma_tab_set_auto_save_interval(tab, g_value_get_int(value));
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID(object, prop_id, pspec);
      break;
...
static void pluma_tab_finalize(GObject *object) {
  PlumaTab *tab = PLUMA_TAB(object);

  if (tab->priv->timer != NULL) g_timer_destroy(tab->priv->timer);

  g_free(tab->priv->tmp_save_uri);

  if (tab->priv->auto_save_timeout > 0) remove_auto_save_timeout(tab);
...
static void pluma_tab_class_init(PlumaTabClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string("name", "Name", "The tab's name", NULL,
                          G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));

  g_object_class_install_property(
      object_class, PROP_STATE,
      g_param_spec_enum("state", "State", "The tab's state",
                        PLUMA_TYPE_TAB_STATE, PLUMA_TAB_STATE_NORMAL,
                        G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));

  g_object_class_install_property(
      object_class, PROP_AUTO_SAVE,
      g_param_spec_boolean("autosave", "Autosave", "Autosave feature", TRUE,
                           G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  g_object_class_install_property(
      object_class, PROP_AUTO_SAVE_INTERVAL,
      g_param_spec_int("autosave-interval", "AutosaveInterval",
                       "Time between two autosaves", 0, G_MAXINT, 0,
                       G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
}
...
static void set_cursor_according_to_state(GtkTextView *view,
                                          PlumaTabState state) {
  GdkCursor *cursor;
  ...
  if ((state == PLUMA_TAB_STATE_LOADING) ||
    ...
    cursor = gdk_cursor_new_for_display(
        gtk_widget_get_display(GTK_WIDGET(view)), GDK_WATCH);

    if (text_window != NULL) gdk_window_set_cursor(text_window, cursor);
    if (left_window != NULL) gdk_window_set_cursor(left_window, cursor);

    g_object_unref(cursor);
  } else {
    cursor = gdk_cursor_new_for_display(
        gtk_widget_get_display(GTK_WIDGET(view)), GDK_XTERM);

    if (text_window != NULL) gdk_window_set_cursor(text_window, cursor);
    if (left_window != NULL) gdk_window_set_cursor(left_window, NULL);

    g_object_unref(cursor);
  }
}

static void view_realized(GtkTextView *view, PlumaTab *tab) {
  set_cursor_according_to_state(view, tab->priv->state);
}

static void set_view_properties_according_to_state(PlumaTab *tab,
                                                   PlumaTabState state) {
  gboolean val;
  gboolean hl_current_line;

  hl_current_line = g_settings_get_boolean(
      tab->priv->editor_settings, PLUMA_SETTINGS_HIGHLIGHT_CURRENT_LINE);

  val = ((state == PLUMA_TAB_STATE_NORMAL) &&
         (tab->priv->print_preview == NULL) && !tab->priv->not_editable);
  ...
  val = ((state != PLUMA_TAB_STATE_LOADING) &&
         (state != PLUMA_TAB_STATE_CLOSING) && hl_current_line);
  gtk_source_view_set_highlight_current_line(GTK_SOURCE_VIEW(tab->priv->view),
                                             val);
}

static void pluma_tab_set_state(PlumaTab *tab, PlumaTabState state) {
  g_return_if_fail(PLUMA_IS_TAB(tab));
  g_return_if_fail((state >= 0) && (state < PLUMA_TAB_NUM_OF_STATES));

  if (tab->priv->state == state) return;
  ...
  if ((state ==
       PLUMA_TAB_STATE_LOADING_ERROR) || /* FIXME: add other states if needed */
...
static void document_uri_notify_handler(PlumaDocument *document,
                                        GParamSpec *pspec, PlumaTab *tab) {
  pluma_debug(DEBUG_TAB);
...
static void document_shortname_notify_handler(PlumaDocument *document,
                                              GParamSpec *pspec,
                                              PlumaTab *tab) {
  pluma_debug(DEBUG_TAB);
...
static void document_modified_changed(GtkTextBuffer *document, PlumaTab *tab) {
  g_object_notify(G_OBJECT(tab), "name");
}

static void set_message_area(PlumaTab *tab, GtkWidget *message_area) {
  if (tab->priv->message_area == message_area) return;
...
static void remove_tab(PlumaTab *tab) {
...
static void io_loading_error_message_area_response(GtkWidget *message_area,
                                                   gint response_id,
                                                   PlumaTab *tab) {
  PlumaDocument *doc;
  PlumaView *view;
  ...
  switch (response_id) {
    case GTK_RESPONSE_OK:
      ...
      pluma_document_load(doc, uri, tab->priv->tmp_encoding,
                          tab->priv->tmp_line_pos, FALSE);
      break;
    ...
    case GTK_RESPONSE_NO:
      ...
      break;
    default:
      _pluma_recent_remove(
          PLUMA_WINDOW(gtk_widget_get_toplevel(GTK_WIDGET(tab))), uri);
...
static void file_already_open_warning_message_area_response(
    GtkWidget *message_area, gint response_id, PlumaTab *tab) {
  PlumaView *view;
...
static void load_cancelled(GtkWidget *area, gint response_id, PlumaTab *tab) {
...
static gboolean scroll_to_cursor(PlumaTab *tab) {
  pluma_view_scroll_to_cursor(PLUMA_VIEW(tab->priv->view));
  tab->priv->idle_scroll = 0;
  return FALSE;
}

static void unrecoverable_reverting_error_message_area_response(
    GtkWidget *message_area, gint response_id, PlumaTab *tab) {
  PlumaView *view;
...
static void show_loading_message_area(PlumaTab *tab) {
  GtkWidget *area;
  ...
  if (tab->priv->message_area != NULL) return;
  ...
  if (len > MAX_MSG_LENGTH) {
    ...
    if (file != NULL) {
       ...
       */
      dirname =
          pluma_utils_str_middle_truncate(str, MAX(20, MAX_MSG_LENGTH - len));
      g_free(str);
    }
  }
  ...
  if (tab->priv->state == PLUMA_TAB_STATE_REVERTING) {
    if (dirname != NULL) {
      ...
      msg = g_strdup_printf(_("Reverting %s from %s"), name_markup,
                            dirname_markup);
    ...
    area = pluma_progress_message_area_new("document-revert", msg, TRUE);
  } else {
    if (dirname != NULL) {
      dirname_markup = g_markup_printf_escaped("<b>%s</b>", dirname);
      ...
      /* Translators: the first %s is a file name (e.g. test.txt) the second one
         is a directory (e.g. ssh://master.gnome.org/home/users/paolo) */
      msg =
          g_strdup_printf(_("Loading %s from %s"), name_markup, dirname_markup);
      g_free(dirname_markup);
    } else {
      msg = g_strdup_printf(_("Loading %s"), name_markup);
    }

    area = pluma_progress_message_area_new("document-open", msg, TRUE);
  }

  g_signal_connect(area, "response", G_CALLBACK(load_cancelled), tab);
...
static void show_saving_message_area(PlumaTab *tab) {
  GtkWidget *area;
  PlumaDocument *doc = NULL;
  gchar *short_name;
  gchar *from;
   ...
   */
  if (len > MAX_MSG_LENGTH) {
    from = pluma_utils_str_middle_truncate(short_name, MAX_MSG_LENGTH);
    g_free(short_name);
  } else {
    gchar *str;
    ...
    str = pluma_utils_str_middle_truncate(to, MAX(20, MAX_MSG_LENGTH - len));
    g_free(to);
  ...
  area = pluma_progress_message_area_new("document-save", msg, FALSE);
...
static void message_area_set_progress(PlumaTab *tab, goffset size,
                                      goffset total_size) {
  if (tab->priv->message_area == NULL) return;

  pluma_debug_message(DEBUG_TAB, "%" G_GUINT64_FORMAT "/%" G_GUINT64_FORMAT,
                      size, total_size);
  ...
  if (total_size == 0) {
    if (size != 0)
      ...
      pluma_progress_message_area_set_fraction(
          PLUMA_PROGRESS_MESSAGE_AREA(tab->priv->message_area), 0);
  } else {
    gdouble frac;
    ...
    pluma_progress_message_area_set_fraction(
        PLUMA_PROGRESS_MESSAGE_AREA(tab->priv->message_area), frac);
  }
}

static void document_loading(PlumaDocument *document, goffset size,
                             goffset total_size, PlumaTab *tab) {
  gdouble et;
  gdouble total_time;
...
static gboolean remove_tab_idle(PlumaTab *tab) {
  remove_tab(tab);

  return FALSE;
}

static void document_loaded(PlumaDocument *document, const GError *error,
                            PlumaTab *tab) {
  GFile *location;
  gchar *uri;
  ...
  if (error != NULL &&
    ...
    if (error->domain == G_IO_ERROR && error->code == G_IO_ERROR_CANCELLED) {
      ...
      if (tab->priv->state == PLUMA_TAB_STATE_LOADING_ERROR) {
        emsg = pluma_io_loading_error_message_area_new(
            uri, tab->priv->tmp_encoding, error);
        ...
        emsg = pluma_unrecoverable_reverting_error_message_area_new(uri, error);
    ...
    if (error && error->domain == PLUMA_DOCUMENT_ERROR &&
        error->code == PLUMA_DOCUMENT_ERROR_CONVERSION_FALLBACK) {
      GtkWidget *emsg;
    ...
    for (l = all_documents; l != NULL; l = g_list_next(l)) {
      ...
      if (d != document) {
        ...
        if (loc != NULL) g_object_unref(loc);
      }
    }
...
static void document_saving(PlumaDocument *document, goffset size,
                            goffset total_size, PlumaTab *tab) {
  gdouble et;
  gdouble total_time;
...
static void end_saving(PlumaTab *tab) {
...
static void unrecoverable_saving_error_message_area_response(
    GtkWidget *message_area, gint response_id, PlumaTab *tab) {
  PlumaView *view;
...
static void no_backup_error_message_area_response(GtkWidget *message_area,
                                                  gint response_id,
                                                  PlumaTab *tab) {
  if (response_id == GTK_RESPONSE_YES) {
    PlumaDocument *doc;
    ...
    unrecoverable_saving_error_message_area_response(message_area, response_id,
                                                     tab);
  }
}

static void externally_modified_error_message_area_response(
    GtkWidget *message_area, gint response_id, PlumaTab *tab) {
  if (response_id == GTK_RESPONSE_YES) {
    PlumaDocument *doc;
    ...
    /* Force saving */
    pluma_document_save(
        doc, tab->priv->save_flags | PLUMA_DOCUMENT_SAVE_IGNORE_MTIME);
  } else {
    unrecoverable_saving_error_message_area_response(message_area, response_id,
                                                     tab);
  }
...
static void recoverable_saving_error_message_area_response(
    GtkWidget *message_area, gint response_id, PlumaTab *tab) {
  PlumaDocument *doc;
  ...
  if (response_id == GTK_RESPONSE_OK) {
    ...
    pluma_debug_message(DEBUG_TAB, "Force saving with URI '%s'",
                        tab->priv->tmp_save_uri);

    g_return_if_fail(tab->priv->auto_save_timeout <= 0);

    pluma_document_save_as(doc, tab->priv->tmp_save_uri,
                           tab->priv->tmp_encoding, tab->priv->save_flags);
  } else {
    unrecoverable_saving_error_message_area_response(message_area, response_id,
                                                     tab);
  }
...
static void document_saved(PlumaDocument *document, const GError *error,
                           PlumaTab *tab) {
  GtkWidget *emsg;
  ...
  if (error != NULL) {
    ...
    if (error->domain == PLUMA_DOCUMENT_ERROR &&
      ...
      _pluma_recent_remove(
          PLUMA_WINDOW(gtk_widget_get_toplevel(GTK_WIDGET(tab))),
          tab->priv->tmp_save_uri);

      emsg = pluma_unrecoverable_saving_error_message_area_new(
          tab->priv->tmp_save_uri, error);
      g_return_if_fail(emsg != NULL);
      ...
      emsg = pluma_conversion_error_while_saving_message_area_new(
          tab->priv->tmp_save_uri, tab->priv->tmp_encoding, error);
...
static void externally_modified_notification_message_area_response(
    GtkWidget *message_area, gint response_id, PlumaTab *tab) {
  PlumaView *view;
...
static void display_externally_modified_notification(PlumaTab *tab) {
  GtkWidget *message_area;
  PlumaDocument *doc;
  gchar *uri;
  ...
  message_area =
      pluma_externally_modified_message_area_new(uri, document_modified);
  g_free(uri);
...
static gboolean view_focused_in(GtkWidget *widget, GdkEventFocus *event,
                                PlumaTab *tab) {
  PlumaDocument *doc;
...
static GMountOperation *tab_mount_operation_factory(PlumaDocument *doc,
                                                    gpointer userdata) {
...
static void pluma_tab_init(PlumaTab *tab) {
  GtkWidget *hbox;
  GtkWidget *map;
  GtkWidget *sw;
  PlumaDocument *doc;
  ...
  auto_save_interval = g_settings_get_uint(tab->priv->editor_settings,
                                           PLUMA_SETTINGS_AUTO_SAVE_INTERVAL);

  lockdown = pluma_app_get_lockdown(pluma_app_get_default());
  tab->priv->auto_save = auto_save && !(lockdown & PLUMA_LOCKDOWN_SAVE_TO_DISK);
  ...
  _pluma_document_set_mount_operation_factory(doc, tab_mount_operation_factory,
                                              tab);
  ...
  gtk_css_provider_load_from_data(
      provider, "textview { font-family: Monospace; font-size: 1pt; }", -1,
      NULL);
  ...
  gtk_source_map_set_view(GTK_SOURCE_MAP(map),
                          GTK_SOURCE_VIEW(tab->priv->view));
  ...
  g_settings_bind(tab->priv->editor_settings,
                  ...
                  tab->priv->view_map_frame, "visible",
                  G_SETTINGS_BIND_GET | G_SETTINGS_BIND_NO_SENSITIVITY);

  g_signal_connect(doc, "notify::uri", G_CALLBACK(document_uri_notify_handler),
                   tab);
  ...
  g_signal_connect(doc, "loading", G_CALLBACK(document_loading), tab);
  g_signal_connect(doc, "loaded", G_CALLBACK(document_loaded), tab);
  g_signal_connect(doc, "saving", G_CALLBACK(document_saving), tab);
  g_signal_connect(doc, "saved", G_CALLBACK(document_saved), tab);

  g_signal_connect_after(tab->priv->view, "focus-in-event",
                         G_CALLBACK(view_focused_in), tab);

  g_signal_connect_after(tab->priv->view, "realize", G_CALLBACK(view_realized),
                         tab);
}
...
GtkWidget *_pluma_tab_new_from_uri(const gchar *uri,
  ...
  _pluma_tab_load(tab, uri, encoding, line_pos, create);
...
PlumaDocument *pluma_tab_get_document(PlumaTab *tab) {
  return PLUMA_DOCUMENT(
      gtk_text_view_get_buffer(GTK_TEXT_VIEW(tab->priv->view)));
}
...
static GdkPixbuf *resize_icon(GdkPixbuf *pixbuf, gint size) {
  gint width, height;
...
static GdkPixbuf *get_stock_icon(GtkIconTheme *theme, const gchar *icon_name,
                                 gint size) {
  GdkPixbuf *pixbuf;
...
static GdkPixbuf *get_icon(GtkIconTheme *theme, GFile *location, gint size) {
  GdkPixbuf *pixbuf;
  GtkIconInfo *icon_info;
  GFileInfo *info;
  GIcon *gicon;

  if (location == NULL) return get_stock_icon(theme, "text-x-generic", size);

  /* FIXME: Doing a sync stat is bad, this should be fixed */
  info = g_file_query_info(location, G_FILE_ATTRIBUTE_STANDARD_ICON,
                           G_FILE_QUERY_INFO_NONE, NULL, NULL);
  if (info == NULL) return get_stock_icon(theme, "text-x-generic", size);
  ...
  if (icon_info == NULL) return get_stock_icon(theme, "text-x-generic", size);

  pixbuf = gtk_icon_info_load_icon(icon_info, NULL);
  g_object_unref(icon_info);

  if (pixbuf == NULL) return get_stock_icon(theme, "text-x-generic", size);
...
GdkPixbuf *_pluma_tab_get_icon(PlumaTab *tab) {
  ...
  switch (tab->priv->state) {
    case PLUMA_TAB_STATE_LOADING:
      pixbuf = get_stock_icon(theme, "document-open", icon_size);
      break;

    case PLUMA_TAB_STATE_REVERTING:
      pixbuf = get_stock_icon(theme, "document-revert", icon_size);
      break;

    case PLUMA_TAB_STATE_SAVING:
      pixbuf = get_stock_icon(theme, "document-save", icon_size);
      break;

    case PLUMA_TAB_STATE_PRINTING:
      pixbuf = get_stock_icon(theme, "document-print", icon_size);
      break;

    case PLUMA_TAB_STATE_PRINT_PREVIEWING:
    case PLUMA_TAB_STATE_SHOWING_PRINT_PREVIEW:
      pixbuf = get_stock_icon(theme, "gtk-print-preview", icon_size);
      break;
    ...
    case PLUMA_TAB_STATE_GENERIC_ERROR:
      pixbuf = get_stock_icon(theme, "dialog-error", icon_size);
      break;

    case PLUMA_TAB_STATE_EXTERNALLY_MODIFIED_NOTIFICATION:
      pixbuf = get_stock_icon(theme, "dialog-warning", icon_size);
      break;
...
void _pluma_tab_load(PlumaTab *tab, const gchar *uri,
                     const PlumaEncoding *encoding, gint line_pos,
                     gboolean create) {
  PlumaDocument *doc;
  ...
  pluma_document_load(doc, uri, encoding, line_pos, create);
}
...
void _pluma_tab_revert(PlumaTab *tab) {
  ...
  pluma_document_load(doc, uri, tab->priv->tmp_encoding, 0, FALSE);
...
static gboolean pluma_tab_auto_save(PlumaTab *tab) {
  ...
  if ((tab->priv->state != PLUMA_TAB_STATE_NORMAL) &&
    ...
    /* Add a new timeout */
    timeout = g_timeout_add_seconds(30, (GSourceFunc)pluma_tab_auto_save, tab);
  ...
  pluma_document_save(
      doc, tab->priv->save_flags | PLUMA_DOCUMENT_SAVE_PRESERVE_BACKUP);
...
void _pluma_tab_save_as(PlumaTab *tab, const gchar *uri,
                        const PlumaEncoding *encoding,
...
static GtkPageSetup *get_page_setup(PlumaTab *tab) {
  gpointer data;
  PlumaDocument *doc;

  doc = pluma_tab_get_document(tab);

  data = g_object_get_data(G_OBJECT(doc), PLUMA_PAGE_SETUP_KEY);
...
static GtkPrintSettings *get_print_settings(PlumaTab *tab) {
  gpointer data;
  PlumaDocument *doc;
  ...
  data = g_object_get_data(G_OBJECT(doc), PLUMA_PRINT_SETTINGS_KEY);
  ...
  name = pluma_document_get_short_name_for_display(doc);
  uri =
      g_strconcat("file://", g_get_user_special_dir(G_USER_DIRECTORY_DOCUMENTS),
                  "/", name, ".pdf", NULL);
...
static void printing_cb(PlumaPrintJob *job, PlumaPrintJobStatus status,
                        PlumaTab *tab) {
  g_return_if_fail(PLUMA_IS_PROGRESS_MESSAGE_AREA(tab->priv->message_area));
...
static void store_print_settings(PlumaTab *tab, PlumaPrintJob *job) {
  PlumaDocument *doc;
  GtkPrintSettings *settings;
  ...
  g_object_set_data_full(G_OBJECT(doc), PLUMA_PRINT_SETTINGS_KEY,
                         g_object_ref(settings),
                         (GDestroyNotify)g_object_unref);

  /* make them the default */
  _pluma_app_set_default_print_settings(pluma_app_get_default(), settings);

  page_setup = pluma_print_job_get_page_setup(job);

  /* remember page setup for this document */
  g_object_set_data_full(G_OBJECT(doc), PLUMA_PAGE_SETUP_KEY,
                         g_object_ref(page_setup),
                         (GDestroyNotify)g_object_unref);

  /* make it the default */
  _pluma_app_set_default_page_setup(pluma_app_get_default(), page_setup);
}

static void done_printing_cb(PlumaPrintJob *job, PlumaPrintJobResult result,
                             const GError *error, PlumaTab *tab) {
  PlumaView *view;
...
static void show_printing_message_area(PlumaTab *tab, gboolean preview) {
  GtkWidget *area;

  if (preview)
    area = pluma_progress_message_area_new("document-print-preview", "", TRUE);
  else
    area = pluma_progress_message_area_new("document-print", "", TRUE);

  g_signal_connect(area, "response", G_CALLBACK(print_cancelled), tab);

  set_message_area(tab, area);
}

static void pluma_tab_print_or_print_preview(
    PlumaTab *tab, GtkPrintOperationAction print_action) {
  PlumaView *view;
  gboolean is_preview;
  GtkPageSetup *setup;
  GtkPrintSettings *settings;
  ...
  g_signal_connect(tab->priv->print_job, "printing", G_CALLBACK(printing_cb),
                   tab);
  g_signal_connect(tab->priv->print_job, "show-preview",
                   G_CALLBACK(show_preview_cb), tab);
  g_signal_connect(tab->priv->print_job, "done", G_CALLBACK(done_printing_cb),
                   tab);
  ...
  res = pluma_print_job_print(
      tab->priv->print_job, print_action, setup, settings,
      GTK_WINDOW(gtk_widget_get_toplevel(GTK_WIDGET(tab))), &error);
...
void _pluma_tab_print_preview(PlumaTab *tab) {
  ...
  pluma_tab_print_or_print_preview(tab, GTK_PRINT_OPERATION_ACTION_PREVIEW);
}
...
gboolean _pluma_tab_can_close(PlumaTab *tab) {
  ...
  if ((ts == PLUMA_TAB_STATE_LOADING) ||
      ...
      (ts == PLUMA_TAB_STATE_REVERTING_ERROR) || /* CHECK: I'm not sure this is
                                                    the right behavior for
                                                    REVERTING ERROR */
      (!gtk_text_buffer_get_modified(GTK_TEXT_BUFFER(doc))) ||
      (!g_settings_get_boolean(tab->priv->editor_settings,
                               PLUMA_SETTINGS_SHOW_SAVE_CONFIRMATION)))
    return TRUE;

  /* Do not close tab with saving errors */
  if (ts == PLUMA_TAB_STATE_SAVING_ERROR) return FALSE;
 ...
 **/
void pluma_tab_set_auto_save_enabled(PlumaTab *tab, gboolean enable) {
  PlumaDocument *doc = NULL;
  PlumaLockdownMask lockdown;
  ...
  lockdown = pluma_app_get_lockdown(pluma_app_get_default());
  if (lockdown & PLUMA_LOCKDOWN_SAVE_TO_DISK) enable = FALSE;

  doc = pluma_tab_get_document(tab);

  if (tab->priv->auto_save == enable) return;
 ...
 * argument.
 **/
void pluma_tab_set_auto_save_interval(PlumaTab *tab, gint interval) {
  PlumaDocument *doc = NULL;
  ...
  if (tab->priv->auto_save_interval == interval) return;

  tab->priv->auto_save_interval = interval;

  if (!tab->priv->auto_save) return;
...
void pluma_tab_set_info_bar(PlumaTab *tab, GtkWidget *info_bar) {
  g_return_if_fail(PLUMA_IS_TAB(tab));
