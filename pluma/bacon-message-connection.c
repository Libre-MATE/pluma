...
static gboolean test_is_socket(const char *path) {
  struct stat s;

  if (stat(path, &s) == -1) return FALSE;

  if (S_ISSOCK(s.st_mode)) return TRUE;
...
static gboolean is_owned_by_user_and_socket(const char *path) {
  ...
  if (s.st_uid != geteuid()) return FALSE;
...
static gboolean server_cb(GIOChannel *source, GIOCondition condition,
                          gpointer data);
...
static gboolean server_cb(GIOChannel *source, GIOCondition condition,
  ...
  while (finished == FALSE && *subs != '\0') {
    if (conn->func != NULL) (*conn->func)(subs, conn->data);

    subs += strlen(subs) + 1;
    if (subs - message >= offset) finished = TRUE;
  }
...
static char *find_file_with_pattern(const char *dir, const char *pattern) {
  ...
  while ((filename = g_dir_read_name(filedir))) {
    if (g_pattern_match_string(pat, filename)) {
      ...
      if (is_owned_by_user_and_socket(tmp)) found_filename = g_strdup(filename);
      g_free(tmp);
    }

    if (found_filename != NULL) break;
  }
...
static char *socket_filename(const char *prefix) {
  ...
  if (filename == NULL) {
    newfile =
        g_strdup_printf("%s.%s.%u", prefix, g_get_user_name(), g_random_int());
...
static gboolean try_server(BaconMessageConnection *conn) {
  ...
  if (!setup_connection(conn)) return FALSE;
  return TRUE;
}
