...
G_DEFINE_TYPE_WITH_PRIVATE(PlumaDocumentsPanel, pluma_documents_panel,
                           GTK_TYPE_BOX)

enum { PROP_0, PROP_WINDOW };

enum { PIXBUF_COLUMN, NAME_COLUMN, TAB_COLUMN, N_COLUMNS };

#define MAX_DOC_NAME_LENGTH 60

static gchar *tab_get_name(PlumaTab *tab) {
  PlumaDocument *doc;
  gchar *name;
  gchar *docname;
  gchar *tab_name;
  ...
  if (gtk_text_buffer_get_modified(GTK_TEXT_BUFFER(doc))) {
    if (pluma_document_get_readonly(doc)) {
      tab_name = g_markup_printf_escaped("<i>%s</i> [<i>%s</i>]", docname,
                                         _("Read-Only"));
    } else {
    ...
    if (pluma_document_get_readonly(doc)) {
      tab_name =
          g_markup_printf_escaped("%s [<i>%s</i>]", docname, _("Read-Only"));
    } else {
...
static void get_iter_from_tab(PlumaDocumentsPanel *panel, PlumaTab *tab,
                              GtkTreeIter *iter) {
  gint num;
  GtkWidget *nb;
  GtkTreePath *path;
...
static void window_active_tab_changed(PlumaWindow *window, PlumaTab *tab,
  ...
  if (!_pluma_window_is_removing_tabs(window)) {
    ...
    if (gtk_list_store_iter_is_valid(GTK_LIST_STORE(panel->priv->model),
                                     &iter)) {
      selection =
          gtk_tree_view_get_selection(GTK_TREE_VIEW(panel->priv->treeview));
...
static void refresh_list(PlumaDocumentsPanel *panel) {
  ...
  while (l != NULL) {
    ...
    gtk_list_store_set(list_store, &iter, PIXBUF_COLUMN, pixbuf, NAME_COLUMN,
                       name, TAB_COLUMN, l->data, -1);
...
static void sync_name_and_icon(PlumaTab *tab, GParamSpec *pspec,
                               PlumaDocumentsPanel *panel) {
  GdkPixbuf *pixbuf;
  gchar *name;
  GtkTreeIter iter;
  ...
  gtk_list_store_set(GTK_LIST_STORE(panel->priv->model), &iter, PIXBUF_COLUMN,
                     pixbuf, NAME_COLUMN, name, TAB_COLUMN, tab, -1);

  g_free(name);
  if (pixbuf != NULL) g_object_unref(pixbuf);
}

static void window_tab_removed(PlumaWindow *window, PlumaTab *tab,
                               PlumaDocumentsPanel *panel) {
  g_signal_handlers_disconnect_by_func(tab, G_CALLBACK(sync_name_and_icon),
                                       panel);
...
static void window_tab_added(PlumaWindow *window, PlumaTab *tab,
                             PlumaDocumentsPanel *panel) {
  GtkTreeIter iter;
  GtkTreeIter sibling;
  GdkPixbuf *pixbuf;
  ...
  if (gtk_list_store_iter_is_valid(GTK_LIST_STORE(panel->priv->model),
                                   &sibling)) {
    gtk_list_store_insert_after(GTK_LIST_STORE(panel->priv->model), &iter,
                                &sibling);
  } else {
    PlumaTab *active_tab;
...
static void window_tabs_reordered(PlumaWindow *window,
                                  PlumaDocumentsPanel *panel) {
  if (panel->priv->is_reodering) return;

  refresh_list(panel);
}

static void set_window(PlumaDocumentsPanel *panel, PlumaWindow *window) {
  ...
  g_signal_connect(window, "tab_removed", G_CALLBACK(window_tab_removed),
                   panel);
  g_signal_connect(window, "tabs_reordered", G_CALLBACK(window_tabs_reordered),
                   panel);
  g_signal_connect(window, "active_tab_changed",
                   G_CALLBACK(window_active_tab_changed), panel);
}
...
static void treeview_cursor_changed(GtkTreeView *view,
  ...
  if (gtk_tree_selection_get_selected(selection, NULL, &iter)) {
    gtk_tree_model_get(panel->priv->model, &iter, TAB_COLUMN, &tab, -1);

    if (pluma_window_get_active_tab(panel->priv->window) != tab) {
      pluma_window_set_active_tab(panel->priv->window, PLUMA_TAB(tab));
    }
  }
}

static void pluma_documents_panel_set_property(GObject *object, guint prop_id,
                                               const GValue *value,
                                               GParamSpec *pspec) {
...
static void pluma_documents_panel_get_property(GObject *object, guint prop_id,
                                               GValue *value,
                                               GParamSpec *pspec) {
  PlumaDocumentsPanel *panel = PLUMA_DOCUMENTS_PANEL(object);
...
static void pluma_documents_panel_finalize(GObject *object) {
...
static void pluma_documents_panel_dispose(GObject *object) {
  PlumaDocumentsPanel *panel = PLUMA_DOCUMENTS_PANEL(object);
...
static void pluma_documents_panel_class_init(PlumaDocumentsPanelClass *klass) {
...
static gboolean show_popup_menu(PlumaDocumentsPanel *panel,
                                GdkEventButton *event) {
  GtkWidget *menu;

  menu = gtk_ui_manager_get_widget(
      pluma_window_get_ui_manager(panel->priv->window), "/NotebookPopup");
...
static gboolean panel_button_press_event(GtkTreeView *treeview,
                                         GdkEventButton *event,
                                         PlumaDocumentsPanel *panel) {
...
static gboolean panel_popup_menu(GtkWidget *treeview,
                                 PlumaDocumentsPanel *panel) {
...
static gboolean treeview_query_tooltip(GtkWidget *widget, gint x, gint y,
  ...
  if (keyboard_tip) {
    ...
    gtk_tree_view_convert_widget_to_bin_window_coords(tree_view, x, y, &bin_x,
                                                      &bin_y);

    if (!gtk_tree_view_get_path_at_pos(tree_view, bin_x, bin_y, &path, NULL,
                                       NULL, NULL)) {
      return FALSE;
    }
  }
...
static void treeview_row_inserted(GtkTreeModel *tree_model, GtkTreePath *path,
                                  GtkTreeIter *iter,
                                  PlumaDocumentsPanel *panel) {
  PlumaTab *tab;
  gint *indeces;
  ...
  if (panel->priv->adding_tab) return;
  ...
  /* g_debug ("New Index: %d (path: %s)", indeces[0], gtk_tree_path_to_string
   * (path));*/
  ...
  if (new_position > old_position) new_position = MAX(0, new_position - 1);

  pluma_notebook_reorder_tab(PLUMA_NOTEBOOK(nb), tab, new_position);

  panel->priv->is_reodering = FALSE;
}

static void pluma_documents_panel_init(PlumaDocumentsPanel *panel) {
  GtkWidget *sw;
  ...
  panel->priv->model = GTK_TREE_MODEL(gtk_list_store_new(
      N_COLUMNS, GDK_TYPE_PIXBUF, G_TYPE_STRING, G_TYPE_POINTER));
  ...
  gtk_tree_view_set_headers_visible(GTK_TREE_VIEW(panel->priv->treeview),
                                    FALSE);
  ...
  g_signal_connect(panel->priv->treeview, "cursor_changed",
                   G_CALLBACK(treeview_cursor_changed), panel);
  g_signal_connect(panel->priv->treeview, "button-press-event",
                   G_CALLBACK(panel_button_press_event), panel);
  g_signal_connect(panel->priv->treeview, "popup-menu",
                   G_CALLBACK(panel_popup_menu), panel);
  g_signal_connect(panel->priv->treeview, "query-tooltip",
                   G_CALLBACK(treeview_query_tooltip), NULL);

  g_signal_connect(panel->priv->model, "row-inserted",
                   G_CALLBACK(treeview_row_inserted), panel);
}
...
GtkWidget *pluma_documents_panel_new(PlumaWindow *window) {
  ...
  return GTK_WIDGET(
      g_object_new(PLUMA_TYPE_DOCUMENTS_PANEL, "window", window, NULL));
}
