...
static void pluma_tab_label_finalize(GObject *object) {
  G_OBJECT_CLASS(pluma_tab_label_parent_class)->finalize(object);
}

static void pluma_tab_label_set_property(GObject *object, guint prop_id,
                                         const GValue *value,
                                         GParamSpec *pspec) {
...
static void pluma_tab_label_get_property(GObject *object, guint prop_id,
...
static void sync_tip(PlumaTab *tab, PlumaTabLabel *tab_label) {
  gchar *str;
...
static void sync_name(PlumaTab *tab, GParamSpec *pspec,
                      PlumaTabLabel *tab_label) {
  gchar *str;
...
static void sync_state(PlumaTab *tab, GParamSpec *pspec,
                       PlumaTabLabel *tab_label) {
  PlumaTabState state;
  ...
  if ((state == PLUMA_TAB_STATE_LOADING) || (state == PLUMA_TAB_STATE_SAVING) ||
...
static void pluma_tab_label_constructed(GObject *object) {
  PlumaTabLabel *tab_label = PLUMA_TAB_LABEL(object);
...
static void pluma_tab_label_class_init(PlumaTabLabelClass *klass) {
  ...
  signals[CLOSE_CLICKED] = g_signal_new(
      "close-clicked", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PlumaTabLabelClass, close_clicked), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  g_object_class_install_property(
      object_class, PROP_TAB,
      g_param_spec_object("tab", "Tab", "The PlumaTab", PLUMA_TYPE_TAB,
                          G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
}

static void pluma_tab_label_init(PlumaTabLabel *tab_label) {
  GtkWidget *ebox;
  GtkWidget *hbox;
  GtkWidget *close_button;
  GtkWidget *spinner;
...
void pluma_tab_label_set_close_button_sensitive(PlumaTabLabel *tab_label,
  ...
  if (sensitive == tab_label->priv->close_button_sensitive) return;
...
GtkWidget *pluma_tab_label_new(PlumaTab *tab) {
  ...
  tab_label = g_object_new(PLUMA_TYPE_TAB_LABEL, "homogeneous", FALSE, "tab",
                           tab, NULL);
