...
G_DEFINE_TYPE_WITH_PRIVATE(PlumaStatusComboBox, pluma_status_combo_box,
                           GTK_TYPE_EVENT_BOX)

static void pluma_status_combo_box_finalize(GObject *object) {
  G_OBJECT_CLASS(pluma_status_combo_box_parent_class)->finalize(object);
}

static void pluma_status_combo_box_get_property(GObject *object, guint prop_id,
                                                GValue *value,
                                                GParamSpec *pspec) {
...
static void pluma_status_combo_box_set_property(GObject *object, guint prop_id,
                                                const GValue *value,
                                                GParamSpec *pspec) {
  PlumaStatusComboBox *obj = PLUMA_STATUS_COMBO_BOX(object);
...
static void pluma_status_combo_box_constructed(GObject *object) {
...
static void pluma_status_combo_box_changed(PlumaStatusComboBox *combo,
                                           GtkMenuItem *item) {
  const gchar *text;
...
static void pluma_status_combo_box_class_init(PlumaStatusComboBoxClass *klass) {
  ...
  signals[CHANGED] = g_signal_new(
      ...
      G_STRUCT_OFFSET(PlumaStatusComboBoxClass, changed), NULL, NULL,
      g_cclosure_marshal_VOID__OBJECT, G_TYPE_NONE, 1, GTK_TYPE_MENU_ITEM);

  g_object_class_install_property(
      object_class, PROP_LABEL,
      g_param_spec_string("label", "LABEL", "The label", NULL,
                          G_PARAM_READWRITE));
}

static void menu_deactivate(GtkMenu *menu, PlumaStatusComboBox *combo) {
...
static void button_press_event(GtkWidget *widget, GdkEventButton *event,
  ...
  if (request.height > max_height) {
    ...
    gtk_widget_set_size_request(gtk_widget_get_toplevel(combo->priv->menu), -1,
                                max_height);
  }

  gtk_menu_popup_at_widget(
      GTK_MENU(combo->priv->menu), gtk_widget_get_parent(widget),
      GDK_GRAVITY_NORTH_WEST, GDK_GRAVITY_SOUTH_WEST, (const GdkEvent *)event);
...
static void set_shadow_type(PlumaStatusComboBox *combo) {
  GtkStyleContext *context;
...
static void pluma_status_combo_box_init(PlumaStatusComboBox *self) {
  ...
  gtk_box_pack_start(GTK_BOX(self->priv->hbox), self->priv->label, FALSE, TRUE,
                     0);
  ...
  gtk_box_pack_start(GTK_BOX(self->priv->hbox), self->priv->item, TRUE, TRUE,
                     0);
  ...
  gtk_box_pack_start(GTK_BOX(self->priv->hbox), self->priv->arrow, FALSE, TRUE,
                     0);
  ...
  g_signal_connect(self->priv->menu, "deactivate", G_CALLBACK(menu_deactivate),
                   self);
}
...
static void item_activated(GtkMenuItem *item, PlumaStatusComboBox *combo) {
...
void pluma_status_combo_box_add_item(PlumaStatusComboBox *combo,
                                     GtkMenuItem *item, const gchar *text) {
  g_return_if_fail(PLUMA_IS_STATUS_COMBO_BOX(combo));
...
void pluma_status_combo_box_set_item_text(PlumaStatusComboBox *combo,
  ...
  g_object_set_data_full(G_OBJECT(item), COMBO_BOX_TEXT_DATA, g_strdup(text),
