...
enum { PROP_0, PROP_HAS_CANCEL_BUTTON };
...
G_DEFINE_TYPE_WITH_PRIVATE(PlumaProgressMessageArea,
                           pluma_progress_message_area, GTK_TYPE_INFO_BAR)

static void pluma_progress_message_area_set_has_cancel_button(
    PlumaProgressMessageArea *area, gboolean has_button) {
  if (has_button)
    gtk_button_set_image(
        GTK_BUTTON(gtk_info_bar_add_button(GTK_INFO_BAR(area), _("_Cancel"),
                                           GTK_RESPONSE_CANCEL)),
...
static void pluma_progress_message_area_set_property(GObject *object,
                                                     guint prop_id,
                                                     const GValue *value,
                                                     GParamSpec *pspec) {
  ...
  switch (prop_id) {
    case PROP_HAS_CANCEL_BUTTON:
      pluma_progress_message_area_set_has_cancel_button(
          area, g_value_get_boolean(value));
      break;
    default:
...
static void pluma_progress_message_area_get_property(GObject *object,
                                                     guint prop_id,
                                                     GValue *value,
                                                     GParamSpec *pspec) {
  switch (prop_id) {
...
static void pluma_progress_message_area_class_init(
    PlumaProgressMessageAreaClass *klass) {
  ...
  g_object_class_install_property(
      gobject_class, PROP_HAS_CANCEL_BUTTON,
      g_param_spec_boolean(
          "has-cancel-button", "Has Cancel Button",
          "If the message area has a cancel button", TRUE,
          G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS));
}

static void pluma_progress_message_area_init(PlumaProgressMessageArea *area) {
  GtkWidget *vbox;
  GtkWidget *hbox;
...
GtkWidget *pluma_progress_message_area_new(const gchar *image_id,
  ...
  area = PLUMA_PROGRESS_MESSAGE_AREA(g_object_new(
      PLUMA_TYPE_PROGRESS_MESSAGE_AREA, "has-cancel-button", has_cancel, NULL));

  pluma_progress_message_area_set_image(area, image_id);

  pluma_progress_message_area_set_markup(area, markup);
...
void pluma_progress_message_area_set_image(PlumaProgressMessageArea *area,
  ...
  gtk_image_set_from_icon_name(GTK_IMAGE(area->priv->image), image_id,
                               GTK_ICON_SIZE_SMALL_TOOLBAR);
}
