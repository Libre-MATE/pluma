...
enum { PROP_0, PROP_DOCUMENT, PROP_URI, PROP_ENCODING, PROP_NEWLINE_TYPE };
...
#define REMOTE_QUERY_ATTRIBUTES                                                \
  G_FILE_ATTRIBUTE_STANDARD_CONTENT_TYPE                                       \
      ...
      "," G_FILE_ATTRIBUTE_STANDARD_SIZE "," G_FILE_ATTRIBUTE_ACCESS_CAN_WRITE \
...
G_DEFINE_TYPE_WITH_PRIVATE(PlumaDocumentLoader, pluma_document_loader,
                           G_TYPE_OBJECT)

static void pluma_document_loader_set_property(GObject *object, guint prop_id,
                                               const GValue *value,
                                               GParamSpec *pspec) {
...
static void pluma_document_loader_get_property(GObject *object, guint prop_id,
                                               GValue *value,
                                               GParamSpec *pspec) {
  PlumaDocumentLoader *loader = PLUMA_DOCUMENT_LOADER(object);
...
static void pluma_document_loader_finalize(GObject *object) {
  PlumaDocumentLoaderPrivate *priv =
      pluma_document_loader_get_instance_private(PLUMA_DOCUMENT_LOADER(object));
...
static void pluma_document_loader_dispose(GObject *object) {
  PlumaDocumentLoaderPrivate *priv =
      pluma_document_loader_get_instance_private(PLUMA_DOCUMENT_LOADER(object));
...
static void pluma_document_loader_class_init(PlumaDocumentLoaderClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_boxed(
          "encoding", "Encoding", "The encoding of the saved file",
          PLUMA_TYPE_ENCODING,
          G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS));
  ...
  signals[LOADING] = g_signal_new(
      "loading", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PlumaDocumentLoaderClass, loading), NULL, NULL, NULL,
      G_TYPE_NONE, 2, G_TYPE_BOOLEAN, G_TYPE_POINTER);
}

static void pluma_document_loader_init(PlumaDocumentLoader *loader) {
...
static AsyncData *async_data_new(PlumaDocumentLoader *loader) {
  AsyncData *async;
...
static const PlumaEncoding *get_metadata_encoding(PlumaDocumentLoader *loader) {
  ...
  if (g_file_info_has_attribute(info, PLUMA_METADATA_ATTRIBUTE_ENCODING)) {
    ...
    charset = g_file_info_get_attribute_string(
        info, PLUMA_METADATA_ATTRIBUTE_ENCODING);
...
static void remote_load_completed_or_failed(PlumaDocumentLoader *loader,
                                            AsyncData *async) {
  pluma_document_loader_loading(loader, TRUE, loader->priv->error);
...
static void close_input_stream_ready_cb(GInputStream *stream, GAsyncResult *res,
                                        AsyncData *async) {
  GError *error = NULL;
  ...
  if (!g_output_stream_close(async->loader->priv->output, async->cancellable,
                             &error)) {
    async_failed(async, error);
    return;
  }
...
static void async_read_cb(GInputStream *stream, GAsyncResult *res,
                          AsyncData *async) {
  pluma_debug(DEBUG_LOADER);
  PlumaDocumentLoader *loader;
  GError *error = NULL;
  ...
  if (loader->priv->bytes_read + async->read < loader->priv->bytes_read) {
    g_set_error(&loader->priv->error, PLUMA_DOCUMENT_ERROR,
                PLUMA_DOCUMENT_ERROR_TOO_BIG, "File too big");
  ...
  if (async->read == 0) {
    g_output_stream_flush(loader->priv->output, NULL, &loader->priv->error);
    ...
    loader->priv->auto_detected_newline_type =
        pluma_document_output_stream_detect_newline_type(
            PLUMA_DOCUMENT_OUTPUT_STREAM(loader->priv->output));
    ...
    /*if ((pluma_smart_charset_converter_get_num_fallbacks
    (loader->priv->converter) != 0) && loader->priv->error == NULL)
    {
...
static void read_file_chunk(AsyncData *async) {
  ...
  g_input_stream_read_async(G_INPUT_STREAM(loader->priv->stream),
                            loader->priv->buffer, READ_CHUNK_SIZE,
                            G_PRIORITY_HIGH, async->cancellable,
                            (GAsyncReadyCallback)async_read_cb, async);
}

static GSList *get_candidate_encodings(PlumaDocumentLoader *loader) {
...
static void finish_query_info(AsyncData *async) {
  PlumaDocumentLoader *loader;
  ...
  if (g_file_info_has_attribute(info, G_FILE_ATTRIBUTE_STANDARD_TYPE) &&
    ...
    g_set_error(&loader->priv->error, G_IO_ERROR, G_IO_ERROR_NOT_REGULAR_FILE,
                "Not a regular file");
  ...
  if (loader->priv->encoding == NULL) {
    ...
    candidate_encodings =
        g_slist_prepend(NULL, (gpointer)loader->priv->encoding);
  }

  loader->priv->converter =
      pluma_smart_charset_converter_new(candidate_encodings);
  g_slist_free(candidate_encodings);

  conv_stream = g_converter_input_stream_new(
      loader->priv->stream, G_CONVERTER(loader->priv->converter));
  ...
  /* Output stream */
  loader->priv->output =
      pluma_document_output_stream_new(loader->priv->document);

  /* start reading */
  read_file_chunk(async);
}

static void query_info_cb(GFile *source, GAsyncResult *res, AsyncData *async) {
  GFileInfo *info;
  GError *error = NULL;
  ...
  info = g_file_query_info_finish(async->loader->priv->gfile, res, &error);
...
static void recover_not_mounted(AsyncData *async) {
  ...
  g_file_mount_enclosing_volume(
      async->loader->priv->gfile, G_MOUNT_MOUNT_NONE, mount_operation,
      async->cancellable, (GAsyncReadyCallback)mount_ready_callback, async);

  g_object_unref(mount_operation);
}

static void async_read_ready_callback(GObject *source, GAsyncResult *res,
                                      AsyncData *async) {
  GError *error = NULL;
  PlumaDocumentLoader *loader;
  ...
  loader->priv->stream =
      G_INPUT_STREAM(g_file_read_finish(loader->priv->gfile, res, &error));
   ...
   */
  g_file_query_info_async(loader->priv->gfile, REMOTE_QUERY_ATTRIBUTES,
                          G_FILE_QUERY_INFO_NONE, G_PRIORITY_HIGH,
                          async->cancellable,
                          (GAsyncReadyCallback)query_info_cb, async);
}

static void open_async_read(AsyncData *async) {
  g_file_read_async(async->loader->priv->gfile, G_PRIORITY_HIGH,
                    async->cancellable,
                    (GAsyncReadyCallback)async_read_ready_callback, async);
}

void pluma_document_loader_loading(PlumaDocumentLoader *loader,
                                   gboolean completed, GError *error) {
...
void pluma_document_loader_load(PlumaDocumentLoader *loader) {
  ...
  /* loading start */
  pluma_document_loader_loading(PLUMA_DOCUMENT_LOADER(loader), FALSE, NULL);
...
gboolean pluma_document_loader_cancel(PlumaDocumentLoader *loader) {
  ...
  if (loader->priv->cancellable == NULL) return FALSE;

  g_cancellable_cancel(loader->priv->cancellable);

  g_set_error(&loader->priv->error, G_IO_ERROR, G_IO_ERROR_CANCELLED,
              "Operation cancelled");
...
const PlumaEncoding *pluma_document_loader_get_encoding(
    PlumaDocumentLoader *loader) {
  g_return_val_if_fail(PLUMA_IS_DOCUMENT_LOADER(loader), NULL);

  if (loader->priv->encoding != NULL) return loader->priv->encoding;
...
PlumaDocumentNewlineType pluma_document_loader_get_newline_type(
    PlumaDocumentLoader *loader) {
