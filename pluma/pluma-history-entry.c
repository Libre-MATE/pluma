...
G_DEFINE_TYPE_WITH_PRIVATE(PlumaHistoryEntry, pluma_history_entry,
                           GTK_TYPE_COMBO_BOX_TEXT)

static void pluma_history_entry_set_property(GObject *object, guint prop_id,
                                             const GValue *value,
                                             GParamSpec *spec) {
  ...
  switch (prop_id) {
    ...
    case PROP_HISTORY_LENGTH:
      pluma_history_entry_set_history_length(entry, g_value_get_uint(value));
      break;
    default:
      break;
  }
...
static void pluma_history_entry_get_property(GObject *object, guint prop_id,
...
static void pluma_history_entry_dispose(GObject *object) {
  pluma_history_entry_set_enable_completion(PLUMA_HISTORY_ENTRY(object), FALSE);

  G_OBJECT_CLASS(pluma_history_entry_parent_class)->dispose(object);
}

static void pluma_history_entry_finalize(GObject *object) {
  PlumaHistoryEntryPrivate *priv;
...
static void pluma_history_entry_class_init(PlumaHistoryEntryClass *klass) {
...
static GtkListStore *get_history_store(PlumaHistoryEntry *entry) {
  GtkTreeModel *store;
...
static GSList *get_history_list(PlumaHistoryEntry *entry) {
  GtkListStore *store;
  GtkTreeIter iter;
  gboolean valid;
  GSList *list = NULL;
...
static void pluma_history_entry_save_history(PlumaHistoryEntry *entry) {
  ...
  pluma_settings_set_list(entry->priv->settings, entry->priv->history_id,
                          settings_items);
...
static void insert_history_item(PlumaHistoryEntry *entry, const gchar *text,
                                gboolean prepend) {
  GtkListStore *store;
  GtkTreeIter iter;
...
static void pluma_history_entry_load_history(PlumaHistoryEntry *entry) {
  ...
  settings_items =
      pluma_settings_get_list(entry->priv->settings, entry->priv->history_id);

  gtk_list_store_clear(store);

  for (l = settings_items, i = 0; l != NULL && i < entry->priv->history_length;
       l = l->next, i++) {
    gtk_list_store_append(store, &iter);
...
static void pluma_history_entry_init(PlumaHistoryEntry *entry) {
  PlumaHistoryEntryPrivate *priv;
...
guint pluma_history_entry_get_history_length(PlumaHistoryEntry *entry) {
...
void pluma_history_entry_set_enable_completion(PlumaHistoryEntry *entry,
                                               gboolean enable) {
  g_return_if_fail(PLUMA_IS_HISTORY_ENTRY(entry));
...
gboolean pluma_history_entry_get_enable_completion(PlumaHistoryEntry *entry) {
...
GtkWidget *pluma_history_entry_new(const gchar *history_id,
  ...
  ret =
      g_object_new(PLUMA_TYPE_HISTORY_ENTRY, "history-id", history_id, "model",
                   store, "has-entry", TRUE, "entry-text-column", 0, NULL);
...
void pluma_history_entry_set_escape_func(
    PlumaHistoryEntry *entry, PlumaHistoryEntryEscapeFunc escape_func) {
  GList *cells;
