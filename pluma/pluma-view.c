...
static gint pluma_view_focus_out(GtkWidget *widget, GdkEventFocus *event);
...
static gboolean pluma_view_drag_drop(GtkWidget *widget, GdkDragContext *context,
...
static void hide_search_window(PlumaView *view, gboolean cancel);

static gboolean pluma_view_draw(GtkWidget *widget, cairo_t *cr);

static void search_highlight_updated_cb(PlumaDocument *doc, GtkTextIter *start,
                                        GtkTextIter *end, PlumaView *view);
...
static void document_read_only_notify_handler(PlumaDocument *document,
                                              GParamSpec *pspec,
                                              PlumaView *view) {
...
static void pluma_view_class_init(PlumaViewClass *klass) {
  ...
  view_signals[START_INTERACTIVE_SEARCH] =
      g_signal_new("start_interactive_search", G_TYPE_FROM_CLASS(object_class),
                   G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                   G_STRUCT_OFFSET(PlumaViewClass, start_interactive_search),
                   NULL, NULL, NULL, G_TYPE_BOOLEAN, 0);

  view_signals[START_INTERACTIVE_GOTO_LINE] = g_signal_new(
      "start_interactive_goto_line", G_TYPE_FROM_CLASS(object_class),
      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
      G_STRUCT_OFFSET(PlumaViewClass, start_interactive_goto_line), NULL, NULL,
      NULL, G_TYPE_BOOLEAN, 0);

  view_signals[RESET_SEARCHED_TEXT] =
      g_signal_new("reset_searched_text", G_TYPE_FROM_CLASS(object_class),
                   G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                   G_STRUCT_OFFSET(PlumaViewClass, reset_searched_text), NULL,
                   NULL, NULL, G_TYPE_BOOLEAN, 0);
  ...
  view_signals[DROP_URIS] =
      g_signal_new("drop_uris", G_TYPE_FROM_CLASS(object_class),
                   G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                   G_STRUCT_OFFSET(PlumaViewClass, drop_uris), NULL, NULL, NULL,
  ...
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_u, GDK_MOD1_MASK,
                               "change_case", 1, G_TYPE_ENUM,
                               GTK_SOURCE_CHANGE_CASE_TOGGLE);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_l, GDK_MOD1_MASK,
                               "change_case", 1, G_TYPE_ENUM,
                               GTK_SOURCE_CHANGE_CASE_TITLE);
}

static void current_buffer_removed(PlumaView *view) {
  if (view->priv->current_buffer) {
    g_signal_handlers_disconnect_by_func(
        view->priv->current_buffer, document_read_only_notify_handler, view);
    g_signal_handlers_disconnect_by_func(view->priv->current_buffer,
                                         search_highlight_updated_cb, view);
...
static void on_notify_buffer_cb(PlumaView *view, GParamSpec *arg1,
                                gpointer userdata) {
  ...
  if (buffer == NULL || !PLUMA_IS_DOCUMENT(buffer)) return;

  view->priv->current_buffer = g_object_ref(buffer);
  g_signal_connect(buffer, "notify::read-only",
                   G_CALLBACK(document_read_only_notify_handler), view);

  gtk_text_view_set_editable(GTK_TEXT_VIEW(view), !pluma_document_get_readonly(
                                                      PLUMA_DOCUMENT(buffer)));

  g_signal_connect(buffer, "search_highlight_updated",
                   G_CALLBACK(search_highlight_updated_cb), view);
}
...
void pluma_set_source_space_drawer_by_level(GtkSourceView *view, gint level,
  ...
  GtkSourceSpaceLocationFlags levels[] = {
      0, GTK_SOURCE_SPACE_LOCATION_TRAILING,
      GTK_SOURCE_SPACE_LOCATION_INSIDE_TEXT |
          GTK_SOURCE_SPACE_LOCATION_TRAILING |
  ...
  if (level >= G_N_ELEMENTS(levels) || level < 0) level = 0;
...
#ifdef GTK_SOURCE_VERSION_3_24
static void pluma_set_source_space_drawer(GSettings *settings,
                                          GtkSourceView *view) {
  pluma_set_source_space_drawer_by_level(
      view, g_settings_get_enum(settings, PLUMA_SETTINGS_DRAWER_SPACE),
      GTK_SOURCE_SPACE_TYPE_SPACE);
  pluma_set_source_space_drawer_by_level(
      view, g_settings_get_enum(settings, PLUMA_SETTINGS_DRAWER_TAB),
  ...
  pluma_set_source_space_drawer_by_level(
      view, g_settings_get_enum(settings, PLUMA_SETTINGS_DRAWER_NBSP),
      GTK_SOURCE_SPACE_TYPE_NBSP);
  gtk_source_space_drawer_set_enable_matrix(
      gtk_source_view_get_space_drawer(view), TRUE);
}
#else
static void pluma_set_source_space_drawer(GSettings *settings,
                                          GtkSourceView *view) {
  ...
  flags |= GTK_SOURCE_DRAW_SPACES_TRAILING | GTK_SOURCE_DRAW_SPACES_TEXT |
...
static void pluma_view_init(PlumaView *view) {
  ...
  display_line_numbers = g_settings_get_boolean(
      view->priv->editor_settings, PLUMA_SETTINGS_DISPLAY_LINE_NUMBERS);
  auto_indent = g_settings_get_boolean(view->priv->editor_settings,
                                       PLUMA_SETTINGS_AUTO_INDENT);
  tabs_size = g_settings_get_uint(view->priv->editor_settings,
  ...
  display_right_margin = g_settings_get_boolean(
      view->priv->editor_settings, PLUMA_SETTINGS_DISPLAY_RIGHT_MARGIN);
  right_margin_position = g_settings_get_uint(
      view->priv->editor_settings, PLUMA_SETTINGS_RIGHT_MARGIN_POSITION);
  hl_current_line = g_settings_get_boolean(
      view->priv->editor_settings, PLUMA_SETTINGS_HIGHLIGHT_CURRENT_LINE);
  ...
  g_object_set(G_OBJECT(view), "wrap_mode", wrap_mode, "show_line_numbers",
               display_line_numbers, "auto_indent", auto_indent, "tab_width",
               tabs_size, "insert_spaces_instead_of_tabs", insert_spaces,
               "show_right_margin", display_right_margin,
               "right_margin_position", right_margin_position,
               "highlight_current_line", hl_current_line, "smart_home_end",
               smart_home_end, "indent_on_tab", TRUE, NULL);

  g_settings_bind(view->priv->editor_settings,
                  PLUMA_SETTINGS_BACKGROUND_PATTERN, view, "background-pattern",
                  G_SETTINGS_BIND_DEFAULT);

  pluma_set_source_space_drawer(view->priv->editor_settings,
  ...
  if (tl != NULL) gtk_target_list_add_uri_targets(tl, TARGET_URI_LIST);

  view->priv->extensions =
      peas_extension_set_new(PEAS_ENGINE(pluma_plugins_engine_get_default()),
                             PLUMA_TYPE_VIEW_ACTIVATABLE, "view", view, NULL);

  /* Act on buffer change */
  g_signal_connect(view, "notify::buffer", G_CALLBACK(on_notify_buffer_cb),
                   NULL);
}
...
static gint pluma_view_focus_out(GtkWidget *widget, GdkEventFocus *event) {
  ...
  if (view->priv->search_window != NULL) hide_search_window(view, FALSE);
...
void pluma_view_cut_clipboard(PlumaView *view) {
  ...
  gtk_text_buffer_cut_clipboard(
      buffer, clipboard, !pluma_document_get_readonly(PLUMA_DOCUMENT(buffer)));

  gtk_text_view_scroll_to_mark(GTK_TEXT_VIEW(view),
                               gtk_text_buffer_get_insert(buffer),
                               PLUMA_VIEW_SCROLL_MARGIN, FALSE, 0.0, 0.0);
}
...
void pluma_view_delete_selection(PlumaView *view) {
  ...
  gtk_text_buffer_delete_selection(
      buffer, TRUE, !pluma_document_get_readonly(PLUMA_DOCUMENT(buffer)));
...
void pluma_view_upper_case_selection(PlumaView *view) {
  ...
  if (gtk_text_buffer_get_selection_bounds(buffer, &start, &end)) {
    gtk_source_buffer_change_case(GTK_SOURCE_BUFFER(buffer),
                                  GTK_SOURCE_CHANGE_CASE_UPPER, &start, &end);
  }
}
...
void pluma_view_lower_case_selection(PlumaView *view) {
  ...
  if (gtk_text_buffer_get_selection_bounds(buffer, &start, &end)) {
    gtk_source_buffer_change_case(GTK_SOURCE_BUFFER(buffer),
                                  GTK_SOURCE_CHANGE_CASE_LOWER, &start, &end);
  }
}
...
void pluma_view_invert_case_selection(PlumaView *view) {
  ...
  if (gtk_text_buffer_get_selection_bounds(buffer, &start, &end)) {
    gtk_source_buffer_change_case(GTK_SOURCE_BUFFER(buffer),
                                  GTK_SOURCE_CHANGE_CASE_TOGGLE, &start, &end);
  }
}
...
void pluma_view_title_case_selection(PlumaView *view) {
  ...
  if (gtk_text_buffer_get_selection_bounds(buffer, &start, &end)) {
    gtk_source_buffer_change_case(GTK_SOURCE_BUFFER(buffer),
                                  GTK_SOURCE_CHANGE_CASE_TITLE, &start, &end);
  }
}
...
void pluma_view_scroll_to_cursor(PlumaView *view) {
  ...
  gtk_text_view_scroll_to_mark(GTK_TEXT_VIEW(view),
                               gtk_text_buffer_get_insert(buffer), 0.25, FALSE,
                               0.0, 0.0);
}

static void update_css_provider(PlumaView *view) {
  gchar *str;
  gchar *css;
 ...
 **/
void pluma_view_set_font(PlumaView *view, gboolean def,
...
static void add_search_completion_entry(const gchar *str) {
  gchar *text;
  gboolean valid;
  GtkTreeModel *model;
  GtkTreeIter iter;
...
static void set_entry_state(GtkWidget *entry, PlumaSearchEntryState state) {
...
static gboolean run_search(PlumaView *view, const gchar *entry_text,
                           gboolean search_backward, gboolean wrap_around,
                           gboolean typing) {
  ...
  if (*entry_text != '\0') {
    if (!search_backward) {
      if (!typing) {
        ...
        gtk_text_buffer_get_selection_bounds(GTK_TEXT_BUFFER(doc), &start_iter,
                                             &match_end);
      ...
      /* run search */
      found = pluma_document_search_forward(doc, &start_iter, NULL,
      ...
      /* run search */
      found = pluma_document_search_backward(doc, NULL, &start_iter,
                                             &match_start, &match_end);
    } else {
    ...
    if (!found && wrap_around) {
      if (!search_backward)
        found = pluma_document_search_forward(
            doc, NULL, NULL, /* FIXME: set the end_inter */
            &match_start, &match_end);
      else
        found = pluma_document_search_backward(
            doc, NULL, /* FIXME: set the start_inter */
  ...
  if (found) {
    ...
    gtk_text_buffer_move_mark_by_name(GTK_TEXT_BUFFER(doc), "selection_bound",
                                      &match_end);
  } else {
    if (typing) {
...
static gboolean search_entry_flush_timeout(PlumaView *view) {
  view->priv->typeselect_flush_timeout = 0;
...
static void update_search_window_position(PlumaView *view) {
  gint x, y;
  gint view_x, view_y;
...
static gboolean search_window_delete_event(GtkWidget *widget,
                                           GdkEventAny *event,
                                           PlumaView *view) {
  hide_search_window(view, FALSE);
...
static gboolean search_window_button_press_event(GtkWidget *widget,
                                                 GdkEventButton *event,
                                                 PlumaView *view) {
  hide_search_window(view, FALSE);
...
static void search_again(PlumaView *view, gboolean search_backward) {
  ...
  run_search(view, entry_text, search_backward, view->priv->wrap_around, FALSE);
}

static gboolean search_window_scroll_event(GtkWidget *widget,
                                           GdkEventScroll *event,
                                           PlumaView *view) {
  gboolean retval = FALSE;

  if (view->priv->search_mode == GOTO_LINE) return retval;
...
static gboolean search_window_key_press_event(GtkWidget *widget,
                                              GdkEventKey *event,
                                              PlumaView *view) {
  gboolean retval = FALSE;
  guint modifiers;
  ...
  if (event->keyval == GDK_KEY_Escape) {
    if (view->priv->search_mode == SEARCH) {
      ...
      doc = PLUMA_DOCUMENT(gtk_text_view_get_buffer(GTK_TEXT_VIEW(view)));
      pluma_document_set_search_text(doc, view->priv->old_search_text,
...
static void search_entry_activate(GtkEntry *entry, PlumaView *view) {
  hide_search_window(view, FALSE);
}

static void wrap_around_menu_item_toggled(GtkCheckMenuItem *checkmenuitem,
                                          PlumaView *view) {
  view->priv->wrap_around = gtk_check_menu_item_get_active(checkmenuitem);
}

static void match_entire_word_menu_item_toggled(GtkCheckMenuItem *checkmenuitem,
                                                PlumaView *view) {
  PLUMA_SEARCH_SET_ENTIRE_WORD(view->priv->search_flags,
                               gtk_check_menu_item_get_active(checkmenuitem));
}

static void match_case_menu_item_toggled(GtkCheckMenuItem *checkmenuitem,
                                         PlumaView *view) {
  PLUMA_SEARCH_SET_CASE_SENSITIVE(
      view->priv->search_flags, gtk_check_menu_item_get_active(checkmenuitem));
}

static void parse_escapes_menu_item_toggled(GtkCheckMenuItem *checkmenuitem,
                                            PlumaView *view) {
  PLUMA_SEARCH_SET_PARSE_ESCAPES(view->priv->search_flags,
                                 gtk_check_menu_item_get_active(checkmenuitem));
}

static gboolean real_search_enable_popdown(gpointer data) {
...
static void search_enable_popdown(GtkWidget *widget, PlumaView *view) {
...
static void search_entry_populate_popup(GtkEntry *entry, GtkMenu *menu,
                                        PlumaView *view) {
  GtkWidget *menu_item;
  ...
  if (view->priv->search_mode == GOTO_LINE) return;
  ...
  g_signal_connect(menu_item, "toggled",
                   G_CALLBACK(wrap_around_menu_item_toggled), view);
  gtk_menu_shell_prepend(GTK_MENU_SHELL(menu), menu_item);
  ...
  g_signal_connect(menu_item, "toggled",
                   G_CALLBACK(match_entire_word_menu_item_toggled), view);
  gtk_menu_shell_prepend(GTK_MENU_SHELL(menu), menu_item);
  ...
  g_signal_connect(menu_item, "toggled",
                   G_CALLBACK(match_case_menu_item_toggled), view);
  gtk_menu_shell_prepend(GTK_MENU_SHELL(menu), menu_item);
  gtk_check_menu_item_set_active(
      GTK_CHECK_MENU_ITEM(menu_item),
  ...
  menu_item = gtk_check_menu_item_new_with_mnemonic(
      _("_Parse escape sequences (e.g. \n)"));
  g_signal_connect(menu_item, "toggled",
                   G_CALLBACK(parse_escapes_menu_item_toggled), view);
  gtk_menu_shell_prepend(GTK_MENU_SHELL(menu), menu_item);
  gtk_check_menu_item_set_active(
      GTK_CHECK_MENU_ITEM(menu_item),
...
static void search_entry_insert_text(GtkEditable *editable, const gchar *text,
...
static void customize_for_search_mode(PlumaView *view) {
  if (view->priv->search_mode == SEARCH) {
    ...
    gtk_entry_set_icon_from_icon_name(GTK_ENTRY(view->priv->search_entry),
                                      GTK_ENTRY_ICON_PRIMARY, "go-jump");
...
static gboolean completion_func(GtkEntryCompletion *completion, const char *key,
  ...
  if (priv->search_mode == GOTO_LINE) return FALSE;

  real_key =
      gtk_entry_get_text(GTK_ENTRY(gtk_entry_completion_get_entry(completion)));
  ...
  g_return_val_if_fail(
      gtk_tree_model_get_column_type(model, 0) == G_TYPE_STRING, FALSE);
  ...
  if (PLUMA_SEARCH_IS_CASE_SENSITIVE(priv->search_flags)) {
    if (!strncmp(real_key, item, strlen(real_key))) ret = TRUE;
  } else {
    ...
    if (!strncmp(key, case_normalized_string, strlen(key))) ret = TRUE;
...
static void ensure_search_window(PlumaView *view) {
  GtkWidget *frame;
  GtkWidget *vbox;
  GtkWidget *toplevel;
  ...
  g_signal_connect(view->priv->search_window, "delete_event",
                   G_CALLBACK(search_window_delete_event), view);
  g_signal_connect(view->priv->search_window, "key_press_event",
                   G_CALLBACK(search_window_key_press_event), view);
  g_signal_connect(view->priv->search_window, "button_press_event",
                   G_CALLBACK(search_window_button_press_event), view);
  g_signal_connect(view->priv->search_window, "scroll_event",
                   G_CALLBACK(search_window_scroll_event), view);
  ...
  g_signal_connect(view->priv->search_entry, "populate_popup",
                   G_CALLBACK(search_entry_populate_popup), view);
  g_signal_connect(view->priv->search_entry, "activate",
                   G_CALLBACK(search_entry_activate), view);
  g_signal_connect(view->priv->search_entry, "insert_text",
                   G_CALLBACK(search_entry_insert_text), view);
  ...
  gtk_entry_completion_set_match_func(completion, completion_func, view->priv,
                                      NULL);

  /* Assign the completion to the entry */
  gtk_entry_set_completion(GTK_ENTRY(view->priv->search_entry), completion);
...
static gboolean get_selected_text(GtkTextBuffer *doc, gchar **selected_text,
                                  gint *len) {
  GtkTextIter start, end;
  ...
  *selected_text = gtk_text_buffer_get_slice(doc, &start, &end, TRUE);
  ...
  if (len != NULL) *len = g_utf8_strlen(*selected_text, -1);

  return TRUE;
}

static void init_search_entry(PlumaView *view) {
  GtkTextBuffer *buffer;
  ...
  if (view->priv->search_mode == GOTO_LINE) {
    ...
    old_find_text =
        pluma_document_get_search_text(PLUMA_DOCUMENT(buffer), &old_find_flags);
    ...
    selection_exists = get_selected_text(buffer, &find_text, &sel_len);
...
static void search_init(GtkWidget *entry, PlumaView *view) {
  PlumaDocument *doc;
  const gchar *entry_text;
  ...
  if (view->priv->search_mode == SEARCH) {
    ...
    if ((search_text == NULL) || (strcmp(search_text, entry_text) != 0) ||
      ...
      pluma_document_set_search_text(doc, entry_text, view->priv->search_flags);
    }

    g_free(search_text);

    run_search(view, entry_text, FALSE, view->priv->wrap_around, TRUE);
  } else {
    if (*entry_text != '\0') {
      ...
      if (*text == '-') {
        ...
        if (*(text + 1) != '\0') offset_line = MAX(atoi(text + 1), 0);
...
static gboolean start_interactive_search_real(PlumaView *view) {
  GtkTextBuffer *buffer;
  ...
  if (!gtk_widget_has_focus(GTK_WIDGET(view))) return FALSE;

  buffer = gtk_text_view_get_buffer(GTK_TEXT_VIEW(view));

  if (view->priv->search_mode == SEARCH)
    gtk_text_buffer_get_selection_bounds(buffer, &view->priv->start_search_iter,
                                         NULL);
  else
    gtk_text_buffer_get_iter_at_mark(buffer, &view->priv->start_search_iter,
                                     gtk_text_buffer_get_insert(buffer));
  ...
  if (view->priv->search_entry_changed_id == 0) {
    view->priv->search_entry_changed_id = g_signal_connect(
        view->priv->search_entry, "changed", G_CALLBACK(search_init), view);
  }
...
static gboolean pluma_view_draw(GtkWidget *widget, cairo_t *cr) {
  ...
  if (gtk_cairo_should_draw_window(cr, window) &&
    ...
    _pluma_document_search_region(doc, &iter1, &iter2);
  }
...
static gboolean pluma_view_drag_motion(GtkWidget *widget,
  ...
  result = GTK_WIDGET_CLASS(pluma_view_parent_class)
...
static void pluma_view_drag_data_received(GtkWidget *widget,
  ...
  if (info == TARGET_URI_LIST) {
    ...
    GTK_WIDGET_CLASS(pluma_view_parent_class)
        ->drag_data_received(widget, context, x, y, selection_data, info,
                             timestamp);
  }
}
...
static GtkWidget *create_line_numbers_menu(GtkWidget *view) {
  GtkWidget *menu;
  GtkWidget *item;
  ...
  g_settings_bind(PLUMA_VIEW(view)->priv->editor_settings,
                  PLUMA_SETTINGS_DISPLAY_LINE_NUMBERS, item, "active",
                  G_SETTINGS_BIND_SET);
...
static void show_line_numbers_menu(GtkWidget *view, GdkEventButton *event) {
  GtkWidget *menu;
...
static void extension_added(PeasExtensionSet *extensions, PeasPluginInfo *info,
                            PeasExtension *exten, PlumaView *view) {
  pluma_view_activatable_activate(PLUMA_VIEW_ACTIVATABLE(exten));
}

static void extension_removed(PeasExtensionSet *extensions,
...
static void pluma_view_unrealize(GtkWidget *widget) {
  ...
  g_signal_handlers_disconnect_by_func(view->priv->extensions, extension_added,
                                       view);
  g_signal_handlers_disconnect_by_func(view->priv->extensions,
                                       extension_removed, view);
...
static gboolean pluma_view_button_press_event(GtkWidget *widget,
  ...
  if (txt_clip) {
    ...
    gtk_clipboard_set_text(gtk_clipboard_get(GDK_SELECTION_PRIMARY), primtxt,
                           strlen(primtxt));

  if ((event->type == GDK_BUTTON_PRESS) &&
      (event->window ==
       gtk_text_view_get_window(GTK_TEXT_VIEW(widget), GTK_TEXT_WINDOW_LEFT))) {
    if (event->button == 3) show_line_numbers_menu(widget, event);
  ...
  return GTK_WIDGET_CLASS(pluma_view_parent_class)
      ->button_press_event(widget, event);
}

static gboolean pluma_view_button_release_event(GtkWidget *widget,
                                                GdkEventButton *event) {
  if (event->button == 2) middle_or_right_down = FALSE;
...
static void search_highlight_updated_cb(PlumaDocument *doc, GtkTextIter *start,
  ...
  if (gdk_rectangle_intersect(&updated_rect, &visible_rect, &redraw_rect)) {
    ...
    gtk_text_view_buffer_to_window_coords(text_view, GTK_TEXT_WINDOW_WIDGET,
                                          redraw_rect.x, redraw_rect.y,
                                          &widget_rect.x, &widget_rect.y);

    widget_rect.width = redraw_rect.width;
    widget_rect.height = redraw_rect.height;

    gtk_widget_queue_draw_area(GTK_WIDGET(text_view), widget_rect.x,
                               widget_rect.y, widget_rect.width,
                               widget_rect.height);
  }
}
...
static void delete_line(GtkTextView *text_view, gint count) {
  ...
  if (count > 0) {
    ...
    while (count < 0) {
      if (!gtk_text_iter_backward_line(&start)) break;
...
static void pluma_view_delete_from_cursor(GtkTextView *text_view,
  ...
  switch (type) {
    case GTK_DELETE_PARAGRAPHS:
      ...
      break;
    default:
      GTK_TEXT_VIEW_CLASS(pluma_view_parent_class)
