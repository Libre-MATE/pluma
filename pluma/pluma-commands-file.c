...
static void tab_state_changed_while_saving(PlumaTab *tab, GParamSpec *pspec,
                                           PlumaWindow *window);
...
static PlumaTab *get_tab_from_file(GList *docs, GFile *file) {
  PlumaTab *tab = NULL;
...
static gboolean is_duplicated_file(GSList *files, GFile *file) {
  while (files != NULL) {
    if (g_file_equal(files->data, file)) return TRUE;
...
/* File loading */
static gint load_file_list(PlumaWindow *window, GSList *files,
  ...
  if (files_to_load == NULL) return loaded_files;
  ...
  while (l != NULL) {
    ...
    // FIXME: pass the GFile to tab when api is there
    uri = g_file_get_uri(l->data);
    tab = pluma_window_create_tab_from_uri(window, uri, encoding, line_pos,
                                           create, jump_to);
    g_free(uri);
  ...
  if (loaded_files == 1) {
    ...
    pluma_statusbar_flash_message(
        ...
        window->priv->generic_message_cid,
        ngettext("Loading %d file\342\200\246", "Loading %d files\342\200\246",
                 loaded_files),
        loaded_files);
  }
...
static gint load_uri_list(PlumaWindow *window, const GSList *uris,
                          const PlumaEncoding *encoding, gint line_pos,
                          gboolean create) {
  GSList *files = NULL;
  const GSList *u;
 ...
 */
static gint pluma_commands_load_files(PlumaWindow *window, GSList *files,
...
static void open_dialog_destroyed(PlumaWindow *window,
                                  PlumaFileChooserDialog *dialog) {
  pluma_debug(DEBUG_COMMANDS);

  g_object_set_data(G_OBJECT(window), PLUMA_OPEN_DIALOG_KEY, NULL);
}

static void open_dialog_response_cb(PlumaFileChooserDialog *dialog,
                                    gint response_id, PlumaWindow *window) {
  GSList *files;
  const PlumaEncoding *encoding;
  ...
  pluma_commands_load_files(window, files, encoding, 0);
...
void _pluma_cmd_file_open(GtkAction *action, PlumaWindow *window) {
  ...
  open_dialog = pluma_file_chooser_dialog_new(
      _("Open Files"), GTK_WINDOW(window), GTK_FILE_CHOOSER_ACTION_OPEN, NULL,
      "process-stop", GTK_RESPONSE_CANCEL, "document-open", GTK_RESPONSE_OK,
      NULL);

  g_object_set_data(G_OBJECT(window), PLUMA_OPEN_DIALOG_KEY, open_dialog);

  g_object_weak_ref(G_OBJECT(open_dialog), (GWeakNotify)open_dialog_destroyed,
                    window);
  ...
  g_signal_connect(open_dialog, "response", G_CALLBACK(open_dialog_response_cb),
                   window);
...
static gboolean is_read_only(GFile *location) {
  gboolean ret = TRUE; /* default to read only */
  GFileInfo *info;

  pluma_debug(DEBUG_COMMANDS);

  info = g_file_query_info(location, G_FILE_ATTRIBUTE_ACCESS_CAN_WRITE,
                           G_FILE_QUERY_INFO_NONE, NULL, NULL);

  if (info != NULL) {
    if (g_file_info_has_attribute(info, G_FILE_ATTRIBUTE_ACCESS_CAN_WRITE)) {
      ret = !g_file_info_get_attribute_boolean(
...
static gboolean replace_read_only_file(GtkWindow *parent, GFile *file) {
  ...
  pluma_dialog_add_button(GTK_DIALOG(dialog), _("_Cancel"), "process-stop",
                          GTK_RESPONSE_CANCEL);

  pluma_dialog_add_button(GTK_DIALOG(dialog), _("_Replace"), "document-save-as",
                          GTK_RESPONSE_YES);
...
static void save_dialog_response_cb(PlumaFileChooserDialog *dialog,
                                    gint response_id, PlumaWindow *window) {
  GFile *file;
  const PlumaEncoding *encoding;
  PlumaTab *tab;
  ...
  tab = PLUMA_TAB(g_object_get_data(G_OBJECT(dialog), PLUMA_TAB_TO_SAVE_AS));
...
save_next_tab:
  ...
  data = g_object_get_data(G_OBJECT(window), PLUMA_LIST_OF_TABS_TO_SAVE_AS);
  if (data == NULL) return;
  ...
  g_object_set_data(G_OBJECT(window), PLUMA_LIST_OF_TABS_TO_SAVE_AS,
                    tabs_to_save_as);

  if (tabs_to_save_as != NULL) {
    tab = PLUMA_TAB(tabs_to_save_as->data);

    if (GPOINTER_TO_BOOLEAN(
            g_object_get_data(G_OBJECT(tab), PLUMA_IS_CLOSING_TAB)) == TRUE) {
      g_object_set_data(G_OBJECT(tab), PLUMA_IS_CLOSING_TAB, NULL);

      /* Trace tab state changes */
      g_signal_connect(tab, "notify::state",
...
static GtkFileChooserConfirmation confirm_overwrite_callback(
    GtkFileChooser *dialog, gpointer data) {
  gchar *uri;
  GFile *file;
...
static void file_save_as(PlumaTab *tab, PlumaWindow *window) {
  ...
  save_dialog = pluma_file_chooser_dialog_new(
      _("Save As\342\200\246"), GTK_WINDOW(window),
      GTK_FILE_CHOOSER_ACTION_SAVE, NULL, "process-stop", GTK_RESPONSE_CANCEL,
      "document-save", GTK_RESPONSE_OK, NULL);

  gtk_file_chooser_set_do_overwrite_confirmation(GTK_FILE_CHOOSER(save_dialog),
                                                 TRUE);
  g_signal_connect(save_dialog, "confirm-overwrite",
  ...
  if (file != NULL) {
    uri_set =
        gtk_file_chooser_set_file(GTK_FILE_CHOOSER(save_dialog), file, NULL);
  ...
  pluma_file_chooser_dialog_set_newline_type(
      PLUMA_FILE_CHOOSER_DIALOG(save_dialog), newline_type);

  g_object_set_data(G_OBJECT(save_dialog), PLUMA_TAB_TO_SAVE_AS, tab);

  g_signal_connect(save_dialog, "response", G_CALLBACK(save_dialog_response_cb),
                   window);

  gtk_widget_show(save_dialog);
}

static void file_save(PlumaTab *tab, PlumaWindow *window) {
  PlumaDocument *doc;
  gchar *uri_for_display;
  ...
  if (pluma_document_is_untitled(doc) || pluma_document_get_readonly(doc)) {
...
static gboolean document_needs_saving(PlumaDocument *doc) {
  if (gtk_text_buffer_get_modified(GTK_TEXT_BUFFER(doc))) return TRUE;
...
void _pluma_cmd_file_save_documents_list(PlumaWindow *window, GList *docs) {
  ...
  g_return_if_fail(
      !(pluma_window_get_state(window) &
        (PLUMA_WINDOW_STATE_PRINTING | PLUMA_WINDOW_STATE_SAVING_SESSION)));
  ...
  while (l != NULL) {
    ...
    if ((state == PLUMA_TAB_STATE_NORMAL) ||
      ...
      /* If the state is:
         - PLUMA_TAB_STATE_LOADING: we do not save since we are sure the file is
         unmodified
         ...
         - PLUMA_TAB_STATE_SAVING: well, we are already saving (no need to save
         again)
         - PLUMA_TAB_STATE_PRINTING, PLUMA_TAB_STATE_PRINT_PREVIEWING: there is
         not a real reason for not saving in this case, we do not save to avoid
         to run two operations using the message area at the same time (may be
         we can remove this limitation in the future). Note that SaveAll,
         ClosAll and Quit are unsensitive if the window state is PRINTING.
         - PLUMA_TAB_STATE_GENERIC_ERROR: we do not save since the document
         contains errors (I don't think this is a very frequent case, we should
         probably remove this state)
         - PLUMA_TAB_STATE_LOADING_ERROR: there is nothing to save
         - PLUMA_TAB_STATE_REVERTING_ERROR: there is nothing to save and saving
         the current document will overwrite the copy of the file the user wants
         to go back to
         - PLUMA_TAB_STATE_SAVING_ERROR: we do not save since we just failed to
         save, so there is no reason to automatically retry... we wait for user
         intervention
      ...
      */
      ...
      uri_for_display = pluma_document_get_uri_for_display(doc);
      pluma_debug_message(DEBUG_COMMANDS, "File '%s' not saved. State: %d",
...
/* File revert */
static void do_revert(PlumaWindow *window, PlumaTab *tab) {
  PlumaDocument *doc;
  gchar *docname;
...
static void revert_dialog_response_cb(GtkDialog *dialog, gint response_id,
                                      PlumaWindow *window) {
  PlumaTab *tab;
...
static GtkWidget *revert_dialog(PlumaWindow *window, PlumaDocument *doc) {
  GtkWidget *dialog;
  gchar *docname;
  gchar *primary_msg;
  gchar *secondary_msg;
  ...
  pluma_dialog_add_button(GTK_DIALOG(dialog), _("_Revert"), "document-revert",
                          GTK_RESPONSE_OK);
...
void _pluma_cmd_file_revert(GtkAction *action, PlumaWindow *window) {
  ...
  g_signal_connect(dialog, "response", G_CALLBACK(revert_dialog_response_cb),
                   window);
...
/* Close tab */
static gboolean really_close_tab(PlumaTab *tab) {
  GtkWidget *toplevel;
  PlumaWindow *window;
  ...
  if (pluma_window_get_active_tab(window) == NULL) {
    ...
    is_quitting = GPOINTER_TO_BOOLEAN(
        g_object_get_data(G_OBJECT(window), PLUMA_IS_QUITTING));

    if (is_quitting) gtk_widget_destroy(GTK_WIDGET(window));
  }
...
static void tab_state_changed_while_saving(PlumaTab *tab, GParamSpec *pspec,
  ...
  if (ts == PLUMA_TAB_STATE_NORMAL) {
    ...
    if (document_needs_saving(doc)) return;
    ...
    g_idle_add_full(G_PRIORITY_HIGH_IDLE, (GSourceFunc)really_close_tab, tab,
                    NULL);
  }
}

static void save_and_close(PlumaTab *tab, PlumaWindow *window) {
  pluma_debug(DEBUG_COMMANDS);
...
static void save_as_and_close(PlumaTab *tab, PlumaWindow *window) {
  pluma_debug(DEBUG_COMMANDS);
...
static void save_and_close_all_documents(const GList *docs,
                                         PlumaWindow *window) {
  GList *tabs;
  GList *l;
  GSList *sl;
  ...
  g_return_if_fail(
      !(pluma_window_get_state(window) & PLUMA_WINDOW_STATE_PRINTING));
  ...
  while (l != NULL) {
    ...
    /* If the state is: ([*] invalid states)
       ...
       - PLUMA_TAB_STATE_REVERTING: since the user wants
         to return back to the version of the file she previously saved, we can
       close without saving (CHECK: are we sure this is the right behavior,
       suppose the case the original file has been deleted)
       - [*] PLUMA_TAB_STATE_SAVING: invalid, ClosAll
         and Quit are unsensitive if the window state is SAVING.
       - [*] PLUMA_TAB_STATE_PRINTING, PLUMA_TAB_STATE_PRINT_PREVIEWING: there
       is not a real reason for not closing in this case, we do not save to
       avoid to run two operations using the message area at the same time (may
       be we can remove this limitation in the future). Note that ClosAll and
       Quit are unsensitive if the window state is PRINTING.
       - PLUMA_TAB_STATE_SHOWING_PRINT_PREVIEW: close (and if needed save)
       - PLUMA_TAB_STATE_LOADING_ERROR: close without saving (if the state is
       LOADING_ERROR then the document is not modified)
       - PLUMA_TAB_STATE_REVERTING_ERROR: we do not close since the document
       contains errors
       - PLUMA_TAB_STATE_SAVING_ERROR: we do not close since the document
       contains errors
       - PLUMA_TAB_STATE_GENERIC_ERROR: we do not close since the document
    ...
    */
    ...
    if ((state != PLUMA_TAB_STATE_SAVING_ERROR) &&
      ...
      if ((g_list_index((GList *)docs, doc) >= 0) &&
          ...
          (state != PLUMA_TAB_STATE_REVERTING)) /* CHECK: is this the right
                                                   behavior with REVERTING ?*/
      {
        ...
        if (pluma_document_is_untitled(doc) ||
          ...
          g_object_set_data(G_OBJECT(t), PLUMA_IS_CLOSING_TAB,
  ...
  while (sl != NULL) {
    save_and_close(PLUMA_TAB(sl->data), window);
    sl = g_slist_next(sl);
  }
...
static void save_and_close_document(const GList *docs, PlumaWindow *window) {
  PlumaTab *tab;
...
static void close_all_tabs(PlumaWindow *window) {
  gboolean is_quitting;
...
static void close_document(PlumaWindow *window, PlumaDocument *doc) {
  PlumaTab *tab;
...
static void close_confirmation_dialog_response_handler(
    PlumaCloseConfirmationDialog *dlg, gint response_id, PlumaWindow *window) {
  ...
  is_closing_all = GPOINTER_TO_BOOLEAN(
      g_object_get_data(G_OBJECT(window), PLUMA_IS_CLOSING_ALL));
  ...
  switch (response_id) {
    case GTK_RESPONSE_YES: /* Save and Close */
      selected_documents =
          pluma_close_confirmation_dialog_get_selected_documents(dlg);
    ...
    case GTK_RESPONSE_NO: /* Close without Saving */
      if (is_closing_all) {
        ...
        unsaved_documents =
            pluma_close_confirmation_dialog_get_unsaved_documents(dlg);
        g_return_if_fail(unsaved_documents->next == NULL);

        close_document(window, PLUMA_DOCUMENT(unsaved_documents->data));
      }
    ...
    default: /* Do not close */
      ...
      g_object_set_data(G_OBJECT(window), PLUMA_IS_QUITTING,
...
static gboolean tab_can_close(PlumaTab *tab, GtkWindow *window) {
  PlumaDocument *doc;
  ...
  if (!_pluma_tab_can_close(tab)) {
    ...
    dlg = pluma_close_confirmation_dialog_new_single(window, doc, FALSE);
...
void _pluma_cmd_file_close_tab(PlumaTab *tab, PlumaWindow *window) {
  ...
  g_return_if_fail(GTK_WIDGET(window) ==
                   gtk_widget_get_toplevel(GTK_WIDGET(tab)));

  g_object_set_data(G_OBJECT(window), PLUMA_IS_CLOSING_ALL,
                    GBOOLEAN_TO_POINTER(FALSE));

  g_object_set_data(G_OBJECT(window), PLUMA_IS_QUITTING,
                    GBOOLEAN_TO_POINTER(FALSE));

  g_object_set_data(G_OBJECT(window), PLUMA_IS_QUITTING_ALL,
...
/* Close all tabs */
static void file_close_all(PlumaWindow *window, gboolean is_quitting) {
  GList *unsaved_docs;
  GtkWidget *dlg;
  ...
  g_return_if_fail(!(pluma_window_get_state(window) &
                     (PLUMA_WINDOW_STATE_SAVING | PLUMA_WINDOW_STATE_PRINTING |
  ...
  if (unsaved_docs->next == NULL) {
    ...
    dlg = pluma_close_confirmation_dialog_new_single(GTK_WINDOW(window), doc,
                                                     FALSE);
  } else {
    dlg = pluma_close_confirmation_dialog_new(GTK_WINDOW(window), unsaved_docs,
                                              FALSE);
  }
