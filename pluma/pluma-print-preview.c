...
static void pluma_print_preview_get_property(GObject *object, guint prop_id,
...
static void pluma_print_preview_set_property(GObject *object, guint prop_id,
                                             const GValue *value,
                                             GParamSpec *pspec) {
  // PlumaPrintPreview *preview = PLUMA_PRINT_PREVIEW (object);
...
static void pluma_print_preview_finalize(GObject *object) {
  // PlumaPrintPreview *preview = PLUMA_PRINT_PREVIEW (object);

  G_OBJECT_CLASS(pluma_print_preview_parent_class)->finalize(object);
}

static void pluma_print_preview_grab_focus(GtkWidget *widget) {
...
static void pluma_print_preview_class_init(PlumaPrintPreviewClass *klass) {
...
static void update_layout_size(PlumaPrintPreview *preview) {
  PlumaPrintPreviewPrivate *priv;

  priv = preview->priv;

  /* force size of the drawing area to make the scrolled window work */
  gtk_layout_set_size(GTK_LAYOUT(priv->layout), priv->tile_w * priv->cols,
                      priv->tile_h * priv->rows);

  gtk_widget_queue_draw(preview->priv->layout);
}

static void set_rows_and_cols(PlumaPrintPreview *preview, gint rows,
                              gint cols) {
...
static double get_paper_width(PlumaPrintPreview *preview) {
  return preview->priv->paper_w * preview->priv->dpi;
}

static double get_paper_height(PlumaPrintPreview *preview) {
...
static void update_tile_size(PlumaPrintPreview *preview) {
  PlumaPrintPreviewPrivate *priv;
  gint w, h;
...
static void set_zoom_factor(PlumaPrintPreview *preview, double zoom) {
  PlumaPrintPreviewPrivate *priv;
...
static void set_zoom_fit_to_size(PlumaPrintPreview *preview) {
  PlumaPrintPreviewPrivate *priv;
  double width, height;
...
static void zoom_in(PlumaPrintPreview *preview) {
  set_zoom_factor(preview, preview->priv->scale * ZOOM_IN_FACTOR);
}

static void zoom_out(PlumaPrintPreview *preview) {
  set_zoom_factor(preview, preview->priv->scale * ZOOM_OUT_FACTOR);
}

static void goto_page(PlumaPrintPreview *preview, gint page) {
  gchar c[32];
  ...
  gtk_widget_set_sensitive(
      GTK_WIDGET(preview->priv->next),
      (page != (preview->priv->n_pages - 1)) && (preview->priv->n_pages > 1));
...
static void prev_button_clicked(GtkWidget *button, PlumaPrintPreview *preview) {
  GdkEvent *event;
  gint page;
...
static void next_button_clicked(GtkWidget *button, PlumaPrintPreview *preview) {
  GdkEvent *event;
  gint page;
...
static void page_entry_activated(GtkEntry *entry, PlumaPrintPreview *preview) {
  const gchar *text;
  gint page;
...
static void page_entry_insert_text(GtkEditable *editable, const gchar *text,
...
static gboolean page_entry_focus_out(GtkWidget *widget, GdkEventFocus *event,
                                     PlumaPrintPreview *preview) {
  const gchar *text;
  gint page;
...
static void on_1x1_clicked(GtkMenuItem *i, PlumaPrintPreview *preview) {
  set_rows_and_cols(preview, 1, 1);
}

static void on_1x2_clicked(GtkMenuItem *i, PlumaPrintPreview *preview) {
  set_rows_and_cols(preview, 1, 2);
}

static void on_2x1_clicked(GtkMenuItem *i, PlumaPrintPreview *preview) {
  set_rows_and_cols(preview, 2, 1);
}

static void on_2x2_clicked(GtkMenuItem *i, PlumaPrintPreview *preview) {
  set_rows_and_cols(preview, 2, 2);
}

static void multi_button_clicked(GtkWidget *button,
                                 PlumaPrintPreview *preview) {
  GtkWidget *m, *i;

  m = gtk_menu_new();
  gtk_widget_show(m);
  g_signal_connect(m, "selection_done", G_CALLBACK(gtk_widget_destroy), m);
...
static void zoom_one_button_clicked(GtkWidget *button,
                                    PlumaPrintPreview *preview) {
  set_zoom_factor(preview, 1);
}

static void zoom_fit_button_clicked(GtkWidget *button,
                                    PlumaPrintPreview *preview) {
  set_zoom_fit_to_size(preview);
}

static void zoom_in_button_clicked(GtkWidget *button,
                                   PlumaPrintPreview *preview) {
  zoom_in(preview);
}

static void zoom_out_button_clicked(GtkWidget *button,
                                    PlumaPrintPreview *preview) {
  zoom_out(preview);
}

static void close_button_clicked(GtkWidget *button,
                                 PlumaPrintPreview *preview) {
  gtk_widget_destroy(GTK_WIDGET(preview));
}

static gboolean ignore_mouse_buttons(GtkWidget *widget, GdkEventKey *event,
                                     PlumaPrintPreview *preview) {
  return TRUE;
}

static void create_bar(PlumaPrintPreview *preview) {
  PlumaPrintPreviewPrivate *priv;
  GtkWidget *toolbar;
  GtkToolItem *i;
  AtkObject *atko;
  ...
  g_signal_connect(priv->prev, "clicked", G_CALLBACK(prev_button_clicked),
                   preview);
  ...
  g_signal_connect(priv->next, "clicked", G_CALLBACK(next_button_clicked),
                   preview);
  ...
  gtk_box_pack_start(
      ...
      gtk_label_new(_("of")), FALSE, FALSE, 0);
  ...
  atk_object_set_description(atko,
                             _("The total number of pages in the document"));
  ...
  g_signal_connect(priv->multi, "clicked", G_CALLBACK(multi_button_clicked),
                   preview);
  ...
  gtk_tool_button_set_icon_name(GTK_TOOL_BUTTON(priv->zoom_one),
                                "zoom-original");
  ...
  gtk_tool_button_set_icon_name(GTK_TOOL_BUTTON(priv->zoom_fit),
                                "zoom-fit-best");
  gtk_tool_item_set_tooltip_text(priv->zoom_fit,
  ...
  g_signal_connect(priv->zoom_in, "clicked", G_CALLBACK(zoom_in_button_clicked),
                   preview);
  ...
  g_signal_connect(i, "clicked", G_CALLBACK(close_button_clicked), preview);
  gtk_widget_show(GTK_WIDGET(i));
  gtk_toolbar_insert(GTK_TOOLBAR(toolbar), i, -1);
...
static gint get_first_page_displayed(PlumaPrintPreview *preview) {
  PlumaPrintPreviewPrivate *priv;
...
static gint get_page_at_coords(PlumaPrintPreview *preview, gint x, gint y) {
  PlumaPrintPreviewPrivate *priv;
  ...
  if (priv->tile_h <= 0 || priv->tile_w <= 0) return -1;
  ...
  if (c > priv->cols) return -1;

  pg = get_first_page_displayed(preview) - 1;
  pg += (r - 1) * priv->cols + c;

  if (pg >= priv->n_pages) return -1;
...
static gboolean preview_layout_query_tooltip(GtkWidget *widget, gint x, gint y,
...
static gint preview_layout_key_press(GtkWidget *widget, GdkEventKey *event,
                                     PlumaPrintPreview *preview) {
  PlumaPrintPreviewPrivate *priv;
  GtkAdjustment *hadj, *vadj;
  double x, y;
  ...
  g_object_get(hadj, "lower", &hlower, "upper", &hupper, "page-size", &hpage,
               NULL);
  g_object_get(vadj, "lower", &vlower, "upper", &vupper, "page-size", &vpage,
               NULL);
  ...
  switch (event->keyval) {
    ...
    case GDK_KEY_KP_Up:
    case GDK_KEY_Up:
      if (event->state & GDK_SHIFT_MASK) goto page_up;
      y = MAX(vlower, y - vstep);
      domove = TRUE;
      break;
    case GDK_KEY_KP_Down:
    case GDK_KEY_Down:
      if (event->state & GDK_SHIFT_MASK) goto page_down;
...
static void create_preview_layout(PlumaPrintPreview *preview) {
  PlumaPrintPreviewPrivate *priv;
  AtkObject *atko;
  ...
  atk_object_set_description(
      atko, _("The preview of a page in the document to be printed"));

  gtk_widget_add_events(priv->layout, GDK_POINTER_MOTION_MASK |
...
static void pluma_print_preview_init(PlumaPrintPreview *preview) {
  PlumaPrintPreviewPrivate *priv;
...
static void draw_page_content(cairo_t *cr, gint page_number,
                              PlumaPrintPreview *preview) {
  ...
  if ((preview->priv->orientation == GTK_PAGE_ORIENTATION_LANDSCAPE) ||
    ...
    cairo_matrix_init(&matrix, 0, -1, 1, 0, 0, get_paper_width(preview));
    cairo_transform(cr, &matrix);
  }

  gtk_print_context_set_cairo_context(preview->priv->context, cr,
 ...
 * the orientation */
static void draw_page_frame(cairo_t *cr, PlumaPrintPreview *preview) {
  double w, h;
...
static void draw_page(cairo_t *cr, double x, double y, gint page_number,
                      PlumaPrintPreview *preview) {
  cairo_save(cr);
...
static gboolean preview_draw(GtkWidget *widget, cairo_t *cr,
  ...
  if (!gtk_cairo_should_draw_window(cr, bin_window)) return TRUE;
...
static double get_screen_dpi(PlumaPrintPreview *preview) {
  GdkScreen *screen;
  double dpi;
...
static void set_n_pages(PlumaPrintPreview *preview, gint n_pages) {
  gchar *str;
...
static void update_paper_size(PlumaPrintPreview *preview,
...
static void preview_got_page_size(GtkPrintOperationPreview *gtk_preview,
                                  GtkPrintContext *context,
                                  GtkPageSetup *page_setup,
                                  PlumaPrintPreview *preview) {
...
static cairo_surface_t *create_preview_surface_platform(
    GtkPaperSize *paper_size, double *dpi_x, double *dpi_y) {
  double width, height;
  cairo_surface_t *sf;
  ...
  *dpi_x = *dpi_y = PRINTER_DPI;
  ...
  sf = cairo_pdf_surface_create_for_stream(dummy_write_func, NULL, width,
                                           height);
  return sf;
}

static cairo_surface_t *create_preview_surface(PlumaPrintPreview *preview,
...
GtkWidget *pluma_print_preview_new(GtkPrintOperation *op,
  ...
  g_signal_connect(gtk_preview, "ready", G_CALLBACK(preview_ready), preview);
