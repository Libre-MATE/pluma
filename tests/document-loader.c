...
static GFile *create_document(const gchar *filename, const gchar *contents) {
  GError *error = NULL;
...
static void delete_document(GFile *location) {
...
static void on_document_loaded(PlumaDocument *document, GError *error,
                               LoaderTestData *data) {
...
static void test_loader(const gchar *filename, const gchar *contents,
                        const gchar *in_buffer, gint newline_type) {
  GFile *file;
  gchar *uri;
  ...
  g_signal_connect(document, "loaded", G_CALLBACK(on_document_loaded), data);
...
static void test_end_line_stripping() {
  test_loader("document-loader.txt", "hello world\n", "hello world", -1);

  test_loader("document-loader.txt", "hello world", "hello world", -1);
...
static void test_end_new_line_detection() {
  test_loader("document-loader.txt", "hello world\n", NULL,
  ...
  test_loader("document-loader.txt", "hello world\r", NULL,
              PLUMA_DOCUMENT_NEWLINE_TYPE_CR);
}

static void test_begin_new_line_detection() {
  test_loader("document-loader.txt", "\nhello world", NULL,
              PLUMA_DOCUMENT_NEWLINE_TYPE_LF);
  ...
  test_loader("document-loader.txt", "\rhello world", NULL,
              PLUMA_DOCUMENT_NEWLINE_TYPE_CR);
}
...
int main(int argc, char *argv[]) {
  ...
  g_test_add_func("/document-loader/end-line-stripping",
                  test_end_line_stripping);
  g_test_add_func("/document-loader/end-new-line-detection",
                  test_end_new_line_detection);
  g_test_add_func("/document-loader/begin-new-line-detection",
