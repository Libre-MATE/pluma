...
#define UNOWNED_LOCAL_URI \
  "/tmp/pluma-document-saver-unowned/pluma-document-saver-test.txt"

#define UNOWNED_REMOTE_URI                             \
  "sftp://localhost/tmp/pluma-document-saver-unowned/" \
  "pluma-document-saver-test.txt"

#define UNOWNED_GROUP_LOCAL_URI "/tmp/pluma-document-saver-unowned-group.txt"
#define UNOWNED_GROUP_REMOTE_URI \
  "sftp://localhost/tmp/pluma-document-saver-unowned-group.txt"
...
static SaverTestData *saver_test_data_new(const gchar *uri,
                                          const gchar *test_contents,
                                          gpointer data) {
...
static void saver_test_data_free(SaverTestData *data) {
  if (data == NULL) {
    return;
  }
...
static PlumaDocument *create_document(const gchar *contents) {
...
static void complete_test_error(PlumaDocument *document, GError *error,
                                SaverTestData *data) {
  g_assert_no_error(error);
}

static const gchar *read_file(const gchar *uri) {
  ...
  g_input_stream_read_all(stream, buffer, sizeof(buffer) - 1, &read, NULL,
                          &error);
  g_assert_no_error(error);
...
static void complete_test(PlumaDocument *document, GError *error,
                          SaverTestData *data) {
  test_completed = TRUE;
...
static void mount_ready_callback(GObject *object, GAsyncResult *result,
                                 gpointer data) {
  GError *error = NULL;
  mount_success =
      g_file_mount_enclosing_volume_finish(G_FILE(object), result, &error);
...
static gboolean ensure_mounted(GFile *file) {
  GMountOperation *mo;
  ...
  g_file_mount_enclosing_volume(file, G_MOUNT_MOUNT_NONE, mo, NULL,
                                mount_ready_callback, NULL);
...
static void test_saver(const gchar *filename_or_uri, const gchar *contents,
                       PlumaDocumentNewlineType newline_type,
                       PlumaDocumentSaveFlags save_flags,
                       GCallback saved_callback, SaverTestData *data) {
  GFile *file;
  gchar *uri;
...
static void test_new_line(const gchar *filename,
                          PlumaDocumentSaveFlags save_flags) {
  gint i;
  ...
  for (i = 0; i < num; ++i) {
    ...
    test_saver(filename, nt->text, nt->type, save_flags, NULL,
               saver_test_data_new(filename, nt->result, NULL));
  }
}

static void test_local_newline() { test_new_line(DEFAULT_LOCAL_URI, 0); }

static void test_local() {
  test_saver(DEFAULT_LOCAL_URI, "hello world", PLUMA_DOCUMENT_NEWLINE_TYPE_LF,
             0, NULL,
             saver_test_data_new(DEFAULT_LOCAL_URI, "hello world\n", NULL));

  test_saver(DEFAULT_LOCAL_URI, "hello world\r\n",
             PLUMA_DOCUMENT_NEWLINE_TYPE_LF, 0, NULL,
             saver_test_data_new(DEFAULT_LOCAL_URI, "hello world\n\n", NULL));
...
static void test_remote_newline() { test_new_line(DEFAULT_REMOTE_URI, 0); }

static void test_remote() {
  test_saver(DEFAULT_REMOTE_URI, "hello world", PLUMA_DOCUMENT_NEWLINE_TYPE_LF,
             0, NULL,
             saver_test_data_new(DEFAULT_REMOTE_URI, "hello world\n", NULL));

  test_saver(DEFAULT_REMOTE_URI, "hello world\r\n",
             PLUMA_DOCUMENT_NEWLINE_TYPE_LF, 0, NULL,
             saver_test_data_new(DEFAULT_REMOTE_URI, "hello world\n\n", NULL));
...
static void check_permissions(GFile *file, guint permissions) {
  GError *error = NULL;
  GFileInfo *info;
...
static void check_permissions_saved(PlumaDocument *document, GError *error,
                                    SaverTestData *data) {
...
static void test_permissions(const gchar *uri, guint permissions) {
  GError *error = NULL;
  GFile *file = g_file_new_for_commandline_arg(uri);
  ...
  test_saver(uri, DEFAULT_CONTENT, PLUMA_DOCUMENT_NEWLINE_TYPE_LF, 0,
             G_CALLBACK(check_permissions_saved),
             saver_test_data_new(uri, DEFAULT_CONTENT_RESULT,
...
static void test_local_permissions() {
...
static void test_local_unowned_directory() {
  test_saver(
      UNOWNED_LOCAL_URI, DEFAULT_CONTENT, PLUMA_DOCUMENT_NEWLINE_TYPE_LF, 0,
      NULL,
      saver_test_data_new(UNOWNED_LOCAL_URI, DEFAULT_CONTENT_RESULT, NULL));
}

static void test_remote_unowned_directory() {
  test_saver(
      UNOWNED_REMOTE_URI, DEFAULT_CONTENT, PLUMA_DOCUMENT_NEWLINE_TYPE_LF, 0,
      NULL,
      saver_test_data_new(UNOWNED_REMOTE_URI, DEFAULT_CONTENT_RESULT, NULL));
}

static void test_remote_permissions() {
...
static void test_unowned_group_permissions(PlumaDocument *document,
                                           GError *error, SaverTestData *data) {
  ...
  GFileInfo *info = g_file_query_info(
      file, G_FILE_ATTRIBUTE_OWNER_GROUP "," G_FILE_ATTRIBUTE_UNIX_MODE,
...
static void test_unowned_group(const gchar *uri) {
  test_saver(uri, DEFAULT_CONTENT, PLUMA_DOCUMENT_NEWLINE_TYPE_LF, 0,
             G_CALLBACK(test_unowned_group_permissions),
             saver_test_data_new(uri, DEFAULT_CONTENT_RESULT, NULL));
}

static void test_local_unowned_group() {
  test_unowned_group(UNOWNED_GROUP_LOCAL_URI);
}

static void test_remote_unowned_group() {
  test_unowned_group(UNOWNED_GROUP_REMOTE_URI);
}

static gboolean check_unowned_directory() {
  ...
  info = g_file_query_info(unowned, G_FILE_ATTRIBUTE_ACCESS_CAN_WRITE,
                           G_FILE_QUERY_INFO_NONE, NULL, &error);
  ...
  info = g_file_query_info(unowned_file, G_FILE_ATTRIBUTE_ACCESS_CAN_WRITE,
                           G_FILE_QUERY_INFO_NONE, NULL, &error);
...
static gboolean check_unowned_group() {
  ...
  if (g_strcmp0(
    ...
    g_printf(
        "NO: group is not root (%s)\n",
        g_file_info_get_attribute_string(info, G_FILE_ATTRIBUTE_OWNER_GROUP));
    g_object_unref(info);
    return FALSE;
  }

  if ((g_file_info_get_attribute_uint32(info, G_FILE_ATTRIBUTE_UNIX_MODE) &
       ACCESSPERMS) != 0660) {
    g_object_unref(unowned);
...
int main(int argc, char *argv[]) {
  ...
  if (have_unowned) {
    g_test_add_func("/document-saver/local-unowned-directory",
  ...
  if (have_unowned_group) {
    g_test_add_func("/document-saver/local-unowned-group",
                    test_local_unowned_group);
  }

  g_test_add_func("/document-saver/remote-permissions",
