...
static void print_hex(gchar *ptr, gint len) {
  gint i;
...
static gchar *get_encoded_text(const gchar *text, gsize nread,
  ...
  converter = g_charset_converter_new(pluma_encoding_get_charset(to),
                                      pluma_encoding_get_charset(from), NULL);
  ...
  *bytes_written_aux = 0;
  ...
  do {
    res = g_converter_convert(G_CONVERTER(converter), text + bytes_read_aux,
                              nread, out_aux, 200, G_CONVERTER_INPUT_AT_END,
...
static GSList *get_all_encodings() {
  GSList *encs = NULL;
  gint i = 0;
...
static gchar *do_test(const gchar *test_in, const gchar *enc, GSList *encodings,
                      gsize nread, const PlumaEncoding **guessed) {
  ...
  if (enc != NULL) {
    ...
    encodings = g_slist_prepend(encodings,
                                (gpointer)pluma_encoding_get_from_charset(enc));
  }
  ...
  do {
    res = g_converter_convert(G_CONVERTER(converter), test_in + bytes_read_aux,
                              nread, out_aux, 200, G_CONVERTER_INPUT_AT_END,
                              &bytes_read, &bytes_written, &err);
    memcpy(out + bytes_written_aux, out_aux, bytes_written);
    bytes_read_aux += bytes_read;
...
static void do_test_roundtrip(const char *str, const char *charset) {
  gsize len;
  gchar *buf, *p;
  GInputStream *in, *tmp;
  ...
  enc =
      g_slist_prepend(enc, (gpointer)pluma_encoding_get_from_charset(charset));
...
static void test_utf8_utf8() {
  ...
  aux =
      do_test("foobar\xc3\xa8\xc3\xa8\xc3\xa8zzzzzz", "UTF-8", NULL, 18, NULL);
...
static void test_xxx_xxx() {
  ...
  for (l = encs; l != NULL; l = g_slist_next(l)) {
    do_test_roundtrip(TEXT_TO_CONVERT, pluma_encoding_get_charset(
                                           (const PlumaEncoding *)l->data));
  }
...
static void test_empty() {
  ...
  encodings = g_slist_prepend(
      encodings, (gpointer)pluma_encoding_get_from_charset("UTF-16"));
  encodings = g_slist_prepend(
      encodings, (gpointer)pluma_encoding_get_from_charset("ISO-8859-15"));
...
static void test_guessed() {
  GSList *encs = NULL;
  gchar *aux, *aux2, *fail;
  gsize aux_len, fail_len;
  const PlumaEncoding *guessed;

  aux = get_encoded_text(
      TEXT_TO_GUESS, -1, pluma_encoding_get_from_charset("UTF-16"),
      pluma_encoding_get_from_charset("UTF-8"), &aux_len, TRUE);

  fail = get_encoded_text(
      aux, aux_len, pluma_encoding_get_from_charset("UTF-8"),
      pluma_encoding_get_from_charset("ISO-8859-15"), &fail_len, FALSE);

  g_assert(fail == NULL);

  /* ISO-8859-15 should fail */
  encs = g_slist_append(
      encs, (gpointer)pluma_encoding_get_from_charset("ISO-8859-15"));
  encs =
      g_slist_append(encs, (gpointer)pluma_encoding_get_from_charset("UTF-16"));
