...
#define##(PLUGIN_ID.upper)_PLUGIN_GET_PRIVATE(object)(                     \
    G_TYPE_INSTANCE_GET_PRIVATE((object), TYPE_##(PLUGIN_ID.upper)_PLUGIN, \
                                ##(PLUGIN_ID.camel)PluginPrivate))

struct _##(PLUGIN_ID.camel)PluginPrivate {
  gpointer dummy;
};

PLUMA_PLUGIN_REGISTER_TYPE(##(PLUGIN_ID.camel)Plugin,
...
##endif
    ...
    static void##(PLUGIN_ID.lower)
        _plugin_init(##(PLUGIN_ID.camel)Plugin *plugin) {
...
static void##(PLUGIN_ID.lower)_plugin_finalize(GObject *object) {
...
##ifdef WITH_MENU static void free_window_data(WindowData *data) {
...
##endif
  ...
  ##ifdef WITH_MENU GtkUIManager *manager;
  WindowData *data;
  ##endif

      pluma_debug(DEBUG_PLUGINS);

  ##ifdef WITH_MENU data = g_new(WindowData, 1);
...
static void impl_deactivate(PlumaPlugin *plugin, PlumaWindow *window) {
  ##ifdef WITH_MENU GtkUIManager *manager;
  WindowData *data;
  ##endif
  ...
  ##ifdef WITH_MENU manager = pluma_window_get_ui_manager(window);
...
static void impl_update_ui(PlumaPlugin *plugin, PlumaWindow *window) {
  pluma_debug(DEBUG_PLUGINS);
}

##ifdef WITH_CONFIGURE_DIALOG static GtkWidget *impl_create_configure_dialog(
    PlumaPlugin *plugin) {
  pluma_debug(DEBUG_PLUGINS);
}
##endif
    ...
    static void##(PLUGIN_ID.lower)
        _plugin_class_init(##(PLUGIN_ID.camel)PluginClass *klass) {
  ...
  ##ifdef WITH_CONFIGURE_DIALOG plugin_class->create_configure_dialog =
