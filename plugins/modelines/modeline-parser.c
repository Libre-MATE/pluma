...
void modeline_parser_init(const gchar *data_dir) {
  if (modelines_data_dir == NULL) modelines_data_dir = g_strdup(data_dir);
}

void modeline_parser_shutdown() {
  if (vim_languages != NULL) g_hash_table_destroy(vim_languages);

  if (emacs_languages != NULL) g_hash_table_destroy(emacs_languages);

  if (kate_languages != NULL) g_hash_table_destroy(kate_languages);
...
static GHashTable *load_language_mappings_group(GKeyFile *key_file,
                                                const gchar *group) {
  GHashTable *table;
  gchar **keys;
  gsize length = 0;
  ...
  pluma_debug_message(DEBUG_PLUGINS, "%" G_GSIZE_FORMAT " mappings in group %s",
                      length, group);
...
static void load_language_mappings(void) {
  ...
  fname = g_build_filename(modelines_data_dir, MODELINES_LANGUAGE_MAPPINGS_FILE,
                           NULL);

  mappings = g_key_file_new();

  if (g_key_file_load_from_file(mappings, fname, 0, &error)) {
    pluma_debug_message(DEBUG_PLUGINS, "Loaded language mappings from %s",
                        fname);
    ...
    pluma_debug_message(DEBUG_PLUGINS,
                        "Failed to loaded language mappings from %s: %s", fname,
                        error->message);
...
static gchar *parse_vim_modeline(gchar *s, ModelineOptions *options) {
  ...
  while (*s != '\0' && !(in_set && *s == ':')) {
    while (*s != '\0' && (*s == ':' || g_ascii_isspace(*s))) s++;
    ...
    if (strcmp(key->str, "ft") == 0 || strcmp(key->str, "filetype") == 0) {
      ...
      if (intval) {
        ...
        options->set |=
            MODELINE_SET_SHOW_RIGHT_MARGIN | MODELINE_SET_RIGHT_MARGIN_POSITION;
      }
    }
  }
...
static gchar *parse_emacs_modeline(gchar *s, ModelineOptions *options) {
  ...
  while (*s != '\0') {
    ...
    pluma_debug_message(DEBUG_PLUGINS, "Emacs modeline bit: %s = %s", key->str,
                        value->str);
...
static gchar *parse_kate_modeline(gchar *s, ModelineOptions *options) {
  ...
  while (*s != '\0') {
    ...
    pluma_debug_message(DEBUG_PLUGINS, "Kate modeline bit: %s = %s", key->str,
                        value->str);
    ...
    if (strcmp(key->str, "hl") == 0 || strcmp(key->str, "syntax") == 0) {
      ...
      if (intval) {
        ...
        options->set |=
            MODELINE_SET_RIGHT_MARGIN_POSITION | MODELINE_SET_SHOW_RIGHT_MARGIN;
      }
    }
  }
...
static gboolean check_previous(GtkSourceView *view, ModelineOptions *previous,
                               ModelineSet set) {
  ...
  if (!(previous->set & set)) return FALSE;
...
void modeline_parser_apply_modeline(GtkSourceView *view) {
  ...
  while ((gtk_text_iter_get_line(&iter) < 10) && !gtk_text_iter_is_end(&iter)) {
    gchar *line;
    ...
    parse_modeline(line, 1 + gtk_text_iter_get_line(&iter), line_count,
                   &options);
  ...
  if (has_option(&options, MODELINE_SET_LANGUAGE) && options.language_id) {
    ...
    language =
        gtk_source_language_manager_get_language(manager, options.language_id);
  ...
  if (has_option(&options, MODELINE_SET_INSERT_SPACES)) {
    ...
    gtk_source_view_set_insert_spaces_instead_of_tabs(
        view, g_settings_get_boolean(settings, PLUMA_SETTINGS_INSERT_SPACES));
  }
  ...
  if (has_option(&options, MODELINE_SET_TAB_WIDTH)) {
    ...
    gtk_source_view_set_tab_width(
        view, g_settings_get_uint(settings, PLUMA_SETTINGS_TABS_SIZE));
  }
  ...
  } else if (check_previous(view, previous,
  ...
  if (previous) {
    ...
    *previous = options;
    ...
    g_object_set_data_full(G_OBJECT(buffer), MODELINE_OPTIONS_DATA_KEY,
                           previous, (GDestroyNotify)free_modeline_options);
  }
...
void modeline_parser_deactivate(GtkSourceView *view) {
  g_object_set_data(G_OBJECT(gtk_text_view_get_buffer(GTK_TEXT_VIEW(view))),
                    MODELINE_OPTIONS_DATA_KEY, NULL);
}
