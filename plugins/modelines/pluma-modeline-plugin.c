...
G_DEFINE_DYNAMIC_TYPE_EXTENDED(
    PlumaModelinePlugin, pluma_modeline_plugin, PEAS_TYPE_EXTENSION_BASE, 0,
...
static void document_data_free(DocumentData *ddata) {
  g_slice_free(DocumentData, ddata);
}

static void pluma_modeline_plugin_constructed(GObject *object) {
  gchar *data_dir;
...
static void pluma_modeline_plugin_init(PlumaModelinePlugin *plugin) {
...
static void pluma_modeline_plugin_dispose(GObject *object) {
...
static void pluma_modeline_plugin_set_property(GObject *object, guint prop_id,
                                               const GValue *value,
                                               GParamSpec *pspec) {
  PlumaModelinePlugin *plugin = PLUMA_MODELINE_PLUGIN(object);
...
static void pluma_modeline_plugin_get_property(GObject *object, guint prop_id,
                                               GValue *value,
                                               GParamSpec *pspec) {
  PlumaModelinePlugin *plugin = PLUMA_MODELINE_PLUGIN(object);
...
static void on_document_loaded_or_saved(PlumaDocument *document,
                                        const GError *error,
                                        GtkSourceView *view) {
  modeline_parser_apply_modeline(view);
}

static void connect_handlers(PlumaView *view) {
  DocumentData *data;
  GtkTextBuffer *doc;
  ...
  data->document_loaded_handler_id = g_signal_connect(
      doc, "loaded", G_CALLBACK(on_document_loaded_or_saved), view);
  data->document_saved_handler_id = g_signal_connect(
      doc, "saved", G_CALLBACK(on_document_loaded_or_saved), view);

  g_object_set_data_full(G_OBJECT(doc), DOCUMENT_DATA_KEY, data,
                         (GDestroyNotify)document_data_free);
}

static void disconnect_handlers(PlumaView *view) {
  DocumentData *data;
  GtkTextBuffer *doc;
...
static void on_window_tab_added(PlumaWindow *window, PlumaTab *tab,
                                gpointer user_data) {
  connect_handlers(pluma_tab_get_view(tab));
}

static void on_window_tab_removed(PlumaWindow *window, PlumaTab *tab,
                                  gpointer user_data) {
  disconnect_handlers(pluma_tab_get_view(tab));
}

static void pluma_modeline_plugin_activate(
  ...
  data->tab_added_handler_id = g_signal_connect(
      window, "tab-added", G_CALLBACK(on_window_tab_added), NULL);

  data->tab_removed_handler_id = g_signal_connect(
      window, "tab-removed", G_CALLBACK(on_window_tab_removed), NULL);
}

static void pluma_modeline_plugin_deactivate(
    PlumaWindowActivatable *activatable) {
  PlumaModelinePluginPrivate *data;
  PlumaWindow *window;
  GList *views;
...
static void pluma_modeline_plugin_class_init(PlumaModelinePluginClass *klass) {
...
static void pluma_modeline_plugin_class_finalize(
    PlumaModelinePluginClass *klass) {
...
G_MODULE_EXPORT void peas_register_types(PeasObjectModule *module) {
  ...
  peas_object_module_register_extension_type(
      module, PLUMA_TYPE_WINDOW_ACTIVATABLE, PLUMA_TYPE_MODELINE_PLUGIN);
}
