...
enum { COLUMN_TAG_NAME, COLUMN_TAG_INDEX_IN_GROUP, NUM_COLUMNS };
...
G_DEFINE_DYNAMIC_TYPE_EXTENDED(PlumaTaglistPluginPanel,
                               pluma_taglist_plugin_panel, GTK_TYPE_BOX, 0,
...
static void set_window(PlumaTaglistPluginPanel *panel, PlumaWindow *window) {
...
static void pluma_taglist_plugin_panel_set_property(GObject *object,
                                                    guint prop_id,
                                                    const GValue *value,
                                                    GParamSpec *pspec) {
...
static void pluma_taglist_plugin_panel_get_property(GObject *object,
                                                    guint prop_id,
                                                    GValue *value,
                                                    GParamSpec *pspec) {
  PlumaTaglistPluginPanel *panel = PLUMA_TAGLIST_PLUGIN_PANEL(object);
...
static void pluma_taglist_plugin_panel_finalize(GObject *object) {
  PlumaTaglistPluginPanel *panel = PLUMA_TAGLIST_PLUGIN_PANEL(object);
...
static void pluma_taglist_plugin_panel_class_init(
    PlumaTaglistPluginPanelClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object(
          ...
          PLUMA_TYPE_WINDOW, G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
}

static void pluma_taglist_plugin_panel_class_finalize(
    PlumaTaglistPluginPanelClass *klass) {
  /* dummy function - used by G_DEFINE_DYNAMIC_TYPE_EXTENDED */
}

static void insert_tag(PlumaTaglistPluginPanel *panel, Tag *tag,
                       gboolean grab_focus) {
  PlumaView *view;
  ...
  if (tag->begin != NULL) {
    sel = gtk_text_buffer_get_selection_bounds(buffer, &start, &end);

    gtk_text_buffer_insert(buffer, &start, (gchar *)tag->begin, -1);

    /* get iterators again since they have been invalidated and move
     * the cursor after the selection */
    gtk_text_buffer_get_selection_bounds(buffer, &start, &cursor);
  }

  if (tag->end != NULL) {
    sel = gtk_text_buffer_get_selection_bounds(buffer, &start, &end);

    gtk_text_buffer_insert(buffer, &end, (gchar *)tag->end, -1);
    ...
    if (!sel) {
      ...
      offset =
          gtk_text_iter_get_offset(&end) - g_utf8_strlen((gchar *)tag->end, -1);
  ...
  if (grab_focus) gtk_widget_grab_focus(GTK_WIDGET(view));
}
...
static gboolean tag_list_key_press_event_cb(GtkTreeView *tag_list,
                                            GdkEventKey *event,
                                            PlumaTaglistPluginPanel *panel) {
  gboolean grab_focus;
...
static GtkTreeModel *create_model(PlumaTaglistPluginPanel *panel) {
  gint i = 0;
  GtkListStore *store;
  GtkTreeIter iter;
  GList *list;
  ...
  while (list != NULL) {
    ...
    gtk_list_store_set(store, &iter, COLUMN_TAG_NAME, tag_name,
                       COLUMN_TAG_INDEX_IN_GROUP, i, -1);
    ++i;
  ...
  pluma_debug_message(
      DEBUG_PLUGINS, "Rows: %d ",
      gtk_tree_model_iter_n_children(GTK_TREE_MODEL(store), NULL));

  return GTK_TREE_MODEL(store);
}

static void populate_tags_list(PlumaTaglistPluginPanel *panel) {
  GtkTreeModel *model;
  ...
  gtk_tree_view_set_model(GTK_TREE_VIEW(panel->priv->tags_list), model);
...
static void populate_tag_groups_combo(PlumaTaglistPluginPanel *panel) {
  GList *l;
  GtkComboBox *combo;
...
static void selected_group_changed(GtkComboBox *combo,
                                   PlumaTaglistPluginPanel *panel) {
  gchar *group_name;
  ...
  if ((panel->priv->selected_tag_group == NULL) ||
      (strcmp(group_name, (gchar *)panel->priv->selected_tag_group->name) !=
       0)) {
    panel->priv->selected_tag_group = find_tag_group(group_name);
    g_return_if_fail(panel->priv->selected_tag_group != NULL);

    pluma_debug_message(DEBUG_PLUGINS, "New selected group: %s",
...
static gchar *create_preview_string(Tag *tag) {
  GString *str;
...
static void update_preview(PlumaTaglistPluginPanel *panel, Tag *tag) {
  gchar *str;

  str = create_preview_string(tag);

  gtk_label_set_markup(GTK_LABEL(panel->priv->preview), str);

  g_free(str);
}

static void tag_list_cursor_changed_cb(GtkTreeView *tag_list, gpointer data) {
  GtkTreeModel *model;
  GtkTreeSelection *selection;
  GtkTreeIter iter;
  gint index;
  ...
  if (gtk_tree_selection_get_selected(selection, NULL, &iter)) {
    ...
    update_preview(panel, (Tag *)g_list_nth_data(
                              panel->priv->selected_tag_group->tags, index));
  }
}

static gboolean tags_list_query_tooltip_cb(GtkWidget *widget, gint x, gint y,
...
static gboolean draw_event_cb(GtkWidget *panel, cairo_t *cr,
                              gpointer user_data) {
  ...
  if (taglist == NULL) create_taglist(ppanel->priv->data_dir);
...
static void realize_tag_groups_combo(GtkWidget *combo, gpointer data) {
  gtk_container_forall(GTK_CONTAINER(combo), set_combo_tooltip, NULL);
}

static void add_preview_widget(PlumaTaglistPluginPanel *panel) {
  GtkWidget *expander;
  GtkWidget *frame;
  ...
  gtk_label_set_selectable(GTK_LABEL(panel->priv->preview), TRUE);
  gtk_label_set_ellipsize(GTK_LABEL(panel->priv->preview), PANGO_ELLIPSIZE_END);

  frame = gtk_frame_new(0);
  gtk_frame_set_shadow_type(GTK_FRAME(frame), GTK_SHADOW_IN);

  gtk_container_add(GTK_CONTAINER(frame), panel->priv->preview);
...
static void pluma_taglist_plugin_panel_init(PlumaTaglistPluginPanel *panel) {
  GtkWidget *sw;
  GtkTreeViewColumn *column;
  ...
  gtk_box_pack_start(GTK_BOX(panel), panel->priv->tag_groups_combo, FALSE, TRUE,
                     0);

  g_signal_connect(panel->priv->tag_groups_combo, "realize",
                   G_CALLBACK(realize_tag_groups_combo), panel);
  ...
  pluma_utils_set_atk_name_description(panel->priv->tags_list, _("Tags"), NULL);
  ...
  gtk_tree_view_set_headers_visible(GTK_TREE_VIEW(panel->priv->tags_list),
                                    FALSE);
  ...
  column = gtk_tree_view_column_new_with_attributes(_("Tags"), cell, "text",
                                                    COLUMN_TAG_NAME, NULL);
  gtk_tree_view_append_column(GTK_TREE_VIEW(panel->priv->tags_list), column);
  ...
  gtk_container_set_focus_chain(GTK_CONTAINER(panel), focus_chain);
  ...
  g_signal_connect_after(panel->priv->tags_list, "row_activated",
                         G_CALLBACK(tag_list_row_activated_cb), panel);
  g_signal_connect(panel->priv->tags_list, "key_press_event",
                   G_CALLBACK(tag_list_key_press_event_cb), panel);
  g_signal_connect(panel->priv->tags_list, "query-tooltip",
                   G_CALLBACK(tags_list_query_tooltip_cb), panel);
  g_signal_connect(panel->priv->tags_list, "cursor_changed",
                   G_CALLBACK(tag_list_cursor_changed_cb), panel);
  g_signal_connect(panel->priv->tag_groups_combo, "changed",
                   G_CALLBACK(selected_group_changed), panel);
  g_signal_connect(panel, "draw", G_CALLBACK(draw_event_cb), NULL);
}
...
GtkWidget *pluma_taglist_plugin_panel_new(PlumaWindow *window,
  ...
  panel = g_object_new(PLUMA_TYPE_TAGLIST_PLUGIN_PANEL, "window", window, NULL);
...
void _pluma_taglist_plugin_panel_register_type(GTypeModule *type_module) {
