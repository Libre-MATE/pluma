...
/* Signals */
enum { URI_ACTIVATED, ERROR, CONFIRM_DELETE, CONFIRM_NO_TRASH, NUM_SIGNALS };
...
static void set_enable_delete(PlumaFileBrowserWidget *obj, gboolean enable);
static void on_model_set(GObject *gobject, GParamSpec *arg1,
                         PlumaFileBrowserWidget *obj);
static void on_treeview_error(PlumaFileBrowserView *tree_view, guint code,
                              gchar *message, PlumaFileBrowserWidget *obj);
static void on_file_store_error(PlumaFileBrowserStore *store, guint code,
                                gchar *message, PlumaFileBrowserWidget *obj);
static gboolean on_file_store_no_trash(PlumaFileBrowserStore *store,
                                       GList *files,
                                       PlumaFileBrowserWidget *obj);
static void on_combo_changed(GtkComboBox *combo, PlumaFileBrowserWidget *obj);
...
static void on_location_jump_activate(GtkMenuItem *item,
                                      PlumaFileBrowserWidget *obj);
static void on_bookmarks_row_changed(GtkTreeModel *model, GtkTreePath *path,
                                     GtkTreeIter *iter,
                                     PlumaFileBrowserWidget *obj);
static void on_bookmarks_row_deleted(GtkTreeModel *model, GtkTreePath *path,
                                     PlumaFileBrowserWidget *obj);
...
static void on_action_directory_new(GtkAction *action,
                                    PlumaFileBrowserWidget *obj);
static void on_action_file_open(GtkAction *action, PlumaFileBrowserWidget *obj);
static void on_action_file_new(GtkAction *action, PlumaFileBrowserWidget *obj);
...
G_DEFINE_DYNAMIC_TYPE_EXTENDED(PlumaFileBrowserWidget,
                               pluma_file_browser_widget, GTK_TYPE_BOX, 0,
...
static void free_name_icon(gpointer data) {
  ...
  if (item->icon) g_object_unref(item->icon);

  g_free(item);
}

static FilterFunc *filter_func_new(PlumaFileBrowserWidget *obj,
                                   PlumaFileBrowserWidgetFilterFunc func,
...
static void location_free(Location *loc) {
  if (loc->root) g_object_unref(loc->root);

  if (loc->virtual_root) g_object_unref(loc->virtual_root);

  g_free(loc);
}

static gboolean combo_find_by_id(PlumaFileBrowserWidget *obj, guint id,
                                 GtkTreeIter *iter) {
  guint checkid;
...
static void remove_path_items(PlumaFileBrowserWidget *obj) {
  GtkTreeIter iter;
...
static void cancel_async_operation(PlumaFileBrowserWidget *widget) {
...
static void pluma_file_browser_widget_finalize(GObject *object) {
  PlumaFileBrowserWidget *obj = PLUMA_FILE_BROWSER_WIDGET(object);
  GList *loc;

  remove_path_items(obj);
  pluma_file_browser_store_set_filter_func(obj->priv->file_store, NULL, NULL);
...
static void pluma_file_browser_widget_get_property(GObject *object,
...
static void pluma_file_browser_widget_set_property(GObject *object,
                                                   guint prop_id,
                                                   const GValue *value,
                                                   GParamSpec *pspec) {
  PlumaFileBrowserWidget *obj = PLUMA_FILE_BROWSER_WIDGET(object);
...
static void pluma_file_browser_widget_class_init(
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string("filter-pattern", "Filter Pattern",
                          "The filter pattern", NULL, G_PARAM_READWRITE));
  g_object_class_install_property(
      object_class, PROP_ENABLE_DELETE,
      g_param_spec_boolean("enable-delete", "Enable delete",
                           "Enable permanently deleting items", TRUE,
                           G_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  signals[URI_ACTIVATED] = g_signal_new(
      "uri-activated", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PlumaFileBrowserWidgetClass, uri_activated), NULL, NULL,
      NULL, G_TYPE_NONE, 1, G_TYPE_STRING);
  signals[ERROR] = g_signal_new(
      "error", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PlumaFileBrowserWidgetClass, error), NULL, NULL, NULL,
  ...
  signals[CONFIRM_NO_TRASH] = g_signal_new(
      "confirm-no-trash", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PlumaFileBrowserWidgetClass, confirm_no_trash),
      g_signal_accumulator_true_handled, NULL, NULL, G_TYPE_BOOLEAN, 1,
      G_TYPE_POINTER);
}

static void pluma_file_browser_widget_class_finalize(
    PlumaFileBrowserWidgetClass *klass) {
  /* dummy function - used by G_DEFINE_DYNAMIC_TYPE_EXTENDED */
}

static void add_signal(PlumaFileBrowserWidget *obj, gpointer object,
                       gulong id) {
...
static void clear_signals(PlumaFileBrowserWidget *obj) {
  GSList *item;
  SignalNode *node;
...
static gboolean get_from_bookmark_file(PlumaFileBrowserWidget *obj, GFile *file,
...
static void insert_path_item(PlumaFileBrowserWidget *obj, GFile *file,
                             GtkTreeIter *after, GtkTreeIter *iter,
                             guint indent) {
  gchar *unescape;
  GdkPixbuf *icon = NULL;
  ...
  gtk_tree_store_set(obj->priv->combo_model, iter, COLUMN_INDENT, indent,
                     COLUMN_ICON, icon, COLUMN_NAME, unescape, COLUMN_FILE,
                     file, COLUMN_ID, PATH_ID, -1);
...
static void insert_separator_item(PlumaFileBrowserWidget *obj) {
  GtkTreeIter iter;

  gtk_tree_store_insert(obj->priv->combo_model, &iter, NULL, 1);
  gtk_tree_store_set(obj->priv->combo_model, &iter, COLUMN_ICON, NULL,
                     COLUMN_NAME, NULL, COLUMN_ID, SEPARATOR_ID, -1);
}

static void combo_set_active_by_id(PlumaFileBrowserWidget *obj, guint id) {
  GtkTreeIter iter;
...
static guint uri_num_parents(GFile *from, GFile *to) {
  ...
  while ((parent = g_file_get_parent(from)) &&
         !(to && g_file_equal(from, to))) {
    g_object_unref(from);
    from = parent;
...
static void insert_location_path(PlumaFileBrowserWidget *obj) {
  Location *loc;
  GFile *current = NULL;
  GFile *tmp;
  ...
  while (current != NULL) {
    ...
    if (current == loc->virtual_root) {
      ...
      gtk_combo_box_set_active_iter(GTK_COMBO_BOX(obj->priv->combo), &iter);
      g_signal_handlers_unblock_by_func(obj->priv->combo, on_combo_changed,
                                        obj);
    }

    if (g_file_equal(current, loc->root) ||
        !pluma_utils_file_has_parent(current)) {
      if (current != loc->virtual_root) g_object_unref(current);
      break;
    }
...
static void check_current_item(PlumaFileBrowserWidget *obj,
                               gboolean show_path) {
  GtkTreeIter separator;
  gboolean has_sep;
  ...
  if (show_path) {
    if (!has_sep) insert_separator_item(obj);
...
static void fill_combo_model(PlumaFileBrowserWidget *obj) {
  GtkTreeStore *store = obj->priv->combo_model;
  GtkTreeIter iter;
  GdkPixbuf *icon;

  icon =
      pluma_file_browser_utils_pixbuf_from_theme("go-home", GTK_ICON_SIZE_MENU);

  gtk_tree_store_append(store, &iter, NULL);
  gtk_tree_store_set(store, &iter, COLUMN_ICON, icon, COLUMN_NAME,
                     _("Bookmarks"), COLUMN_ID, BOOKMARKS_ID, -1);
  g_object_unref(icon);
...
static void indent_cell_data_func(GtkCellLayout *cell_layout,
...
static void create_combo(PlumaFileBrowserWidget *obj) {
  GtkCellRenderer *renderer;

  obj->priv->combo_model =
      gtk_tree_store_new(N_COLUMNS, G_TYPE_UINT, GDK_TYPE_PIXBUF, G_TYPE_STRING,
                         G_TYPE_FILE, G_TYPE_UINT);
  obj->priv->combo =
      gtk_combo_box_new_with_model(GTK_TREE_MODEL(obj->priv->combo_model));
  ...
  gtk_cell_layout_set_cell_data_func(GTK_CELL_LAYOUT(obj->priv->combo),
                                     renderer, indent_cell_data_func, obj,
                                     NULL);

  renderer = gtk_cell_renderer_pixbuf_new();
  gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(obj->priv->combo), renderer,
                             FALSE);
  gtk_cell_layout_add_attribute(GTK_CELL_LAYOUT(obj->priv->combo), renderer,
                                "pixbuf", COLUMN_ICON);
  ...
  g_object_set(renderer, "ellipsize-set", TRUE, "ellipsize",
               PANGO_ELLIPSIZE_END, NULL);

  gtk_box_pack_start(GTK_BOX(obj), GTK_WIDGET(obj->priv->combo), FALSE, FALSE,
                     0);

  fill_combo_model(obj);
  g_signal_connect(obj->priv->combo, "changed", G_CALLBACK(on_combo_changed),
                   obj);
...
static const GtkActionEntry tree_actions_file_selection[] = {
    {"FileOpen", "document-open", N_("_Open"), NULL, N_("Open selected file"),
     G_CALLBACK(on_action_file_open)}};

static const GtkActionEntry tree_actions[] = {
    {"DirectoryUp", "go-up", N_("Up"), NULL, N_("Open the parent folder"),
     G_CALLBACK(on_action_directory_up)}};
...
static const GtkActionEntry tree_actions_single_most_selection[] = {
    ...
    {"FileNew", "document-new", N_("New F_ile"), NULL, N_("Add new empty file"),
...
static const GtkActionEntry tree_actions_sensitive[] = {
    {"DirectoryPrevious", "go-previous", N_("_Previous Location"), NULL,
    ...
    {"DirectoryNext", "go-next", N_("_Next Location"), NULL,
     ...
     G_CALLBACK(on_action_directory_next)},
...
static const GtkToggleActionEntry tree_actions_toggle[] = {
    {"FilterHidden", "dialog-password", N_("Show _Hidden"), NULL,
     N_("Show hidden files and folders"), G_CALLBACK(on_action_filter_hidden),
     FALSE},
    {"FilterBinary", NULL, N_("Show _Binary"), NULL, N_("Show binary files"),
     G_CALLBACK(on_action_filter_binary), FALSE}};
...
static void create_toolbar(PlumaFileBrowserWidget *obj, const gchar *data_dir) {
  GtkUIManager *manager;
  GError *error = NULL;
  GtkActionGroup *action_group;
  GtkWidget *toolbar;
  ...
  if (error != NULL) {
    g_warning("Error in adding ui from file %s: %s", XML_UI_FILE,
              error->message);
    g_error_free(error);
    return;
  }
  ...
  gtk_action_group_add_actions(action_group, toplevel_actions,
                               G_N_ELEMENTS(toplevel_actions), obj);
  ...
  gtk_action_group_add_toggle_actions(action_group, tree_actions_toggle,
                                      G_N_ELEMENTS(tree_actions_toggle), obj);
  gtk_ui_manager_insert_action_group(manager, action_group, 0);
  ...
  gtk_action_group_add_actions(action_group, tree_actions_selection,
                               G_N_ELEMENTS(tree_actions_selection), obj);
  gtk_ui_manager_insert_action_group(manager, action_group, 0);
  obj->priv->action_group_selection = action_group;

  action_group =
      gtk_action_group_new("FileBrowserWidgetFileSelectionActionGroup");
  gtk_action_group_set_translation_domain(action_group, NULL);
  gtk_action_group_add_actions(action_group, tree_actions_file_selection,
                               G_N_ELEMENTS(tree_actions_file_selection), obj);
  gtk_ui_manager_insert_action_group(manager, action_group, 0);
  ...
  gtk_action_group_add_actions(action_group, tree_actions_sensitive,
                               G_N_ELEMENTS(tree_actions_sensitive), obj);
  gtk_ui_manager_insert_action_group(manager, action_group, 0);
  obj->priv->action_group_sensitive = action_group;

  action_group = gtk_action_group_new("FileBrowserWidgetBookmarkActionGroup");
  gtk_action_group_set_translation_domain(action_group, NULL);
  gtk_action_group_add_actions(action_group, bookmark_actions,
                               G_N_ELEMENTS(bookmark_actions), obj);
  gtk_ui_manager_insert_action_group(manager, action_group, 0);
  ...
  gtk_menu_set_reserve_toggle_size(GTK_MENU(obj->priv->location_previous_menu),
                                   FALSE);

  gtk_widget_show(obj->priv->location_previous_menu);

  widget = GTK_WIDGET(gtk_menu_tool_button_new(
  ...
  gtk_menu_tool_button_set_arrow_tooltip_text(
      GTK_MENU_TOOL_BUTTON(widget), _("Go to a previously opened location"));
  ...
  gtk_menu_set_reserve_toggle_size(GTK_MENU(obj->priv->location_next_menu),
                                   FALSE);
...
static void set_enable_delete(PlumaFileBrowserWidget *obj, gboolean enable) {
  ...
  if (obj->priv->action_group_selection == NULL) return;
...
static gboolean filter_real(PlumaFileBrowserStore *model, GtkTreeIter *iter,
                            PlumaFileBrowserWidget *obj) {
  GSList *item;
  FilterFunc *func;
  ...
  for (item = obj->priv->filter_funcs; item; item = item->next) {
    ...
    if (!func->func(obj, model, iter, func->user_data)) return FALSE;
  }

  return TRUE;
}

static void add_bookmark_hash(PlumaFileBrowserWidget *obj, GtkTreeIter *iter) {
  GtkTreeModel *model;
  GdkPixbuf *pixbuf;
  gchar *name;
  gchar *uri;
  ...
  gtk_tree_model_get(model, iter, PLUMA_FILE_BOOKMARKS_STORE_COLUMN_ICON,
                     &pixbuf, PLUMA_FILE_BOOKMARKS_STORE_COLUMN_NAME, &name,
                     -1);
  ...
  g_hash_table_insert(obj->priv->bookmarks_hash, file, item);
}

static void init_bookmarks_hash(PlumaFileBrowserWidget *obj) {
  GtkTreeIter iter;
  GtkTreeModel *model;
...
static void on_begin_loading(PlumaFileBrowserStore *model, GtkTreeIter *iter,
                             PlumaFileBrowserWidget *obj) {
...
static void on_end_loading(PlumaFileBrowserStore *model, GtkTreeIter *iter,
                           PlumaFileBrowserWidget *obj) {
  if (!GDK_IS_WINDOW(gtk_widget_get_window(GTK_WIDGET(obj->priv->treeview))))
    return;
...
static void create_tree(PlumaFileBrowserWidget *obj) {
  GtkWidget *sw;

  obj->priv->file_store = pluma_file_browser_store_new(NULL);
  obj->priv->bookmarks_store = pluma_file_bookmarks_store_new();
  obj->priv->treeview = PLUMA_FILE_BROWSER_VIEW(pluma_file_browser_view_new());
  ...
  pluma_file_browser_store_set_filter_func(
      obj->priv->file_store, (PlumaFileBrowserStoreFilterFunc)filter_real, obj);
  ...
  g_signal_connect(obj->priv->treeview, "error", G_CALLBACK(on_treeview_error),
                   obj);
...
static void create_filter(PlumaFileBrowserWidget *obj) {
  GtkWidget *expander;
  GtkWidget *vbox;
  GtkWidget *entry;
...
static void pluma_file_browser_widget_init(PlumaFileBrowserWidget *obj) {
  GdkDisplay *display;
...
static void update_sensitivity(PlumaFileBrowserWidget *obj) {
  GtkTreeModel *model =
  ...
  if (PLUMA_IS_FILE_BROWSER_STORE(model)) {
    ...
    action =
        gtk_action_group_get_action(obj->priv->action_group, "FilterHidden");
...
static gboolean pluma_file_browser_widget_get_first_selected(
    PlumaFileBrowserWidget *obj, GtkTreeIter *iter) {
...
static gboolean popup_menu(PlumaFileBrowserWidget *obj, GdkEventButton *event,
                           GtkTreeModel *model) {
  GtkWidget *menu;
...
static gboolean filter_glob(PlumaFileBrowserWidget *obj,
                            PlumaFileBrowserStore *store, GtkTreeIter *iter,
                            gpointer user_data) {
  gchar *name;
  gboolean result;
  guint flags;

  if (obj->priv->filter_pattern == NULL) return TRUE;

  gtk_tree_model_get(GTK_TREE_MODEL(store), iter,
                     PLUMA_FILE_BROWSER_STORE_COLUMN_NAME, &name,
                     PLUMA_FILE_BROWSER_STORE_COLUMN_FLAGS, &flags, -1);
...
static void rename_selected_file(PlumaFileBrowserWidget *obj) {
  GtkTreeModel *model;
  GtkTreeIter iter;

  model = gtk_tree_view_get_model(GTK_TREE_VIEW(obj->priv->treeview));

  if (!PLUMA_IS_FILE_BROWSER_STORE(model)) return;

  if (pluma_file_browser_widget_get_first_selected(obj, &iter))
    pluma_file_browser_view_start_rename(obj->priv->treeview, &iter);
}

static GList *get_deletable_files(PlumaFileBrowserWidget *obj) {
  GtkTreeSelection *selection;
  GtkTreeModel *model;
  GList *rows;
  GList *row;
  ...
  for (row = rows; row; row = row->next) {
    ...
    gtk_tree_model_get(model, &iter, PLUMA_FILE_BROWSER_STORE_COLUMN_FLAGS,
                       &flags, -1);

    if (FILE_IS_DUMMY(flags)) continue;
...
static gboolean delete_selected_files(PlumaFileBrowserWidget *obj,
                                      gboolean trash) {
  GtkTreeModel *model;
  gboolean confirm;
  ...
  if (!PLUMA_IS_FILE_BROWSER_STORE(model)) return FALSE;
...
static GtkWidget *create_goto_menu_item(PlumaFileBrowserWidget *obj,
                                        GList *item, GdkPixbuf *icon_pixbuf) {
  GtkWidget *menu_item;
  gchar *label_text;
  ...
  if (!get_from_bookmark_file(obj, loc->virtual_root, &label_text, &pixbuf)) {
    ...
    if (icon_pixbuf) pixbuf = g_object_ref(icon_pixbuf);
  }
  ...
  g_signal_connect(menu_item, "activate", G_CALLBACK(on_location_jump_activate),
                   obj);
...
static void jump_to_location(PlumaFileBrowserWidget *obj, GList *item,
                             gboolean previous) {
  Location *loc;
  GtkWidget *widget;
  GList *children;
  ...
  while (obj->priv->current_location != item) {
    ...
    if (obj->priv->current_location == NULL) {
      ...
      if (obj->priv->current_location == item) break;
    } else {
  ...
  pluma_file_browser_widget_set_root_and_virtual_root(obj, root, virtual_root);
...
static void clear_next_locations(PlumaFileBrowserWidget *obj) {
  GList *children;
  GList *item;

  if (obj->priv->current_location == NULL) return;
  ...
  children =
      gtk_container_get_children(GTK_CONTAINER(obj->priv->location_next_menu));

  for (item = children; item; item = item->next) {
    gtk_container_remove(GTK_CONTAINER(obj->priv->location_next_menu),
...
static void update_filter_mode(PlumaFileBrowserWidget *obj, GtkAction *action,
  ...
  if (PLUMA_IS_FILE_BROWSER_STORE(model)) {
    now = pluma_file_browser_store_get_filter_mode(
        PLUMA_FILE_BROWSER_STORE(model));
    ...
    pluma_file_browser_store_set_filter_mode(PLUMA_FILE_BROWSER_STORE(model),
                                             now);
  }
}

static void set_filter_pattern_real(PlumaFileBrowserWidget *obj,
                                    gchar const *pattern,
  ...
  if (pattern != NULL && *pattern == '\0') pattern = NULL;

  if (pattern == NULL && obj->priv->filter_pattern_str == NULL) return;
  ...
  if (pattern == NULL) {
    if (obj->priv->glob_filter_id != 0) {
      pluma_file_browser_widget_remove_filter(obj, obj->priv->glob_filter_id);
    ...
    if (obj->priv->glob_filter_id == 0)
      obj->priv->glob_filter_id =
          pluma_file_browser_widget_add_filter(obj, filter_glob, NULL, NULL);
  }
  ...
  if (PLUMA_IS_FILE_BROWSER_STORE(model))
    pluma_file_browser_store_refilter(PLUMA_FILE_BROWSER_STORE(model));
...
static void show_files_real(PlumaFileBrowserWidget *obj,
...
void pluma_file_browser_widget_set_root_and_virtual_root(
    PlumaFileBrowserWidget *obj, gchar const *root, gchar const *virtual_root) {
  PlumaFileBrowserStoreResult result;

  if (!virtual_root)
    result = pluma_file_browser_store_set_root_and_virtual_root(
...
void pluma_file_browser_widget_set_root(PlumaFileBrowserWidget *obj,
  ...
  if (!virtual_root) {
    pluma_file_browser_widget_set_root_and_virtual_root(obj, root, NULL);
    return;
  }
  ...
  pluma_file_browser_widget_set_root_and_virtual_root(obj, str, root);
...
PlumaFileBrowserStore *pluma_file_browser_widget_get_browser_store(
    PlumaFileBrowserWidget *obj) {
  return obj->priv->file_store;
}

PlumaFileBookmarksStore *pluma_file_browser_widget_get_bookmarks_store(
    PlumaFileBrowserWidget *obj) {
  return obj->priv->bookmarks_store;
}

PlumaFileBrowserView *pluma_file_browser_widget_get_browser_view(
    PlumaFileBrowserWidget *obj) {
  return obj->priv->treeview;
}

GtkUIManager *pluma_file_browser_widget_get_ui_manager(
    PlumaFileBrowserWidget *obj) {
  return obj->priv->manager;
}

GtkWidget *pluma_file_browser_widget_get_filter_entry(
    PlumaFileBrowserWidget *obj) {
  return obj->priv->filter_entry;
}

gulong pluma_file_browser_widget_add_filter(
    PlumaFileBrowserWidget *obj, PlumaFileBrowserWidgetFilterFunc func,
    gpointer user_data, GDestroyNotify notify) {
  FilterFunc *f;
  GtkTreeModel *model =
      gtk_tree_view_get_model(GTK_TREE_VIEW(obj->priv->treeview));
...
void pluma_file_browser_widget_remove_filter(PlumaFileBrowserWidget *obj,
  ...
  for (item = obj->priv->filter_funcs; item; item = item->next) {
    ...
    if (func->id == id) {
      if (func->destroy_notify) func->destroy_notify(func->user_data);
...
gboolean pluma_file_browser_widget_get_selected_directory(
    PlumaFileBrowserWidget *obj, GtkTreeIter *iter) {
  GtkTreeModel *model =
      gtk_tree_view_get_model(GTK_TREE_VIEW(obj->priv->treeview));
  GtkTreeIter parent;
  ...
  gtk_tree_model_get(model, iter, PLUMA_FILE_BROWSER_STORE_COLUMN_FLAGS, &flags,
                     -1);
...
static guint pluma_file_browser_widget_get_num_selected_files_or_directories(
    PlumaFileBrowserWidget *obj, guint *files, guint *dirs) {
  GList *rows, *row;
  GtkTreePath *path;
  GtkTreeIter iter;
...
static AsyncData *async_data_new(PlumaFileBrowserWidget *widget) {
  AsyncData *ret;
...
static void set_busy(PlumaFileBrowserWidget *obj, gboolean busy) {
  GdkWindow *window;
...
static void activate_mount(PlumaFileBrowserWidget *widget, GVolume *volume,
                           GMount *mount) {
  GFile *root;
  gchar *uri;
  ...
  if (!mount) {
    ...
    g_signal_emit(widget, signals[ERROR], 0, PLUMA_FILE_BROWSER_ERROR_SET_ROOT,
                  message);
...
static void try_activate_drive(PlumaFileBrowserWidget *widget, GDrive *drive) {
  GList *volumes;
  GVolume *volume;
  GMount *mount;
...
static void poll_for_media_cb(GDrive *drive, GAsyncResult *res,
                              AsyncData *async) {
  GError *error = NULL;
  ...
  if (g_drive_poll_for_media_finish(drive, res, &error) &&
    ...
    g_signal_emit(async->widget, signals[ERROR], 0,
                  PLUMA_FILE_BROWSER_ERROR_SET_ROOT, message);
...
static void mount_volume_cb(GVolume *volume, GAsyncResult *res,
                            AsyncData *async) {
  GError *error = NULL;
...
static void activate_drive(PlumaFileBrowserWidget *obj, GtkTreeIter *iter) {
  GDrive *drive;
  AsyncData *async;

  gtk_tree_model_get(GTK_TREE_MODEL(obj->priv->bookmarks_store), iter,
                     PLUMA_FILE_BOOKMARKS_STORE_COLUMN_OBJECT, &drive, -1);

  /* most common use case is a floppy drive, we'll poll for media and
     go from there */
  async = async_data_new(obj);
  g_drive_poll_for_media(drive, async->cancellable,
                         (GAsyncReadyCallback)poll_for_media_cb, async);
...
static void try_mount_volume(PlumaFileBrowserWidget *widget, GVolume *volume) {
  ...
  operation = gtk_mount_operation_new(
      GTK_WINDOW(gtk_widget_get_toplevel(GTK_WIDGET(widget))));
  async = async_data_new(widget);

  g_volume_mount(volume, G_MOUNT_MOUNT_NONE, operation, async->cancellable,
...
static void activate_volume(PlumaFileBrowserWidget *obj, GtkTreeIter *iter) {
  GVolume *volume;

  gtk_tree_model_get(GTK_TREE_MODEL(obj->priv->bookmarks_store), iter,
                     PLUMA_FILE_BOOKMARKS_STORE_COLUMN_OBJECT, &volume, -1);
...
void pluma_file_browser_widget_refresh(PlumaFileBrowserWidget *obj) {
  ...
  if (PLUMA_IS_FILE_BROWSER_STORE(model))
    ...
    pluma_file_bookmarks_store_refresh(PLUMA_FILE_BOOKMARKS_STORE(model));
  }
}
...
static void bookmark_open(PlumaFileBrowserWidget *obj, GtkTreeModel *model,
                          GtkTreeIter *iter) {
  gchar *uri;
  gint flags;

  gtk_tree_model_get(model, iter, PLUMA_FILE_BOOKMARKS_STORE_COLUMN_FLAGS,
                     &flags, -1);
  ...
  uri = pluma_file_bookmarks_store_get_uri(PLUMA_FILE_BOOKMARKS_STORE(model),
                                           iter);
  ...
  if (uri) {
    ...
    if ((flags & PLUMA_FILE_BOOKMARKS_STORE_IS_MOUNT) ||
      ...
      pluma_file_browser_widget_set_root(obj, uri, FALSE);
    } else {
      pluma_file_browser_widget_set_root(obj, uri, TRUE);
    }
  } else {
    g_warning("No uri!");
  }
...
static void file_open(PlumaFileBrowserWidget *obj, GtkTreeModel *model,
                      GtkTreeIter *iter) {
  gchar *uri;
  gint flags;

  gtk_tree_model_get(model, iter, PLUMA_FILE_BROWSER_STORE_COLUMN_FLAGS, &flags,
                     PLUMA_FILE_BROWSER_STORE_COLUMN_URI, &uri, -1);
...
static gboolean directory_open(PlumaFileBrowserWidget *obj, GtkTreeModel *model,
                               GtkTreeIter *iter) {
  ...
  if (FILE_IS_DIR(flags)) {
    ...
    if (!gtk_show_uri_on_window(NULL, uri, GDK_CURRENT_TIME, &error)) {
      g_signal_emit(obj, signals[ERROR], 0,
                    PLUMA_FILE_BROWSER_ERROR_OPEN_DIRECTORY, error->message);
...
static void on_bookmark_activated(PlumaFileBrowserView *tree_view,
                                  GtkTreeIter *iter,
                                  PlumaFileBrowserWidget *obj) {
...
static void on_file_activated(PlumaFileBrowserView *tree_view,
                              GtkTreeIter *iter, PlumaFileBrowserWidget *obj) {
  GtkTreeModel *model = gtk_tree_view_get_model(GTK_TREE_VIEW(tree_view));

  file_open(obj, model, iter);
}

static gboolean virtual_root_is_root(PlumaFileBrowserWidget *obj,
                                     PlumaFileBrowserStore *model) {
  GtkTreeIter root;
  GtkTreeIter virtual_root;

  if (!pluma_file_browser_store_get_iter_root(model, &root)) return TRUE;
  ...
  if (pluma_file_browser_store_get_iter_virtual_root(model, &iter)) {
    ...
    if (pluma_file_browser_store_get_iter_root(model, &root)) {
      if (!obj->priv->changing_location) {
        ...
        if (obj->priv->current_location) clear_next_locations(obj);

        root_uri = pluma_file_browser_store_get_root(model);
        ...
        gtk_tree_model_get(GTK_TREE_MODEL(model), &iter,
                           PLUMA_FILE_BROWSER_STORE_COLUMN_ICON, &pixbuf, -1);

        obj->priv->current_location = obj->priv->locations;
        obj->priv->current_location_menu_item =
            create_goto_menu_item(obj, obj->priv->current_location, pixbuf);
      ...
      action =
          gtk_action_group_get_action(obj->priv->action_group, "DirectoryUp");
...
static void on_combo_changed(GtkComboBox *combo, PlumaFileBrowserWidget *obj) {
  ...
  gtk_tree_model_get(GTK_TREE_MODEL(obj->priv->combo_model), &iter, COLUMN_ID,
                     &id, -1);
...
static gboolean do_change_directory(PlumaFileBrowserWidget *obj,
                                    GdkEventKey *event) {
...
static gboolean on_treeview_key_press_event(PlumaFileBrowserView *treeview,
  ...
  if (do_change_directory(obj, event)) return TRUE;
...
static void on_selection_changed(GtkTreeSelection *selection,
  ...
  if (PLUMA_IS_FILE_BROWSER_STORE(model)) {
    selected = pluma_file_browser_widget_get_num_selected_files_or_directories(
        obj, &files, &dirs);
  }
...
static void on_filter_mode_changed(PlumaFileBrowserStore *model,
  ...
  action = GTK_TOGGLE_ACTION(
      gtk_action_group_get_action(obj->priv->action_group, "FilterBinary"));
...
static void on_action_directory_new(GtkAction *action,
  ...
  if (!pluma_file_browser_widget_get_selected_directory(obj, &parent)) return;

  if (pluma_file_browser_store_new_directory(PLUMA_FILE_BROWSER_STORE(model),
                                             &parent, &iter)) {
    pluma_file_browser_view_start_rename(obj->priv->treeview, &iter);
  }
}
...
static void on_action_file_new(GtkAction *action, PlumaFileBrowserWidget *obj) {
  ...
  if (pluma_file_browser_store_new_file(PLUMA_FILE_BROWSER_STORE(model),
                                        &parent, &iter)) {
    pluma_file_browser_view_start_rename(obj->priv->treeview, &iter);
  }
}
...
static void on_action_bookmark_open(GtkAction *action,
  ...
  if (!PLUMA_IS_FILE_BOOKMARKS_STORE(model)) return;

  if (gtk_tree_selection_get_selected(selection, NULL, &iter))
    bookmark_open(obj, model, &iter);
}

void _pluma_file_browser_widget_register_type(GTypeModule *type_module) {
