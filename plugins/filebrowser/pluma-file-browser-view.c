...
struct _PlumaFileBrowserViewPrivate {
  ...
  GtkTreePath *double_click_path[2]; /* Both clicks in a double click need to be
                                        on the same row */
...
G_DEFINE_DYNAMIC_TYPE_EXTENDED(PlumaFileBrowserView, pluma_file_browser_view,
                               GTK_TYPE_TREE_VIEW, 0,
                               G_ADD_PRIVATE_DYNAMIC(PlumaFileBrowserView))

static void on_cell_edited(GtkCellRendererText *cell, gchar *path,
                           gchar *new_text, PlumaFileBrowserView *tree_view);
...
static void on_unload(PlumaFileBrowserStore *model, gchar const *uri,
                      PlumaFileBrowserView *view);

static void on_row_inserted(PlumaFileBrowserStore *model, GtkTreePath *path,
                            GtkTreeIter *iter, PlumaFileBrowserView *view);

static void pluma_file_browser_view_finalize(GObject *object) {
  PlumaFileBrowserView *obj = PLUMA_FILE_BROWSER_VIEW(object);

  if (obj->priv->hand_cursor) g_object_unref(obj->priv->hand_cursor);

  if (obj->priv->hover_path) gtk_tree_path_free(obj->priv->hover_path);
  ...
  G_OBJECT_CLASS(pluma_file_browser_view_parent_class)->finalize(object);
}

static void add_expand_state(PlumaFileBrowserView *view, gchar const *uri) {
  GFile *file;
...
static void remove_expand_state(PlumaFileBrowserView *view, gchar const *uri) {
  GFile *file;
...
static void row_expanded(GtkTreeView *tree_view, GtkTreeIter *iter,
                         GtkTreePath *path) {
  ...
  if (GTK_TREE_VIEW_CLASS(pluma_file_browser_view_parent_class)->row_expanded)
    GTK_TREE_VIEW_CLASS(pluma_file_browser_view_parent_class)
        ->row_expanded(tree_view, iter, path);

  if (!PLUMA_IS_FILE_BROWSER_STORE(view->priv->model)) return;
  ...
  _pluma_file_browser_store_iter_expanded(
      PLUMA_FILE_BROWSER_STORE(view->priv->model), iter);
}

static void row_collapsed(GtkTreeView *tree_view, GtkTreeIter *iter,
                          GtkTreePath *path) {
  PlumaFileBrowserView *view = PLUMA_FILE_BROWSER_VIEW(tree_view);
  gchar *uri;
  ...
  _pluma_file_browser_store_iter_collapsed(
      PLUMA_FILE_BROWSER_STORE(view->priv->model), iter);
}
...
static gboolean leave_notify_event(GtkWidget *widget, GdkEventCrossing *event) {
  ...
  // Chainup
  return GTK_WIDGET_CLASS(pluma_file_browser_view_parent_class)
...
static gboolean enter_notify_event(GtkWidget *widget, GdkEventCrossing *event) {
  ...
  if (view->priv->click_policy == PLUMA_FILE_BROWSER_VIEW_CLICK_POLICY_SINGLE) {
    ...
    gtk_tree_view_get_path_at_pos(GTK_TREE_VIEW(widget), event->x, event->y,
                                  &view->priv->hover_path, NULL, NULL, NULL);
...
static gboolean motion_notify_event(GtkWidget *widget, GdkEventMotion *event) {
  ...
  if (view->priv->click_policy == PLUMA_FILE_BROWSER_VIEW_CLICK_POLICY_SINGLE) {
    ...
    if (old_hover_path != NULL) gtk_tree_path_free(old_hover_path);
  }
...
static void set_click_policy_property(
    PlumaFileBrowserView *obj, PlumaFileBrowserViewClickPolicy click_policy) {
  GtkTreeIter iter;
  GdkDisplay *display;
  GdkWindow *win;
  ...
  if (click_policy == PLUMA_FILE_BROWSER_VIEW_CLICK_POLICY_SINGLE) {
    ...
    if (obj->priv->hover_path != NULL) {
      if (gtk_tree_model_get_iter(GTK_TREE_MODEL(obj->priv->model), &iter,
                                  obj->priv->hover_path))
...
static void directory_activated(PlumaFileBrowserView *view, GtkTreeIter *iter) {
  pluma_file_browser_store_set_virtual_root(
      PLUMA_FILE_BROWSER_STORE(view->priv->model), iter);
}

static void activate_selected_files(PlumaFileBrowserView *view) {
  ...
  for (row = rows; row; row = row->next) {
    ...
    /* Get iter from path */
    if (!gtk_tree_model_get_iter(view->priv->model, &iter, path)) continue;

    gtk_tree_model_get(view->priv->model, &iter,
                       PLUMA_FILE_BROWSER_STORE_COLUMN_FLAGS, &flags, -1);

    if (FILE_IS_DIR(flags)) {
      if (directory == NULL) directory = path;
...
static void activate_selected_bookmark(PlumaFileBrowserView *view) {
  GtkTreeView *tree_view = GTK_TREE_VIEW(view);
  GtkTreeSelection *selection = gtk_tree_view_get_selection(tree_view);
  GtkTreeIter iter;
...
static void activate_selected_items(PlumaFileBrowserView *view) {
...
static void toggle_hidden_filter(PlumaFileBrowserView *view) {
...
static void drag_begin(GtkWidget *widget, GdkDragContext *context) {
  ...
  /* Chain up */
  GTK_WIDGET_CLASS(pluma_file_browser_view_parent_class)
      ->drag_begin(widget, context);
}

static void did_not_drag(PlumaFileBrowserView *view, GdkEventButton *event) {
  ...
  if (gtk_tree_view_get_path_at_pos(tree_view, event->x, event->y, &path, NULL,
                                    NULL, NULL)) {
...
static gboolean button_press_event(GtkWidget *widget, GdkEventButton *event) {
  ...
  if (gtk_tree_view_get_path_at_pos(tree_view, event->x, event->y, &path, NULL,
    ...
    if (event->type == GDK_2BUTTON_PRESS) {
      ...
      if (event->button == 3 && selected) call_parent = FALSE;
      ...
      if ((event->button == 1 || event->button == 2) &&
        ...
        if (selected) {
          ...
          view->priv->ignore_release =
              call_parent && view->priv->click_policy !=
                                 PLUMA_FILE_BROWSER_VIEW_CLICK_POLICY_SINGLE;
...
static void fill_expand_state(PlumaFileBrowserView *view, GtkTreeIter *iter) {
  GtkTreePath *path;
  GtkTreeIter child;
  gchar *uri;

  if (!gtk_tree_model_iter_has_child(view->priv->model, iter)) return;
...
static void uninstall_restore_signals(PlumaFileBrowserView *tree_view,
                                      GtkTreeModel *model) {
...
static void install_restore_signals(PlumaFileBrowserView *tree_view,
                                    GtkTreeModel *model) {
  g_signal_connect(model, "begin-refresh", G_CALLBACK(on_begin_refresh),
                   tree_view);
  ...
  g_signal_connect_after(model, "row-inserted", G_CALLBACK(on_row_inserted),
                         tree_view);
}

static void set_restore_expand_state(PlumaFileBrowserView *view,
                                     gboolean state) {
  if (state == view->priv->restore_expand_state) return;
  ...
  } else if (view->priv->model &&
             PLUMA_IS_FILE_BROWSER_STORE(view->priv->model)) {
...
static void pluma_file_browser_view_class_init(
    PlumaFileBrowserViewClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_enum("click-policy", "Click Policy", "The click policy",
  ...
  g_object_class_install_property(
      ...
      g_param_spec_boolean("restore-expand-state", "Restore Expand State",
  ...
  signals[ERROR] = g_signal_new(
      ...
      G_STRUCT_OFFSET(PlumaFileBrowserViewClass, error), NULL, NULL, NULL,
      G_TYPE_NONE, 2, G_TYPE_UINT, G_TYPE_STRING);
  signals[FILE_ACTIVATED] = g_signal_new(
      "file-activated", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PlumaFileBrowserViewClass, file_activated), NULL, NULL,
  ...
  signals[DIRECTORY_ACTIVATED] = g_signal_new(
      ...
      G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PlumaFileBrowserViewClass, directory_activated), NULL,
  ...
  signals[BOOKMARK_ACTIVATED] = g_signal_new(
      ...
      G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PlumaFileBrowserViewClass, bookmark_activated), NULL,
      NULL, NULL, G_TYPE_NONE, 1, GTK_TYPE_TREE_ITER);
}

static void pluma_file_browser_view_class_finalize(
    PlumaFileBrowserViewClass *klass) {
  /* dummy function - used by G_DEFINE_DYNAMIC_TYPE_EXTENDED */
}

static void cell_data_cb(GtkTreeViewColumn *tree_column, GtkCellRenderer *cell,
...
static void pluma_file_browser_view_init(PlumaFileBrowserView *obj) {
  GdkDisplay *display;
  ...
  gtk_tree_view_column_pack_start(obj->priv->column, obj->priv->pixbuf_renderer,
                                  FALSE);
  gtk_tree_view_column_add_attribute(obj->priv->column,
                                     obj->priv->pixbuf_renderer, "pixbuf",
                                     PLUMA_FILE_BROWSER_STORE_COLUMN_ICON);

  obj->priv->text_renderer = gtk_cell_renderer_text_new();
  gtk_tree_view_column_pack_start(obj->priv->column, obj->priv->text_renderer,
                                  TRUE);
  gtk_tree_view_column_add_attribute(obj->priv->column,
                                     obj->priv->text_renderer, "text",
...
static gboolean bookmarks_separator_func(GtkTreeModel *model, GtkTreeIter *iter,
                                         gpointer user_data) {
  guint flags;
...
GtkWidget *pluma_file_browser_view_new(void) {
  PlumaFileBrowserView *obj =
      PLUMA_FILE_BROWSER_VIEW(g_object_new(PLUMA_TYPE_FILE_BROWSER_VIEW, NULL));
...
void pluma_file_browser_view_set_model(PlumaFileBrowserView *tree_view,
  ...
  if (tree_view->priv->model == model) return;
...
void pluma_file_browser_view_start_rename(PlumaFileBrowserView *tree_view,
  ...
  g_return_if_fail(PLUMA_IS_FILE_BROWSER_STORE(tree_view->priv->model));
  g_return_if_fail(iter != NULL);

  gtk_tree_model_get(tree_view->priv->model, iter,
                     PLUMA_FILE_BROWSER_STORE_COLUMN_FLAGS, &flags, -1);

  if (!(FILE_IS_DIR(flags) || !FILE_IS_DUMMY(flags))) return;
  ...
  gtk_tree_view_scroll_to_cell(
      GTK_TREE_VIEW(tree_view),
      gtk_tree_row_reference_get_path(tree_view->priv->editable),
      tree_view->priv->column, FALSE, 0.0, 0.0);
}
...
static void on_cell_edited(GtkCellRendererText *cell, gchar *path,
  ...
  if (new_text == NULL || *new_text == '\0') return;

  treepath = gtk_tree_path_new_from_string(path);
  ret = gtk_tree_model_get_iter(GTK_TREE_MODEL(tree_view->priv->model), &iter,
                                treepath);
  gtk_tree_path_free(treepath);

  if (ret) {
    if (pluma_file_browser_store_rename(
            PLUMA_FILE_BROWSER_STORE(tree_view->priv->model), &iter, new_text,
            &error)) {
      treepath = gtk_tree_model_get_path(GTK_TREE_MODEL(tree_view->priv->model),
                                         &iter);
      ...
      if (error) {
        g_signal_emit(tree_view, signals[ERROR], 0, error->code,
                      error->message);
        g_error_free(error);
      }
    }
...
static void on_unload(PlumaFileBrowserStore *model, gchar const *uri,
  ...
  if (!view->priv->restore_expand_state || view->priv->is_refresh) return;

  remove_expand_state(view, uri);
}

static void restore_expand_state(PlumaFileBrowserView *view,
...
void _pluma_file_browser_view_register_type(GTypeModule *type_module) {
