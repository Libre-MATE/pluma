...
#define BUS_CONNECT(bus, name, data)                                         \
  pluma_message_bus_connect(bus, MESSAGE_OBJECT_PATH, #name,                 \
                            (PlumaMessageCallback)message_##name##_cb, data, \
                            NULL)
...
static WindowData *window_data_new(PlumaWindow *window,
                                   PlumaFileBrowserWidget *widget) {
  ...
  data->filters = g_hash_table_new_full(g_str_hash, g_str_equal,
...
static WindowData *get_window_data(PlumaWindow *window) {
  return (WindowData *)(g_object_get_data(G_OBJECT(window), WINDOW_DATA_KEY));
}

static void window_data_free(PlumaWindow *window) {
...
static FilterData *filter_data_new(PlumaWindow *window, PlumaMessage *message) {
...
static void filter_data_free(FilterData *data) {
  ...
  identifier = pluma_message_type_identifier(
      pluma_message_get_object_path(data->message),
...
static void message_cache_data_free(MessageCacheData *data, GClosure *closure) {
  g_object_unref(data->message);
  g_slice_free(MessageCacheData, data);
}

static MessageCacheData *message_cache_data_new(PlumaWindow *window,
                                                PlumaMessage *message) {
...
static void message_get_root_cb(PlumaMessageBus *bus, PlumaMessage *message,
                                WindowData *data) {
...
static void message_set_root_cb(PlumaMessageBus *bus, PlumaMessage *message,
                                WindowData *data) {
  gchar *root = NULL;
  gchar *virtual = NULL;
  ...
  if (virtual)
    pluma_file_browser_widget_set_root_and_virtual_root(data->widget, root,
                                                        virtual);
  else
...
static void message_set_emblem_cb(PlumaMessageBus *bus, PlumaMessage *message,
                                  WindowData *data) {
  gchar *id = NULL;
  gchar *emblem = NULL;
  GtkTreePath *path;
  ...
  if (path != NULL) {
    ...
    pixbuf = gtk_icon_theme_load_icon(gtk_icon_theme_get_default(), emblem, 10,
                                      0, &error);
    ...
    if (pixbuf) {
      ...
      if (gtk_tree_model_get_iter(GTK_TREE_MODEL(store), &iter, path)) {
        ...
        pluma_file_browser_store_set_value(
            store, &iter, PLUMA_FILE_BROWSER_STORE_COLUMN_EMBLEM, &value);
...
static gchar *item_id(const gchar *path, const gchar *uri) {
  return g_strconcat(path, "::", uri, NULL);
}

static gchar *track_row(WindowData *data, PlumaFileBrowserStore *store,
                        GtkTreePath *path, const gchar *uri) {
  GtkTreeRowReference *ref;
  gchar *id;
  gchar *pathstr;
...
static void set_item_message(WindowData *data, GtkTreeIter *iter,
                             GtkTreePath *path, PlumaMessage *message) {
  PlumaFileBrowserStore *store;
  gchar *uri = NULL;
  guint flags = 0;
  gchar *track_id;
  ...
  pluma_message_set(message, "id", track_id, "uri", uri, NULL);

  if (pluma_message_has_key(message, "is_directory")) {
    pluma_message_set(message, "is_directory", FILE_IS_DIR(flags), NULL);
  }
...
static gboolean custom_message_filter_func(PlumaFileBrowserWidget *widget,
                                           PlumaFileBrowserStore *store,
                                           GtkTreeIter *iter,
                                           FilterData *data) {
  WindowData *wdata = get_window_data(data->window);
...
static void message_add_filter_cb(PlumaMessageBus *bus, PlumaMessage *message,
                                  PlumaWindow *window) {
  ...
  pluma_message_get(message, "object_path", &object_path, "method", &method,
                    NULL);
  ...
  if (pluma_message_type_lookup(message_type, "id") != G_TYPE_STRING ||
      ...
      pluma_message_type_lookup(message_type, "is_directory") !=
          G_TYPE_BOOLEAN ||
      pluma_message_type_lookup(message_type, "filter") != G_TYPE_BOOLEAN) {
    return;
  }

  cbmessage = pluma_message_type_instantiate(message_type, "id", NULL, "uri",
                                             NULL, "is_directory", FALSE,
                                             "filter", FALSE, NULL);

  // Register the custom filter on the widget
  filter_data = filter_data_new(window, cbmessage);
  id = pluma_file_browser_widget_add_filter(
      data->widget,
      (PlumaFileBrowserWidgetFilterFunc)custom_message_filter_func, filter_data,
      (GDestroyNotify)filter_data_free);

  filter_data->id = id;
}

static void message_remove_filter_cb(PlumaMessageBus *bus,
                                     PlumaMessage *message, WindowData *data) {
  gulong id = 0;
...
static void message_up_cb(PlumaMessageBus *bus, PlumaMessage *message,
                          WindowData *data) {
  PlumaFileBrowserStore *store =
      pluma_file_browser_widget_get_browser_store(data->widget);

  pluma_file_browser_store_set_virtual_root_up(store);
}

static void message_history_back_cb(PlumaMessageBus *bus, PlumaMessage *message,
                                    WindowData *data) {
  pluma_file_browser_widget_history_back(data->widget);
}

static void message_history_forward_cb(PlumaMessageBus *bus,
                                       PlumaMessage *message,
                                       WindowData *data) {
  pluma_file_browser_widget_history_forward(data->widget);
}

static void message_refresh_cb(PlumaMessageBus *bus, PlumaMessage *message,
                               WindowData *data) {
  pluma_file_browser_widget_refresh(data->widget);
}

static void message_set_show_hidden_cb(PlumaMessageBus *bus,
                                       PlumaMessage *message,
                                       WindowData *data) {
...
static void message_set_show_binary_cb(PlumaMessageBus *bus,
                                       PlumaMessage *message,
                                       WindowData *data) {
  gboolean active = FALSE;
  PlumaFileBrowserStore *store;
...
static void message_show_bookmarks_cb(PlumaMessageBus *bus,
                                      PlumaMessage *message, WindowData *data) {
  pluma_file_browser_widget_show_bookmarks(data->widget);
}

static void message_show_files_cb(PlumaMessageBus *bus, PlumaMessage *message,
                                  WindowData *data) {
  pluma_file_browser_widget_show_files(data->widget);
}

static void message_add_context_item_cb(PlumaMessageBus *bus,
                                        PlumaMessage *message,
                                        WindowData *data) {
  ...
  pluma_message_get(message, "action", &action, "path", &path, NULL);
  ...
  gtk_ui_manager_add_ui(manager, merge_id, path, name,
                        gtk_action_get_name(action), GTK_UI_MANAGER_AUTO,
                        FALSE);

  if (gtk_ui_manager_get_widget(manager, path)) {
    data->merge_ids =
        g_list_prepend(data->merge_ids, GINT_TO_POINTER(merge_id));
...
static void message_remove_context_item_cb(PlumaMessageBus *bus,
                                           PlumaMessage *message,
                                           WindowData *data) {
  guint merge_id = 0;
  GtkUIManager *manager;
...
static void message_get_view_cb(PlumaMessageBus *bus, PlumaMessage *message,
                                WindowData *data) {
...
static void register_methods(PlumaWindow *window,
                             PlumaFileBrowserWidget *widget) {
  PlumaMessageBus *bus = pluma_window_get_message_bus(window);
  WindowData *data = get_window_data(window);

  /* Register method calls */
  pluma_message_bus_register(bus, MESSAGE_OBJECT_PATH, "get_root", 1, "uri",
                             G_TYPE_STRING, NULL);

  pluma_message_bus_register(bus, MESSAGE_OBJECT_PATH, "set_root", 1, "uri",
                             G_TYPE_STRING, "virtual", G_TYPE_STRING, NULL);

  pluma_message_bus_register(bus, MESSAGE_OBJECT_PATH, "set_emblem", 0, "id",
                             G_TYPE_STRING, "emblem", G_TYPE_STRING, NULL);

  pluma_message_bus_register(bus, MESSAGE_OBJECT_PATH, "add_filter", 1,
                             "object_path", G_TYPE_STRING, "method",
                             G_TYPE_STRING, "id", G_TYPE_ULONG, NULL);

  pluma_message_bus_register(bus, MESSAGE_OBJECT_PATH, "remove_filter", 0, "id",
                             G_TYPE_ULONG, NULL);

  pluma_message_bus_register(bus, MESSAGE_OBJECT_PATH, "add_context_item", 1,
                             "action", GTK_TYPE_ACTION, "path", G_TYPE_STRING,
                             "id", G_TYPE_UINT, NULL);

  pluma_message_bus_register(bus, MESSAGE_OBJECT_PATH, "remove_context_item", 0,
                             "id", G_TYPE_UINT, NULL);
  ...
  pluma_message_bus_register(bus, MESSAGE_OBJECT_PATH, "history_forward", 0,
                             NULL);

  pluma_message_bus_register(bus, MESSAGE_OBJECT_PATH, "refresh", 0, NULL);

  pluma_message_bus_register(bus, MESSAGE_OBJECT_PATH, "set_show_hidden", 0,
                             "active", G_TYPE_BOOLEAN, NULL);
  pluma_message_bus_register(bus, MESSAGE_OBJECT_PATH, "set_show_binary", 0,
                             "active", G_TYPE_BOOLEAN, NULL);

  pluma_message_bus_register(bus, MESSAGE_OBJECT_PATH, "show_bookmarks", 0,
                             NULL);
  pluma_message_bus_register(bus, MESSAGE_OBJECT_PATH, "show_files", 0, NULL);

  pluma_message_bus_register(bus, MESSAGE_OBJECT_PATH, "get_view", 1, "view",
...
static void store_row_inserted(PlumaFileBrowserStore *store, GtkTreePath *path,
                               GtkTreeIter *iter, MessageCacheData *data) {
  gchar *uri = NULL;
  guint flags = 0;
...
static void store_row_deleted(PlumaFileBrowserStore *store, GtkTreePath *path,
...
static void store_virtual_root_changed(PlumaFileBrowserStore *store,
                                       GParamSpec *spec,
                                       MessageCacheData *data) {
  WindowData *wdata = get_window_data(data->window);
  gchar *uri;
  ...
  pluma_message_set(data->message, "uri", uri, NULL);
...
static void store_begin_loading(PlumaFileBrowserStore *store, GtkTreeIter *iter,
                                MessageCacheData *data) {
  GtkTreePath *path;
  WindowData *wdata = get_window_data(data->window);
...
static void store_end_loading(PlumaFileBrowserStore *store, GtkTreeIter *iter,
                              MessageCacheData *data) {
  GtkTreePath *path;
  WindowData *wdata = get_window_data(data->window);
...
static void register_signals(PlumaWindow *window,
                             PlumaFileBrowserWidget *widget) {
  PlumaMessageBus *bus = pluma_window_get_message_bus(window);
  PlumaFileBrowserStore *store;
  ...
  /* Register signals */
  root_changed_type = pluma_message_bus_register(
      bus, MESSAGE_OBJECT_PATH, "root_changed", 0, "id", G_TYPE_STRING, "uri",
      G_TYPE_STRING, NULL);

  begin_loading_type = pluma_message_bus_register(
      bus, MESSAGE_OBJECT_PATH, "begin_loading", 0, "id", G_TYPE_STRING, "uri",
      G_TYPE_STRING, NULL);

  end_loading_type = pluma_message_bus_register(
      bus, MESSAGE_OBJECT_PATH, "end_loading", 0, "id", G_TYPE_STRING, "uri",
      G_TYPE_STRING, NULL);

  inserted_type = pluma_message_bus_register(
      bus, MESSAGE_OBJECT_PATH, "inserted", 0, "id", G_TYPE_STRING, "uri",
      G_TYPE_STRING, "is_directory", G_TYPE_BOOLEAN, NULL);

  deleted_type = pluma_message_bus_register(
      bus, MESSAGE_OBJECT_PATH, "deleted", 0, "id", G_TYPE_STRING, "uri",
      G_TYPE_STRING, "is_directory", G_TYPE_BOOLEAN, NULL);

  store = pluma_file_browser_widget_get_browser_store(widget);

  message = pluma_message_type_instantiate(inserted_type, "id", NULL, "uri",
                                           NULL, "is_directory", FALSE, NULL);
  ...
  message = pluma_message_type_instantiate(deleted_type, "id", NULL, "uri",
                                           NULL, "is_directory", FALSE, NULL);
  data->row_deleted_id =
      g_signal_connect_data(store, "row-deleted", G_CALLBACK(store_row_deleted),
                            message_cache_data_new(window, message),
                            (GClosureNotify)message_cache_data_free, 0);

  message = pluma_message_type_instantiate(root_changed_type, "id", NULL, "uri",
                                           NULL, NULL);
  ...
  message = pluma_message_type_instantiate(begin_loading_type, "id", NULL,
                                           "uri", NULL, NULL);
  ...
  message = pluma_message_type_instantiate(end_loading_type, "id", NULL, "uri",
                                           NULL, NULL);
  data->end_loading_id =
      g_signal_connect_data(store, "end_loading", G_CALLBACK(store_end_loading),
                            message_cache_data_new(window, message),
                            (GClosureNotify)message_cache_data_free, 0);
}

static void message_unregistered(PlumaMessageBus *bus,
                                 PlumaMessageType *message_type,
                                 PlumaWindow *window) {
  gchar *identifier = pluma_message_type_identifier(
      pluma_message_type_get_object_path(message_type),
  ...
  if (data) pluma_file_browser_widget_remove_filter(wdata->widget, data->id);
...
void pluma_file_browser_messages_register(PlumaWindow *window,
  ...
  g_signal_connect(pluma_window_get_message_bus(window), "unregistered",
                   G_CALLBACK(message_unregistered), window);
}

static void cleanup_signals(PlumaWindow *window) {
  WindowData *data = get_window_data(window);
  PlumaFileBrowserStore *store;
