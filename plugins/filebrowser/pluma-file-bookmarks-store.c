...
static void on_fs_changed(GVolumeMonitor *monitor, GObject *object,
                          PlumaFileBookmarksStore *model);

static void on_bookmarks_file_changed(GFileMonitor *monitor, GFile *file,
                                      GFile *other_file,
                                      GFileMonitorEvent event_type,
                                      PlumaFileBookmarksStore *model);
static gboolean find_with_flags(GtkTreeModel *model, GtkTreeIter *iter,
                                gpointer obj, guint flags, guint notflags);

G_DEFINE_DYNAMIC_TYPE_EXTENDED(PlumaFileBookmarksStore,
                               pluma_file_bookmarks_store, GTK_TYPE_TREE_STORE,
                               0,
                               G_ADD_PRIVATE_DYNAMIC(PlumaFileBookmarksStore))

static void pluma_file_bookmarks_store_dispose(GObject *object) {
...
static void pluma_file_bookmarks_store_finalize(GObject *object) {
  G_OBJECT_CLASS(pluma_file_bookmarks_store_parent_class)->finalize(object);
}

static void pluma_file_bookmarks_store_class_init(
    PlumaFileBookmarksStoreClass *klass) {
...
static void pluma_file_bookmarks_store_class_finalize(
    PlumaFileBookmarksStoreClass *klass) {
  /* dummy function - used by G_DEFINE_DYNAMIC_TYPE_EXTENDED */
}

static void pluma_file_bookmarks_store_init(PlumaFileBookmarksStore *obj) {
  obj->priv = pluma_file_bookmarks_store_get_instance_private(obj);
}

/* Private */
static void add_node(PlumaFileBookmarksStore *model, GdkPixbuf *pixbuf,
                     const gchar *name, GObject *obj, guint flags,
                     GtkTreeIter *iter) {
  GtkTreeIter newiter;
  ...
  gtk_tree_store_set(GTK_TREE_STORE(model), &newiter,
                     ...
                     PLUMA_FILE_BOOKMARKS_STORE_COLUMN_FLAGS, flags, -1);

  if (iter != NULL) *iter = newiter;
}

static gboolean add_file(PlumaFileBookmarksStore *model, GFile *file,
  ...
  if (flags & PLUMA_FILE_BOOKMARKS_STORE_IS_HOME)
    pixbuf = pluma_file_browser_utils_pixbuf_from_theme("user-home",
                                                        GTK_ICON_SIZE_MENU);
  else if (flags & PLUMA_FILE_BOOKMARKS_STORE_IS_DESKTOP)
    pixbuf = pluma_file_browser_utils_pixbuf_from_theme("user-desktop",
                                                        GTK_ICON_SIZE_MENU);
  else if (flags & PLUMA_FILE_BOOKMARKS_STORE_IS_ROOT)
    pixbuf = pluma_file_browser_utils_pixbuf_from_theme("drive-harddisk",
                                                        GTK_ICON_SIZE_MENU);
  ...
  if (pixbuf == NULL) {
    ...
    if (native) {
      pixbuf =
          pluma_file_browser_utils_pixbuf_from_file(file, GTK_ICON_SIZE_MENU);
    } else {
      pixbuf = pluma_file_browser_utils_pixbuf_from_theme("folder",
                                                          GTK_ICON_SIZE_MENU);
    }
  }
...
static void check_mount_separator(PlumaFileBookmarksStore *model, guint flags,
                                  gboolean added) {
  GtkTreeIter iter;
  gboolean found;

  found = find_with_flags(GTK_TREE_MODEL(model), &iter, NULL,
                          flags | PLUMA_FILE_BOOKMARKS_STORE_IS_SEPARATOR, 0);

  if (added && !found) {
    /* Add the separator */
    add_node(model, NULL, NULL, NULL,
             flags | PLUMA_FILE_BOOKMARKS_STORE_IS_SEPARATOR, NULL);
  } else if (!added && found) {
    remove_node(GTK_TREE_MODEL(model), &iter);
  }
}

static void init_special_directories(PlumaFileBookmarksStore *model) {
  gchar const *path;
  GFile *file;
  ...
  if (path != NULL) {
    file = g_file_new_for_path(path);
    add_file(model, file, NULL,
             PLUMA_FILE_BOOKMARKS_STORE_IS_DOCUMENTS |
                 PLUMA_FILE_BOOKMARKS_STORE_IS_SPECIAL_DIR,
             NULL);
    g_object_unref(file);
  }
  ...
  add_file(model, file, _("File System"), PLUMA_FILE_BOOKMARKS_STORE_IS_ROOT,
           NULL);
  g_object_unref(file);
...
static void get_fs_properties(gpointer fs, gchar **name, GdkPixbuf **pixbuf,
                              guint *flags) {
  GIcon *icon = NULL;
  ...
  *pixbuf = NULL;
  ...
  if (icon) {
    *pixbuf =
        pluma_file_browser_utils_pixbuf_from_icon(icon, GTK_ICON_SIZE_MENU);
    g_object_unref(icon);
  }
}

static void add_fs(PlumaFileBookmarksStore *model, gpointer fs, guint flags,
                   GtkTreeIter *iter) {
  gchar *name;
  GdkPixbuf *pixbuf;
  guint fsflags;
...
static void process_volume_cb(GVolume *volume, PlumaFileBookmarksStore *model) {
  GMount *mount;
...
static void process_drive_novolumes(PlumaFileBookmarksStore *model,
                                    GDrive *drive) {
...
static void process_drive_cb(GDrive *drive, PlumaFileBookmarksStore *model) {
  GList *volumes;
...
static void init_drives(PlumaFileBookmarksStore *model) {
  GList *drives;
...
static void process_volume_nodrive_cb(GVolume *volume,
                                      PlumaFileBookmarksStore *model) {
  GDrive *drive;
...
static void init_volumes(PlumaFileBookmarksStore *model) {
  GList *volumes;
...
static void process_mount_novolume_cb(GMount *mount,
                                      PlumaFileBookmarksStore *model) {
  GVolume *volume;
...
static void init_mounts(PlumaFileBookmarksStore *model) {
  GList *mounts;
...
static void init_fs(PlumaFileBookmarksStore *model) {
  if (model->priv->volume_monitor == NULL) {
    ...
    for (ptr = signals; *ptr; ptr++) {
      g_signal_connect(model->priv->volume_monitor, *ptr,
...
static gboolean add_bookmark(PlumaFileBookmarksStore *model, gchar const *name,
                             gchar const *uri) {
  GFile *file;
  gboolean ret;
...
static gchar *get_bookmarks_file(void) {
  return g_build_filename(g_get_user_config_dir(), "gtk-3.0", "bookmarks",
                          NULL);
}
...
static gboolean parse_bookmarks_file(PlumaFileBookmarksStore *model,
...
static void init_bookmarks(PlumaFileBookmarksStore *model) {
  gchar *bookmarks;
...
static gint flags_order[] = {PLUMA_FILE_BOOKMARKS_STORE_IS_HOME,
                             PLUMA_FILE_BOOKMARKS_STORE_IS_DESKTOP,
                             PLUMA_FILE_BOOKMARKS_STORE_IS_SPECIAL_DIR,
...
static gint bookmarks_compare_names(GtkTreeModel *model, GtkTreeIter *a,
                                    GtkTreeIter *b) {
  gchar *n1;
  gchar *n2;
  gint result;
  ...
  gtk_tree_model_get(model, a, PLUMA_FILE_BOOKMARKS_STORE_COLUMN_NAME, &n1,
                     PLUMA_FILE_BOOKMARKS_STORE_COLUMN_FLAGS, &f1, -1);
  gtk_tree_model_get(model, b, PLUMA_FILE_BOOKMARKS_STORE_COLUMN_NAME, &n2,
                     PLUMA_FILE_BOOKMARKS_STORE_COLUMN_FLAGS, &f2, -1);
...
static gint bookmarks_compare_flags(GtkTreeModel *model, GtkTreeIter *a,
                                    GtkTreeIter *b) {
  guint f1;
  guint f2;
  gint *flags;
  ...
  gtk_tree_model_get(model, a, PLUMA_FILE_BOOKMARKS_STORE_COLUMN_FLAGS, &f1,
                     -1);
  gtk_tree_model_get(model, b, PLUMA_FILE_BOOKMARKS_STORE_COLUMN_FLAGS, &f2,
                     -1);
...
static gint bookmarks_compare_func(GtkTreeModel *model, GtkTreeIter *a,
                                   GtkTreeIter *b, gpointer user_data) {
  gint result;

  result = bookmarks_compare_flags(model, a, b);

  if (result == 0) result = bookmarks_compare_names(model, a, b);
...
static gboolean find_with_flags(GtkTreeModel *model, GtkTreeIter *iter,
  ...
  if (!gtk_tree_model_get_iter_first(model, &child)) return FALSE;

  do {
    gtk_tree_model_get(model, &child, PLUMA_FILE_BOOKMARKS_STORE_COLUMN_OBJECT,
                       &childobj, PLUMA_FILE_BOOKMARKS_STORE_COLUMN_FLAGS,
                       &childflags, -1);

    fequal = (obj == childobj);

    if (childobj) g_object_unref(childobj);
  ...
  if (!(flags & PLUMA_FILE_BOOKMARKS_STORE_IS_SEPARATOR)) {
    if (flags & PLUMA_FILE_BOOKMARKS_STORE_IS_FS) {
      check_mount_separator(PLUMA_FILE_BOOKMARKS_STORE(model),
                            flags & PLUMA_FILE_BOOKMARKS_STORE_IS_FS, FALSE);
    }
  }
...
static void remove_bookmarks(PlumaFileBookmarksStore *model) {
  GtkTreeIter iter;

  while (find_with_flags(GTK_TREE_MODEL(model), &iter, NULL,
                         PLUMA_FILE_BOOKMARKS_STORE_IS_BOOKMARK, 0)) {
    remove_node(GTK_TREE_MODEL(model), &iter);
  }
}

static void initialize_fill(PlumaFileBookmarksStore *model) {
...
PlumaFileBookmarksStore *pluma_file_bookmarks_store_new(void) {
  PlumaFileBookmarksStore *model;
  GType column_types[] = {GDK_TYPE_PIXBUF, G_TYPE_STRING, G_TYPE_OBJECT,
                          G_TYPE_UINT};
  ...
  gtk_tree_sortable_set_default_sort_func(GTK_TREE_SORTABLE(model),
                                          bookmarks_compare_func, NULL, NULL);
...
gchar *pluma_file_bookmarks_store_get_uri(PlumaFileBookmarksStore *model,
  ...
  gtk_tree_model_get(GTK_TREE_MODEL(model), iter,
                     ...
                     PLUMA_FILE_BOOKMARKS_STORE_COLUMN_OBJECT, &obj, -1);
...
static void on_bookmarks_file_changed(GFileMonitor *monitor, GFile *file,
  ...
  switch (event_type) {
    ...
    case G_FILE_MONITOR_EVENT_CREATED:
      ...
      init_bookmarks(model);
      break;
    case G_FILE_MONITOR_EVENT_DELETED:  // FIXME: shouldn't we also monitor the
                                        // directory?
      /* Remove bookmarks */
      remove_bookmarks(model);
...
void _pluma_file_bookmarks_store_register_type(GTypeModule *type_module) {
