...
static void on_error_cb(PlumaFileBrowserWidget *widget, guint code,
                        gchar const *message,
                        PlumaFileBrowserPluginPrivate *priv);
static void on_model_set_cb(PlumaFileBrowserView *widget, GParamSpec *arg1,
                            PlumaFileBrowserPluginPrivate *priv);
...
static void on_filter_mode_changed_cb(PlumaFileBrowserStore *model,
                                      GParamSpec *param,
                                      PlumaFileBrowserPluginPrivate *priv);
static void on_rename_cb(PlumaFileBrowserStore *model, const gchar *olduri,
                         const gchar *newuri, PlumaWindow *window);
static void on_filter_pattern_changed_cb(PlumaFileBrowserWidget *widget,
                                         GParamSpec *param,
                                         PlumaFileBrowserPluginPrivate *priv);
static void on_tab_added_cb(PlumaWindow *window, PlumaTab *tab,
                            PlumaFileBrowserPluginPrivate *priv);
static gboolean on_confirm_delete_cb(PlumaFileBrowserWidget *widget,
                                     PlumaFileBrowserStore *store, GList *rows,
                                     PlumaFileBrowserPluginPrivate *priv);
static gboolean on_confirm_no_trash_cb(PlumaFileBrowserWidget *widget,
                                       GList *files, PlumaWindow *window);

static void pluma_window_activatable_iface_init(
    PlumaWindowActivatableInterface *iface);

G_DEFINE_DYNAMIC_TYPE_EXTENDED(
    PlumaFileBrowserPlugin, pluma_file_browser_plugin, PEAS_TYPE_EXTENSION_BASE,
    0,
...
static void pluma_file_browser_plugin_init(PlumaFileBrowserPlugin *plugin) {
  plugin->priv = pluma_file_browser_plugin_get_instance_private(plugin);
}

static void pluma_file_browser_plugin_dispose(GObject *object) {
...
static void pluma_file_browser_plugin_set_property(GObject *object,
                                                   guint prop_id,
                                                   const GValue *value,
                                                   GParamSpec *pspec) {
  PlumaFileBrowserPlugin *plugin = PLUMA_FILE_BROWSER_PLUGIN(object);
...
static void pluma_file_browser_plugin_get_property(GObject *object,
...
static void prepare_auto_root(PlumaFileBrowserPluginPrivate *priv) {
...
static void restore_default_location(PlumaFileBrowserPluginPrivate *priv) {
  gchar *root;
  gchar *virtual_root;
  gboolean bookmarks;
  gboolean remote;
  ...
  if (root != NULL && *root != '\0') {
    ...
    if (remote || g_file_is_native(file)) {
      if (virtual_root != NULL && *virtual_root != '\0') {
        ...
        prepare_auto_root(priv);
        pluma_file_browser_widget_set_root(priv->tree_widget, root, TRUE);
      }
    }
...
static void restore_filter(PlumaFileBrowserPluginPrivate *priv) {
  gchar *filter_mode;
  ...
  if (filter_mode != NULL) {
    ...
    } else if (strcmp(filter_mode, "none") == 0 || *filter_mode == '\0') {
  ...
  pluma_file_browser_store_set_filter_mode(
      pluma_file_browser_widget_get_browser_store(priv->tree_widget), mode);

  pattern = g_settings_get_string(priv->settings, "filter-pattern");

  pluma_file_browser_widget_set_filter_pattern(priv->tree_widget, pattern);

  g_free(filter_mode);
  g_free(pattern);
}

static PlumaFileBrowserViewClickPolicy click_policy_from_string(
...
static void on_click_policy_changed(GSettings *settings, gchar *key,
                                    gpointer user_data) {
  PlumaFileBrowserPluginPrivate *priv;
  gchar *click_policy;
...
static void on_enable_delete_changed(GSettings *settings, gchar *key,
                                     gpointer user_data) {
  PlumaFileBrowserPluginPrivate *priv;
...
static void on_confirm_trash_changed(GSettings *settings, gchar *key,
                                     gpointer user_data) {
  PlumaFileBrowserPluginPrivate *priv;
  gboolean enable = FALSE;
...
static gboolean have_click_policy(void) {
...
static void install_caja_prefs(PlumaFileBrowserPluginPrivate *priv) {
  gchar *pref;
  gboolean prefb;
  PlumaFileBrowserViewClickPolicy policy;
  PlumaFileBrowserView *view;
  ...
  if (have_click_policy()) {
    g_signal_connect(priv->caja_settings, "changed::" CAJA_CLICK_POLICY_KEY,
                     G_CALLBACK(on_click_policy_changed), priv);
  }

  g_signal_connect(priv->caja_settings, "changed::" CAJA_ENABLE_DELETE_KEY,
                   G_CALLBACK(on_enable_delete_changed), priv);

  g_signal_connect(priv->caja_settings, "changed::" CAJA_CONFIRM_TRASH_KEY,
                   G_CALLBACK(on_confirm_trash_changed), priv);
...
static void set_root_from_doc(PlumaFileBrowserPluginPrivate *priv,
                              PlumaDocument *doc) {
  GFile *file;
  GFile *parent;
...
static void on_action_set_active_root(GtkAction *action,
                                      PlumaFileBrowserPluginPrivate *priv) {
  set_root_from_doc(
      priv, pluma_window_get_active_document(PLUMA_WINDOW(priv->window)));
}

static gchar *get_terminal(PlumaFileBrowserPluginPrivate *priv) {
  gchar *terminal;

  terminal = g_settings_get_string(priv->terminal_settings, TERMINAL_EXEC_KEY);
...
static void on_action_open_terminal(GtkAction *action,
                                    PlumaFileBrowserPluginPrivate *priv) {
  gchar *terminal;
  gchar *wd = NULL;
  gchar *local;
  ...
  if (!pluma_file_browser_widget_get_selected_directory(priv->tree_widget,
                                                        &iter))
    return;

  store = pluma_file_browser_widget_get_browser_store(priv->tree_widget);
  gtk_tree_model_get(GTK_TREE_MODEL(store), &iter,
                     PLUMA_FILE_BROWSER_STORE_COLUMN_URI, &wd, -1);
  ...
  g_spawn_async(local, argv, NULL, G_SPAWN_SEARCH_PATH, NULL, NULL, NULL, NULL);
...
static void on_selection_changed_cb(GtkTreeSelection *selection,
                                    PlumaFileBrowserPluginPrivate *priv) {
  ...
  tree_view = GTK_TREE_VIEW(
      pluma_file_browser_widget_get_browser_view(priv->tree_widget));
  model = gtk_tree_view_get_model(tree_view);

  if (!PLUMA_IS_FILE_BROWSER_STORE(model)) return;

  sensitive = pluma_file_browser_widget_get_selected_directory(
      priv->tree_widget, &iter);

  if (sensitive) {
    gtk_tree_model_get(model, &iter, PLUMA_FILE_BROWSER_STORE_COLUMN_URI, &uri,
                       -1);
...
static GtkActionEntry extra_actions[] = {
    {"SetActiveRoot", "go-jump", N_("_Set root to active document"), NULL,
...
static GtkActionEntry extra_single_selection_actions[] = {
    {"OpenTerminal", "utilities-terminal", N_("_Open terminal here"), NULL,
     N_("Open a terminal at the currently opened directory"),
     G_CALLBACK(on_action_open_terminal)}};

static void add_popup_ui(PlumaFileBrowserPluginPrivate *priv) {
  ...
  gtk_action_group_add_actions(action_group, extra_actions,
                               G_N_ELEMENTS(extra_actions), priv);
  ...
  priv->merge_id =
      gtk_ui_manager_add_ui_from_string(manager, POPUP_UI, -1, &error);
...
static void remove_popup_ui(PlumaFileBrowserPluginPrivate *priv) {
  GtkUIManager *manager;
...
static void pluma_file_browser_plugin_update_state(
  ...
  gtk_action_set_sensitive(
      gtk_action_group_get_action(priv->action_group, "SetActiveRoot"),
      doc != NULL && !pluma_document_is_untitled(doc));
}

static void pluma_file_browser_plugin_activate(
    PlumaWindowActivatable *activatable) {
  PlumaFileBrowserPluginPrivate *priv;
  PlumaWindow *window;
  PlumaPanel *panel;
  ...
  priv->tree_widget =
      PLUMA_FILE_BROWSER_WIDGET(pluma_file_browser_widget_new(data_dir));
  g_free(data_dir);
  ...
  g_signal_connect(priv->tree_widget, "error", G_CALLBACK(on_error_cb), priv);

  g_signal_connect(priv->tree_widget, "notify::filter-pattern",
                   G_CALLBACK(on_filter_pattern_changed_cb), priv);

  g_signal_connect(priv->tree_widget, "confirm-delete",
                   G_CALLBACK(on_confirm_delete_cb), priv);

  g_signal_connect(priv->tree_widget, "confirm-no-trash",
                   G_CALLBACK(on_confirm_no_trash_cb), window);

  g_signal_connect(
      gtk_tree_view_get_selection(GTK_TREE_VIEW(
          pluma_file_browser_widget_get_browser_view(priv->tree_widget))),
      "changed", G_CALLBACK(on_selection_changed_cb), priv);
  ...
  pluma_panel_add_item(panel, GTK_WIDGET(priv->tree_widget), _("File Browser"),
                       image);
  ...
  g_signal_connect(
      pluma_file_browser_widget_get_browser_view(priv->tree_widget),
      "notify::model", G_CALLBACK(on_model_set_cb), priv);

  store = pluma_file_browser_widget_get_browser_store(priv->tree_widget);
  g_signal_connect(store, "notify::virtual-root",
                   G_CALLBACK(on_virtual_root_changed_cb), priv);

  g_signal_connect(store, "notify::filter-mode",
                   G_CALLBACK(on_filter_mode_changed_cb), priv);

  g_signal_connect(store, "rename", G_CALLBACK(on_rename_cb), window);

  g_signal_connect(window, "tab-added", G_CALLBACK(on_tab_added_cb), priv);
...
static void pluma_file_browser_plugin_deactivate(
    PlumaWindowActivatable *activatable) {
  PlumaFileBrowserPluginPrivate *priv;
  PlumaWindow *window;
  PlumaPanel *panel;
  ...
  /* Disconnect signals */
  g_signal_handlers_disconnect_by_func(window, G_CALLBACK(on_tab_added_cb),
                                       priv);
  ...
  if (priv->caja_settings) g_object_unref(priv->caja_settings);
...
static void pluma_file_browser_plugin_class_init(
    PlumaFileBrowserPluginClass *klass) {
...
static void pluma_file_browser_plugin_class_finalize(
    PlumaFileBrowserPluginClass *klass) {
...
G_MODULE_EXPORT void peas_register_types(PeasObjectModule *module) {
  ...
  peas_object_module_register_extension_type(
      module, PLUMA_TYPE_WINDOW_ACTIVATABLE, PLUMA_TYPE_FILE_BROWSER_PLUGIN);
}

/* Callbacks */
static void on_uri_activated_cb(PlumaFileBrowserWidget *tree_widget,
                                gchar const *uri, PlumaWindow *window) {
  pluma_commands_load_uri(window, uri, NULL, 0);
}

static void on_error_cb(PlumaFileBrowserWidget *tree_widget, guint code,
                        gchar const *message,
                        PlumaFileBrowserPluginPrivate *priv) {
  gchar *title;
  GtkWidget *dlg;
  ...
  switch (code) {
    ...
    case PLUMA_FILE_BROWSER_ERROR_OPEN_DIRECTORY:
      title =
          _("An error occurred while opening a directory in the file manager");
      break;
  ...
  dlg =
      gtk_message_dialog_new(GTK_WINDOW(priv->window),
...
static void on_model_set_cb(PlumaFileBrowserView *widget, GParamSpec *arg1,
  ...
  g_settings_set_boolean(priv->onload_settings, "tree-view",
...
static void on_rename_cb(PlumaFileBrowserStore *store, const gchar *olduri,
                         const gchar *newuri, PlumaWindow *window) {
  PlumaApp *app;
  GList *documents;
  GList *item;
...
static void on_virtual_root_changed_cb(PlumaFileBrowserStore *store,
                                       GParamSpec *param,
                                       PlumaFileBrowserPluginPrivate *priv) {
  gchar *root;
  gchar *virtual_root;
  ...
  g_signal_handlers_disconnect_by_func(PLUMA_WINDOW(priv->window),
                                       G_CALLBACK(on_tab_added_cb), priv);
...
static void on_tab_added_cb(PlumaWindow *window, PlumaTab *tab,
  ...
  if (load_default) restore_default_location(priv);
...
static gboolean on_confirm_no_trash_cb(PlumaFileBrowserWidget *widget,
  ...
  result = pluma_file_browser_utils_confirmation_dialog(
      window, GTK_MESSAGE_QUESTION, message, secondary);
  g_free(secondary);
...
static gboolean on_confirm_delete_cb(PlumaFileBrowserWidget *widget,
                                     PlumaFileBrowserStore *store, GList *paths,
                                     PlumaFileBrowserPluginPrivate *priv) {
  gchar *normal;
  gchar *message;
  gchar *secondary;
  ...
  if (!priv->confirm_trash) return TRUE;

  if (paths->next == NULL) {
    normal = get_filename_from_path(GTK_TREE_MODEL(store),
  ...
  result = pluma_file_browser_utils_confirmation_dialog(
      PLUMA_WINDOW(priv->window), GTK_MESSAGE_QUESTION, message, secondary);
