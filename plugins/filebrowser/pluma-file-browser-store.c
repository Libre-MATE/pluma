...
static FileBrowserNode *model_find_node(PlumaFileBrowserStore *model,
                                        FileBrowserNode *node, GFile *uri);
...
static GtkTreeModelFlags pluma_file_browser_store_get_flags(
...
static gboolean pluma_file_browser_store_iter_children(GtkTreeModel *tree_model,
                                                       GtkTreeIter *iter,
                                                       GtkTreeIter *parent);
static gboolean pluma_file_browser_store_iter_has_child(
    GtkTreeModel *tree_model, GtkTreeIter *iter);
static gint pluma_file_browser_store_iter_n_children(GtkTreeModel *tree_model,
                                                     GtkTreeIter *iter);
static gboolean pluma_file_browser_store_iter_nth_child(
...
static void pluma_file_browser_store_drag_source_init(
    GtkTreeDragSourceIface *iface);
static gboolean pluma_file_browser_store_row_draggable(
    GtkTreeDragSource *drag_source, GtkTreePath *path);
static gboolean pluma_file_browser_store_drag_data_delete(
    GtkTreeDragSource *drag_source, GtkTreePath *path);
static gboolean pluma_file_browser_store_drag_data_get(
    GtkTreeDragSource *drag_source, GtkTreePath *path,
    GtkSelectionData *selection_data);

static void file_browser_node_free(PlumaFileBrowserStore *model,
                                   FileBrowserNode *node);
static void model_add_node(PlumaFileBrowserStore *model, FileBrowserNode *child,
                           FileBrowserNode *parent);
static void model_clear(PlumaFileBrowserStore *model, gboolean free_nodes);
...
G_DEFINE_DYNAMIC_TYPE_EXTENDED(
    PlumaFileBrowserStore, pluma_file_browser_store, G_TYPE_OBJECT, 0,
...
static void cancel_mount_operation(PlumaFileBrowserStore *obj) {
...
static void pluma_file_browser_store_finalize(GObject *object) {
...
static void pluma_file_browser_store_get_property(GObject *object,
...
static void pluma_file_browser_store_set_property(GObject *object,
                                                  guint prop_id,
                                                  const GValue *value,
                                                  GParamSpec *pspec) {
  PlumaFileBrowserStore *obj = PLUMA_FILE_BROWSER_STORE(object);
  ...
  switch (prop_id) {
    case PROP_FILTER_MODE:
      pluma_file_browser_store_set_filter_mode(obj, g_value_get_flags(value));
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID(object, prop_id, pspec);
      break;
...
static void pluma_file_browser_store_class_init(
    PlumaFileBrowserStoreClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string("root", "Root", "The root uri", NULL,
                          G_PARAM_READABLE));

  g_object_class_install_property(
      object_class, PROP_VIRTUAL_ROOT,
      g_param_spec_string("virtual-root", "Virtual Root",
                          "The virtual root uri", NULL, G_PARAM_READABLE));

  g_object_class_install_property(
      object_class, PROP_FILTER_MODE,
      g_param_spec_flags("filter-mode", "Filter Mode", "The filter mode",
  ...
  model_signals[BEGIN_LOADING] = g_signal_new(
      ...
      G_STRUCT_OFFSET(PlumaFileBrowserStoreClass, begin_loading), NULL, NULL,
  ...
  model_signals[END_LOADING] = g_signal_new(
      ...
      G_STRUCT_OFFSET(PlumaFileBrowserStoreClass, end_loading), NULL, NULL,
      NULL, G_TYPE_NONE, 1, GTK_TYPE_TREE_ITER);
  model_signals[ERROR] = g_signal_new(
      "error", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PlumaFileBrowserStoreClass, error), NULL, NULL, NULL,
  ...
  model_signals[RENAME] = g_signal_new(
      ...
      G_STRUCT_OFFSET(PlumaFileBrowserStoreClass, rename), NULL, NULL, NULL,
      G_TYPE_NONE, 2, G_TYPE_STRING, G_TYPE_STRING);
  model_signals[BEGIN_REFRESH] = g_signal_new(
      "begin-refresh", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PlumaFileBrowserStoreClass, begin_refresh), NULL, NULL,
      NULL, G_TYPE_NONE, 0);
  model_signals[END_REFRESH] = g_signal_new(
      "end-refresh", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PlumaFileBrowserStoreClass, end_refresh), NULL, NULL,
      NULL, G_TYPE_NONE, 0);
  model_signals[UNLOAD] = g_signal_new(
      "unload", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PlumaFileBrowserStoreClass, unload), NULL, NULL, NULL,
      G_TYPE_NONE, 1, G_TYPE_STRING);
}

static void pluma_file_browser_store_class_finalize(
    PlumaFileBrowserStoreClass *klass) {
...
static void pluma_file_browser_store_init(PlumaFileBrowserStore *obj) {
  obj->priv = pluma_file_browser_store_get_instance_private(obj);

  obj->priv->column_types[PLUMA_FILE_BROWSER_STORE_COLUMN_URI] = G_TYPE_STRING;
  obj->priv->column_types[PLUMA_FILE_BROWSER_STORE_COLUMN_NAME] = G_TYPE_STRING;
  obj->priv->column_types[PLUMA_FILE_BROWSER_STORE_COLUMN_FLAGS] = G_TYPE_UINT;
...
static gboolean node_has_parent(FileBrowserNode *node,
                                FileBrowserNode *parent) {
...
static gboolean node_in_tree(PlumaFileBrowserStore *model,
                             FileBrowserNode *node) {
  return node_has_parent(node, model->priv->virtual_root);
}

static gboolean model_node_visibility(PlumaFileBrowserStore *model,
                                      FileBrowserNode *node) {
  if (node == NULL) return FALSE;

  if (NODE_IS_DUMMY(node)) return !NODE_IS_HIDDEN(node);

  if (node == model->priv->virtual_root) return TRUE;

  if (!node_has_parent(node, model->priv->virtual_root)) return FALSE;

  return !NODE_IS_FILTERED(node);
}

static gboolean model_node_inserted(PlumaFileBrowserStore *model,
                                    FileBrowserNode *node) {
...
static GType pluma_file_browser_store_get_column_type(GtkTreeModel *tree_model,
                                                      gint idx) {
  g_return_val_if_fail(PLUMA_IS_FILE_BROWSER_STORE(tree_model), G_TYPE_INVALID);
  g_return_val_if_fail(idx < PLUMA_FILE_BROWSER_STORE_COLUMN_NUM && idx >= 0,
                       G_TYPE_INVALID);
...
static gboolean pluma_file_browser_store_get_iter(GtkTreeModel *tree_model,
  ...
  for (i = 0; i < depth; ++i) {
    ...
    if (!NODE_IS_DIR(node)) return FALSE;
...
static GtkTreePath *pluma_file_browser_store_get_path_real(
    PlumaFileBrowserStore *model, FileBrowserNode *node) {
  GtkTreePath *path;
  gint num = 0;
  ...
  while (node != model->priv->virtual_root) {
    ...
    for (item = FILE_BROWSER_NODE_DIR(node->parent)->children; item;
         item = item->next) {
      ...
      if (model_node_visibility(model, check) &&
        ...
        if (NODE_IS_DUMMY(node)) g_warning("Dummy not visible???");
...
static GtkTreePath *pluma_file_browser_store_get_path(GtkTreeModel *tree_model,
  ...
  return pluma_file_browser_store_get_path_real(
      PLUMA_FILE_BROWSER_STORE(tree_model),
...
static void pluma_file_browser_store_get_value(GtkTreeModel *tree_model,
  ...
  g_value_init(
      value, PLUMA_FILE_BROWSER_STORE(tree_model)->priv->column_types[column]);
...
static gint pluma_file_browser_store_iter_n_children(GtkTreeModel *tree_model,
  ...
  if (!NODE_IS_DIR(node)) return 0;

  for (item = FILE_BROWSER_NODE_DIR(node)->children; item; item = item->next)
    if (model_node_inserted(model, (FileBrowserNode *)(item->data))) ++num;
...
static gboolean pluma_file_browser_store_iter_parent(GtkTreeModel *tree_model,
  ...
  if (!node_in_tree(model, node)) return FALSE;
...
/* Private */
static void model_begin_loading(PlumaFileBrowserStore *model,
                                FileBrowserNode *node) {
  GtkTreeIter iter;
...
static void model_end_loading(PlumaFileBrowserStore *model,
                              FileBrowserNode *node) {
  GtkTreeIter iter;
...
static void model_node_update_visibility(PlumaFileBrowserStore *model,
                                         FileBrowserNode *node) {
  GtkTreeIter iter;
...
static void model_resort_node(PlumaFileBrowserStore *model,
                              FileBrowserNode *node) {
  FileBrowserNodeDir *dir;
  GSList *item;
  FileBrowserNode *child;
  ...
  if (!model_node_visibility(model, node->parent)) {
    ...
    for (item = dir->children; item; item = item->next) {
      ...
      if (model_node_visibility(model, child)) child->pos = pos++;
    }
    ...
    for (item = dir->children; item; item = item->next) {
      ...
      if (model_node_visibility(model, child)) neworder[pos++] = child->pos;
    }

    iter.user_data = node->parent;
    path = pluma_file_browser_store_get_path_real(model, node->parent);
...
static void row_changed(PlumaFileBrowserStore *model, GtkTreePath **path,
                        GtkTreeIter *iter) {
...
static void row_inserted(PlumaFileBrowserStore *model, GtkTreePath **path,
                         GtkTreeIter *iter) {
...
static void row_deleted(PlumaFileBrowserStore *model, const GtkTreePath *path) {
  GtkTreePath *copy = gtk_tree_path_copy(path);
...
static void model_refilter_node(PlumaFileBrowserStore *model,
                                FileBrowserNode *node, GtkTreePath **path) {
  ...
  if (path == NULL) {
    if (in_tree)
      tmppath = pluma_file_browser_store_get_path_real(model, node);
    else
  ...
  if (NODE_IS_DIR(node)) {
    if (in_tree) gtk_tree_path_down(*path);
    ...
    if (in_tree) gtk_tree_path_up(*path);
  }
  ...
  if (tmppath) gtk_tree_path_free(tmppath);
}

static void model_refilter(PlumaFileBrowserStore *model) {
...
static void file_browser_node_init(FileBrowserNode *node, GFile *file,
                                   FileBrowserNode *parent) {
  if (file != NULL) {
...
static FileBrowserNode *file_browser_node_new(GFile *file,
                                              FileBrowserNode *parent) {
...
static FileBrowserNode *file_browser_node_dir_new(PlumaFileBrowserStore *model,
                                                  GFile *file,
                                                  FileBrowserNode *parent) {
...
static void file_browser_node_free_children(PlumaFileBrowserStore *model,
                                            FileBrowserNode *node) {
  GSList *item;
...
static void file_browser_node_free(PlumaFileBrowserStore *model,
  ...
  if (node->icon) g_object_unref(node->icon);

  if (node->emblem) g_object_unref(node->emblem);
 ...
 * a node.
 **/
static void model_remove_node_children(PlumaFileBrowserStore *model,
                                       FileBrowserNode *node, GtkTreePath *path,
                                       gboolean free_nodes) {
  FileBrowserNodeDir *dir;
  ...
  if (!model_node_visibility(model, node)) {
    ...
    // be freed
    if (free_nodes) file_browser_node_free_children(model, node);

    return;
  }

  if (path == NULL)
    path_child = pluma_file_browser_store_get_path_real(model, node);
  else
  ...
  for (item = list; item; item = item->next) {
    model_remove_node(model, (FileBrowserNode *)(item->data), path_child,
                      free_nodes);
  }
...
static void model_remove_node(PlumaFileBrowserStore *model,
  ...
  else if (parent && model_node_visibility(model, parent) &&
           !(free_nodes && NODE_IS_DUMMY(node)))
    model_check_dummy(model, parent);

  /* Now free the node if necessary */
  if (free_nodes) file_browser_node_free(model, node);
}
...
static void file_browser_node_unload(PlumaFileBrowserStore *model,
                                     FileBrowserNode *node,
  ...
  if (remove_children) model_remove_node_children(model, node, NULL, TRUE);
...
static void model_recomposite_icon_real(PlumaFileBrowserStore *tree_model,
                                        FileBrowserNode *node,
                                        GFileInfo *info) {
  GdkPixbuf *icon;
  ...
  if (info) {
    ...
    if (gicon != NULL)
      icon =
          pluma_file_browser_utils_pixbuf_from_icon(gicon, GTK_ICON_SIZE_MENU);
    else
      icon = NULL;
  } else {
    icon = pluma_file_browser_utils_pixbuf_from_file(node->file,
                                                     GTK_ICON_SIZE_MENU);
  }
  ...
  if (node->emblem) {
    ...
    gdk_pixbuf_composite(node->emblem, node->icon, icon_size - 10,
                         icon_size - 10, 10, 10, icon_size - 10, icon_size - 10,
...
static void model_recomposite_icon(PlumaFileBrowserStore *tree_model,
                                   GtkTreeIter *iter) {
  g_return_if_fail(PLUMA_IS_FILE_BROWSER_STORE(tree_model));
  g_return_if_fail(iter != NULL);
  g_return_if_fail(iter->user_data != NULL);

  model_recomposite_icon_real(tree_model, (FileBrowserNode *)(iter->user_data),
                              NULL);
}

static FileBrowserNode *model_create_dummy_node(PlumaFileBrowserStore *model,
                                                FileBrowserNode *parent) {
  FileBrowserNode *dummy;
...
static FileBrowserNode *model_add_dummy_node(PlumaFileBrowserStore *model,
                                             FileBrowserNode *parent) {
  FileBrowserNode *dummy;
  ...
  if (NODE_IS_DIR(node)) {
    ...
    if (!filter_tree_model_iter_has_child_real(model, node)) {
      ...
      if (FILE_IS_HIDDEN(flags)) {
        ...
        path = pluma_file_browser_store_get_path_real(model, dummy);
...
static void insert_node_sorted(PlumaFileBrowserStore *model,
                               FileBrowserNode *child,
                               FileBrowserNode *parent) {
  FileBrowserNodeDir *dir;
...
static void model_add_nodes_batch(PlumaFileBrowserStore *model,
                                  GSList *children, FileBrowserNode *parent) {
...
static gchar const *backup_content_type(GFileInfo *info) {
  ...
  if (!g_file_info_get_is_backup(info)) return NULL;
...
static void file_browser_node_set_from_info(PlumaFileBrowserStore *model,
                                            FileBrowserNode *node,
                                            GFileInfo *info, gboolean isadded) {
  gchar const *content;
  ...
  if (info == NULL) {
    info = g_file_query_info(node->file, STANDARD_ATTRIBUTE_TYPES,
  ...
  if (g_file_info_get_file_type(info) == G_FILE_TYPE_DIRECTORY)
    ...
    if (!content || g_content_type_is_unknown(content) ||
  ...
  if (free_info) g_object_unref(info);
...
static FileBrowserNode *node_list_contains_file(GSList *children, GFile *file) {
  ...
  for (item = children; item; item = item->next) {
    ...
    if (node->file != NULL && g_file_equal(node->file, file)) return node;
  }

  return NULL;
}

static FileBrowserNode *model_add_node_from_file(PlumaFileBrowserStore *model,
                                                 FileBrowserNode *parent,
  ...
  if ((node = node_list_contains_file(FILE_BROWSER_NODE_DIR(parent)->children,
                                      file)) == NULL) {
    if (info == NULL) {
      info = g_file_query_info(file, STANDARD_ATTRIBUTE_TYPES,
                               G_FILE_QUERY_INFO_NONE, NULL, &error);
      free_info = TRUE;
    }
    ...
    if (info && free_info) g_object_unref(info);
  }
 ...
 * added */
static void model_add_nodes_from_files(PlumaFileBrowserStore *model,
                                       FileBrowserNode *parent,
                                       GSList *original_children,
                                       GList *files) {
  GList *item;
  ...
  if (nodes) model_add_nodes_batch(model, nodes, parent);
}

static FileBrowserNode *model_add_node_from_dir(PlumaFileBrowserStore *model,
                                                FileBrowserNode *parent,
                                                GFile *file) {
  FileBrowserNode *node;
  ...
  if ((node = node_list_contains_file(FILE_BROWSER_NODE_DIR(parent)->children,
    ...
    if (node->icon == NULL) {
      node->icon = pluma_file_browser_utils_pixbuf_from_theme(
          "folder", GTK_ICON_SIZE_MENU);
    }
...
static void on_directory_monitor_event(GFileMonitor *monitor, GFile *file,
                                       GFile *other_file,
...
static void model_iterate_next_files_cb(GFileEnumerator *enumerator,
                                        GAsyncResult *result,
                                        AsyncNode *async) {
  GList *files;
  GError *error = NULL;
  ...
  if (files == NULL) {
    ...
    if (!error) {
      ...
      g_signal_emit(dir->model, model_signals[ERROR], 0,
                    PLUMA_FILE_BROWSER_ERROR_LOAD_DIRECTORY, error->message);
    ...
    model_add_nodes_from_files(dir->model, parent, async->original_children,
                               files);
...
static void model_iterate_children_cb(GFile *file, GAsyncResult *result,
                                      AsyncNode *async) {
  GError *error = NULL;
...
static void model_load_directory(PlumaFileBrowserStore *model,
                                 FileBrowserNode *node) {
  FileBrowserNodeDir *dir;
  AsyncNode *async;
...
static GList *get_parent_files(PlumaFileBrowserStore *model, GFile *file) {
  GList *result = NULL;
...
static void model_fill(PlumaFileBrowserStore *model, FileBrowserNode *node,
                       GtkTreePath **path) {
  gboolean free_path = FALSE;
  GtkTreeIter iter = {
      0,
...
static void set_virtual_root_from_node(PlumaFileBrowserStore *model,
  ...
  while (prev != model->priv->root) {
    ...
    if (prev != node) file_browser_node_unload(model, next, FALSE);
  ...
  for (item = FILE_BROWSER_NODE_DIR(node)->children; item; item = item->next) {
    ...
    if (NODE_IS_DIR(check)) {
      ...
      check->flags |= PLUMA_FILE_BROWSER_STORE_FLAG_IS_HIDDEN;
    }
  }
  ...
  /* Notify that the virtual-root has changed before loading up new nodes so
     that the "root_changed" signal can be emitted before any "inserted" signals
   */
  g_object_notify(G_OBJECT(model), "virtual-root");

  model_fill(model, NULL, &empty);

  if (!NODE_LOADED(node)) model_load_directory(model, node);
}

static void set_virtual_root_from_file(PlumaFileBrowserStore *model,
                                       GFile *file) {
  GList *files;
  GList *item;
  FileBrowserNode *parent;
...
static FileBrowserNode *model_find_node_children(PlumaFileBrowserStore *model,
                                                 FileBrowserNode *parent,
                                                 GFile *file) {
  FileBrowserNodeDir *dir;
  FileBrowserNode *child;
  ...
  if (!NODE_IS_DIR(parent)) return NULL;
...
static FileBrowserNode *model_find_node(PlumaFileBrowserStore *model,
                                        FileBrowserNode *node, GFile *file) {
  if (node == NULL) node = model->priv->root;

  if (node->file && g_file_equal(node->file, file)) return node;
...
static GQuark pluma_file_browser_store_error_quark(void) {
...
static GFile *unique_new_name(GFile *directory, gchar const *name) {
  ...
  while (newuri == NULL || g_file_query_exists(newuri, NULL)) {
    if (newuri != NULL) g_object_unref(newuri);
...
static PlumaFileBrowserStoreResult model_root_mounted(
    PlumaFileBrowserStore *model, gchar const *virtual_root) {
  model_check_dummy(model, model->priv->root);
  g_object_notify(G_OBJECT(model), "root");

  if (virtual_root != NULL)
    return pluma_file_browser_store_set_virtual_root_from_string(model,
                                                                 virtual_root);
  else
...
static void handle_root_error(PlumaFileBrowserStore *model, GError *error) {
  FileBrowserNode *root;

  g_signal_emit(model, model_signals[ERROR], 0,
                PLUMA_FILE_BROWSER_ERROR_SET_ROOT, error->message);
...
static PlumaFileBrowserStoreResult model_mount_root(
    PlumaFileBrowserStore *model, gchar const *virtual_root) {
  GFileInfo *info;
  GError *error = NULL;
  MountInfo *mount_info;
  ...
  info =
      g_file_query_info(model->priv->root->file, G_FILE_ATTRIBUTE_STANDARD_TYPE,
                        G_FILE_QUERY_INFO_NONE, NULL, &error);
  ...
  if (!info) {
    if (error->code == G_IO_ERROR_NOT_MOUNTED) {
      ...
      mount_info->cancellable =
          g_object_ref(FILE_BROWSER_NODE_DIR(model->priv->root)->cancellable);
...
PlumaFileBrowserStore *pluma_file_browser_store_new(gchar const *root) {
  PlumaFileBrowserStore *obj = PLUMA_FILE_BROWSER_STORE(
      g_object_new(PLUMA_TYPE_FILE_BROWSER_STORE, NULL));
...
void pluma_file_browser_store_set_value(PlumaFileBrowserStore *tree_model,
  ...
  g_return_if_fail(PLUMA_IS_FILE_BROWSER_STORE(tree_model));
  g_return_if_fail(column == PLUMA_FILE_BROWSER_STORE_COLUMN_EMBLEM);
  ...
  if (data) g_return_if_fail(GDK_IS_PIXBUF(data));
  ...
  if (model_node_visibility(tree_model, node)) {
    path = pluma_file_browser_store_get_path(GTK_TREE_MODEL(tree_model), iter);
    row_changed(tree_model, &path, iter);
    gtk_tree_path_free(path);
  }
}

PlumaFileBrowserStoreResult pluma_file_browser_store_set_virtual_root(
    PlumaFileBrowserStore *model, GtkTreeIter *iter) {
...
PlumaFileBrowserStoreResult pluma_file_browser_store_set_virtual_root_top(
    PlumaFileBrowserStore *model) {
  g_return_val_if_fail(PLUMA_IS_FILE_BROWSER_STORE(model),
                       PLUMA_FILE_BROWSER_STORE_RESULT_NO_CHANGE);
...
PlumaFileBrowserStoreResult pluma_file_browser_store_set_virtual_root_up(
    PlumaFileBrowserStore *model) {
  g_return_val_if_fail(PLUMA_IS_FILE_BROWSER_STORE(model),
                       PLUMA_FILE_BROWSER_STORE_RESULT_NO_CHANGE);
...
gboolean pluma_file_browser_store_get_iter_virtual_root(
    PlumaFileBrowserStore *model, GtkTreeIter *iter) {
  g_return_val_if_fail(PLUMA_IS_FILE_BROWSER_STORE(model), FALSE);
  g_return_val_if_fail(iter != NULL, FALSE);

  if (model->priv->virtual_root == NULL) return FALSE;
...
gboolean pluma_file_browser_store_get_iter_root(PlumaFileBrowserStore *model,
  ...
  if (model->priv->root == NULL) return FALSE;
...
void pluma_file_browser_store_cancel_mount_operation(
...
PlumaFileBrowserStoreResult pluma_file_browser_store_set_root_and_virtual_root(
    PlumaFileBrowserStore *model, gchar const *root,
...
PlumaFileBrowserStoreResult pluma_file_browser_store_set_root(
    PlumaFileBrowserStore *model, gchar const *root) {
  g_return_val_if_fail(PLUMA_IS_FILE_BROWSER_STORE(model),
                       PLUMA_FILE_BROWSER_STORE_RESULT_NO_CHANGE);
  return pluma_file_browser_store_set_root_and_virtual_root(model, root, NULL);
}
...
PlumaFileBrowserStoreFilterMode pluma_file_browser_store_get_filter_mode(
    PlumaFileBrowserStore *model) {
  return model->priv->filter_mode;
}

void pluma_file_browser_store_set_filter_mode(
    PlumaFileBrowserStore *model, PlumaFileBrowserStoreFilterMode mode) {
  g_return_if_fail(PLUMA_IS_FILE_BROWSER_STORE(model));

  if (model->priv->filter_mode == mode) return;
...
void pluma_file_browser_store_set_filter_func(
    PlumaFileBrowserStore *model, PlumaFileBrowserStoreFilterFunc func,
    gpointer user_data) {
  g_return_if_fail(PLUMA_IS_FILE_BROWSER_STORE(model));
...
void pluma_file_browser_store_refresh(PlumaFileBrowserStore *model) {
  ...
  if (model->priv->root == NULL || model->priv->virtual_root == NULL) return;
...
gboolean pluma_file_browser_store_rename(PlumaFileBrowserStore *model,
  ...
  if (g_file_move(node->file, file, G_FILE_COPY_NONE, NULL, NULL, NULL, &err)) {
    ...
    if (model_node_visibility(model, node)) {
      ...
      if (error != NULL)
        *error = g_error_new_literal(
            ...
            _("The renamed file is currently filtered out. You need to adjust "
              "your filter settings to make the file visible"));
      return FALSE;
    }
    ...
    if (err) {
      if (error != NULL) {
        *error =
            g_error_new_literal(pluma_file_browser_store_error_quark(),
                                PLUMA_FILE_BROWSER_ERROR_RENAME, err->message);
      }
...
static void delete_file_finished(GFile *file, GAsyncResult *res,
                                 AsyncData *data) {
  GError *error = NULL;
  gboolean ok;
...
static void delete_files(AsyncData *data) {
  ...
  if (data->trash) {
    g_file_trash_async(file, G_PRIORITY_DEFAULT, data->cancellable,
                       (GAsyncReadyCallback)delete_file_finished, data);
  } else {
    g_file_delete_async(file, G_PRIORITY_DEFAULT, data->cancellable,
                        (GAsyncReadyCallback)delete_file_finished, data);
  }
}

PlumaFileBrowserStoreResult pluma_file_browser_store_delete_all(
    PlumaFileBrowserStore *model, GList *rows, gboolean trash) {
  FileBrowserNode *node;
  AsyncData *data;
  GList *files = NULL;
  GList *row;
  ...
  if (rows == NULL) return PLUMA_FILE_BROWSER_STORE_RESULT_NO_CHANGE;
  ...
  for (row = rows; row; row = row->next) {
    ...
    if (!gtk_tree_model_get_iter(GTK_TREE_MODEL(model), &iter, path)) continue;

    /* Skip if the current path is actually a descendant of the
       previous path */
    if (prev != NULL && gtk_tree_path_is_descendant(path, prev)) continue;
...
PlumaFileBrowserStoreResult pluma_file_browser_store_delete(
    PlumaFileBrowserStore *model, GtkTreeIter *iter, gboolean trash) {
  FileBrowserNode *node;
  GList *rows = NULL;
  ...
  if (NODE_IS_DUMMY(node)) return PLUMA_FILE_BROWSER_STORE_RESULT_NO_CHANGE;

  rows =
      g_list_append(NULL, pluma_file_browser_store_get_path_real(model, node));
...
gboolean pluma_file_browser_store_new_file(PlumaFileBrowserStore *model,
  ...
  g_return_val_if_fail(NODE_IS_DIR((FileBrowserNode *)(parent->user_data)),
                       FALSE);
  g_return_val_if_fail(iter != NULL, FALSE);
  ...
  if (!stream) {
    g_signal_emit(model, model_signals[ERROR], 0,
                  PLUMA_FILE_BROWSER_ERROR_NEW_FILE, error->message);
    g_error_free(error);
  } else {
    g_object_unref(stream);
    node = model_add_node_from_file(model, (FileBrowserNode *)parent_node, file,
                                    NULL);
    ...
    if (model_node_visibility(model, node)) {
      ...
      g_signal_emit(model, model_signals[ERROR], 0,
                    ...
                    _("The new file is currently filtered out. You need to "
                      "adjust your filter settings to make the file visible"));
    }
  }
...
gboolean pluma_file_browser_store_new_directory(PlumaFileBrowserStore *model,
   ...
   * file browser pane. */
  file =
      unique_new_name(((FileBrowserNode *)parent_node)->file, _("directory"));

  if (!g_file_make_directory(file, NULL, &error)) {
    g_signal_emit(model, model_signals[ERROR], 0,
                  PLUMA_FILE_BROWSER_ERROR_NEW_DIRECTORY, error->message);
    g_error_free(error);
  } else {
    node = model_add_node_from_file(model, (FileBrowserNode *)parent_node, file,
                                    NULL);
    ...
    if (model_node_visibility(model, node)) {
      ...
      g_signal_emit(
          model, model_signals[ERROR], 0, PLUMA_FILE_BROWSER_ERROR_NEW_FILE,
          _("The new directory is currently filtered out. You need to adjust "
            "your filter settings to make the directory visible"));
    }
  }
...
void _pluma_file_browser_store_register_type(GTypeModule *type_module) {
