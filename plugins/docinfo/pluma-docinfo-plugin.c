...
G_DEFINE_DYNAMIC_TYPE_EXTENDED(
    PlumaDocInfoPlugin, pluma_docinfo_plugin, PEAS_TYPE_EXTENSION_BASE, 0,
...
static void docinfo_dialog_response_cb(GtkDialog *widget, gint res_id,
                                       PlumaDocInfoPluginPrivate *data);

static void docinfo_dialog_destroy_cb(GObject *obj,
                                      PlumaDocInfoPluginPrivate *data) {
...
static DocInfoDialog *get_docinfo_dialog(PlumaDocInfoPlugin *plugin) {
  PlumaDocInfoPluginPrivate *data;
  PlumaWindow *window;
  DocInfoDialog *dialog;
  gchar *data_dir;
  ...
  ret = pluma_utils_get_ui_objects(
      ui_file, NULL, &error_widget, "dialog", &dialog->dialog,
      "docinfo_dialog_content", &content, "file_name_label",
      &dialog->file_name_label, "words_label", &dialog->words_label,
      "bytes_label", &dialog->bytes_label, "lines_label", &dialog->lines_label,
      "chars_label", &dialog->chars_label, "chars_ns_label",
      &dialog->chars_ns_label, "selection_vbox", &dialog->selection_vbox,
      ...
      "selected_chars_ns_label", &dialog->selected_chars_ns_label, NULL);
  ...
  g_signal_connect(dialog->dialog, "destroy",
                   G_CALLBACK(docinfo_dialog_destroy_cb), data);

  g_signal_connect(dialog->dialog, "response",
                   G_CALLBACK(docinfo_dialog_response_cb), data);

  return dialog;
}

static void calculate_info(PlumaDocument *doc, GtkTextIter *start,
                           GtkTextIter *end, gint *chars, gint *words,
                           gint *white_chars, gint *bytes) {
  gchar *text;
  ...
  *bytes = strlen(text);
  ...
  if (*chars > 0) {
    ...
    pango_get_log_attrs(text, -1, 0, pango_language_from_string("C"), attrs,
                        *chars + 1);
...
static void docinfo_real(PlumaDocument *doc, DocInfoDialog *dialog) {
  ...
  pluma_debug_message(DEBUG_PLUGINS, "Chars non-space: %d",
                      chars - white_chars);
...
static void selectioninfo_real(PlumaDocument *doc, DocInfoDialog *dialog) {
  gboolean sel;
  GtkTextIter start, end;
  gint words = 0;
  gint chars = 0;
  ...
  if (sel) {
    ...
    pluma_debug_message(DEBUG_PLUGINS, "Selected chars non-space: %d",
                        chars - white_chars);
...
static void docinfo_cb(GtkAction *action, PlumaDocInfoPlugin *plugin) {
  PlumaDocInfoPluginPrivate *data;
  PlumaWindow *window;
  PlumaDocument *doc;
...
static const GtkActionEntry action_entries[] = {
    {"DocumentStatistics", NULL, N_("_Document Statistics"), NULL,
     N_("Get statistical information on the current document"),
     G_CALLBACK(docinfo_cb)}};

static void update_ui(PlumaDocInfoPluginPrivate *data) {
  PlumaWindow *window;
  PlumaView *view;
  ...
  if (data->dialog != NULL) {
    gtk_dialog_set_response_sensitive(GTK_DIALOG(data->dialog->dialog),
                                      GTK_RESPONSE_OK, (view != NULL));
  }
}

static void pluma_docinfo_plugin_init(PlumaDocInfoPlugin *plugin) {
...
static void pluma_docinfo_plugin_dispose(GObject *object) {
...
static void pluma_docinfo_plugin_set_property(GObject *object, guint prop_id,
                                              const GValue *value,
                                              GParamSpec *pspec) {
  PlumaDocInfoPlugin *plugin = PLUMA_DOCINFO_PLUGIN(object);
...
static void pluma_docinfo_plugin_get_property(GObject *object, guint prop_id,
                                              GValue *value,
                                              GParamSpec *pspec) {
  PlumaDocInfoPlugin *plugin = PLUMA_DOCINFO_PLUGIN(object);
...
static void pluma_docinfo_plugin_activate(PlumaWindowActivatable *activatable) {
  ...
  gtk_ui_manager_add_ui(manager, data->ui_id, MENU_PATH, "DocumentStatistics",
                        "DocumentStatistics", GTK_UI_MANAGER_MENUITEM, FALSE);

  update_ui(data);
}

static void pluma_docinfo_plugin_deactivate(
...
static void pluma_docinfo_plugin_update_state(
    PlumaWindowActivatable *activatable) {
  pluma_debug(DEBUG_PLUGINS);
...
static void pluma_docinfo_plugin_class_init(PlumaDocInfoPluginClass *klass) {
...
static void pluma_docinfo_plugin_class_finalize(
    PlumaDocInfoPluginClass *klass) {
...
G_MODULE_EXPORT void peas_register_types(PeasObjectModule *module) {
  ...
  peas_object_module_register_extension_type(
      module, PLUMA_TYPE_WINDOW_ACTIVATABLE, PLUMA_TYPE_DOCINFO_PLUGIN);
}
