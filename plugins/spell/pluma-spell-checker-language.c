...
static void bind_iso_domains(void) {
...
static void read_iso_3166_entry(xmlTextReaderPtr reader, GHashTable *table) {
  xmlChar *code, *name;
...
static void load_iso_entries(int iso, GFunc read_entry_func,
                             gpointer user_data) {
  ...
  filename =
      g_strdup_printf(ISO_CODES_PREFIX "/share/xml/iso-codes/iso_%d.xml", iso);
  reader = xmlNewTextReaderFilename(filename);
  if (reader == NULL) goto out;

  xmlStrPrintf(iso_entries, sizeof(iso_entries), (const char *)"iso_%d_entries",
               iso);
  ...
  while (ret == 1) {
    ...
    if (state == STATE_ENTRIES && type == XML_READER_TYPE_ELEMENT &&
        xmlStrEqual(tag, iso_entry)) {
      read_entry_func(reader, user_data);
    } else if (state == STATE_START && type == XML_READER_TYPE_ELEMENT &&
               xmlStrEqual(tag, iso_entries)) {
      state = STATE_ENTRIES;
    } else if (state == STATE_ENTRIES && type == XML_READER_TYPE_END_ELEMENT &&
               xmlStrEqual(tag, iso_entries)) {
      state = STATE_STOP;
...
static GHashTable *create_iso_639_table(void) {
  GHashTable *table;

  bind_iso_domains();
  table =
      g_hash_table_new_full(g_str_hash, g_str_equal, (GDestroyNotify)xmlFree,
...
static GHashTable *create_iso_3166_table(void) {
  GHashTable *table;

  bind_iso_domains();
  table = g_hash_table_new_full(g_str_hash, g_str_equal, (GDestroyNotify)g_free,
                                (GDestroyNotify)xmlFree);
...
static char *create_name_for_language(const char *code) {
  char **str;
  char *name = NULL;
...
static gint key_cmp(gconstpointer a, gconstpointer b, gpointer user_data) {
  return strcmp(a, b);
}

static gint lang_cmp(const PlumaSpellCheckerLanguage *a,
                     const PlumaSpellCheckerLanguage *b) {
  return g_utf8_collate(a->name, b->name);
}

static gboolean build_langs_list(const gchar *key, const gchar *value,
                                 gpointer data) {
  ...
  available_languages =
      g_slist_insert_sorted(available_languages, lang, (GCompareFunc)lang_cmp);
...
const GSList *pluma_spell_checker_get_available_languages(void) {
  ...
  if (available_languages_initialized) return available_languages;
  ...
  dicts = g_tree_new_full(key_cmp, NULL, (GDestroyNotify)g_free,
...
const PlumaSpellCheckerLanguage *pluma_spell_checker_language_from_key(
  ...
  while (langs != NULL) {
    ...
    if (g_ascii_strcasecmp(key, l->abrev) == 0) return l;
