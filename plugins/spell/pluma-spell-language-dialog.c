...
enum { COLUMN_LANGUAGE_NAME = 0, COLUMN_LANGUAGE_POINTER, ENCODING_NUM_COLS };
...
G_DEFINE_TYPE(PlumaSpellLanguageDialog, pluma_spell_language_dialog,
              GTK_TYPE_DIALOG)

static void pluma_spell_language_dialog_class_init(
    PlumaSpellLanguageDialogClass *klass) {
  /* GObjectClass *object_class = G_OBJECT_CLASS (klass); */
}

static void dialog_response_handler(GtkDialog *dlg, gint res_id) {
  if (res_id == GTK_RESPONSE_HELP) {
    pluma_help_display(GTK_WINDOW(dlg), NULL, "pluma-spell-checker-plugin");
...
static void language_row_activated(GtkTreeView *tree_view, GtkTreePath *path,
                                   GtkTreeViewColumn *column,
                                   PlumaSpellLanguageDialog *dialog) {
  gtk_dialog_response(GTK_DIALOG(dialog), GTK_RESPONSE_OK);
}

static void create_dialog(PlumaSpellLanguageDialog *dlg,
  ...
  GtkTreeViewColumn *column;
  gchar *ui_file;
  gchar *root_objects[] = {"content", NULL};
  ...
  ret = pluma_utils_get_ui_objects(ui_file, root_objects, &error_widget,
                                   "content", &content, "languages_treeview",
                                   &dlg->languages_treeview, NULL);
  g_free(ui_file);
  ...
  dlg->model = GTK_TREE_MODEL(
      gtk_list_store_new(ENCODING_NUM_COLS, G_TYPE_STRING, G_TYPE_POINTER));
  ...
  column = gtk_tree_view_column_new_with_attributes(
      _("Languages"), cell, "text", COLUMN_LANGUAGE_NAME, NULL);
...
static void pluma_spell_language_dialog_init(PlumaSpellLanguageDialog *dlg) {}

static void populate_language_list(PlumaSpellLanguageDialog *dlg,
  ...
  while (langs) {
    ...
    name = pluma_spell_checker_language_to_string(
        (const PlumaSpellCheckerLanguage *)langs->data);

    gtk_list_store_append(store, &iter);
    gtk_list_store_set(store, &iter, COLUMN_LANGUAGE_NAME, name,
                       COLUMN_LANGUAGE_POINTER, langs->data, -1);

    if (langs->data == cur_lang) {
      GtkTreeSelection *selection;

      selection =
          gtk_tree_view_get_selection(GTK_TREE_VIEW(dlg->languages_treeview));
      g_return_if_fail(selection != NULL);
...
const PlumaSpellCheckerLanguage *pluma_spell_language_get_selected_language(
  ...
  if (!gtk_tree_selection_get_selected(selection, NULL, &iter)) return NULL;
