...
enum { COLUMN_SUGGESTIONS, NUM_COLUMNS };
...
static void close_button_clicked_handler(GtkButton *button,
                                         PlumaSpellCheckerDialog *dlg);
static void suggestions_list_selection_changed_handler(
    GtkTreeSelection *selection, PlumaSpellCheckerDialog *dlg);
...
static void change_all_button_clicked_handler(GtkButton *button,
                                              PlumaSpellCheckerDialog *dlg);
static void suggestions_list_row_activated_handler(
    GtkTreeView *view, GtkTreePath *path, GtkTreeViewColumn *column,
    PlumaSpellCheckerDialog *dlg);

static guint signals[LAST_SIGNAL] = {0};

G_DEFINE_TYPE(PlumaSpellCheckerDialog, pluma_spell_checker_dialog,
              GTK_TYPE_WINDOW)

static void pluma_spell_checker_dialog_dispose(GObject *object) {
...
static void pluma_spell_checker_dialog_class_init(
    PlumaSpellCheckerDialogClass *klass) {
  ...
  signals[IGNORE] = g_signal_new(
      "ignore", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PlumaSpellCheckerDialogClass, ignore), NULL, NULL, NULL,
      G_TYPE_NONE, 1, G_TYPE_STRING);

  signals[IGNORE_ALL] = g_signal_new(
      "ignore_all", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PlumaSpellCheckerDialogClass, ignore_all), NULL, NULL,
      NULL, G_TYPE_NONE, 1, G_TYPE_STRING);

  signals[CHANGE] = g_signal_new(
      "change", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PlumaSpellCheckerDialogClass, change), NULL, NULL, NULL,
      G_TYPE_NONE, 2, G_TYPE_STRING, G_TYPE_STRING);

  signals[CHANGE_ALL] = g_signal_new(
      "change_all", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PlumaSpellCheckerDialogClass, change_all), NULL, NULL,
  ...
  signals[ADD_WORD_TO_PERSONAL] = g_signal_new(
      ...
      G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PlumaSpellCheckerDialogClass, add_word_to_personal), NULL,
      NULL, NULL, G_TYPE_NONE, 1, G_TYPE_STRING);
}

static void create_dialog(PlumaSpellCheckerDialog *dlg, const gchar *data_dir) {
  ...
  gchar *root_objects[] = {
      "content",      "check_word_image", "add_word_image",   "ignore_image",
      "change_image", "ignore_all_image", "change_all_image", NULL};
  gboolean ret;
  gchar *ui_file;
  ...
  ret = pluma_utils_get_ui_objects(
      ui_file, root_objects, &error_widget,

      "content", &content, "misspelled_word_label", &dlg->misspelled_word_label,
      "word_entry", &dlg->word_entry, "check_word_button",
      &dlg->check_word_button, "ignore_button", &dlg->ignore_button,
      "ignore_all_button", &dlg->ignore_all_button, "change_button",
      &dlg->change_button, "change_all_button", &dlg->change_all_button,
      "add_word_button", &dlg->add_word_button, "close_button",
      &dlg->close_button, "suggestions_list", &dlg->suggestions_list,
      "language_label", &dlg->language_label, NULL);
  g_free(ui_file);
  ...
  dlg->suggestions_list_model =
      GTK_TREE_MODEL(gtk_list_store_new(NUM_COLUMNS, G_TYPE_STRING));
  ...
  column = gtk_tree_view_column_new_with_attributes(
      _("Suggestions"), cell, "text", COLUMN_SUGGESTIONS, NULL);
  ...
  g_signal_connect(selection, "changed",
                   G_CALLBACK(suggestions_list_selection_changed_handler), dlg);
...
static void pluma_spell_checker_dialog_init(PlumaSpellCheckerDialog *dlg) {}
...
void pluma_spell_checker_dialog_set_spell_checker(PlumaSpellCheckerDialog *dlg,
  ...
  if (dlg->spell_checker != NULL) g_object_unref(dlg->spell_checker);
  ...
  if (dlg->misspelled_word != NULL)
    pluma_spell_checker_dialog_set_misspelled_word(dlg, dlg->misspelled_word,
                                                   -1);
  else
...
void pluma_spell_checker_dialog_set_misspelled_word(
  ...
  g_return_if_fail(
      !pluma_spell_checker_check_word(dlg->spell_checker, word, -1));

  /* build_suggestions_list */
  if (dlg->misspelled_word != NULL) g_free(dlg->misspelled_word);
...
static void update_suggestions_list_model(PlumaSpellCheckerDialog *dlg,
  ...
  if (suggestions == NULL) {
    ...
    gtk_list_store_set(
        ...
        COLUMN_SUGGESTIONS, _("(no suggested words)"), -1);
  ...
  while (suggestions != NULL) {
    gtk_list_store_append(store, &iter);
    gtk_list_store_set(store, &iter, COLUMN_SUGGESTIONS,
                       (gchar *)suggestions->data, -1);
...
static void check_word_button_clicked_handler(GtkButton *button,
  ...
  if (pluma_spell_checker_check_word(dlg->spell_checker, word, len)) {
    ...
    gtk_list_store_set(store, &iter,
                       /* Translators: Displayed in the "Check Spelling" dialog
                          if the current word isn't misspelled */
                       COLUMN_SUGGESTIONS, _("(correct spelling)"), -1);

    gtk_widget_set_sensitive(dlg->suggestions_list, FALSE);
  } else {
    GSList *sug;

    sug = pluma_spell_checker_get_suggestions(dlg->spell_checker, word, len);
...
static void change_button_clicked_handler(GtkButton *button,
  ...
  pluma_spell_checker_set_correction(dlg->spell_checker, dlg->misspelled_word,
                                     -1, change, -1);
