...
static void pluma_window_activatable_iface_init(
...
G_DEFINE_DYNAMIC_TYPE_EXTENDED(
    PlumaSpellPlugin, pluma_spell_plugin, PEAS_TYPE_EXTENSION_BASE, 0,
...
static const GtkActionEntry action_entries[] = {
    {"CheckSpell", "tools-check-spelling", N_("_Check Spelling..."),
     "<shift>F7", N_("Check the current document for incorrect spelling"),
     G_CALLBACK(spell_cb)},

    {"ConfigSpell", NULL, N_("Set _Language..."), NULL,
     N_("Set the language of the current document"),
     G_CALLBACK(set_language_cb)}};

static const GtkToggleActionEntry toggle_action_entries[] = {
    {"AutoSpell", NULL, N_("_Autocheck Spelling"), "<control>F7",
     N_("Automatically spell-check the current document"),
     G_CALLBACK(auto_spell_cb), FALSE}};
...
static void pluma_spell_plugin_init(PlumaSpellPlugin *plugin) {
...
static void pluma_spell_plugin_dispose(GObject *object) {
...
static void set_spell_language_cb(PlumaSpellChecker *spell,
  ...
  pluma_document_set_metadata(doc, PLUMA_METADATA_ATTRIBUTE_SPELL_LANGUAGE, key,
                              NULL);
}

static void set_language_from_metadata(PlumaSpellChecker *spell,
                                       PlumaDocument *doc) {
  const PlumaSpellCheckerLanguage *lang = NULL;
  gchar *value = NULL;

  value =
      pluma_document_get_metadata(doc, PLUMA_METADATA_ATTRIBUTE_SPELL_LANGUAGE);
...
static PlumaSpellPluginAutocheckType get_autocheck_type(
    PlumaSpellPlugin *plugin) {
  PlumaSpellPluginAutocheckType autocheck_type;

  autocheck_type =
      g_settings_get_enum(plugin->priv->settings, AUTOCHECK_TYPE_KEY);

  return autocheck_type;
}

static void set_autocheck_type(GSettings *settings,
                               PlumaSpellPluginAutocheckType autocheck_type) {
...
static PlumaSpellChecker *get_spell_checker_from_document(PlumaDocument *doc) {
  ...
  if (data == NULL) {
    ...
    g_object_set_qdata_full(G_OBJECT(doc), spell_checker_id, spell,
                            (GDestroyNotify)g_object_unref);

    g_signal_connect(spell, "set_language", G_CALLBACK(set_spell_language_cb),
                     doc);
  } else {
...
static CheckRange *get_check_range(PlumaDocument *doc) {
  CheckRange *range;
...
static void update_current(PlumaDocument *doc, gint current) {
  CheckRange *range;
  GtkTextIter iter;
...
static void set_check_range(PlumaDocument *doc, GtkTextIter *start,
                            GtkTextIter *end) {
  CheckRange *range;
  GtkTextIter iter;
  ...
  if (range == NULL) {
    ...
    range->start_mark = gtk_text_buffer_create_mark(
        GTK_TEXT_BUFFER(doc), "check_range_start_mark", &iter, TRUE);

    range->end_mark = gtk_text_buffer_create_mark(
        GTK_TEXT_BUFFER(doc), "check_range_end_mark", &iter, FALSE);

    range->current_mark = gtk_text_buffer_create_mark(
        GTK_TEXT_BUFFER(doc), "check_range_current_mark", &iter, TRUE);

    g_object_set_qdata_full(G_OBJECT(doc), check_range_id, range,
...
static gchar *get_current_word(PlumaDocument *doc, gint *start, gint *end) {
  ...
  gtk_text_buffer_get_iter_at_mark(GTK_TEXT_BUFFER(doc), &current_iter,
                                   range->current_mark);
  ...
  *end = MIN(gtk_text_iter_get_offset(&end_iter), range_end);
  ...
  pluma_debug_message(DEBUG_PLUGINS, "Current word extends [%d, %d]", *start,
                      *end);

  if (!(*start < *end)) return NULL;

  return gtk_text_buffer_get_slice(GTK_TEXT_BUFFER(doc), &current_iter,
                                   &end_iter, TRUE);
}

static gboolean goto_next_word(PlumaDocument *doc) {
  CheckRange *range;
  GtkTextIter current_iter;
...
static gchar *get_next_misspelled_word(PlumaView *view) {
  PlumaDocument *doc;
  CheckRange *range;
  gint start, end;
  gchar *word;
  ...
  while (pluma_spell_checker_check_word(spell, word, -1)) {
    ...
    if (!goto_next_word(doc)) return NULL;
...
static void ignore_cb(PlumaSpellCheckerDialog *dlg, const gchar *w,
                      PlumaView *view) {
  gchar *word = NULL;
  ...
  pluma_spell_checker_dialog_set_misspelled_word(
      PLUMA_SPELL_CHECKER_DIALOG(dlg), word, -1);

  g_free(word);
}

static void change_cb(PlumaSpellCheckerDialog *dlg, const gchar *word,
                      const gchar *change, PlumaView *view) {
  PlumaDocument *doc;
  CheckRange *range;
  gchar *w = NULL;
  ...
  gtk_text_buffer_get_iter_at_offset(GTK_TEXT_BUFFER(doc), &start,
                                     range->mw_start);
  if (range->mw_end < 0)
    gtk_text_buffer_get_end_iter(GTK_TEXT_BUFFER(doc), &end);
  else
    gtk_text_buffer_get_iter_at_offset(GTK_TEXT_BUFFER(doc), &end,
                                       range->mw_end);
...
static void change_all_cb(PlumaSpellCheckerDialog *dlg, const gchar *word,
                          const gchar *change, PlumaView *view) {
  PlumaDocument *doc;
  CheckRange *range;
  gchar *w = NULL;
...
static void add_word_cb(PlumaSpellCheckerDialog *dlg, const gchar *word,
                        PlumaView *view) {
  g_return_if_fail(view != NULL);
  g_return_if_fail(word != NULL);
...
static void language_dialog_response(GtkDialog *dlg, gint res_id,
                                     PlumaSpellChecker *spell) {
  if (res_id == GTK_RESPONSE_OK) {
    const PlumaSpellCheckerLanguage *lang;

    lang = pluma_spell_language_get_selected_language(
        PLUMA_SPELL_LANGUAGE_DIALOG(dlg));
    if (lang != NULL) pluma_spell_checker_set_language(spell, lang);
  }

  gtk_widget_destroy(GTK_WIDGET(dlg));
}

static SpellConfigureDialog *get_configure_dialog(PlumaSpellPlugin *plugin) {
  ...
  ret = pluma_utils_get_ui_objects(
      ui_file, root_objects, &error_widget, "spell_dialog_content",
      &dialog->content, "autocheck_never", &dialog->never, "autocheck_document",
      &dialog->document, "autocheck_always", &dialog->always, NULL);
...
static void configure_dialog_button_toggled(GtkToggleButton *button,
  ...
  if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(dialog->always))) {
    set_autocheck_type(dialog->settings, AUTOCHECK_ALWAYS);
                 ...
                 GTK_TOGGLE_BUTTON(dialog->document))) {
...
static void configure_dialog_destroyed(GtkWidget *widget, gpointer data) {
...
static void set_language_cb(GtkAction *action, PlumaSpellPlugin *plugin) {
  ...
  data_dir = peas_extension_base_get_data_dir(PEAS_EXTENSION_BASE(plugin));
  dlg = pluma_spell_language_dialog_new(GTK_WINDOW(window), lang, data_dir);
  g_free(data_dir);
  ...
  g_signal_connect(dlg, "response", G_CALLBACK(language_dialog_response),
                   spell);
...
static void spell_cb(GtkAction *action, PlumaSpellPlugin *plugin) {
  ...
  if (gtk_text_buffer_get_char_count(GTK_TEXT_BUFFER(doc)) <= 0) {
    ...
    pluma_statusbar_flash_message(PLUMA_STATUSBAR(statusbar), data->message_cid,
                                  _("The document is empty."));

    return;
  }

  if (!gtk_text_buffer_get_selection_bounds(GTK_TEXT_BUFFER(doc), &start,
                                            &end)) {
...
static void set_auto_spell(PlumaWindow *window, PlumaDocument *doc,
                           gboolean active) {
  ...
  if (active) {
    ...
    if (autospell != NULL) pluma_automatic_spell_checker_free(autospell);
  }
}
...
static void auto_spell_cb(GtkAction *action, PlumaSpellPlugin *plugin) {
  ...
  pluma_debug_message(DEBUG_PLUGINS, active ? "Auto Spell activated"
                                            : "Auto Spell deactivated");

  doc = pluma_window_get_active_document(window);
  if (doc == NULL) return;

  if (get_autocheck_type(plugin) == AUTOCHECK_DOCUMENT) {
    pluma_document_set_metadata(doc, PLUMA_METADATA_ATTRIBUTE_SPELL_ENABLED,
                                active ? "1" : NULL, NULL);
  }
...
static void update_ui(PlumaSpellPlugin *plugin) {
  PlumaSpellPluginPrivate *data;
  PlumaWindow *window;
  PlumaDocument *doc;
  PlumaView *view;
...
static void set_auto_spell_from_metadata(PlumaSpellPlugin *plugin,
                                         PlumaDocument *doc,
                                         GtkActionGroup *action_group) {
  ...
  switch (autocheck_type) {
    ...
    case AUTOCHECK_DOCUMENT: {
      active_str = pluma_document_get_metadata(
          doc, PLUMA_METADATA_ATTRIBUTE_SPELL_ENABLED);
      break;
    }
...
static void on_document_loaded(PlumaDocument *doc, const GError *error,
                               PlumaSpellPlugin *plugin) {
  if (error == NULL) {
    PlumaSpellChecker *spell;

    spell = PLUMA_SPELL_CHECKER(
        g_object_get_qdata(G_OBJECT(doc), spell_checker_id));
    if (spell != NULL) {
      set_language_from_metadata(spell, doc);
    }
...
static void on_document_saved(PlumaDocument *doc, const GError *error,
                              PlumaSpellPlugin *plugin) {
  PlumaAutomaticSpellChecker *autospell;
  PlumaSpellChecker *spell;
  const gchar *key;
  ...
  autospell = pluma_automatic_spell_checker_get_from_document(doc);
  spell =
      PLUMA_SPELL_CHECKER(g_object_get_qdata(G_OBJECT(doc), spell_checker_id));

  if (spell != NULL) {
    key = pluma_spell_checker_language_to_key(
        pluma_spell_checker_get_language(spell));
  } else {
    key = NULL;
  }
  ...
  if (get_autocheck_type(plugin) == AUTOCHECK_DOCUMENT) {
    pluma_document_set_metadata(doc, PLUMA_METADATA_ATTRIBUTE_SPELL_ENABLED,
                                ...
                                PLUMA_METADATA_ATTRIBUTE_SPELL_LANGUAGE, key,
                                NULL);
  } else {
...
static void tab_added_cb(PlumaWindow *window, PlumaTab *tab,
                         PlumaSpellPlugin *plugin) {
  PlumaDocument *doc;
  gchar *uri;
  ...
  g_signal_connect(doc, "loaded", G_CALLBACK(on_document_loaded), plugin);

  g_signal_connect(doc, "saved", G_CALLBACK(on_document_saved), plugin);
}

static void tab_removed_cb(PlumaWindow *window, PlumaTab *tab,
                           PlumaSpellPlugin *plugin) {
  PlumaDocument *doc;
...
static void pluma_spell_plugin_activate(PlumaWindowActivatable *activatable) {
  PlumaSpellPlugin *plugin;
  PlumaSpellPluginPrivate *data;
  PlumaWindow *window;
  ...
  gtk_action_group_add_toggle_actions(data->action_group, toggle_action_entries,
  ...
  gtk_ui_manager_add_ui(manager, data->ui_id, MENU_PATH, "CheckSpell",
                        "CheckSpell", GTK_UI_MANAGER_MENUITEM, FALSE);

  gtk_ui_manager_add_ui(manager, data->ui_id, MENU_PATH, "AutoSpell",
                        "AutoSpell", GTK_UI_MANAGER_MENUITEM, FALSE);

  gtk_ui_manager_add_ui(manager, data->ui_id, MENU_PATH, "ConfigSpell",
                        "ConfigSpell", GTK_UI_MANAGER_MENUITEM, FALSE);
  ...
  for (l = docs; l != NULL; l = g_list_next(l)) {
    ...
    set_auto_spell_from_metadata(plugin, doc, data->action_group);
  ...
  data->tab_added_id =
      g_signal_connect(window, "tab-added", G_CALLBACK(tab_added_cb), plugin);
  data->tab_removed_id = g_signal_connect(window, "tab-removed",
                                          G_CALLBACK(tab_removed_cb), plugin);
}

static void pluma_spell_plugin_deactivate(PlumaWindowActivatable *activatable) {
  PlumaSpellPluginPrivate *data;
  PlumaWindow *window;
  GtkUIManager *manager;
...
static void pluma_spell_plugin_set_property(GObject *object, guint prop_id,
                                            const GValue *value,
                                            GParamSpec *pspec) {
  PlumaSpellPlugin *plugin = PLUMA_SPELL_PLUGIN(object);
...
static void pluma_spell_plugin_get_property(GObject *object, guint prop_id,
...
static GtkWidget *pluma_spell_plugin_create_configure_widget(
    PeasGtkConfigurable *configurable) {
  SpellConfigureDialog *dialog;

  dialog = get_configure_dialog(PLUMA_SPELL_PLUGIN(configurable));

  g_signal_connect(dialog->always, "toggled",
                   G_CALLBACK(configure_dialog_button_toggled), dialog);
  g_signal_connect(dialog->document, "toggled",
                   G_CALLBACK(configure_dialog_button_toggled), dialog);
  g_signal_connect(dialog->never, "toggled",
                   G_CALLBACK(configure_dialog_button_toggled), dialog);

  g_signal_connect(dialog->content, "destroy",
                   G_CALLBACK(configure_dialog_destroyed), dialog);

  return dialog->content;
}

static void pluma_spell_plugin_class_init(PlumaSpellPluginClass *klass) {
  ...
  if (check_range_id == 0) check_range_id = g_quark_from_string("CheckRangeID");
}

static void pluma_spell_plugin_class_finalize(PlumaSpellPluginClass *klass) {
...
G_MODULE_EXPORT void peas_register_types(PeasObjectModule *module) {
  ...
  peas_object_module_register_extension_type(
      module, PLUMA_TYPE_WINDOW_ACTIVATABLE, PLUMA_TYPE_SPELL_PLUGIN);

  peas_object_module_register_extension_type(module, PEAS_GTK_TYPE_CONFIGURABLE,
