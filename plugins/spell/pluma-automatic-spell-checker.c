...
static void view_destroy(PlumaView *view, PlumaAutomaticSpellChecker *spell) {
  pluma_automatic_spell_checker_detach_view(spell, view);
}

static void check_word(PlumaAutomaticSpellChecker *spell, GtkTextIter *start,
                       GtkTextIter *end) {
  gchar *word;

  word =
      gtk_text_buffer_get_text(GTK_TEXT_BUFFER(spell->doc), start, end, FALSE);
  ...
  */
  ...
  if (!pluma_spell_checker_check_word(spell->spell_checker, word, -1)) {
    ...
    g_print ("Apply tag: [%d - %d]\n", gtk_text_iter_get_offset (start),
                                    gtk_text_iter_get_offset (end));
    */
    gtk_text_buffer_apply_tag(GTK_TEXT_BUFFER(spell->doc), spell->tag_highlight,
                              start, end);
  }
...
static void check_range(PlumaAutomaticSpellChecker *spell, GtkTextIter start,
  ...
  */
  ...
  if (gtk_text_iter_inside_word(&end)) gtk_text_iter_forward_word_end(&end);
  ...
  gtk_text_buffer_remove_tag(GTK_TEXT_BUFFER(spell->doc), spell->tag_highlight,
                             &start, &end);
...
static void check_deferred_range(PlumaAutomaticSpellChecker *spell,
                                 gboolean force_all) {
  GtkTextIter start, end;

  gtk_text_buffer_get_iter_at_mark(GTK_TEXT_BUFFER(spell->doc), &start,
                                   spell->mark_insert_start);
  gtk_text_buffer_get_iter_at_mark(GTK_TEXT_BUFFER(spell->doc), &end,
...
static void insert_text_before(GtkTextBuffer *buffer, GtkTextIter *iter,
                               gchar *text, gint len,
                               PlumaAutomaticSpellChecker *spell) {
  gtk_text_buffer_move_mark(buffer, spell->mark_insert_start, iter);
}

static void insert_text_after(GtkTextBuffer *buffer, GtkTextIter *iter,
                              gchar *text, gint len,
                              PlumaAutomaticSpellChecker *spell) {
  GtkTextIter start;
...
static void delete_range_after(GtkTextBuffer *buffer, GtkTextIter *start,
                               GtkTextIter *end,
                               PlumaAutomaticSpellChecker *spell) {
  check_range(spell, *start, *end, FALSE);
}

static void mark_set(GtkTextBuffer *buffer, GtkTextIter *iter,
                     GtkTextMark *mark, PlumaAutomaticSpellChecker *spell) {
...
static void get_word_extents_from_mark(GtkTextBuffer *buffer,
...
static void remove_tag_to_word(PlumaAutomaticSpellChecker *spell,
                               const gchar *word) {
  GtkTextIter iter;
  ...
  while (found) {
    found = gtk_text_iter_forward_search(
        &iter, word, GTK_TEXT_SEARCH_VISIBLE_ONLY | GTK_TEXT_SEARCH_TEXT_ONLY,
    ...
    if (found) {
      if (gtk_text_iter_starts_word(&match_start) &&
        ...
        gtk_text_buffer_remove_tag(GTK_TEXT_BUFFER(spell->doc),
                                   spell->tag_highlight, &match_start,
                                   &match_end);
      }
...
static void add_to_dictionary(GtkWidget *menuitem,
                              PlumaAutomaticSpellChecker *spell) {
  gchar *word;

  GtkTextIter start, end;

  get_word_extents_from_mark(GTK_TEXT_BUFFER(spell->doc), &start, &end,
                             spell->mark_click);

  word = gtk_text_buffer_get_text(GTK_TEXT_BUFFER(spell->doc), &start, &end,
                                  FALSE);
...
static void ignore_all(GtkWidget *menuitem, PlumaAutomaticSpellChecker *spell) {
  gchar *word;
...
static void replace_word(GtkWidget *menuitem,
                         PlumaAutomaticSpellChecker *spell) {
  gchar *oldword;
  const gchar *newword;
  ...
  oldword = gtk_text_buffer_get_text(GTK_TEXT_BUFFER(spell->doc), &start, &end,
                                     FALSE);
  ...
  pluma_spell_checker_set_correction(spell->spell_checker, oldword,
                                     strlen(oldword), newword, strlen(newword));

  g_free(oldword);
}

static GtkWidget *build_suggestion_menu(PlumaAutomaticSpellChecker *spell,
                                        const gchar *word) {
  ...
  suggestions =
      pluma_spell_checker_get_suggestions(spell->spell_checker, word, -1);
  ...
  if (suggestions == NULL) {
    ...
    while (suggestions != NULL) {
      ...
      g_object_set_qdata_full(G_OBJECT(mi), suggestion_id,
  ...
  gtk_image_menu_item_set_image(
      ...
      gtk_image_new_from_icon_name("go-bottom", GTK_ICON_SIZE_MENU));
...
static void add_word_signal_cb(PlumaSpellChecker *checker, const gchar *word,
                               gint len, PlumaAutomaticSpellChecker *spell) {
  gchar *w;
...
static void set_language_cb(PlumaSpellChecker *checker,
                            const PlumaSpellCheckerLanguage *lang,
                            PlumaAutomaticSpellChecker *spell) {
  pluma_automatic_spell_checker_recheck_all(spell);
}

static void clear_session_cb(PlumaSpellChecker *checker,
                             PlumaAutomaticSpellChecker *spell) {
  pluma_automatic_spell_checker_recheck_all(spell);
}
 ...
 */
static gboolean button_press_event(GtkTextView *view, GdkEventButton *event,
                                   PlumaAutomaticSpellChecker *spell) {
  if (event->button == 3) {
    gint x, y;
    GtkTextIter iter;
    ...
    if (spell->deferred_check) check_deferred_range(spell, TRUE);

    gtk_text_view_window_to_buffer_coords(view, GTK_TEXT_WINDOW_TEXT, event->x,
                                          event->y, &x, &y);
 ...
 */
static gboolean popup_menu_event(GtkTextView *view,
                                 PlumaAutomaticSpellChecker *spell) {
  GtkTextIter iter;
...
static void tag_table_changed(GtkTextTagTable *table,
                              PlumaAutomaticSpellChecker *spell) {
...
static void tag_added_or_removed(GtkTextTagTable *table, GtkTextTag *tag,
                                 PlumaAutomaticSpellChecker *spell) {
  tag_table_changed(table, spell);
}

static void tag_changed(GtkTextTagTable *table, GtkTextTag *tag,
                        gboolean size_changed,
                        PlumaAutomaticSpellChecker *spell) {
  tag_table_changed(table, spell);
}

static void highlight_updated(GtkSourceBuffer *buffer, GtkTextIter *start,
                              GtkTextIter *end,
                              PlumaAutomaticSpellChecker *spell) {
  check_range(spell, *start, *end, FALSE);
}

static void spell_tag_destroyed(PlumaAutomaticSpellChecker *spell,
...
PlumaAutomaticSpellChecker *pluma_automatic_spell_checker_new(
  ...
  g_return_val_if_fail(
      (spell = pluma_automatic_spell_checker_get_from_document(doc)) == NULL,
      spell);
  ...
  g_object_set_qdata_full(
      G_OBJECT(doc), automatic_spell_checker_id, spell,
  ...
  g_signal_connect_after(doc, "insert-text", G_CALLBACK(insert_text_after),
                         spell);
  g_signal_connect_after(doc, "delete-range", G_CALLBACK(delete_range_after),
                         spell);
  g_signal_connect(doc, "mark-set", G_CALLBACK(mark_set), spell);

  g_signal_connect(doc, "highlight-updated", G_CALLBACK(highlight_updated),
                   spell);
  ...
  spell->tag_highlight =
      gtk_text_buffer_create_tag(GTK_TEXT_BUFFER(doc), "gtkspell-misspelled",
                                 "underline", PANGO_UNDERLINE_ERROR, NULL);

  g_object_weak_ref(G_OBJECT(spell->tag_highlight),
                    (GWeakNotify)spell_tag_destroyed, spell);
  ...
  g_signal_connect(tag_table, "tag-added", G_CALLBACK(tag_added_or_removed),
                   spell);
  g_signal_connect(tag_table, "tag-removed", G_CALLBACK(tag_added_or_removed),
                   spell);
  g_signal_connect(tag_table, "tag-changed", G_CALLBACK(tag_changed), spell);

  /* we create the mark here, but we don't use it until text is
   * inserted, so we don't really care where iter points.  */
  gtk_text_buffer_get_bounds(GTK_TEXT_BUFFER(doc), &start, &end);

  spell->mark_insert_start = gtk_text_buffer_get_mark(
      GTK_TEXT_BUFFER(doc), "pluma-automatic-spell-checker-insert-start");

  if (spell->mark_insert_start == NULL) {
    spell->mark_insert_start = gtk_text_buffer_create_mark(
        GTK_TEXT_BUFFER(doc), "pluma-automatic-spell-checker-insert-start",
        &start, TRUE);
  } else {
    gtk_text_buffer_move_mark(GTK_TEXT_BUFFER(doc), spell->mark_insert_start,
                              &start);
  }

  spell->mark_insert_end = gtk_text_buffer_get_mark(
      GTK_TEXT_BUFFER(doc), "pluma-automatic-spell-checker-insert-end");

  if (spell->mark_insert_end == NULL) {
    spell->mark_insert_end = gtk_text_buffer_create_mark(
        GTK_TEXT_BUFFER(doc), "pluma-automatic-spell-checker-insert-end",
        &start, TRUE);
  } else {
    gtk_text_buffer_move_mark(GTK_TEXT_BUFFER(doc), spell->mark_insert_end,
                              &start);
  }

  spell->mark_click = gtk_text_buffer_get_mark(
      GTK_TEXT_BUFFER(doc), "pluma-automatic-spell-checker-click");

  if (spell->mark_click == NULL) {
    spell->mark_click = gtk_text_buffer_create_mark(
        GTK_TEXT_BUFFER(doc), "pluma-automatic-spell-checker-click", &start,
        TRUE);
  } else {
...
PlumaAutomaticSpellChecker *pluma_automatic_spell_checker_get_from_document(
  ...
  if (automatic_spell_checker_id == 0) return NULL;
...
void pluma_automatic_spell_checker_free(PlumaAutomaticSpellChecker *spell) {
  g_return_if_fail(spell != NULL);
  g_return_if_fail(
      pluma_automatic_spell_checker_get_from_document(spell->doc) == spell);

  if (automatic_spell_checker_id == 0) return;
...
static void pluma_automatic_spell_checker_free_internal(
  ...
  if (table != NULL && spell->tag_highlight != NULL) {
    ...
    g_signal_handlers_disconnect_matched(G_OBJECT(table), G_SIGNAL_MATCH_DATA,
                                         0, 0, NULL, NULL, spell);
  ...
  g_signal_handlers_disconnect_matched(
      G_OBJECT(spell->doc), G_SIGNAL_MATCH_DATA, 0, 0, NULL, NULL, spell);
  ...
  while (list != NULL) {
    ...
    g_signal_handlers_disconnect_matched(G_OBJECT(view), G_SIGNAL_MATCH_DATA, 0,
                                         0, NULL, NULL, spell);
...
void pluma_automatic_spell_checker_attach_view(
  ...
  g_signal_connect(view, "button-press-event", G_CALLBACK(button_press_event),
                   spell);
  g_signal_connect(view, "popup-menu", G_CALLBACK(popup_menu_event), spell);
  g_signal_connect(view, "populate-popup", G_CALLBACK(populate_popup), spell);
  g_signal_connect(view, "destroy", G_CALLBACK(view_destroy), spell);
