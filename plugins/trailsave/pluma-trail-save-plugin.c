...
G_DEFINE_DYNAMIC_TYPE_EXTENDED(
    PlumaTrailSavePlugin, pluma_trail_save_plugin, PEAS_TYPE_EXTENSION_BASE, 0,
...
static void strip_trailing_spaces(GtkTextBuffer *text_buffer) {
  ...
  for (line_num = 0; line_num < line_count; ++line_num) {
    ...
    slice =
        gtk_text_buffer_get_slice(text_buffer, &line_start, &line_end, TRUE);
    ...
    if (should_strip) {
      gtk_text_buffer_get_iter_at_line_index(text_buffer, &strip_start,
                                             line_num, strip_start_index);
      gtk_text_buffer_get_iter_at_line_index(text_buffer, &strip_end, line_num,
                                             strip_end_index);
...
static void on_save(PlumaDocument *document, const gchar *uri,
                    PlumaEncoding *encoding, PlumaDocumentSaveFlags save_flags,
...
static void on_tab_added(PlumaWindow *window, PlumaTab *tab,
                         PlumaTrailSavePlugin *plugin) {
...
static void on_tab_removed(PlumaWindow *window, PlumaTab *tab,
                           PlumaTrailSavePlugin *plugin) {
  PlumaDocument *document;
...
static void pluma_trail_save_plugin_activate(
  ...
  for (documents_iter = documents; documents_iter && documents_iter->data;
...
static void pluma_trail_save_plugin_deactivate(
    PlumaWindowActivatable *activatable) {
  PlumaTrailSavePlugin *plugin;
  PlumaWindow *window;
  GList *documents;
...
static void pluma_trail_save_plugin_init(PlumaTrailSavePlugin *plugin) {
...
static void pluma_trail_save_plugin_dispose(GObject *object) {
...
static void pluma_trail_save_plugin_set_property(GObject *object, guint prop_id,
                                                 const GValue *value,
                                                 GParamSpec *pspec) {
  PlumaTrailSavePlugin *plugin = PLUMA_TRAIL_SAVE_PLUGIN(object);
...
static void pluma_trail_save_plugin_get_property(GObject *object, guint prop_id,
                                                 GValue *value,
                                                 GParamSpec *pspec) {
  PlumaTrailSavePlugin *plugin = PLUMA_TRAIL_SAVE_PLUGIN(object);
...
static void pluma_trail_save_plugin_class_init(
    PlumaTrailSavePluginClass *klass) {
...
static void pluma_trail_save_plugin_class_finalize(
    PlumaTrailSavePluginClass *klass) {
...
G_MODULE_EXPORT void peas_register_types(PeasObjectModule *module) {
  ...
  peas_object_module_register_extension_type(
      module, PLUMA_TYPE_WINDOW_ACTIVATABLE, PLUMA_TYPE_TRAIL_SAVE_PLUGIN);
}
