...
G_DEFINE_DYNAMIC_TYPE_EXTENDED(
    PlumaSortPlugin, pluma_sort_plugin, PEAS_TYPE_EXTENSION_BASE, 0,
...
static const GtkActionEntry action_entries[] = {
    {"Sort", "view-sort-ascending", N_("S_ort..."), NULL,
     N_("Sort the current document or selection"), G_CALLBACK(sort_cb)}};

static void do_sort(PlumaSortPlugin *plugin) {
  ...
  if (gtk_toggle_button_get_active(
          GTK_TOGGLE_BUTTON(priv->ignore_case_checkbutton))) {
  ...
  gtk_source_buffer_sort_lines(GTK_SOURCE_BUFFER(doc), &priv->start, &priv->end,
                               sort_flags, starting_column);

  pluma_debug_message(DEBUG_PLUGINS, "Done.");
}

static void sort_dialog_response_handler(GtkDialog *dialog, gint res_id,
  ...
  switch (res_id) {
    ...
    case GTK_RESPONSE_HELP:
      pluma_help_display(GTK_WINDOW(dialog), NULL, "pluma-sort-plugin");
      break;
...
static void get_current_selection(PlumaSortPlugin *plugin) {
  PlumaSortPluginPrivate *priv;
  PlumaDocument *doc;
  ...
  if (!gtk_text_buffer_get_selection_bounds(GTK_TEXT_BUFFER(doc), &priv->start,
                                            &priv->end)) {
...
static void create_sort_dialog(PlumaSortPlugin *plugin) {
  PlumaSortPluginPrivate *priv;
  ...
  ret = pluma_utils_get_ui_objects(
      ui_file, NULL, &error_widget, "sort_dialog", &priv->dialog,
      ...
      "remove_dups_checkbutton", &priv->remove_dups_checkbutton, NULL);
  g_free(data_dir);
  g_free(ui_file);
  ...
  if (!ret) {
    ...
    pluma_warning(GTK_WINDOW(priv->window), "%s", err_message);
  ...
  g_signal_connect(priv->dialog, "destroy", G_CALLBACK(gtk_widget_destroyed),
                   &priv->dialog);
...
static void pluma_sort_plugin_set_property(GObject *object, guint prop_id,
                                           const GValue *value,
                                           GParamSpec *pspec) {
...
static void pluma_sort_plugin_get_property(GObject *object, guint prop_id,
...
static void update_ui(PlumaSortPlugin *plugin) {
  PlumaView *view;
  ...
  gtk_action_group_set_sensitive(
      plugin->priv->ui_action_group,
      (view != NULL) && gtk_text_view_get_editable(GTK_TEXT_VIEW(view)));
}

static void pluma_sort_plugin_activate(PlumaWindowActivatable *activatable) {
  PlumaSortPluginPrivate *priv;
  ...
  gtk_action_group_set_translation_domain(priv->ui_action_group,
                                          GETTEXT_PACKAGE);
  gtk_action_group_add_actions(priv->ui_action_group, action_entries,
  ...
  gtk_ui_manager_add_ui(manager, priv->ui_id, MENU_PATH, "Sort", "Sort",
                        GTK_UI_MANAGER_MENUITEM, FALSE);

  update_ui(PLUMA_SORT_PLUGIN(activatable));
}

static void pluma_sort_plugin_deactivate(PlumaWindowActivatable *activatable) {
  PlumaSortPluginPrivate *priv;
  GtkUIManager *manager;
...
static void pluma_sort_plugin_update_state(
...
static void pluma_sort_plugin_class_init(PlumaSortPluginClass *klass) {
...
static void pluma_sort_plugin_class_finalize(PlumaSortPluginClass *klass) {
  /* dummy function - used by G_DEFINE_DYNAMIC_TYPE_EXTENDED */
}

static void peas_activatable_iface_init(
...
G_MODULE_EXPORT void peas_register_types(PeasObjectModule *module) {
  ...
  peas_object_module_register_extension_type(
      module, PLUMA_TYPE_WINDOW_ACTIVATABLE, PLUMA_TYPE_SORT_PLUGIN);
}
