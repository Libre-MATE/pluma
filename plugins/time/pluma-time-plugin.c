...
G_DEFINE_DYNAMIC_TYPE_EXTENDED(
    PlumaTimePlugin, pluma_time_plugin, PEAS_TYPE_EXTENSION_BASE, 0,
...
static const GtkActionEntry action_entries[] = {
    {"InsertDateAndTime", NULL, N_("In_sert Date and Time..."), NULL,
     N_("Insert current date and time at the cursor position"),
     G_CALLBACK(time_cb)},
};

static void pluma_time_plugin_init(PlumaTimePlugin *plugin) {
...
static void pluma_time_plugin_finalize(GObject *object) {
...
static void pluma_time_plugin_dispose(GObject *object) {
  PlumaTimePlugin *plugin = PLUMA_TIME_PLUGIN(object);
...
static void update_ui(PlumaTimePluginPrivate *data) {
  PlumaWindow *window;
  PlumaView *view;
  GtkAction *action;
  ...
  action = gtk_action_group_get_action(data->action_group, "InsertDateAndTime");
  gtk_action_set_sensitive(action, (view != NULL) && gtk_text_view_get_editable(
                                                         GTK_TEXT_VIEW(view)));
}

static void pluma_time_plugin_activate(PlumaWindowActivatable *activatable) {
  ...
  gtk_ui_manager_add_ui(manager, data->ui_id, MENU_PATH, "InsertDateAndTime",
                        "InsertDateAndTime", GTK_UI_MANAGER_MENUITEM, FALSE);

  update_ui(data);
}

static void pluma_time_plugin_deactivate(PlumaWindowActivatable *activatable) {
  PlumaTimePluginPrivate *data;
  PlumaWindow *window;
  GtkUIManager *manager;
...
static void pluma_time_plugin_update_state(
...
static PlumaTimePluginPromptType get_prompt_type(PlumaTimePlugin *plugin) {
...
static void set_prompt_type(GSettings *settings,
                            PlumaTimePluginPromptType prompt_type) {
  if (!g_settings_is_writable(settings, PROMPT_TYPE_KEY)) {
    return;
  }

  g_settings_set_enum(settings, PROMPT_TYPE_KEY, prompt_type);
}

/* The selected format in the list */
static gchar *get_selected_format(PlumaTimePlugin *plugin) {
  gchar *sel_format;
...
static void set_selected_format(GSettings *settings, const gchar *format) {
  g_return_if_fail(format != NULL);

  if (!g_settings_is_writable(settings, SELECTED_FORMAT_KEY)) {
    return;
  }
...
static gchar *get_custom_format(PlumaTimePlugin *plugin) {
  gchar *format;
...
static void set_custom_format(GSettings *settings, const gchar *format) {
  g_return_if_fail(format != NULL);

  if (!g_settings_is_writable(settings, CUSTOM_FORMAT_KEY)) return;

  g_settings_set_string(settings, CUSTOM_FORMAT_KEY, format);
}
...
static void choose_format_dialog_destroyed(GtkWidget *widget, gpointer data) {
  pluma_debug(DEBUG_PLUGINS);
...
static void updated_custom_format_example(GtkEntry *format_entry,
...
static void choose_format_dialog_button_toggled(GtkToggleButton *button,
...
static void configure_dialog_selection_changed(GtkTreeSelection *selection,
                                               TimeConfigureDialog *dialog) {
  gint sel_format;
...
static TimeConfigureDialog *get_configure_dialog(PlumaTimePlugin *plugin) {
  ...
  ret = pluma_utils_get_ui_objects(
      ui_file, root_objects, &error_widget, "time_dialog_content",
      &dialog->content, "formats_viewport", &viewport, "formats_tree",
      &dialog->list, "always_prompt", &dialog->prompt, "never_prompt",
      &dialog->use_list, "use_custom", &dialog->custom, "custom_entry",
      &dialog->custom_entry, "custom_format_example",
      &dialog->custom_format_example, NULL);
  ...
  g_settings_bind(dialog->settings, CUSTOM_FORMAT_KEY, dialog->custom_entry,
  ...
  g_signal_connect(dialog->prompt, "toggled",
                   G_CALLBACK(configure_dialog_button_toggled), dialog);
  ...
  g_signal_connect(selection, "changed",
                   G_CALLBACK(configure_dialog_selection_changed), dialog);
...
static void choose_format_dialog_row_activated(GtkTreeView *list,
                                               GtkTreePath *path,
                                               GtkTreeViewColumn *column,
                                               ChooseFormatDialog *dialog) {
  gint sel_format;
...
static ChooseFormatDialog *get_choose_format_dialog(
    GtkWindow *parent, PlumaTimePluginPromptType prompt_type,
    PlumaTimePlugin *plugin) {
  ChooseFormatDialog *dialog;
  gchar *data_dir;
  gchar *ui_file;
  ...
  ret = pluma_utils_get_ui_objects(
      ui_file, NULL, &error_widget, "choose_format_dialog", &dialog->dialog,
      "choice_list", &dialog->list, "use_sel_format_radiobutton",
      &dialog->use_list, "use_custom_radiobutton", &dialog->custom,
      "custom_entry", &dialog->custom_entry, "custom_format_example",
      &dialog->custom_format_example, NULL);
  ...
  if (!ret) {
    ...
    pluma_dialog_add_button(GTK_DIALOG(err_dialog), _("_OK"), "gtk-ok",
                            GTK_RESPONSE_ACCEPT);

    if (wg != NULL) gtk_window_group_add_window(wg, GTK_WINDOW(err_dialog));
    ...
    gtk_container_add(
        GTK_CONTAINER(gtk_dialog_get_content_area(GTK_DIALOG(err_dialog))),
        error_widget);

    g_signal_connect(err_dialog, "response", G_CALLBACK(gtk_widget_destroy),
                     NULL);
...
static void choose_format_dialog_response_cb(GtkWidget *widget, gint response,
                                             ChooseFormatDialog *dialog) {
  switch (response) {
    case GTK_RESPONSE_HELP: {
      pluma_debug_message(DEBUG_PLUGINS, "GTK_RESPONSE_HELP");
      pluma_help_display(GTK_WINDOW(widget), NULL,
...
static void time_cb(GtkAction *action, PlumaTimePlugin *plugin) {
  ...
  if (prompt_type == USE_CUSTOM_FORMAT) {
    ...
    dialog = get_choose_format_dialog(GTK_WINDOW(window), prompt_type, plugin);
    if (dialog != NULL) {
...
static GtkWidget *pluma_time_plugin_create_configure_widget(
...
static void pluma_time_plugin_set_property(GObject *object, guint prop_id,
                                           const GValue *value,
                                           GParamSpec *pspec) {
  PlumaTimePlugin *plugin = PLUMA_TIME_PLUGIN(object);
...
static void pluma_time_plugin_get_property(GObject *object, guint prop_id,
...
static void pluma_time_plugin_class_init(PlumaTimePluginClass *klass) {
...
static void pluma_time_plugin_class_finalize(PlumaTimePluginClass *klass) {
...
G_MODULE_EXPORT void peas_register_types(PeasObjectModule *module) {
  ...
  peas_object_module_register_extension_type(
      module, PLUMA_TYPE_WINDOW_ACTIVATABLE, PLUMA_TYPE_TIME_PLUGIN);
